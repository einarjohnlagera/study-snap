...
- `v0.141.0 - Formulas That Render` (In Progress); previous: `v0.140.0 - Pending Work in Reach` (Released); previous: `v0.139.0 - Reopened` (Released); previous: `v0.138.0 - Stated and Enforced` (Released); previous: `v0.137.0 - Deploy Integrity` (Released); previous: `v0.136.0 - Contribution Surface` (Released); previous: `v0.135.0 - Update Signal` (Released); previous: `v0.134.0 - Notification Foundations` (Released); previous: `v0.133.0 - Education Family` (Released); previous: `v0.132.0 - Publication Boundary` (Released); previous: `v0.131.0 - Inbox Polish` (Released); previous: `v0.130.0 - Notification Inbox` (Released); previous: `v0.129.0 - Adoption Signal` (Released); previous: `v0.128.0 - Onboarding Unfrozen` (Released); previous: `v0.127.0 - Failure Attribution and Learner Dates` (Released); previous: `v0.126.0 - Context Budget` (Released); previous: `v0.125.0 - Bounded Reads` (Released); previous: `v0.124.0 - Collection Path Performance` (Released); previous: `v0.123.0 - Collection Builder Integrity` (Released); previous: `v0.122.0 - Shared Quiz Discoverability` (Released); previous: `v0.121.0 - Shared Quiz Recipient Experience` (Released); previous: `v0.120.0 - Canonical Note Title Integrity` (Released); previous: `v0.119.1 - Public Catalog Bounds` (Released); previous: `v0.119.0 - Curator Bulk Regeneration` (Released); previous: `v0.118.0 - Note and Study Pack Regeneration` (Released); previous: `v0.117.0 - Authoring and Quiz Legibility` (Released); previous: `v0.116.0 - Additive Review Set Updates` (Released); previous: `v0.115.0 - Learner Publication Authority` (Released, deployed); previous: `v0.114.0 - Connection Evidence` (Released); previous: `v0.113.1 - Anchoring Hardening` (Released); previous: `v0.113.0 - Session Anchoring` (Released, deployed); previous: `v0.112.0 - Connection Pool Integrity` (Released, deployed); previous: `v0.111.0 - Multidisciplinary Domain Context` (Released); previous: `v0.110.2 - Shared Link Integrity` (Released); previous: `v0.110.1 - Quiz Text Integrity` (Released, deployed); previous: `v0.110.0 - Supporter Combined Quiz` (Released, deployed); previous: `v0.109.0 - Assessment Discoverability` (Released, deployed); previous: `v0.108.0 - Session Identity` (Released, deployed); previous: `v0.107.0 - Curriculum-Scale Remediation` (Released, deployed); previous: `v0.106.0 - Board Exam Review Set Identity` (Released, deployed); previous: `v0.105.0 - Curriculum-Scale Exams` (Released, deployed)
...
**⚠️ In a NATIVE query, never test a named parameter for null without casting it.** PostgreSQL types native-query parameters itself, and a bare `:param is null` gives it nothing to infer from — it fails the entire statement at **parse** time with `could not determine data type of parameter $n`, so the endpoint 500s on every call, not just a paged one. Write `cast(:param as timestamptz) is null`. **JPQL is unaffected** (Hibernate types those parameters), which is why the same shape is safe in JPQL repositories. **⚠️ H2 accepts the uncast form, so the test suite cannot catch this** — it shipped undetected in `v0.91.0` and 500'd the *Shared with you* Library section against real PostgreSQL until a user reported it. `NativeQueryParameterTypingTest` remains the Docker-free source tripwire; do not weaken it. `NativeQueryPostgresIntegrationTest` automatically reflects over every repository `@Query(nativeQuery = true)`, `PREPARE`s it on the pinned PostgreSQL container (**`postgres:18` since `v0.113.1` item 5** — several places still say 16), runs the full Flyway migration set and executes both PostgreSQL-only Library implementations. New annotated native queries require no inventory edit. The normal backend `./mvnw clean install` therefore requires Docker; the only opt-out is the explicit `-Dnativequery.pg.skip=true`, which must be treated as leaving PostgreSQL SQL and migrations unverified. Keep the shared H2 test `application.yaml` unchanged.

**⚠️ WHEN YOU HAND-BUILD A SERVICE INSIDE `NativeQueryPostgresIntegrationTest`, ITS `@Transactional` IS INERT — AND THAT CAN MAKE A "NOTHING WAS WRITTEN" GUARD PASS FOR THE WRONG REASON.** The class is `@DataJpaTest`, so services are not loaded and are constructed by hand (the `LinkedLearnerService` and `StudyPackService` harnesses both do this). A hand-built instance is not a Spring proxy, so its `@Transactional` methods do nothing, while the Spring Data repositories it calls **are** proxied. A `@Modifying` write with no active transaction then throws `TransactionRequiredException` instead of committing — so a mutant that performs a forbidden write **outside** the intended transaction blows up harmlessly and the guard stays green, while in production the proxy opens a transaction and the write really lands. **Measured in `v0.118.0`:** the "charge the note-generation meter eagerly" mutant SURVIVED guard 2 for exactly this reason; wrapping the hand-built `UserUsageService`'s increments in a `TransactionTemplate` (default REQUIRED, so it joins an active transaction and opens one otherwise) reproduced production semantics and killed it. **⚠️ Whenever a hand-built collaborator's transactionality is load-bearing for the assertion, reproduce it explicitly — and mutation-verify, because this class of vacuous pass is invisible from reading the test.**

**⚠️ A guard in that class must be `@Transactional(propagation = Propagation.NOT_SUPPORTED)` whenever the code under test uses `dispatchAfterCommit`.** Under `@DataJpaTest`'s default rollback transaction the registered `afterCommit` synchronization **never fires**, so the async worker never runs — and a guard asserting "content unchanged" or "meters unchanged" passes because *nothing happened at all*.
...
- `RELEASES.md` is the canonical release log. Add new sections at the top. **Do not DELETE old release entries — but DO ARCHIVE them.** Once the file holds more than the current version plus the last five, MOVE the older sections to `docs/archive/RELEASES_ARCHIVE.md` and leave a one-line index entry for each. **⚠️ A MOVE IS NOT A DELETE, AND THE DISTINCTION IS THE WHOLE RULE:** content stays verbatim and `git grep`-able, and live + archive together must still contain every section byte-for-byte. `/signoff` carries the step; `docs/archive/README.md` carries the rationale. **⚠️ THIS SENTENCE USED TO READ *"Do not delete old release entries"* FULL STOP, WHICH FORBADE THE ARCHIVING `docs/archive/README.md` HAS MANDATED SINCE 2026-07-10** — a contradiction that let the convention lapse for 85 releases while `RELEASES.md` reached 116 sections, and that a cold pressure test caught three days after `v0.126.0` patched the command file and left this line standing.
