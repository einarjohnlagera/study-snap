...
- `v0.94.0 - Connection Experience` (Released); previous: `v0.93.0 - Progress Refinement` (Released)

Implementation status: Phases 1-4 are **Released** (`v0.91.0`-`v0.94.0`), with **Phase 4 PARTIAL**: shareable invitation links and connection management shipped in `v0.94.0`; **supporter onboarding did NOT** — it was blocked by the onboarding freeze and is the first candidate for `v0.95.0` once `[CHECKPOINT — due 2026-09-11]` lifts. **No public people search is in Phase 4 at all.** **Phase 5 remains uncommitted and must not be stubbed.** **⚠️ An `ACCEPTED` relationship implies no access of any kind** — material, activity and progress each need their own live grant, and streaks/study days are reachable only through `ACTIVITY`.
