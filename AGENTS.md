...
- Course / Program UI rules:
  ...
  - `users.courseProgram` and `notes.courseProgram` remain persisted string fields and the UI rules below describe live behavior. `v0.69.0` shipped `notes.domain_context` and note-level `notes.learner_level` alongside them. `v0.70.0` exact-matched the audited strings into nullable catalog FKs (with only the literal `Bsed` -> `Education` alias); nothing reads either FK. The legacy note string remains load-bearing as the zero-join-row discovery fallback and is still written by authoring. The Exam Hub separately resolves program names from the catalog by `exam_goal_slug` and fails open to its literal fallback lists.
