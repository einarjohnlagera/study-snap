...
- **Generated Challenge bank writes must run in a transaction SEPARATE from the Challenge session's, with the failure caught at the call site.** Use the injected `questionBankTransactionOperations` (`REQUIRES_NEW`, timeout-bounded) and wrap `execute(...)` in the warning catch. **The invariant is isolation, not any particular save shape.**
- **Do not "fix" a bank write failure by catching it inside the caller's transaction — that cannot work.** A failed flush marks the transaction rollback-only (JPA-mandated) *and* PostgreSQL aborts it (SQLSTATE 25P02), so the session's commit fails regardless of the catch. **A `@Transactional(REQUIRES_NEW)` annotation does not work either**: its commit runs in the proxy after the method body returns, leaving an internal catch equally dead. Only a `TransactionTemplate`, which commits inside `execute(...)`, makes the catch real. Per-row saves and per-row `flush()` are **not** the mechanism and must not be reintroduced as one.
