...
- `v0.94.0 - Connection Experience` (In progress); previous: `v0.93.0 - Progress Refinement` (Released)

Implementation status: Phases 1-3 are **Released**. **v0.94.0 opens Phase 4 — Connection Experience — so Phase 4 IS in scope**, but only the two halves below: **supporter onboarding is OUT** until the onboarding freeze lifts at `[CHECKPOINT — due 2026-09-11]`, and **no public people search** is in Phase 4 at all. **Phase 5 remains uncommitted and must not be stubbed.**

**⚠️ `v0.94.0` — Connection Experience (Phase 4 of Learning Connections).** Three items. **(1) SPLIT THE PROGRESS PAYLOAD** — remove `engagement` from `LinkedLearnerProgressResponse` so streaks, longest streak, study days and engagement mode are reachable ONLY through a live `ACTIVITY` grant. **⚠️ This exists because a live `PROGRESS` grant currently makes the learner's activity toggle INERT** — the progress response carries byte-for-byte the payload the ACTIVITY scope gates — found independently by two cold agents at the `v0.93.0` pressure test. **Owner chose SPLIT over coupling the toggles or fixing the copy (2026-08-28)**, because it is the only option where turning activity off actually does something. **⚠️ Re-derive `hasActivity`**, which reads `engagement.studyDaysThisWeek` today. **⚠️ A supporter holding only `PROGRESS` loses streaks and study days — intended, not a regression** — and `v0.93.0`'s *"study activity is visible through shared progress"* copy becomes FALSE and must go. **(2) SHAREABLE INVITATION LINKS.** **⚠️ Scoped on an owner ruling AGAINST a deferral recommendation (2026-08-28); the constraints are anti-drift, not advice.** **⚠️ A link must not reopen the account-existence oracle `v0.90.0` closed** — nothing about whether an address or account exists may be inferable from creation, redemption or failure. **⚠️ Acceptance stays load-bearing**: holding a link never by itself creates an `ACCEPTED` relationship, the redeemer takes an explicit act, and the inviter can revoke an unused link. **⚠️ Guardian consent is NOT bypassable by link** — a minor redeeming one lands in the same `PENDING` + consent path, with birth year still collected at link time from the learner. **⚠️ Invitations stay ONE-AT-A-TIME**: a link delivers ONE relationship and is not a multi-recipient invite; the quiz share link stays the many-recipient mechanism and is neither extended nor merged. **(3) INVITATIONS STOP DYING IN SILENCE** — surface `expiresAt` on `LinkedLearnerInvitationResponse` and show the clock; when one expires, say so and point at re-inviting. **⚠️ Re-inviting ALREADY re-arms the expired row by updating `expires_at`** (`LinkedLearnerInvitationReArmTest`), so this is disclosure of a working path — **do NOT add a resend endpoint that writes a NEW row**, and do not change what a `linked_learner_invitations` row means, because `[CHECKPOINT — due 2026-10-13]` reads it. **⚠️ Standing rules unchanged:** the progress read stays UNIDIRECTIONAL with the explicit caller-is-supporter assertion; `PROGRESS` stays learner-issued only; granting requires `ACCEPTED` while withdrawing does not; every read re-verifies `ACCEPTED`; guardian consent stays re-asserted inside the grant check; no relationship-type column, no new profile type, nothing on `ProfileType`; `NoteVisibility` stays `PRIVATE | PUBLIC`; no endpoint accepts a learner user id. Plan: `docs/claude-plans/learning-connections-phase-plan.md`.
