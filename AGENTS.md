...
- `v0.87.0 - Published Bounds` (In Progress); previous: `v0.86.0 - Generation Recovery` (Released)

**⚠️ `v0.87.0` — every bound enforced on generated content must be PUBLISHED to the model.** **A generated note is judged against limits the model was never given.** `buildGeneratedNoteContent` validates `title` (1–12 **words**), `overview` (8–90) and `keyIdea` (4–40) while the JSON schema publishes **characters** (160 / 1200 / 500) and the prompt states **sentence counts** — three units, one contract, and the Java bound is the tighter one every time. **⚠️ The minimums are unpublished too:** a correct 7-word overview is rejected by a floor of 8 that nothing states, while the prompt pushes toward brevity. Found by `v0.86.0`'s cold-context pressure test, which falsified that release's claim to have fixed *the only* unpublished bound — it fixed the bullet arrays and left these three. **⚠️ They are the leading candidate for the owner-reported Bulk Generate failures in other subject areas that `v0.86.0` could not reproduce**, because they raise `invalid title` / `invalid overview` / `invalid key idea` rather than `invalid core details`. That is **code-traced, not reproduced** — **scope item 1 is reproducing it, and the release rescopes if the bounds never fire.** **⚠️ Do NOT simply raise the numbers** (two units for one contract is the defect, not its size) and **⚠️ do NOT delete the checks** — the schema bound is a far looser fallback, so removal widens what is accepted rather than leaving it unchanged.
