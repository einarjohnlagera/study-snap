...
- `v0.82.0 - Authored Depth Backfill` (In Progress); previous: `v0.81.0 - Challenge Bank Integrity` (Released)
...
- **`BOARD_TAKER` is not self-certifying.** Information Technology is explicitly excluded from the `BOARD_TAKER` depth mapping when it appears in either `note_course_program` / `course_programs.name` or the free-text `notes.course_program`; a note with several programs is excluded when any one is Information Technology. Do not use `course_programs.exam_goal_slug` as a licensure test — it identifies Exam Hubs only and omits legitimate board programs including Civil Engineering.
