...
- `v0.93.0 - Progress Refinement` (Released); previous: `v0.92.0 - Activity Sharing` (Released)

Implementation status: Phases 1, 2 and 3 are **Released** (`v0.91.0`, `v0.92.0`, `v0.93.0`). **⚠️ An `ACCEPTED` relationship no longer implies progress access** — the progress read requires a live learner-issued `PROGRESS` grant, and the read stays unidirectional (supporter reads learner, never the reverse). **Phase 4 — Connection Experience — is NEXT and unstarted; Phase 5 remains uncommitted.** Neither is in scope until a release opens it, and neither may be stubbed.
