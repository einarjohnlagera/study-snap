...
- `v0.89.0 - Regeneration Integrity` (In Progress); previous: `v0.88.0 - Section Authoring` (Released)

**⚠️ `v0.89.0` — Regeneration Integrity. Regenerating a Study Pack can silently DROP a comparison table or a Common Misconceptions block the prior version had** — no warning, no indication, and **no version history to recover from**, because regeneration updates the pack in place. **⚠️ REPRODUCE BEFORE FIXING — this is a stated gate**: if repeated regenerations do not drop the blocks, the hypothesis is wrong and the release rescopes, exactly as `v0.87.0` did. **The leading hypothesis is that `prompts/study-pack-v1/developer.txt:35-36` makes both blocks conditionally optional** (*"omit if not applicable"* / *"omit if no clear misconceptions apply"*), so every regeneration re-decides and nothing carries the prior answer forward — but that is a reading of a prompt, not a measurement. **⚠️ Do NOT re-propose domain drift or a null level signal as the cause**; both were ruled out on the original sample, the second by the note's own history. **⚠️ Regeneration stays IN-PLACE** — the versioning rule is binding and a fix writing a new Study Pack row per version breaks the quiz/session history link. **⚠️ Do NOT make the optional blocks mandatory**; forcing a comparison table onto material with nothing to compare produces a worse pack. The problem is losing a block that applied, not the blocks being conditional. **No new validation or bounds on generated content.** Also ships one small unrelated fix: the recovery sweep at `GenerationRecoveryRowWriter.recoverPool:58-59` sets `FAILED` without writing `generation_status_at`, which makes the bounds half of `[CHECKPOINT — due 2026-09-01]` unanswerable because churn cannot be distinguished without a sweep timestamp.
