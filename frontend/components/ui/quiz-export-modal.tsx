      ...
      <div className="space-y-3">
        {EXPORT_OPTIONS.map((option) => (
          <button
            key={option.mode}
            type="button"
            className="w-full rounded-2xl border border-border bg-background px-4 py-4 text-left transition-colors hover:bg-highlight disabled:cursor-not-allowed disabled:opacity-60"
            onClick={() => onSelect(option.mode)}
            disabled={exporting}
          >
            <div className="flex items-start gap-3">
              <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-border bg-muted/40 text-foreground/70">
