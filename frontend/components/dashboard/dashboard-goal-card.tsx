...
function resolveGoalDestination(goalSummary: GoalNudgeResponse): string {
  if (goalSummary.goalType === "EXAM") {
    return `/exam/${goalSummary.studyGoal}`;
  }
  return `/public/library?courseProgram=${encodeURIComponent(goalSummary.studyGoal)}`;
}

export function DashboardGoalCard({ goalSummary }: Readonly<DashboardGoalCardProps>) {
  const destination = resolveGoalDestination(goalSummary);
        ...
        <Link
          ...
          onClick={() => {
            void trackAnalyticsEvent({
              ...
              metadata: { studyGoal: goalSummary.studyGoal, destination },
            });
          }}
        >
          Browse {goalSummary.goalName} notes &rarr;
        </Link>
        <Link href="/progress" className="text-sm font-medium text-blue-700 hover:underline dark:text-blue-300">
