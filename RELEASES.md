...
- **Quiz session history + review** — Note Detail now keeps completed quiz history tied to the note so past practice is reviewable instead of disposable:
  ...
  - Session Review now includes an `Export` action with two options: `Full Quiz` (all questions) and `Mistakes Only` (incorrect answers only)
  - exported PDFs use note title, quiz type, score summary, weak concepts, per-question answers, explanations, and subtle `Generated by NoteLib` branding for offline review
  - Mistakes Only PDF shows a focused `Mistakes Review` section with mistake count, accuracy, and weak concepts derived from incorrect answers; includes each incorrect question with choices, correct answer highlighted, user answer marked, and full explanation
  - Mistakes Only PDF handles perfect scores gracefully with a `Perfect Score!` message instead of an empty document
  - Mistakes Only filename uses `notelib-mistakes-[title]-[date].pdf` to distinguish from full quiz exports
