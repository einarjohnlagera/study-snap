...
**Known, explicitly out of scope:** `/exam/{slug}` (the Exam Hub page) has a near-identical `ExamNoteCard` — same bespoke pattern, same `summaryPreview || contentPreview` priority, apparently copy-pasted from the subject-landing page before this fix. Not migrated here since it wasn't part of what was asked; flagged in the Backlog Index for a follow-up pass alongside it.
