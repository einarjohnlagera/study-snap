...
- **Item 1 — a `$` delimiter no longer mis-pairs when currency and a formula share a string (frontend).** `startsCurrencyAmount` gives the OPENER the currency-awareness the closer already had, so `$50,000` no longer opens a math span that closes on a formula's `$` four clauses later. **⚠️ POPULATION CORRECTED DOWN, AND THE OLD NUMBER WAS A LOOSER PROXY:** the kickoff said *"22 questions + 49 explanations"*, counted as `$<digit>` with two or more `$`. Scoped precisely to strings carrying **both** money and a formula, it is **44** — 6 questions, 26 explanations, 12 working solutions. Money-only strings were already safe: the closer rejects a `$` preceded by whitespace, so two amounts never paired.
- **⚠️ THE PRODUCTION DATA CAUGHT A WORSE BUG BEFORE ANY CODE WAS WRITTEN, AND IT IS THE REASON THE RULE HAS TWO CLAUSES.** `NPV $10000$` — a real string — is the model writing a NUMBER as maths, with properly paired delimiters. A naive *"digits after `$` means currency"* rule skips that opener, which leaves its **closing** `$` to be read as an opener and swallows the rest of the sentence, formula included. **That is strictly worse than the defect being fixed.** So a `$` immediately after the digits means the pair is already balanced and the span is left alone; and a letter, backslash or operator after the digits means maths (`$3x^2$`, also real). Both are pinned by tests taken verbatim from production, and both mutations were killed by their intended test.
