...
- 17. Show due-concept signals to Free users, action stays gated — **pending explicit product decision**, current docs (`subscriptions-and-usage-limits.md`) call the signal gate intentional; not implemented until that decision is made
- 18. Course/Program discoverability (chips/rail) + fix search-scope coverage

### Planned Scope (Tier 2 addition)

Item 15 has shipped below. Items 12, 13, and 14 were re-verified against the actual codebase before prompting: none need a new backend endpoint (public note totals, `premiumEndsAt`, and the plan-detail endpoint all already exist) — only items 15 and 16+17 (combined into one prompt, since both touch the same due-concepts signal path) genuinely need new backend. All six remaining items (12, 13, 14, 16, 17, 18) are routed through Codex regardless of backend need, per a token-budget call rather than the usual Claude-Code-direct threshold. Item 17's product sign-off (see above) has been given; its prompt is combined with item 16. Sequencing: item 14 depends on item 15's `readyCount` field shape and must run after it, not in parallel — everything else is independent.

### Shipped

- **Public Study Plan practice-readiness metadata (backend + frontend).** Public plan list and detail responses now return a live `readyCount`, calculated with the existing `STUDY_PACK_READY` resolver. Published-plan and Dashboard cards show `{readyCount} of {itemCount} notes practice-ready` as metadata—not a badge—so zero, partial, and fully ready inventory is visible before adoption; older cached list responses retain their existing item-count display.
