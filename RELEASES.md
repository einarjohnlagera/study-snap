...
Items 14, 15, and 18 have shipped below. Items 12 and 13 were re-verified against the actual codebase before prompting: neither needs a new backend endpoint (public note totals and `premiumEndsAt` already exist) — only items 16+17 (combined into one prompt, since both touch the same due-concepts signal path) genuinely need new backend. The four remaining items (12, 13, 16, 17) are routed through Codex regardless of backend need, per a token-budget call rather than the usual Claude-Code-direct threshold. Item 17's product sign-off (see above) has been given; its prompt is combined with item 16.
