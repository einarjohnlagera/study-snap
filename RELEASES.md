...
- **Profile-type integrity + onboarding enforcement** — Completed-but-null legacy users are now routed back to `/onboarding` for a focused profile-type prompt only, while truly incomplete users keep the full onboarding flow. Content-creating and generation mutations now enforce profile setup server-side through `OnboardingGuardService` and return a named 403 `ONBOARDING_REQUIRED` / `COMPLETE_PROFILE_TYPE` error for null `profileType` callers. No backfill or silent defaulting was added; read/auth/onboarding/recovery endpoints remain reachable so users can fix their setup.
