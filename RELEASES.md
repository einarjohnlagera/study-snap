...
- **Quota honesty via per-session deduction** — change Long Exam and Board Exam quota to deduct **1 unit per session** (not per source note), so "12 / 10 sessions" is literally true and more generous; the prescreen quota copy simplifies to "uses 1 of N remaining". Source count still drives question-count + generation; numbers (12/10) and monthly reset unchanged. A deliberate quota-mechanic change (anti-drift line updated to allow exactly this); per-week quotas were considered and rejected.
