...
- **Goal milestone markers** — `/progress` goal view now shows six fixed ConceptHealth-derived checkpoints between the goal summary and next-study card: first concept mastered, 25%, all reviewed, 50%, 70%, and all mastered. `GET /me/progress` now exposes `goalSummary.notPracticedConcepts` so the all-reviewed checkpoint uses the same backend-resolved concept counts as the rest of the progress report.
