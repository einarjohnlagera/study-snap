...
Items 12 and 14–18 have shipped below. Item 13 was re-verified against the actual codebase before prompting: it needs no new backend endpoint because `premiumEndsAt` already exists. It remains the sole unshipped Tier 2 item and is routed through Codex regardless of backend need, per a token-budget call rather than the usual Claude-Code-direct threshold.
