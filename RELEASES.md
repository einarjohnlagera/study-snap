...
Items 14–18 have shipped below. Items 12 and 13 were re-verified against the actual codebase before prompting: neither needs a new backend endpoint (public note totals and `premiumEndsAt` already exist). They remain the two unshipped Tier 2 items and are routed through Codex regardless of backend need, per a token-budget call rather than the usual Claude-Code-direct threshold.
