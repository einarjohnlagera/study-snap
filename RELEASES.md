...
- **Echo Weekly Countdown pacing at session completion (frontend).** When the learner has a Primary Review Set with a target date, the quiz result screen (`PostSessionNextStep`, inline in the three session-mode pages) adds one line of This-Week pacing echo ("that's another session toward this week's target"), following the same general-nudge pattern as the existing `GoalNudgeCard` (shown based on the learner's Primary Review Set, not gated on the completed note's own collection membership). Data and derivation already exist via `GET /collections/{id}/goal` — no backend change.
- **Shared note-card press feedback (frontend).** `motion-pressable`/`motion-surface` applied to `shared-note-card.tsx`'s wrapper, reconciled with the existing bespoke press-scale already on Library/Public Library's card wrapper (`getBrowsingCardClassName`) so two mechanisms don't stack; Dashboard's note-card wrapper currently has no press feedback at all and gets it for the first time.
- **Skeleton-first initial load for Note Detail (frontend).** Note Detail's initial load currently shows plain "Loading note..." text; reuses the existing `StudyPackGeneratingCard` skeleton (with adjusted copy) instead. Review Sets list already has skeleton-first load — no change needed there.
- **Sticky search/filter toolbar (frontend).** Private Library and Public Library toolbars, using the same `sticky top-16 z-20` convention already used by 3 other surfaces in the app.
- **Collapse-by-default Note Detail sections (frontend).** Recent Sessions / Performance Overview collapse by default, replicating (not importing — it's a private in-file pattern) `CompanionDisplayCard`'s "View Full Guide" collapse/expand shape.

Anti-drift: no 6th quiz mode; no auto-assembled or gap-filling review plan (owned content only, explicitly not Smart Review Planning); no freetext exam identity; no per-learner runtime personalization beyond the existing PRO tier design; no pricing, paywall, or quota numbers introduced by this release (tier *direction* only, per Fable's own guardrails); no image-capture/vision-extraction infrastructure (Idea 6, explicitly excluded from this fold-in); no new backend entity, migration, or endpoint (all 5 items are frontend-only, reusing existing data). Full source material in `docs/claude-prompt/new-capability-out/01-new-capability-ideation.md`, `docs/claude-prompt/app-shape-session-plan.md`, and `docs/claude-prompt/conversion-audit-prioritized-backlog.md`.
