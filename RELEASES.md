...
- **"Build" icon fixed (frontend).** Both "Build" entry points on Study Plan detail (`collection-detail-page-client.tsx`) now use a new `build` action icon (`Hammer`) instead of reusing `edit`'s `Pencil`.
