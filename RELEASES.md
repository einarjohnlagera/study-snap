...
- **Deep-linkable Help guides** — Help page opens a specific guide from a URL hash (e.g. `/help#progress-milestones`), so in-app links can land on the exact guide. Hash (not query param) to avoid a Next.js `useSearchParams` Suspense build de-opt.
- **Inline help-link pattern** — a small, persistent "How this works →" link co-located with complex features, paired with a one-sentence inline gist. Reference-grade (always present, re-readable), distinct from the one-time dismissible `GuidanceTip` (reserved for "this feature exists" discovery nudges).

**Progress & Study Focus guide**
- New Help guide covering Goals, Study Focus (subject multi-select), Goal Milestones, how mastery is calculated, and the honest "starting a new term" answer (no reset button; shared subjects carry mastery forward). Profile-type aware: TEACHER has no Study Focus, BOARD_EXAM = "Exam Focus", STUDENT = "Study Focus".
- Inline gist + "How milestones work →" deep-link on the Progress Milestones card.
- Inline "How Study Focus works →" deep-link on the Profile Study Focus section (profile-type-aware framing).

**Exam Hubs guide**
- New Help guide covering what `/exam/ale`, `/exam/pnle`, `/exam/let` are, how they curate public notes, and how to reach them — currently zero Help coverage.
