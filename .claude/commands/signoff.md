...
- [ ] **Archive check — run it here, because nothing else will.** If `RELEASES.md` now holds **more than the current version plus the last five**, move the older sections into `docs/archive/RELEASES_ARCHIVE.md` and leave a one-line index entry for each; do the same for `ROADMAP.md`'s **`## Current Release Baseline`** once it names more than the current version plus the last five → `docs/archive/ROADMAP_ARCHIVE.md`. **⚠️ THAT SECTION IS THE ONE THAT GROWS, and this clause originally pointed at the wrong one:** it said *"any newly-`(Released)` per-version retrospective"*, but kickoff stopped creating those at `v0.85.0` and live `ROADMAP.md` has **zero** `## v` sections — so it would have matched nothing forever while `Current Release Baseline` (**247,345 chars naming 121 versions, one paragraph per release**) kept growing unguarded, structurally identical to the `CLAUDE.md` chain `v0.126.0` removed. **⚠️ A MOVE, NOT A DELETE — content verbatim, and verify afterwards that live + archive together still contain every section byte-for-byte.** **⚠️ THIS STEP EXISTS BECAUSE ITS ABSENCE IS WHY THE CONVENTION LAPSED:** `docs/archive/README.md` documented the design in 2026-07-10, the pass ran once, and nothing triggered it again for **85 releases** — `RELEASES.md` reached **116 sections** against a documented design of *current + last few*. **⚠️ Do NOT extend this to the ROADMAP Backlog Index**: it mixes live `[CHECKPOINT — due …]` gates with closed rows and kickoff step 8 depends on it, so it needs judgment, not a sweep.
