...
- **This ruling binds every surface that renders a learner's Applicable Programs, not just the one that prompted it.** Note Detail is the only such surface today because the private list projection returns an empty array — **the private library card already passes `applicablePrograms` through**, so correcting that projection surfaces programs on learner cards automatically, with no opt-out. That correction must therefore not land before this ruling is implemented.
...
- **Known gap, tied to item 1's outcome:** on an admin-curated **non-copy** learner note, the learner sees read-only programs with no explanation of their origin. Honest but unexplained. The Note Detail provenance sentence is conditioned on `copiedFromNoteId` precisely so it stays true under either resolution of item 1, rather than asserting a copy that did not happen.
