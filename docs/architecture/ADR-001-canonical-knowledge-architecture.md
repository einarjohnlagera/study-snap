...
**Status:** **Accepted** — ratified by the owner 2026-08-03. Drafted, audited against production, and revised the same day; amended 2026-08-16 to ratify the four-axis model and gated Target Audience retirement; amended 2026-09-05 to correct the canonical-shapes table's copied-note row and to record the resolution of the overlapping-representation class; amended 2026-09-16 to make Program Family membership many-to-many.
**Deciders:** Owner
...
**⚠️ DATED AMENDMENT, 2026-09-17 — the 30-day checkpoint on the 80 fired, and re-auditing found the population itself had grown.** The pre-committed `[CHECKPOINT — due 2026-09-16]` (`ROADMAP.md:824`) read the same 80 count unchanged after 30 days: zero were classified, because nobody was ever actually assigned the work. But the true curator-owned public NULL-depth population is **282**, not 80 — 202 `BOARD_TAKER` plus the original 80 `STUDENT` — of which **173 were created after this ADR's own `v0.83.0` implementation date**, entirely via the bulk-generate `makePublic` path, which never inspects Authored Depth. The passive-classification assumption above did not merely fail to shrink the backlog; the backlog more than doubled while it was being tested. Full audit, production evidence and the owner's resulting decision: `docs/claude-plans/authored-depth-legacy-backfill-audit-and-plan.md`. This paragraph's original claims (the 120/26/11/3/80 breakdown as of 2026-08-17) are preserved above, unedited, as the historical record.
...
2. **A family expands to all of its members.** No curated per-family subsets and no expansion presets: membership is the *only* input to expansion. Membership is many-to-many — a Course / Program may participate in zero, one, or several families. If a family should expand to a different set, that is a change to *membership*, not to expansion logic. **⚠️ Amended 2026-09-16 (owner-ratified, storage-neutral):** this clause originally read "no preset table beyond `course_programs.program_family_id`," which named a specific single-FK column as the only legal storage and would have literally forbidden a membership table. The substance is unchanged — expansion is still unconditional and still driven entirely by membership, and the rejected alternatives below (subject-conditioned expansion, curated subset expansion) are still rejected — only the storage clause moves, deliberately naming no physical table so a future storage change never requires re-touching this ADR. *Which* table implements membership lives in `docs/architecture/DATA_MODEL.md`, not here. See `docs/claude-plans/program-family-many-to-many-final-plan.md` §A for the full reasoning.
...
| **Curated note copied by a learner** | as copied — **and see the correction below: this is NULL for every curator note authored since slice 4** | copied rows, preserved as authored metadata |

**⚠️ CORRECTION, 2026-09-05 — ROW THREE OF THIS TABLE IS FACTUALLY WRONG AS WRITTEN, AND IT IS CORRECTED HERE RATHER THAN QUIETLY REWRITTEN.** *"As copied"* is accurate about the MECHANISM and misleading about the RESULT: it reads as though the string is populated. `NoteService.create` sets it `curator ? null : …`, so **every curator note authored since slice 4 carries a NULL string**, and a copy inherits that null. Measured 2026-09-05: **608 of 1360 curated public notes with join rows (44.7%) have a NULL `course_program`, and 551 (40.5%) carry two or more join rows.**

**⚠️ THE TABLE DESCRIBES THE V107-ERA SHAPE, WHICH IS PRECISELY WHY IT LOOKS CORRECT AGAINST TODAY'S DATA AND IS NOT.** All 536 learner copies that exist in production carry an agreeing string, because they were copied before the curator-null behaviour existed. **The row is wrong about copies made today, and no query over existing rows would reveal that.** This is the same failure this ADR already records at *"Status: cleared"* below — a passage carried forward on its own authority instead of being re-read against current code — and the lesson stated there applies verbatim here.

**⚠️ TERMINOLOGY, NARROWED IN THE SAME PASS: this table said "inherited rows" and now says "copied rows".** No live inheritance relationship exists or is intended — a copied note is independent, and the curator retains no ongoing authority over it. **The word "inherited" elsewhere in this ADR should be read in that light; it has NOT been mass-rewritten, because ratified language is not edited for style.**
...
**⚠️ ~~Deliberately left open~~ — RESOLVED 2026-09-05. THE OVERLAPPING-REPRESENTATION CLASS IS CLOSED BY A THIRD OPTION, AND BOTH OPTIONS NAMED BELOW ARE REJECTED. The original text is preserved because its reasoning is still why the question was asked; do NOT re-derive the two options from it.** **The resolution: applicability authority TRANSFERS AT THE `PRIVATE → PUBLIC` TRANSITION** for learner-owned notes — copied rows are cleared at publication, which un-shadows the learner's own field so they state their own classification. Authority is **transferred, not merely revoked**, and the change is one write path with no migration and no public-discovery SQL change. **Not inheriting on copy is REJECTED on product semantics** — 608 of 1360 curated notes carry a NULL string and 551 carry 2+ join rows, so a copy would be left with no representation at all, and one string cannot hold a multi-program note. **Letting a learner's own value clear the copied rows is REJECTED AS A SOLUTION SHAPE**, and separately — as a repo fact rather than an argument — it **cannot fire in the current model at all**, because shadowing is unconditional at one row so the learner's free-text value is never written. **⚠️ Do NOT conflate those two rejections, and do NOT read either as meaning learner ownership does not matter: it does, and it is exercised at publication.** **⚠️ The publish hazard named below is REAL AND UNREALISED — 536 learner copies carry copied rows and ZERO are public** — so this closes a live mechanism before it fires rather than repairing damage. Evidence: `docs/claude-plans/v0.115.0-stage1-note-copy-audit.md`. Original text follows.
...
>
> **⚠️ AMENDED 2026-09-14 (`v0.145.0`, owner decision — this entry is clause (b)'s record for `BASIC_MEDICAL_SCIENCES`).** Name `Basic Medical Sciences` / enum `BASIC_MEDICAL_SCIENCES` / `quantitative = false`. **Clause (a) evidence:** 6 canonical, multi-program Pharmacology notes (`Antibiotics: Mechanism of Action and Resistance` and five siblings) are already authored and already assigned to 3 live health programs (Medicine, Nursing, Pharmacy) `[PROD 2026-09-14]`, plus ~9 firmly-planned PNLE rows — clearing the ~10-note floor without depending on the planned half alone, the strongest evidence any candidate in this ADR's history has cleared clause (a) with. **Boundary (Test D):** does a professional role appear in the knowledge itself, or only in who is reading it? Role in the knowledge → `NURSING`; role only in the audience → `BASIC_MEDICAL_SCIENCES`. Applied to all 18 multi-program Pharmacology notes' real content, this separated them 6 mechanism-framed / 11 role-framed / 1 unclassified with no hard case. `quantitative = false` is a companion decision, not a default: the false-negative repair is `"pharmacokinetic"` added to `QUANTITATIVE_KEYWORDS`, discipline-specific and measured (not assumed) against production to net +17 matches at the Quick Review tier with zero false positives, after two of the three originally-proposed strings (`"half-life"`, `"clearance"`) were measured and rejected as a no-op and a live false positive respectively. **Governance ratio: `12:51` = 0.235** `[PROD 2026-09-14]` (course catalog re-read the same day; unchanged from the day before), still far below the 0.40 re-open threshold.
>
> **⚠️ Two lines above this amendment are corrected here, in the same edit, per `CLAUDE.md`'s rule that a claim about shipped/production state decays exactly as fast as any other claim.** The *"Usage observation, 2026-08-29"* paragraph's *"three unused values — Professional Education, Nursing, Accountancy"* is now stale: `[PROD 2026-09-14]` `Professional Education` carries **232** notes and `Nursing` carries **46** — only `Accountancy` remains at zero. And this amendment's own *"41 programs"* / `8:41` reading is stale: `[PROD 2026-09-14]` the catalog holds **51** rows, which is the denominator the `12:51` ratio above already uses. Neither historical paragraph is rewritten — both are dated observations of their own moment — but a reader must not carry either figure forward as current.
