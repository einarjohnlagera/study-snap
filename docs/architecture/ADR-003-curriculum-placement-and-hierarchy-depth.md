...
- **The term must be carried at every child-copy site, and there are three.** A child Subject Plan is built field by field in `persistAdoptedPlan` (serves `adopt()`, and once per child inside `adoptGoal()`) and in the Official-update addition `createSubjectAddition`, and an already-owned standalone copy is re-parented in `adoptGoal()` and must take the term there too. *(Amended 2026-09-26 at `v0.160.0` implementation: this said "two" until the re-parent branch was found; the decisions A-F are unchanged.)* A further builder, `persistAdoptedGoal`, copies the root and must NOT carry a term, because a root has no placement. Missing `createSubjectAddition` lets the system itself manufacture a mixed placed/unplaced Year with no curator involved.
