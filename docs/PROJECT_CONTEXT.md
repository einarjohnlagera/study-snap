...
- **AI metadata suggestions are non-blocking** — broad or invalid AI subject suggestions such as `Engineering`, `Medicine`, `Business`, and `Law` are ignored safely and must not fail Study Pack generation; valid specific subject suggestions still flow through the normal review/apply behavior
