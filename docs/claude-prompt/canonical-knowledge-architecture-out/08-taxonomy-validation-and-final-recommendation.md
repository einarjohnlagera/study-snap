...
### Finding 1 (blocking): three proposed Content Context values already exist as *subjects*

| Proposed as Content Context in `06` | Already a `notes.subject` value | Notes |
|---|---|---|
...
**Recommendation — turn the collision into a signal, not a prohibition.** Subject and Content Context are genuinely different axes, and a value legitimately appearing in both is not an error (a broad survey note about nursing really is `subject = Nursing`). But for a small team it is a footgun. Since Content Context is a *curated closed set* and Subject is *open text*, the cheap guard is:

> When a note's `subject` equals its `content_context`, surface an admin-side warning that the subject is probably too broad — a nudge, never a hard validation error.

That reuses the intent of a rule the codebase already has, costs one comparison, and needs no new vocabulary. **Do not rename the Content Context values to dodge the collision** — see Q5, where borrowed board-subject-area names are the reason the vocabulary reads well to learners.
...
`Architecture` is a judgment call: its five big subject plans (Site Planning 175, National Building Code 151, Architectural Design 141, Building Technology 140, Building Utilities 137) are Architecture-specific, so a program-name context is correct — but with `Professional Practice & Regulation` and `Construction Materials` carved out, it may be thin enough to fall back initially. **Owner call.**
...
**Do not add thin-program fallbacks to the curated `content_contexts` table at all.** Let them resolve through the existing chain (`content_context` → `course_program` → user's `course_program`). Then **`content_context IS NULL` *is* the marker of "not yet promoted"**, and the promotion backlog becomes a one-line query:

```sql
SELECT course_program, COUNT(*) AS notes_awaiting_a_content_context
FROM notes WHERE content_context IS NULL AND course_program IS NOT NULL
...
**The principle:** the vocabulary reads well exactly where it borrows names learners already encounter — board subject areas and recognized curriculum bundles — and reads badly where I invented a grouping. **Derive Content Context names from real curriculum vocabulary; never invent one.** Both values that failed the learner test were my inventions, and both also failed the governance rule independently. That correlation is not a coincidence: an invented name is usually a sign there is no real shared body of knowledge behind it.
...
### Q6 — Is the Content Context / Program Family separation still cleanest?
...
- **`Engineering Sciences` subjects are shared by different subsets** (Strength of Materials broadly, Hydraulics narrowly). So applicability cannot be derived from Content Context either.

Sharing is ragged and crosses family boundaries. Therefore **explicit per-note Applicable Programs is the only correct model**, Program Family is genuinely only an authoring shortcut, and Content Context must stay the sole generation authority. Your separation is right.

This also **retires `01` §4's alternative A1** (inverse mapping: program → content contexts, notes stay single-valued). I had kept it as a documented fallback if Step 3 proved too expensive. Query K kills it: under ragged cross-family sharing it would need a sparse per-note override table immediately, which is `note_course_program` with extra steps. **Remove it as a live fallback.**

### Q7 — Is "Content Context" the best long-term name?

**Recommendation: `Domain Context`.** One argument decides it — the field's single job in code is to feed the prompt line that reads *"treat the {value} above as the authoritative academic **domain**."* A field named `domain_context` makes its responsibility self-evident to the next engineer who opens `buildGenerationContextBlock`; `content_context` is mildly circular, since all note metadata is context about content.
...
1. **Subject/Content Context collision** — ~334 notes carry a program-name-as-subject, and three proposed context values already exist as subjects. Needs the Finding 1 nudge rule decided.
...
3. **User-authored notes never get a Content Context.** Normal users do not see the field (correctly — low friction), so their notes always resolve through the fallback chain and never receive canonical treatment. That is acceptable, but it should be *stated*, because it means Content Context is an Official-content mechanism, not a product-wide one.
4. **Subject granularity is inconsistent and this architecture does not fix it** — Accountancy has `PPE` and `Inventory` as subjects beside `Financial Accounting`. Content Context may make the inconsistency more visible on cards.

### 3. Hidden long-term risks

- **Content Context becomes the new program field for curators** — because it will be the only *curated* vocabulary, it will attract every classification urge that has nowhere else to go. The governance rule's failure condition (Q3) is the guard; it must actually be reviewed at kickoff, not just written down.
...
3. **Do not catalog thin-program fallbacks** — use `content_context IS NULL` as the promotion marker (Q4).
...
`02-adr-draft.md` is updated in the same change set as this document: revised value set, governance rule, fallback semantics, the ragged-sharing evidence, and the naming decision recorded as an open owner call.
