...
Planning output only. This is a rename MAP for a later implementation session — no copy has been changed *by this document*.

> **Status note added 2026-08-01 — read before executing any row below.** This map is no longer fully current. `v0.68.0` (Company Redefinition Phase 4, §4 items 6 and 8) shipped a *different* terminology initiative that overlaps two of this document's entries, and **reversed the "keep" stance on both**:
>
> - the topic-note action renamed from **"Generate Note" / "Generate a note" → "Create a Note"** (onboarding Step 3, note editor, demo page, and the `Generate from topic` → `Create from topic` entry tab), and
> - the `PLAN_COMPARISON_ROWS` label renamed from **"Topic note generation" → "Topic notes"**, with the topic-note quota's whole user-facing vocabulary moving to "topic note(s)".
>
> Both are marked **keep** below (see the "AI Note Generator" row and the pricing-page policy bullet, each annotated inline). Those "keep" verdicts were correct for *this* document's purpose — neither string contains "AI," so an AI-de-emphasis pass had no reason to touch them — but they are now stale as descriptions of shipped state. **Do not restore either string when executing this map.** The reasoning for the reversal is in `05-packaging-and-terminology.md` §3.3 and §4 item 6, and in `RELEASES.md` v0.68.0.
>
> Worth noting in the other direction: this document's own point 4 below already recommended **"Create Notes / Topic Notes"** as the outcome-first replacement for an "AI Note Generator"-style surface. `v0.68.0` arrived at "Create a Note" and "topic note" independently, via the Generate-verb argument rather than the AI-de-emphasis argument — so the two initiatives agree on the destination, and this map's recommendation is corroborated rather than contradicted. Only the "already-shipped, leave alone" annotations are out of date.
>
> Everything else in this map — the "AI" allowed/banned policy and all remaining rows (`ai-suggestion-modal.tsx`, the "AI Critique" → "Answer Critique" feature rename, the Help/Learn-guide mechanism copy, and the `getCollectionLabels`/`getUpgradeCtas`/`exam-mode-visibility` infra constraints) — is **unexecuted and unaffected**. It still needs its own scoping pass and `/kickoff`; it is not part of `v0.68.0`.
...
- Pricing page *prose* (e.g. trust/positioning sentences) — but plan feature rows keep outcome names (already true: "Summary + Key Concepts", ~~"Topic note generation"~~ → **renamed to "Topic notes" in `v0.68.0`**; the policy is unchanged, only this example's current wording, see the status note above)
...
| "AI Note Generator" (brief example) | "Create Notes" (action) / "Topic Notes" (feature noun). ~~Onboarding already ships "Generate a note" / "Generate Note" — keep; plans row "Topic note generation" — keep~~ **Superseded 2026-08-01: both were renamed in `v0.68.0`** — the action is now **"Create a Note"** (onboarding Step 3, note editor, demo page, `Create from topic` entry tab) and the plans row is now **"Topic notes"**, converging on this row's own recommended terms. Nothing left to do here; do not restore the old strings (see the status note at the top) | Note creation entry points, onboarding Step 3, plan comparison | static (plan row lives in `plans.ts` PLAN_COMPARISON_ROWS but is not profile/CTA infra) |
...
**Consistency guard for future copy:** new feature names are outcome nouns (Key Concepts, Answer Critique, Topic Notes, Practice Quiz); actions are verb-first without "AI" ("Create", "Build", ~~"Generate a note" is acceptable — "generate" describes the action, not the technology~~). **Amended 2026-08-01 (`v0.68.0`):** the "generate" carve-out no longer holds for the topic-note action specifically. "Generate" is now reserved for operations that transform the learner's *own* material (Generate Study Pack, Generate Quiz, Regenerate) — the actual differentiator — so authoring freeform prose from a bare topic is **"Create a Note,"** not "Generate a note." This is a *narrowing* of the rule, not a reversal of its spirit: "generate" still describes an action rather than a technology, but it now also carries a product claim about whose material is being transformed. See `05-packaging-and-terminology.md` §3.2 (the keep bucket) and §3.3 (the rename bucket) for the full instance-by-instance split. "Curation, never generation" stays intact: nothing in this map renames a curator/assistant surface in a way that implies students receive auto-generated curriculum — the Internal Curator vs. Learning Companion split is unaffected.
