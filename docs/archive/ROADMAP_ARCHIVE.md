...
**Signed off 2026-09-09.** `v0.136.0 — Contribution Surface` is **Released**, shipped as PRs #1357 and #1358 on `releases/v0.136.0`, cut from `main` after `v0.135.0` merged as #1356 and tagged `7f371e65`. Source: `docs/claude-plans/your-impact-private-contribution-surface-stage1.md`. **⚠️⚠️ THE HEADLINE IS A LIVE DEFECT, NOT THE IA MOVE: `frontend/app/settings/page.tsx:276-296` calls `getCreatorImpact()` on every Settings load and uses the result for EXACTLY ONE BOOLEAN (`setHasPublicNotes`), pulling the entire payload — 1,587 note records on the owner's account — through three `IN (:noteIds)` grouped queries, on the 20-connection pool that has ALREADY FAILED TWICE from unbounded reads.** It is live today and independent of whether the page ever moves. **❌ THE BRIEF'S PERFORMANCE PREMISE IS FALSE AND MUST NOT BE REPEATED: Impact was NEVER part of the Public Profile payload** — it is a separate `GET /creator-impact/me` fetched client-side behind an `isOwner` gate, and the controller takes NO `userId` parameter, so a public visitor triggers no impact query. **Moving the page saves Public Profile NOTHING; the IA move is a discoverability fix (7 lifetime dashboard views), not a performance one — do not write a test asserting an improvement that cannot occur.** **SCOPE: ALL SEVEN ITEMS by owner decision** — bound the query; add a lightweight summary endpoint and switch Settings to it; a dedicated `/impact` route reusing `CreatorImpactService`; Public Profile keeps only an owner-only `View impact →`; impacted notes first with zero-impact collapsed; zero states; mobile and an accessible collapse control. **⚠️ ANTI-DRIFT: NEVER add a `userId` parameter — its ABSENCE is the privacy control and `/impact?userId=…` must stay UNREPRESENTABLE, not merely blocked**; no leaderboards, badges, XP, follower counts or public metrics; **do NOT merge Impact into Progress or Learning Connections** (Progress = how I am learning; Your Impact = how my shared knowledge helps others); no sidebar entry; no duplicated analytics logic; **`/impact?note=<id>` is DEFERRED — an unused deep-link contract is a commitment with no caller**; **the tie-break is `noteId ASC` ALONE, never `copyCount DESC`** (it is absent from the ranking query and would promote a distribution metric into semantic ranking); **label views "page views" — `PUBLIC_NOTE_VIEWED` has 25,262 rows and ZERO with a `user_id`, so the metric CANNOT support a per-person reading**; **do NOT show a prominent "Awaiting first study · 1,526" counter — it turns a contribution surface into a deficit scoreboard**; preserve the `PRIVATE TO YOU` labelling, the (accurate) headline-dedup copy, the self-copy exclusion, the `impactRequestIdRef` guard and `public-profile-page-client.test.tsx:277`. **⚠️ NO Learning Connections work — `[CHECKPOINT — due 2026-09-19]`, denominator ONE. NO `frontend/app/onboarding` work — `[CHECKPOINT — due 2026-09-11]` is two days out.** **⚠️ DENOMINATOR IS ONE AND THE USUAL DEFERRAL DOES NOT TRANSFER: the single account with impact is the OWNER'S OWN ADMIN account, the owner is asking for the feature, and 56 learners helped is MEASURED impact rather than a speculative event stream.** **⚠️⚠️ THE AUDIT RECOMMENDED SPLITTING THIS INTO TWO RELEASES and the owner took both halves — recorded rather than smoothed over. The stated consequence: 7 items is well past the 3–4 band, so VERIFICATION IS AT LEAST ONE SCOPED COLD AGENT and the tier is re-decided if the diff grows.** **⚠️ Ordering still binds INSIDE the release: bound the query BEFORE the new page consumes it, so the new surface is never briefly worse than the old.** **Cross-reference: `/impact` becomes the canonical `IMPACT_MILESTONE` destination, so the notification audit's §15 must stop pointing at `/progress`.** **Routing: CODEX.** **✅ ALL SEVEN ITEMS SHIPPED.** **⚠️ THE PRE-SIGNOFF COLD AGENT REFUTED NONE OF THE 13 CLAIMS PUT TO IT** — it confirmed the bounded `IN` lists, the required `impacted` parameter, the absence of any user identifier (and that the reflective test genuinely FAILS if one is added), `noteId ASC` as the sole tie-break (and that the integration fixture actually distinguishes it from `copyCount DESC`), and the complement relationship that makes `totalZeroImpact` correct by subtraction. **⚠️ BUT THE INLINE AUDIT BEFORE IT FOUND FOUR DEFECTS A GREEN 4,699-TEST SUITE DID NOT, AND THE WORST WAS INVISIBLE TO `git diff`: the Knowledge Impact digest EMAIL pointed `View Your Impact` at `/public/creator/{username}#your-impact-heading`, an anchor that still exists but now resolves to a card holding only another link.** `RetentionService` appeared in no PR of this release — it was found by sweeping PROSE for what describes impact, which is the fourth release running that this class has cost. **⚠️ `KNOWLEDGE_IMPACT_DASHBOARD_VIEWED` CHANGED MEANING TWICE — the fire site moved AND it is now gated on a successful load — so the 7-lifetime-views figure this release was justified by is NOT comparable to what it now counts.** Full scope in `RELEASES.md`.
...
**Two more baseline paragraphs moved here at the `v0.144.0` kickoff, 2026-09-13** — `v0.137.0` and `v0.138.0`, found lagging behind this section's own archive cadence (their `RELEASES.md` equivalents had already moved at the `v0.143.0` kickoff, but these baseline paragraphs had not). A MOVE, not a delete, verified by line-multiset comparison against the pre-move file rather than string equality, since relocating two paragraphs earlier in this file breaks a naive byte-diff even when nothing is lost. **⚠️ Unlike the `v0.142.0` move below, this one did NOT hold `[CHECKPOINT — due` count identical before and after the live file (133 → 132)** — the moved `v0.137.0` paragraph carried one cross-reference *mention* of `[CHECKPOINT — due 2026-09-19]`, not the row that owns it; the owning row is confirmed still live at `ROADMAP.md:514`. Do not read a future non-identical count on this kind of move as a defect without first checking whether the delta is a mention rather than an owned gate.
...
**Moved from `docs/product/ROADMAP.md`'s Current Release Baseline at the `v0.145.0` kickoff, to keep that section at current + last five.**
