...
Archived section of `RELEASES.md` (v0.40.1 and earlier). Moved here 2026-07-10 to keep
`RELEASES.md` itself lean and under the single-read size limit — this is a MOVE, not a
delete; every version below has its full original content preserved verbatim (a handful of
stale "(In Progress)" labels on long-since-shipped v0.6.0–v0.10.0 headings were corrected
in the move, no other content changed). Condensed, user-facing summaries for the same
versions also live in `docs/releases/vX.Y.Z.md` — those drop implementation detail
(endpoint/column names, invariants, pre-signoff test-gap lists) that this file keeps.
...
- **Post-adopt guidance nudge (frontend).** Reuse `pickActiveGuidance()` to suggest setting a target date after adopting a Goal.
...
- **Target completion date + study intensity backend.** Added nullable `note_collections.target_completion_date` (top-level Goals only, set via `updateMetadata` PATCH-omit-preserves semantics, cleared via dedicated `DELETE /collections/{id}/target-date` since a `LocalDate` has no PATCH-clear sentinel) and nullable `users.study_days_per_week` (`PUT /users/profile/study-days-per-week`, full-replace semantics, 1-7 range validated). Neither field is ever copied on adopt or self-copy. Both new response fields land on the DTOs the weekly countdown derivation reads from (`GoalCollectionDetailResponse` and `MeResponse`).
- **Weekly countdown derivation (backend).** `GET /collections/{id}/goal` now computes `weeksRemaining`, `conceptsRemaining`, and `todaysConceptBudget` — pure derivation from `targetCompletionDate` + `studyDaysPerWeek` + the existing readiness rollup, due concepts as a floor with new-concept pacing filling the rest. Missing `studyDaysPerWeek` defaults to 7 (every day) for the math only; only a missing `targetCompletionDate` hides the countdown. Phase 1 simple ÷ computation only — the weighted largest-remainder subject allocation is Phase 2.
- **Dashboard primary CTA (frontend).** `DashboardStudyPlanSection` now branches on `primaryCollectionId`: when set and found among the user's owned collections, it shows that Goal/plan directly (heading, no course/program subtitle, no "See all" link) instead of the course/program-matched public recommendation, and the CTA always opens the collection directly — never triggers adopt. Falls back to today's course/program flow if the reference isn't found. New `CollectionLabels.primarySingular` field ("Primary Study Plan" / "Primary Review Set" / "Primary Lesson Plan" / "Primary Collection") resolves the heading text profile-aware, per the locked anti-drift rule. Wired into both Dashboard call sites only; the Review Sets list page surface is separate follow-up work.
- **Target-date + intensity input (frontend).** `EditCollectionModal` gained a target-date field and a study-intensity field, shown only for top-level Goals (hidden for child Subject plans). Setting a date goes through the existing metadata PATCH; clearing a previously-set date calls the dedicated `DELETE /collections/{id}/target-date` endpoint instead, since omitting the field would leave it unchanged. Study intensity is sourced/saved separately via `getMe()`/`updateStudyDaysPerWeek` since it's a user-level attribute asked on the same screen, not a collection field. The form now uses `noValidate` so all validation is custom JS (a native `max="7"` constraint was silently blocking submission before the app's own error message could show).
- **Goal detail "This Week" section (frontend).** New `GoalWeeklyCountdownCard` renders above the existing `ReadinessSummary` on the Goal detail page, showing `weeksRemaining`/`conceptsRemaining`/`todaysConceptBudget`; hidden entirely when no target date is set. Editing a Goal's target date now refetches `getCollectionGoal` so the countdown updates immediately rather than going stale until reload. Date display avoids a UTC-parsing day-shift bug via a local-date constructor, verified under a timezone behind UTC. Known scoped limitation: a childless top-level Goal (no Subject plans yet) renders the leaf view, not the Goal view, so the card won't show until at least one child exists — matches the page's existing Goal-vs-leaf branching, not a new gap.
- **Review Sets list page primary CTA (frontend).** The `/collections` list page now wires `primaryCollectionId` into its `DashboardStudyPlanSection` call site, same treatment shipped for Dashboard — the `getMe()` fetch already sourcing `courseProgram` there was renamed `loadProfile` and extended to also read `primaryCollectionId`. Onboarding's call site of the same component is untouched (a new user has no owned collections yet). The Review Sets list page's own empty-state augmentation ("Browse All Official Review Sets") remains separate, later v0.40.1 work.
- **Post-adopt target-date guidance (frontend).** Goal adoption from Dashboard and public plan cards now records a session-only just-adopted flag before routing to the personal Goal. The Goal detail page consumes that flag once and, only when the Goal has no `targetCompletionDate`, shows the existing `GuidanceTip` with a "Set target date" CTA that opens the existing edit modal; leaf-plan adoption, normal dateless Goal visits, and already-dated Goals do not show the tip.
- **Progress primary default (frontend).** `/progress` now resolves `primaryCollectionId` from `GET /auth/me` and, when the URL has no explicit `collectionId`, applies a one-time default to the Primary Review Set's Goal-scoped readiness using `GET /collections/{id}/goal` instead of leaf-plan readiness. Explicit `?collectionId=` selections and the `All subjects` picker option still win after that first resolution, and the picker shows the selected Goal's real title instead of a placeholder.
- **Pre-signoff polish (frontend).** Fixed two real gaps surfaced by a full pre-signoff pressure test of this release: (1) `EditCollectionModal`'s study-intensity field now tracks the value actually loaded from `getMe()` separately from the field's live contents, and only sends `updateStudyDaysPerWeek` when that baseline resolved and the value changed — previously a save-before-prefill race (or a `getMe()` failure) could silently wipe a learner's real intensity to `null`; (2) `DashboardStudyPlanSection` and `PublicStudyPlanCard` now resolve their CTA verb ("Start/Continue this {Goal}") and child-count line ("N {Subject plan}s") through `getCollectionLabels` instead of hardcoded "Goal"/"Subject plan" text, matching the anti-drift rule this release already committed to and the Goal detail page's existing correct behavior; `PublicStudyPlanCard` gained an optional `profileType` prop for this.
- **Target-date reparent leak fix (backend).** `updateParent` now clears `targetCompletionDate` in the same branch that sets a top-level Goal's `parentCollectionId` to non-null — closes an invariant gap where the primary-invariant was reasserted on this exact reparent path but the sibling target-date invariant was not, letting a dated Goal keep a stale date if nested then later detached back to top-level. Surfaced by the full pre-signoff pressure test of this release; verified the new test actually catches the regression by reverting the fix locally first.

### Known limitations (documented, not fixed, at pre-signoff)

Surfaced by a full pre-signoff pressure test (two Explore audits covering every backend/frontend file in this release, plus an independent `advisor` review) — judged low-severity/cosmetic enough to document rather than block signoff on:

- **Progress's stale-primary handling diverges from Dashboard's.** Dashboard falls back silently to the course/program recommendation if `primaryCollectionId` doesn't resolve among owned collections; Progress instead renders a hard "Goal not found" card on a 404. Low severity because deleting a top-level collection already self-heals `primaryCollectionId` to null via `reassertPrimaryInvariant`, so this only matters in a narrow stale-client-cache window.
- The target-date `<input type="date">` in `EditCollectionModal` has no `min`, so a past date is currently selectable.
- `EditCollectionModal`'s `AppModal` description copy ("Update the name or description for this saved set") predates this release's date/intensity fields and hasn't been updated to mention them.
- `dashboard-study-plan-section.tsx` fetches `listCollections()` twice (once for the primary lookup, once for the course/program match) when both a primary and a course/program are set — redundant, not incorrect.
- No test differentiates a non-default `studyDaysPerWeek` (e.g. 3) actually changing the weekly countdown formula's `remainingScheduledDays` term — every existing countdown test uses either 7 or the unset-defaults-to-7 case.
- No boundary test for `conceptsRemaining` at full completion (`masteredConcepts == totalConcepts`) or `remainingDays == 0` (target date = today) as distinct from the already-tested overdue case.
- `adoptGoal`'s primary-invariant reassert runs outside a `@Transactional` boundary (unlike `create`/`delete`/`updateParent`, which reassert inside their transaction) — a small race window vs. a concurrent create, low severity given adopt's idempotency guard.
- `study_days_per_week`'s 1-7 range is enforced only at the DTO/`@Valid` layer, not in `AuthService` or via a DB CHECK constraint — low severity since `computeWeeklyCountdown` floors `remainingScheduledDays` at 1 regardless of input.
...
- **Kill-switch for eager exam-pool pre-warming (backend).** New `studysnap.pricing.exam-pool-prewarm-enabled` flag (default `false`) gates `ExamQuestionPoolService.initiatePoolForMode` — the single choke point used by both the eager (every Study Pack generation) and lazy-refill paths. A real Long/Board Exam start still works when disabled: it falls through to the existing live/on-demand generation path instead of serving from a pre-warmed pool.
- **Goal card misleading "0 notes" (frontend).** The Dashboard "Recommended Study Plan" card showed "N Subject plans · 0 notes" for Goal-type plans, since a Goal never holds notes directly. A true aggregate across children would need a new backend batch query; instead the note count is omitted entirely for Goal cards, matching the pattern `/collections`'s own list already uses.
- **Builder card title truncation (frontend).** The note-row "Move"/"Remove" controls shared one row with the title on `sm:` breakpoints and up, squeezing long titles. Controls now always sit on their own row below the title.
- **"Build" icon collision with "Edit" (frontend).** Both actions resolved to the same `Pencil` icon. Added a distinct `build` icon (`ListTree`) to the shared `ActionIconName` set.
- **Builder sections collapsed by default (frontend).** All Subject Plan sections in the Goal Builder opened expanded, making a plan with 7+ sections × 10+ notes each unworkable to scan. UX-validated (Opus): sections now collapse by default on first load only (later refreshes never reseed, so manual expand/collapse survives edits; a newly created section still opens expanded). Added an Expand-all/Collapse-all control and a distinct "No notes yet" badge on empty sections so collapsing never hides a problem the curator needs to notice.

Anti-drift: no code/logic change to the malloc/metrics items — Dockerfile env var and actuator exposure config only. The exam-pool kill-switch is a config-gated no-op, not new logic — `refreshPool` (refill of an already-existing, actively-served pool) stays ungated, since that's genuine demand-driven work, not speculative pre-warming. OCR was investigated and ruled out as the trigger (its client is already properly closed via try-with-resources, and both triggering requests were topic-based bulk note+Study Pack generation, which doesn't touch OCR). The Study Plan UI fixes are presentation-only — no new endpoint, field, or backend query for the Goal note-count fix (an aggregate was considered and explicitly deferred, not built); the Builder collapse state is local component state only, no new persisted field.
...
- **Kill-switch for eager exam-pool pre-warming (backend).** `ExamQuestionPoolService.initiatePoolForMode` is now gated by `studysnap.pricing.exam-pool-prewarm-enabled` (default `false`). `refreshPool` stays ungated on purpose since it's demand-driven refill, not speculative pre-warming.
- **Goal card misleading "0 notes" fixed (frontend).** `dashboard-study-plan-section.tsx`'s Goal-branch description/detail lines now show only the Subject-plan count, omitting the always-zero direct item count.
- **Builder card layout fixed (frontend).** `SortableNoteCard` and `LeafSortableNoteCard` in the Builder now always stack the Move/Remove controls onto their own row below the title, instead of sharing a row at `sm:` breakpoints and up.
- **"Build" icon fixed (frontend).** Both "Build" entry points on Study Plan detail (`collection-detail-page-client.tsx`) now use a new `build` action icon (`ListTree`) instead of reusing `edit`'s `Pencil`.
- **Builder sections collapsed by default (frontend).** `refreshBuilder` now accepts a `seedCollapsed` option, applied only from the initial `loadBuilder` call — every Subject Plan section starts collapsed on first load, later mutation-triggered refreshes never reseed, and a newly created section (absent from the collapsed set) opens expanded. Added `Expand all`/`Collapse all` buttons and a "No notes yet" badge that replaces the readiness/mastered stats on empty sections.
- **Audit fixes on the Builder collapse feature (frontend).** An independent Opus review of the polish items found: (1) `loadBuilder`'s dependency on `labels.goalSingular` re-fires a second time for TEACHER profiles once auth resolves ("Goal" → "Course"), which would have re-collapsed a section the curator had just expanded in that window — fixed by keying the seed guard to `collectionId` (`seededCollapseForCollectionIdRef`) instead of a plain one-time boolean, so it's robust to the double-fire while still correctly reseeding on navigation to a different Goal plan; (2) the `build` icon (`Hammer`) read as "construction/broken" rather than "organize" — swapped for `ListTree`, which reads as the nested subject→note structure this canvas actually organizes.
...
- **Inline `ReadinessSummary` on leaf plan detail (frontend).** Reuses the existing `GET /collections/{id}/readiness` endpoint and `ProgressReportService` — no new field, endpoint, or mastery signal. Placed between `PlanHeroCard` and the existing execution-progress summary, matching the order Goal detail already uses while keeping the "notes practiced" bar as a distinct signal.
...
- **Pro-gated CTA on the readiness surface (frontend).** Extends the v0.32.1 "surface premium exams as paywall moments" pattern; profile-aware via `exam-mode-visibility.ts` and the existing terminal action (Student -> Long Exam, Board Taker -> Board Exam, Professional -> Interview Practice) — no new plan tier or quota category.
...
- **Quick Review sessions hidden from session-history UI (backend).** Locked decision: Quick Review session rows keep persisting (so `lastSessionCompletedAt`, "Continue where you left off," and practiced-status tracking keep working exactly as today), but completed Quick Review sessions no longer appear in Note Detail's session-history list. This is deliberately *not* full session deletion — that was considered and rejected because it would make any note only ever reviewed via Quick Review show as permanently "not started" across Dashboard and plan detail.

Anti-drift: reuses existing `ConceptHealth` / `ProgressReportService` / `GET /collections/{id}/readiness` and existing premium-exam entitlement checks only. No new mastery signal, no new persisted field, no data migration/backfill, no billing/quota/price change. Flexible review methods (Flashcards/Identification/Enumeration/Memorization) and real spaced-repetition Memorization are scoped separately in `ROADMAP.md`'s v0.38.0 candidate section.

### Shipped

- **Inline leaf-plan readiness and mastery CTAs (frontend).** Leaf Study Plan detail now lazy-loads the existing plan readiness endpoint and renders the shared compact `ReadinessSummary` between the hero and the execution-progress card. The "Review due concepts" CTA appears only for entitled users with due concepts and a concrete note route, while the readiness-surface premium CTA reuses the existing terminal action and premium-exam launch handler instead of duplicating gating.
...
- **Quick Review excluded from the Note Detail session-history list (backend).** `QuizSessionHistoryService.listRecentSessions` now filters out `QUICK_REVIEW`-mode sessions before building the visible "Recent Sessions" list. The underlying rows are untouched: `findLatestSessionCompletedAtByNoteIds` (feeding `lastSessionCompletedAt`, "Continue where you left off," and practiced-status everywhere) still includes Quick Review sessions, and the dedicated session-review page remains reachable by direct URL. The Dashboard's "Strongest Notes"/"Top Performance" widgets (a separate `note-performance` endpoint that intentionally includes Quick Review for best-score highlighting) are unchanged — out of scope for this change.
...
- **Progress navigation continuity (frontend).** Keep the nav label as "Progress" while the page gains plan-scoped readiness.
...
Theme: v0.32.1 surfaced the premium exams and reframed pricing, but conversion was still ~0 of ~153 verified users. The funnel diagnosis ran first and **re-scoped this release**: the real, un-conflicted constraint is **near-zero W1→W2 retention (5.6%, recent cohorts ~0%)** — users activate (68.6%) and engage once (58.8%), then don't return. (An initial read flagged a "broken checkout" from 6 upgrade clicks → 0 `CHECKOUT_INITIATED`; that was a **metric-inception artifact** — `CHECKOUT_INITIATED` was added in v0.31.2 while `UPGRADE_CLICKED` is far older, and a live upgrade reached the real Xendit invoice. Checkout works.) Free is not too generous. **Anti-drift: do not raise exam quota numbers** — quota size is not the constraint; retention is. Full data in `docs/archive/conversion-funnel-finding.md`; see `docs/product/ROADMAP.md` for sequencing.
...
- **Conversion funnel diagnosis** *(done)* — read prod `/admin/funnel`; finding written to `docs/archive/conversion-funnel-finding.md`. Corrected reading: checkout works; retention is the constraint. Drives the priorities below.
