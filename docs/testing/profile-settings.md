...
Verify authenticated navigation labels:

- avatar dropdown shows `My Profile`, `Settings`, and `Sign Out`
- `My Profile` links to `/public/profile/{userId}`
- `Settings` links to `/settings`
- `/profile` remains the private editing surface and is not labeled `Account Settings` in navigation
