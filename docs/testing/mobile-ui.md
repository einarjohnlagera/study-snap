...
- safe mobile pages without sticky bottom CTAs, such as Dashboard, Library, Public Library, and Settings, still show the floating `Send Feedback` launcher
- quiz result and `Review Answers` screens use an inline feedback panel instead of the floating launcher
- quiz result screens show the lightweight `Was this quiz helpful?` prompt with `Yes` and `Give Feedback`
