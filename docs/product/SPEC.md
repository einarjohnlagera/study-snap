...
- clicking `Export` should generate a PDF directly without a modal in v1
- export feedback should stay non-blocking:
  - while generating -> `Exporting PDF...`
  - after success -> `PDF ready`
- exported quiz PDFs must use stored session-review data only:
  - note title
  - quiz type
  - generated date
  - score summary
...
- default filename format:
  - `notelib-quiz-{note-title}-{yyyy-mm-dd}.pdf`
