...
- **Existing-user backfill — DECISION PENDING (compliance).** Signup always wrote `false`, so the column can't distinguish "default off" from "deliberate opt-out"; a blind backfill to `true` could re-enable people who opted out. Scope to users with no email-preference change, or get explicit sign-off — before any migration runs. Out of the Codex prompt until decided.
