...
| Profile-type population mix across the full user base (STUDENT vs. BOARD_EXAM vs. other `ProfileType` values — not just the surge cohort) | Same reprioritization discussion, 2026-07-28 — needed to check whether "Review Set adoption is displacing note-authoring as the primary populate-path" holds product-wide or only for the `BOARD_EXAM`-only practice-first fast-path | **Active — cheap, open.** The 6.9%/58.6% STUDENT/BOARD_EXAM split known today (`13-item4-profiletype-surge-check.sql`) is the surge cohort only, not the base. Practice-first adoption is `BOARD_EXAM`-only (Phase 1 gate) — if `STUDENT` is actually the largest segment overall, most learners still populate their journey by authoring notes, not adopting Review Sets, and "Review Sets are becoming the real product" needs that caveat attached | none — one query against the existing user/profile table | 2026-07-28 |
