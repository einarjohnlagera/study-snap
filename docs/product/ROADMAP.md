...
`v0.29.1 - Bulk Generation Polish` is the release currently in progress.
...
## v0.29.1 - Bulk Generation Polish (in progress)
...
- **Partial-outcome reporting for bulk generation.** Shipped as a bounded terminal-outcome receipt: `POST /notes/bulk-generate` returns a `resultId`, the worker writes one owner-scoped `bulk_generation_result` receipt at batch completion with requested/created counts and exact failed topic strings, and `GET /notes/bulk-generate/results/{id}` returns then deletes it. The Library keeps the immediate `Queued N notes` toast, waits for the existing auto-refresh poller to settle, then reads the receipt and shows a dismissible banner only when content-generation failures occurred. `Retry these` pre-fills `/library/bulk-generate` with the failed topics plus the original subject, course/program, target audience, and public toggle. This is the one v0.29.1 relaxation of the v0.29.0 no-progress-infrastructure rule: it is terminal, write-once/read-once, and expires after 24h; it is not a batch-job entity, live progress table, per-item status row, or new status enum.

- **Open Bulk Generation to all users (gate-flip + quota-aware failure UX + discoverability).** Bulk Generation was built admin-first but architected as a removable role gate; this flips it on for everyone. It is **not** a one-line ungating — the honest version bundles three things, and the gate-flip must not ship without the quota UX:
  - **Gate-flip.** Remove the ADMIN-only gates on the Library Create entry, the `/library/bulk-generate` route guard, and the `POST /notes/bulk-generate` / `GET .../results/{id}` endpoints, so all authenticated users reach the flow. The non-admin path already enforces per-user quota (`enforceLimits = role != ADMIN`); ADMIN still bypasses. Keep the profile-aware field resolution (non-teachers get course/program + target audience from their profile) that already exists behind the gate.
  - **Quota-aware failure UX (make-or-break).** Today every failed topic in the receipt banner reads "couldn't be generated — try again," with a `Retry these` button. For real users that mislabels the dominant new failure mode — **quota exhaustion mid-batch** — and loops them back into the same wall. The receipt must carry a per-failure (or batch-level) **reason** distinguishing `QUOTA_EXCEEDED` from `GENERATION_FAILED`; the banner branches: quota → an upgrade message via `getUpgradeCtas(currentPlan)` (never hardcoded copy), generation-failure → the existing "try again" + retry. Surface **remaining note-generation quota on the Bulk page** so users don't fire 30 topics into a 10-credit wall blindly. Reconcile the two quota surfaces: note-gen quota exhaustion produces no row (a failed topic), while Study-Pack-gen quota exhaustion produces a row with a FAILED Study Pack badge — the messaging should not treat these as unrelated. This delivers the "quota ran out mid-batch" partial-execution messaging deferred since v0.29.0.
  - **Discoverability.** Add a persistent, low-key inline link on the single Note Create "Generate from topic" surface — value-first wording ("Have a list of topics? Generate them all at once →"), a navigational affordance (not a one-time `pickActiveGuidance` tip), linking to `/library/bulk-generate`. Strengthen the in-page intro on the Bulk page (what it does, that notes generate in the background and appear in the Library as they finish) since the audience is no longer admins. Add a Help entry as the reference/support backstop (consistent with the v0.28.0 Study Plans guide) — secondary to the in-page explanation, not a substitute.
  - **Out of scope:** changing quota *limits* or plan tiers; a teacher-specific bulk flow; bulk *quiz* generation (v0.31.0).
