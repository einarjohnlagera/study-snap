...
| Knowledge Impact (creator-recognition dashboard — "your notes helped N learners") | `docs/claude-prompt/company-redefinition-out/09-knowledge-impact.md`, sourced from a GPT-authored roadmap-addition proposal relayed 2026-07-25 | **Parked — data-gated, not a veto. Gate query run 2026-07-28 — fails, more concentrated than the original inference, not less.** `docs/claude-prompt/company-redefinition-out/14-knowledge-impact-gate-check.sql` (using the wider Java-level official-author definition — email-or-admin-role — not the narrower SQL predicate, see that file's own inconsistency note): 3 distinct non-official public-note creators (threshold is order-of-magnitude 20-30+); 697 official public notes vs. 4 community (99.4% official — up from the ~232:3/98.7% CTO-evaluation estimate, the ratio has gotten *more* concentrated since, not less); views/copies split 12,835/2,994 official vs. 231/2 community (community gets ~1.8% of views, ~0.07% of copies); monthly community-publish rate 1 (2026-05), 3 (2026-06), **zero (2026-07)** — a stalled rate, not a rising one. Both halves of the un-park threshold fail. What was a code-level inference in the original CTO evaluation is now a measured fact pointing the same direction, harder than before. The strategic-fork framing and fair-counter point from that evaluation still stand as-is, unaffected by this result. Separately, confirmed a real inconsistency while writing this query: `PublicLibraryRepositoryImpl`'s SQL-level official-author predicate checks admin-role only, missing the email-based check `PublicProfileService.isOfficialAuthor` (Java) uses — the public library's own OFFICIAL/COMMUNITY filter likely mis-buckets the owner's personally-authored public notes as community today. Not fixed here, flagged for its own small fix | re-run `14-knowledge-impact-gate-check.sql` after meaningful time/growth has passed (no fixed re-check date, unlike Item 4's Diagnostic-Read-anchored gate) — un-parks only if distinct non-official creators reach order-of-magnitude 20-30+ and/or the monthly community-publish rate shows sustained (not one-month) upward movement | 2026-07-28 |
