...
| Exam Hub page card inconsistency (`/exam/{slug}`) — `ExamNoteCard` is a near-identical copy of the subject-landing page's pre-fix bespoke card (same `summaryPreview \|\| contentPreview` priority, apparently copy-pasted before the subject-landing page was migrated in v0.50.2), not covered by the note-preview-vs-summary Fable session's four named surfaces | `RELEASES.md` v0.50.2 "Known, explicitly out of scope" | Not yet scoped | needs its own pass — migrate to `SharedNoteCard`'s single-excerpt cascade, same as the subject-landing page fix | 2026-07-16 |
