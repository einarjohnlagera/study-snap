...
`v0.60.3 - Challenge Quiz Shaping & Onboarding Coverage Capture` is the current in-progress release (on `releases/v0.60.3`, cut from `main` after v0.60.2 merged, and after the pre-kickoff scoping/second-opinion work on `docs/v0.60.3-challenge-quiz-shaping-scoping` merged the same way) — kicked off 2026-07-28. Three of its four items (adaptive initial question count, the Redo Missed Questions session-matching fix, the incomplete-submission guard) are clear to implement immediately. The fourth (onboarding coverage-gap capture) is gated at its own kickoff, not the release's: it needs a prod query confirming effectively-zero `STUDENT`-profile presence in the Diagnostic Read's signup-surge measurement cohort before it can start, to avoid contaminating that read's funnel data mid-measurement — owner-run, requires prod `DB_USER` access. See `RELEASES.md` v0.60.3 for Planned Scope, and this doc's own "v0.60.3 — Challenge Quiz Shaping & Onboarding Coverage Capture" section below for full technical detail, including the second-opinion review that shaped the final Item 2 and Item 4 designs. Third patch release off `v0.60.0`'s line, so it does not consume a minor-version slot — `v0.61.0 - Explore Convergence` remains the next minor-version slot, still gated on the Diagnostic Read re-read (due after 2026-08-06).
