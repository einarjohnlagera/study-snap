...
- **Release chunks:** `v0.60.0 — Shared Official Pool Foundation` (3a only, no review-queue dependency, ships standalone) is kicked off (In Progress) — see its section below; a later TBD release for 3b once the review-queue question is resolved; the Reviewer relabel can ship independently in either chunk and has not been assigned to either yet.
...
A new `resolvePoolKey(studyPackId)` step inside `ExamQuestionPoolService` resolves the pool to the Official source's `studyPackId` when the caller's note is Official or a one-hop copy, falling through to per-owner keying otherwise (derived on read, never persisted). Served-question tracking moves to a new per-user child table (`exam_question_pool_progress`) so adopters don't bleed into each other's served-question history; `learnerLevel`-triggered auto-refresh drops for Official pools. Private per-owner pools are fully unchanged. See `docs/claude-prompt/company-redefinition-out/04-reusable-assets-and-reviewer.md` for the full bounded-object-model design; the Reviewer label-only relabel is not assigned to this release and remains a separate, independently-shippable decision.
