...
- **Review Sets list page primary CTA (frontend).** `/collections` wires `primaryCollectionId` into its `DashboardStudyPlanSection` call site, same treatment as Dashboard. Onboarding's call site is untouched (no owned collections yet during onboarding). Remaining frontend work: Progress default-view change. (The list page's own empty-state augmentation is separate v0.40.1 scope.)
