...
`v0.60.0 - Shared Official Pool Foundation` is the current in-progress version (on `releases/v0.60.0`, cut from `main` after v0.59.0 merged) — Phase 3a (cross-user question pool foundation), ratified and kicked off 2026-07-24 after production adoption-concurrency data cleared its proposed gate (see "Company Redefinition Roadmap — Phase Detail" below, Phase 3 section, for the evidence and full scope). **Retargeted 2026-07-24, same day, from the originally-scoped exam-pool (`ExamQuestionPoolService`) design to Challenge Quiz Official template sharing** — the gate evidence's adopters are FREE-tier and Long/Board Exam pools are PRO-gated with ~zero production load; see `RELEASES.md` v0.60.0 for the retargeted scope and the diagnostic trail. **Reclaimed the `v0.60.0` slot from `Explore Convergence`, same day (2026-07-24), because this chunk was ratified and kicked off on real data while Explore's gate remains unmet** — Explore Convergence renumbers to `v0.61.0`. This is **not** the Explore Convergence chunk, which stays un-kicked-off pending the Diagnostic Read re-read — Phase 3a's ratification is unrelated to and does not accelerate that gate.
...
**Status: 3a ratified and kicked off 2026-07-24 as `v0.60.0` (reclaimed from Explore Convergence, which renumbers to `v0.61.0`) — its proposed gate cleared on real adoption data, not on "we already do per-user pooling."** 3b stays drafted, not ratified, parked on its own unrelated review-queue dependency (see below). Source: `company-redefinition-out/04-reusable-assets-and-reviewer.md`.
...
- **Release chunks:** `v0.60.0 — Shared Official Pool Foundation` (3a only, retargeted to Challenge Quiz Official template sharing, no review-queue dependency, ships standalone) is kicked off (In Progress) — see its section below; a later TBD release for 3b once the review-queue question is resolved; the Reviewer relabel can ship independently in either chunk and has not been assigned to either yet.
...
## v0.60.0 - Shared Official Pool Foundation (In Progress, base branch `releases/v0.60.0`)
