...
| Challenge Quiz daily quota (3/10/20 FREE/PLUS/PRO) + a price decrease, both raised 2026-07-27 alongside the item above | Same conversation as the row above — a broader owner proposal to loosen Challenge Quiz quota to a daily model and lower Plus/Pro pricing to "entice" more paying users, with the cost story resting on getting users to share/publish their own notes so pooling covers more content | **Ratified by owner 2026-07-28 — proceeding.** This row's own Gate (right) already named "an explicit owner decision to proceed without one" as a valid unlock path alongside a measured coverage lift — that's what happened here, not a bypass of the stated bar. Owner's reasoning: months of flat retention with no lever having moved it, wants to try retention/engagement levers rather than keep waiting. **Scope note, not a blocker:** the underlying operational gap this row originally flagged — zero token/cost telemetry on the Challenge Quiz generation path, so a cost blowup is only visible after the invoice — is still real and still worth closing. `StudyPackEntity` already has `modelUsed`/`inputTokens`/`outputTokens`/`cachedInputTokens` columns (written today by Study Pack generation); Challenge Quiz's generation path just never writes them. Folding that capture into this item's own scope (not a precondition before starting) makes the quota change's actual effect observable rather than blind — bring it into the kickoff scoping as a small addition, not a separate gate. Price-decrease half: this product's prior finding that pricing isn't the conversion lever ("Free quota is essentially never hit") is unrelated to the retention argument driving this ratification — carry that context into scoping so the price change is evaluated on its own terms, not assumed to solve retention by association | **Not kicked off — needs scoping**, including whether the price-decrease half ships together with the quota change or separately, and the token-telemetry addition above. Anti-drift: plan-tier copy is centralized (`frontend/src/config/plans.ts`'s `getUpgradeCtas`, `docs/features/pricing.md`) — do not let a quota/price change leave stale numbers in either | 2026-07-28 |
