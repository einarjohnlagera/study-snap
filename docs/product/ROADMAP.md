...
`v0.60.1 - Challenge Quiz Fix Pass` is the most recently released version (on `releases/v0.60.1`, cut from `main` after v0.60.0 merged) — a 5-item bug-fix pass on Challenge Quiz, discovered while exercising v0.60.0's Official template sharing in production: an executor-saturation bug blocking the backfill (fixed by moving seed dispatch off the live-generation executor), a never-implemented whole-array shuffle (fixed, MATCHING-block-aware), a silently-inert manual difficulty selector (removed entirely rather than fixed, per product decision), abandoned sessions silently resumed and auto-submitted as timed-out (fixed via server-side expiry-aware forfeit plus a ported Resume/Start Fresh prompt), and a non-functional "Redo Missed Questions" button (fixed — same-route retrigger bug). Kicked off and shipped same-day, 2026-07-25; a same-day pre-signoff pressure test found and fixed three additional refinements (TEACHER-profile resume prompt, redo-missed retrigger guard reset, `generateMoreQuestions` expiry handling) and logged two narrow known limitations plus one scoped entry-point gap — see `RELEASES.md` v0.60.1 for full Shipped detail and the Known Limitations section, and its own "v0.60.1 — Challenge Quiz Fix Pass" section below for the original technical plan. Patch release off `v0.60.0`, so it did not consume a minor-version slot. **`v0.60.2 - Challenge Quiz Known-Limitations Cleanup` was kicked off 2026-07-26 (on `releases/v0.60.2`, cut from `main`) and is now In Progress** — a 3-item cleanup of narrow, ungated concurrency and entry-point gaps logged as v0.60.1's own Known Limitations: a claim-release rollback that can leave bank rows stuck claimed, an expiry-vs-completion race that can silently overwrite a completed session as forfeited, and a missing Resume/Start Fresh prompt at the collection premium-exam entry point. Full technical plan is written out in its own section below, "v0.60.2 — Challenge Quiz Known-Limitations Cleanup." `v0.61.0 - Explore Convergence` remains the next minor-version slot after that, still gated on the Diagnostic Read re-read (due after 2026-08-06) — v0.60.2 does not displace it, since a second patch release doesn't consume a minor-version slot either.
...
| **Challenge Quiz Known-Limitations Cleanup — 3 ungated items logged in v0.60.1** (claim-release rollback under `@Transactional`, expiry-vs-completion lock race, Resume/Start Fresh missing at the collection premium-exam entry point) | Full plan (root causes, verified fix designs, file:line anchors) written directly into this doc's own `## v0.60.2 — Challenge Quiz Known-Limitations Cleanup` section above, verified by direct file read (not inferred) in the same session that kicked it off — no product or data gate, unlike every other Backlog Index candidate reviewed the same day | **Kicked off 2026-07-26, In Progress** on `releases/v0.60.2`, handed to Codex per a deliberate Claude-token-conservation choice rather than the routing table's normal "1–3 files, Claude Code direct" lane for isolated bug fixes — audit the diff before commit, per this project's standard Codex-delivery process, with particular attention to the lock-race fix's recheck-after-lock step | none — in progress | 2026-07-26 |
| Challenge Quiz default question count — start sessions with more than 5 questions, mode-wide (not gated on Official vs. private content) | Follow-up idea raised during `v0.60.0`'s Official Challenge Quiz template sharing work (2026-07-24); independently reviewed by Opus and Fable, both converging on the same verdict as the owner/Claude Code discussion | Parked — rejected as scoped to Official content only. "It's free now for Official content, so why not front-load it" was not accepted as sufficient justification: the template mechanism already captured all available cost savings (the `+5 Questions` path was already unmetered), so front-loading unlocks no further cost benefit — it's a pacing/engagement change wearing a cost-optimization justification. The decisive objection was the provenance-dependent inconsistency it would introduce (adopted content starting at 15-20 questions, a user's own content still starting at 5, in the same mode) | an explicit product decision that Challenge Quiz's default/progressive question count should change for all content, argued on its own UX merits (session pacing, commitment, engagement) — not "because it's now free for some content" | 2026-07-24 |
...
## v0.60.2 — Challenge Quiz Known-Limitations Cleanup (In Progress, base branch `releases/v0.60.2`)
...
### Item 1 — Claim-release rollback under `@Transactional`
...
### Item 2 — Expiry-vs-completion lock race
...
### Item 3 — Resume/Start Fresh missing at the collection premium-exam entry point
