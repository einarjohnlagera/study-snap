...
`v0.47.0 - Conversion Audit Tier 4: Cleanup Batch` is in progress (on `releases/v0.47.0`, cut from `main` after v0.46.0 merged) — 15 low-impact, cheap-cleanup items from the conversion/retention UX audit backlog's Tier 4, grouped into 5 Codex-prompted PRs (Landing & Pricing, Public Note Detail, Discovery & Library, Onboarding copy, Doc hygiene). No new backend entities, migrations, or endpoints. See `RELEASES.md`.
