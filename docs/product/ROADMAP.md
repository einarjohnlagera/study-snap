...
`v0.61.0 - Challenge Quiz Quota Increase` (on `releases/v0.61.0`, cut from `main` after v0.60.3 merged) is the current in-progress release — kicked off 2026-07-28. Reclaims the `v0.61.0` minor-version slot from `Explore Convergence` (renumbered to `v0.62.0` — see the Phase 2 section and dependency spine below), since this item's gate (owner ratification) cleared 2026-07-28 while Explore's gate (the Diagnostic Read showing a discovery problem) remains unmet, following this doc's own established precedent for reclaiming a slot when a different item's gate clears first (`v0.58.0`, `v0.59.0`, and `v0.60.0` each changed hands the same way). Two items: a substantial Challenge Quiz monthly quota increase (FREE 5→20, PLUS 25→100, PRO 50→200) and Challenge Quiz LLM token/cost telemetry, folded into the same release so the quota change's effect on spend is observable. Both ratified by the owner 2026-07-28, overriding this doc's own prior data/instrumentation gates — see the "Challenge Quiz quota increase" Backlog Index row for the full ratification history, and this doc's own "v0.61.0 — Challenge Quiz Quota Increase" section below for full technical detail. A price decrease (the other half of the original held proposal) stays explicitly deferred, not part of this release.
...
| **Target-habit definition** (exam-date-relative return cadence vs. the current calendar-week W1→W2 boolean) | This session, 2026-07-28 — surfaced during a strategic reprioritization discussion (see "Prioritization Lens & Strategic Frame" above and the updated "Product Learning Loop" section at the bottom of this doc) | **Active — do now, before the Diagnostic Read and H1+H5 re-reads land (~2026-08-06).** W1→W2 is a calendar-week boolean; the dominant exam-dated segment already retains at 0/41 *below their own exam date*, which is itself a hint the calendar-week frame may be the wrong shape for a lifecycle that's naturally bounded by an exam date rather than open-ended. Define the actual target habit (e.g. an exam-date-relative return cadence) reusing what's already built — `studyDaysPerWeek`, `target_completion_date`, the weekly countdown, readiness — no new engineering. Do this before interpreting either upcoming read, so the same numbers aren't read twice under two different implicit definitions of "retained" | none — definitional exercise, no engineering, no data dependency | 2026-07-28 |
...
- **Read this against an explicit target-habit definition, not the raw W1→W2 boolean alone** — see the "Target-habit definition" Backlog Index row above. The same underlying numbers can look like failure or success depending on whether the yardstick is calendar-week return or exam-date-relative persistence.
...
## v0.61.0 — Challenge Quiz Quota Increase (Scoped, not yet kicked off)
