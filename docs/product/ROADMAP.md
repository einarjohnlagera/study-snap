...
`v0.54.1 - Public Note Copy Correctness Fixes` is the current in-progress version (on `releases/v0.54.1`, cut from `main` after v0.54.0 merged) — see its section below.
...
| SEO strategy remainder (P1 GSC setup, P3 exam-named Learn guides, P4 hub subject breakdown + full-inventory path, P5 ItemList JSON-LD, P6 organic-referrer analytics) | `seo-strategy-out/01-seo-strategy.md` | **P4/P5/P6 scoped into v0.53.0** — decided directly, ahead of P1's originally-suggested GSC-data-first sequencing, since the product priority shifted explicitly to new-user acquisition (not re-engaging past churned users) and P6 is itself the internal-analytics substitute for GSC in the near term. P1 remains a non-code ops task (needs DNS/domain access); P3 needs human-verified exam-subject content, not to be fabricated — neither is part of v0.53.0 | P1: domain access, still open. P3: a curator who can write accurately about PNLE/LET/ALE structure, still open | 2026-07-21 |
...
| Wave 2 Exam Hub candidate: CPALE (Accountancy) — the N2 depth inventory found ~100+ notes across Accountancy-adjacent subjects (accounting, auditing, taxation, financial management) not covered by any current Exam Hub | `public-library-seo-expansion-out/02-subject-depth-inventory.sql` query 1 results | **Scoped into v0.54.0.** Depth gate confirmed against production (`next-priority-new-user-focus-out/02-h1-h5-cohort-recheck-and-cpale-depth.sql` Query 2) — crosses the ~25-30 note Wave-2 bar | none, in progress | 2026-07-21 |
| Public-note copy idempotency guard never backfills a stale copy | investigated during v0.50.3 signoff follow-up, no session doc | **Scoped into v0.54.1.** `NoteService.copyNote()`'s existing-copy branch returns a prior copy as-is even if it predates the source's Study Pack becoming ready — a user re-copying after their first copy landed draft keeps getting the stale draft forever. Only reachable if a user copies before the source pack exists, then retries later. Fix: if the existing copy has no pack and the source now has a ready one, backfill it instead of returning as-is | none, in progress | 2026-07-21 |
...
## v0.54.1 - Public Note Copy Correctness Fixes (In Progress, base branch `releases/v0.54.1`)
