...
- **Review Sets list page primary CTA (frontend).** `/collections` wires `primaryCollectionId` into its `DashboardStudyPlanSection` call site, same treatment as Dashboard. Onboarding's call site is untouched (no owned collections yet during onboarding). Remaining frontend work: Progress default-view change, post-adopt nudge. (The list page's own empty-state augmentation is separate v0.40.1 scope.)
...
- **Manual "Set as primary" UI.** The backend `PUT`/`DELETE /collections/{id}/primary` endpoints exist, but no frontend button/menu action calls them — `primaryCollectionId` is fully auto-set today (set on a learner's first owned top-level Goal, never touched again automatically). This only matters for a learner with two or more top-level Goals who wants to switch which one is primary; there's currently no way to do that. Surfaced while scoping the post-adopt guidance nudge — the nudge itself ships target-date-only and does not build this. Candidate for its own small follow-up item (a Goal-detail action + the two API wrappers) if multi-Goal usage shows this is a real friction point.
