...
`v0.60.3 - Challenge Quiz Shaping & Onboarding Coverage Capture` is the current in-progress release (on `releases/v0.60.3`, cut from `main` after v0.60.2 merged, and after the pre-kickoff scoping/second-opinion work on `docs/v0.60.3-challenge-quiz-shaping-scoping` merged the same way) — kicked off 2026-07-28. **Three of its four items (adaptive initial question count, the Redo Missed Questions session-matching fix, the incomplete-submission guard) shipped 2026-07-28 (PR #918, routed to Codex as a bundled prompt/single-PR, audited before commit).** The fourth (onboarding coverage-gap capture) is gated at its own kickoff, not the release's: it needed a prod query confirming effectively-zero `STUDENT`-profile presence in the Diagnostic Read's signup-surge measurement cohort before it could start, to avoid contaminating that read's funnel data mid-measurement — owner-run, requires prod `DB_USER` access. **Query run 2026-07-28 — fails the bar** (surge day n=29: `STUDENT` 2/6.9%, not effectively zero; see Item 4's own section below for full results). Item 4 stays parked until the Diagnostic Read closes (~2026-08-06). See `RELEASES.md` v0.60.3 for Planned/Shipped Scope, and this doc's own "v0.60.3 — Challenge Quiz Shaping & Onboarding Coverage Capture" section below for full technical detail, including the second-opinion review that shaped the final Item 2 and Item 4 designs. Third patch release off `v0.60.0`'s line, so it does not consume a minor-version slot — `v0.61.0 - Explore Convergence` remains the next minor-version slot, still gated on the Diagnostic Read re-read (due after 2026-08-06).
...
| **Challenge Quiz Shaping & Onboarding Coverage Capture — adaptive question count, Redo Missed Questions fix, incomplete-submission guard, onboarding coverage-gap capture** (`v0.60.3`, kicked off 2026-07-28) | Question-count idea originally raised during `v0.60.0`'s work (2026-07-24), rejected then (see superseded verdict below); **reopened 2026-07-27** on a different, accepted argument (session pacing / "+5" click friction, not cost) — mode-wide scope resolves the original provenance objection. Redo Missed Questions bug, the submit-time guard, and the onboarding item all surfaced the same day — the first two from direct user testing, the onboarding item as the one piece worth keeping from a broader pricing/quota/growth discussion (see the held-items row directly below). Full plan written directly into this doc's own `## v0.60.3 — Challenge Quiz Shaping & Onboarding Coverage Capture` section above, verified by direct file read. **Independent second opinion run 2026-07-27** via `docs/claude-prompt/v0.60.3-challenge-quiz-shaping-session-plan.md` (GPT, reconciled same day with an Opus follow-up — see `v0.60.3-challenge-quiz-shaping-out/01-second-opinion.md`); Item 2's provenance-signal design and Item 4's sequencing gate both revised and finalized as a result, Items 1 and 3 confirmed clean, no third round | **Kicked off 2026-07-28. Items 1-3 shipped 2026-07-28 (PR #918).** Superseded verdict, kept for history: originally Parked/rejected 2026-07-24 as scoped to Official content only — "it's free now for Official content, so why not front-load it" was rejected as a cost argument wearing a pacing decision's clothes, and the decisive objection was provenance-dependent inconsistency (adopted content starting at 15-20 questions, private content still at 5, in the same mode). That objection doesn't apply to the reopened version: mode-wide, argued on pacing/commitment/click-friction merits, not cost. Item 4 (onboarding) needs a prod query confirming effectively-zero `STUDENT`-profile presence in the Diagnostic Read's measurement cohort before its own kickoff — **query run 2026-07-28, fails the bar** (surge-day STUDENT presence 2/29, 6.9%) — see Item 4's own sequencing note | Item 4's kickoff gate query ran 2026-07-28 (`13-item4-profiletype-surge-check.sql`): surge day (n=29) `BOARD_EXAM` 17/58.6%, no `profile_type` yet 10/34.5%, `STUDENT` 2/6.9% — fails the effectively-zero bar, not a pass on "skews Board Exam." Wider post-surge window (n=73) still has 20 (27.4%) with no `profile_type` set, so the true STUDENT share may be higher once those resolve. Item 4 stays parked until the Diagnostic Read closes (~2026-08-06); re-run the gate query at that point rather than reusing this result. Items 1-3 shipped, no remaining blockers | 2026-07-28 |
...
## v0.60.3 — Challenge Quiz Shaping & Onboarding Coverage Capture (In Progress, base branch `releases/v0.60.3`)
