...
| Subject-landing page card inconsistency (`/public/library/{subject}`) — a third, independent bespoke card implementation discovered mid-v0.50.2-implementation, using `summaryPreview \|\| contentPreview` (the opposite priority order from the documented card-content rule), not covered by the note-preview-vs-summary Fable session's four named surfaces | `RELEASES.md` v0.50.2 "Known, explicitly out of scope" | Not yet scoped | needs its own pass — decide whether to migrate to `SharedNoteCard`'s single-excerpt cascade like the other four surfaces, or is a deliberate exception | 2026-07-16 |
