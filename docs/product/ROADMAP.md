...
- **Target-date + intensity input (frontend).** `EditCollectionModal` gained the target-date and study-intensity fields, shown only for top-level Goals. Target date shares the metadata PATCH to set, uses the dedicated clear endpoint to unset. Study intensity is sourced/saved separately (`getMe()`/`updateStudyDaysPerWeek`) since it's user-level, not a collection field. Remaining frontend work: Goal detail "This Week" section, Review Sets list page, Progress default-view change, post-adopt nudge.
