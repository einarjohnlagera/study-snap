...
| **Challenge Quiz Shaping & Onboarding Coverage Capture — adaptive question count, Redo Missed Questions fix, incomplete-submission guard, onboarding coverage-gap capture** (candidate `v0.60.3`, not yet kicked off) | Question-count idea originally raised during `v0.60.0`'s work (2026-07-24), rejected then (see superseded verdict below); **reopened 2026-07-27** on a different, accepted argument (session pacing / "+5" click friction, not cost) — mode-wide scope resolves the original provenance objection. Redo Missed Questions bug, the submit-time guard, and the onboarding item all surfaced the same day — the first two from direct user testing, the onboarding item as the one piece worth keeping from a broader pricing/quota/growth discussion (see the held-items row directly below). Full plan written directly into this doc's own `## v0.60.3 — Challenge Quiz Shaping & Onboarding Coverage Capture` section above, verified by direct file read. **Independent second opinion requested 2026-07-27** via `docs/claude-prompt/v0.60.3-challenge-quiz-shaping-session-plan.md` (GPT, not yet run — output goes to `v0.60.3-challenge-quiz-shaping-out/01-second-opinion.md`) | **Candidate, fully scoped, not yet kicked off.** Superseded verdict, kept for history: originally Parked/rejected 2026-07-24 as scoped to Official content only — "it's free now for Official content, so why not front-load it" was rejected as a cost argument wearing a pacing decision's clothes, and the decisive objection was provenance-dependent inconsistency (adopted content starting at 15-20 questions, private content still at 5, in the same mode). That objection doesn't apply to the reopened version: mode-wide, argued on pacing/commitment/click-friction merits, not cost. Item 4 (onboarding) additionally needs an explicit go/no-go on Diagnostic Read funnel-timing overlap before kickoff — see Item 4's own sequencing note | none — scoped and ready; awaiting kickoff decision (Item 4 also awaiting a funnel-timing go/no-go) | 2026-07-27 |
| Challenge Quiz daily quota (3/10/20 FREE/PLUS/PRO) + a price decrease, both raised 2026-07-27 alongside the item above | Same conversation as the row above — a broader owner proposal to loosen Challenge Quiz quota to a daily model and lower Plus/Pro pricing to "entice" more paying users, with the cost story resting on getting users to share/publish their own notes so pooling covers more content | **Held, not rejected — pending LLM cost instrumentation that does not exist today.** Verified: zero token/cost telemetry anywhere in the Challenge Quiz generation path; the one adjacent `StudyPackEntity.estimatedCost` field is write-only, never aggregated or surfaced. A 12-18x monthly-volume-ceiling increase (daily × 30) on the non-paying tier, stacked on top of the not-yet-shipped Item 1 adaptive count above, with no way to detect a cost blowup except the eventual invoice — "no harm in trying" does not hold under those conditions. The proposed cost offset ("everyone shares their notes") is the same unproven community-authorship flywheel Knowledge Impact is gated on (~3 community creators ever, ~232 official notes) — sequencing should be sharing-mechanism → measured coverage lift → then revisit quota, not the reverse. Separately, the price-decrease half runs against this product's own prior finding: two independent Fable sessions already ruled pricing out as the conversion lever, citing "Free quota is essentially never hit" — the same fact re-surfaced in this discussion. Plus/Pro are one-time time-boxed passes, not subscriptions, so "small income from plenty of users" doesn't compound the way the subscription-business framing assumes | LLM cost/token telemetry exists per-generation (at minimum for Challenge Quiz) AND either a measured community-content coverage lift, or an explicit owner decision to proceed without one | 2026-07-27 |
| Knowledge Impact (creator-recognition dashboard — "your notes helped N learners") | `docs/claude-prompt/company-redefinition-out/09-knowledge-impact.md`, sourced from a GPT-authored roadmap-addition proposal relayed 2026-07-25 | **Parked — data-gated, not a veto.** CTO evaluation (not a Fable session) verified the proposal's premise against production code: ~232 of ~235 public notes are official/admin-curated, only ~3 notes ever published by non-official users — the cited 12,211 views/2,094 copies almost certainly land overwhelmingly on official content, not community creators (a safe inference from the 232:3 ratio + rich-get-richer discovery scoring, not yet a measured fact — no query has ever split engagement by official-vs-community authorship, none counts distinct community creators). Names an explicit strategic fork the memo otherwise assumes silently: doubling down on curated consumption vs. trying to ignite community authorship. Also credits the fair counter — near-zero publishing may exist *because* creators get nothing back, so this could be a small lever on the publishing rate, not only a reward waiting for scale | the two-query gate in `09`'s "The gate" section (count distinct non-official public-note creators; attribute the views/copies aggregate to official vs. community) shows a real, meaningful creator base and/or a rising community-publish rate | 2026-07-25 |
...
### Item 1 — Adaptive initial question count (10/12/15)
...
### Item 2 — Redo Missed Questions returns the wrong session
...
**Existing signal available, no schema change needed:** `session.setQuotaExempt(true)` (currently line 411) is set in exactly one place in this entire file — `startRedoMissedSession` itself. Every ordinary Challenge/Board Exam session defaults to `quotaExempt=false`. This is a clean, already-persisted way to distinguish "this in-progress session is itself a redo-missed session" from "this is an ordinary session," with no new column.

Fix:
- Give `resolveExistingChallengeSession` a way to require the candidate be quota-exempt (e.g. an added boolean parameter, or a second private variant) — `startSession` passes/behaves as today (no requirement), `startRedoMissedSession` passes the new requirement.
- If `startRedoMissedSession` finds a candidate that is `IN_PROGRESS`/`GENERATING` but **not** quota-exempt (i.e. a stale ordinary session, not a stale redo session): do not return it. Since clicking "Redo Missed Questions" is itself a deliberate, explicit user action — unlike the generic entry-point ambiguity v0.60.1's Resume/Start Fresh prompt exists for — there's no ambiguity to resolve with a prompt here. Forfeit the stale ordinary session (mirror the existing `markSessionForfeited` + save + claims-release pattern already used elsewhere in this class) and proceed to start the fresh redo-missed session, rather than silently resuming it or running two `IN_PROGRESS` rows for the same user+study pack+mode side by side (which would corrupt `findTopByUserIdAndStudyPackIdAndSessionModeAndStatusInOrderByCreatedAtDesc`'s "top" semantics for later lookups).
- If the candidate **is** quota-exempt (a genuinely stale prior redo-missed session), keep today's behavior — resume it, matching how any other in-progress session is treated.
...
**Sequencing risk, not a blocker — confirm before kickoff:** this touches the onboarding funnel while the Diagnostic Read re-read (due after 2026-08-06) is mid-measurement. The Diagnostic Read's own text warns against exactly this ("reorganizing navigation mid-surge would pollute the exact funnel data this read needs to stay clean"). This item is scoped to `STUDENT`, and the surge cohort the read is measuring skews Board-Exam/exam-dated — the overlap may be partial, but that's an assumption, not a confirmed fact. Get an explicit go/no-go on timing before kickoff rather than assuming the scopes don't collide.
