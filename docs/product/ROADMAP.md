...
**Near-term buildable slice = (a) + (c).** A static coach-label mapping (same shape as `getCollectionLabels`) over `CompanionDisplayCard`, order-preserving, plus a conversational frame composed from an already-loaded live signal, positioned above the authored Companion (which stays a stable, unreordered narrative). Frontend-only, no new backend call, no new persisted state — same architectural class as `pickActiveGuidance` (`frontend/lib/guidance-engine.ts`) and `getNextPlanAction`, both deterministic and frontend-only already. (Shipped as `CompanionCoachIntro`, deliberately narrowed to the resolved primary action only — countdown and readiness numbers were dropped from the composition once review found they duplicated `GoalWeeklyCountdownCard`/`ReadinessSummary` a few cards up; see the "Coach vs. Companion" refinement below.)
