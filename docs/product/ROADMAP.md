...
| Production performance audit (Private Library, Public Library, Note Collection detail, Dashboard reported slow) | `production-performance-audit-out/01-production-performance-audit.md` | **Implementation complete — v0.51.0 pre-signoff.** F1, F2, F4, F5, F6, F7, and F8 are implemented in full (F8 — real server-side Public Library pagination — chosen directly over F3's stopgap). F9 (client caching) and F10 (denormalized counts) intentionally remain parked below | F9/F10: gated on post-v0.51.0 production evidence (slow-query logs, whether refetch pain persists once queries are bounded) — not before | 2026-07-19 |
...
## v0.51.0 - Read-Path Performance Pass II (Implementation complete, pre-signoff; base branch `releases/v0.51.0`)
