...
| **Company Redefinition (Boardready-Model Re-Architecture, 6 docs)** | `docs/claude-prompt/company-redefinition-out/01–06`, sourced from `company-redefinition-session-plan.md` (the R0–R5 prompt sequence). Origin: a boardready.ph-inspired owner proposal; a pre-Fable Claude Code challenge found it right-in-direction/wrong-in-sequence (2.4% W1→W2 retention flat across 3 releases, 0% exam-dated retention undiagnosed, pricing already ruled out as the retention lever). Owner explicitly chose to proceed without user interviews (cold outreach and in-app feedback surfacing have both gone unanswered). **v1.0.0 explicitly reserved for later** — owner decision (2026-07-23): tag it only once the redefinition's user-visible core is actually live and the product succeeds, not at the planning/strategic-commitment stage | **Phase 1 ratified and kicked off as v0.57.0** (2026-07-23, see its ROADMAP/RELEASES section); Phases 2–4 remain drafted, not ratified | Phase 2 needs Phase 1's behavioral read (positive-or-ambiguous) before its own `/kickoff`; Phase 3 needs its proposed adoption-volume gate (3a) and the review-queue dependency (3b) resolved; Phase 4 needs `05`'s "Owner must decide" §4 items actually decided | 2026-07-23 |
