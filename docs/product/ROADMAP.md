...
| **Company Redefinition (Boardready-Model Re-Architecture, 6 docs)** | `docs/claude-prompt/company-redefinition-out/01–06`, sourced from `company-redefinition-session-plan.md` (the R0–R5 prompt sequence). Origin: a boardready.ph-inspired owner proposal; a pre-Fable Claude Code challenge found it right-in-direction/wrong-in-sequence (2.4% W1→W2 retention flat across 3 releases, 0% exam-dated retention undiagnosed, pricing already ruled out as the retention lever). Owner explicitly chose to proceed without user interviews (cold outreach and in-app feedback surfacing have both gone unanswered) | 4-phase roadmap drafted, not ratified | owner must ratify each phase individually before any `/kickoff` (see `06-unified-roadmap.md` "Nothing here is authorized for implementation"); Phase 2 additionally needs Phase 1's behavioral read (positive-or-ambiguous); Phase 3 needs its proposed adoption-volume gate (3a) and the review-queue dependency (3b) resolved; Phase 4 needs `05`'s "Owner must decide" §4 items actually decided | 2026-07-23 |
