...
**Study Plan Hierarchy — Phase 1 (Goal → Subject plans; backend + UI).** Add `parent_collection_id` (self-referential, **2-level enforced**: parent must be top-level, child must have no children — cycles impossible); a Goal detail page listing child Subject plans with each child's readiness plus a cheap **Goal % = Σ child.mastered / Σ child.total** (sums per-child counts, so **no cross-subject concept re-dedup**); the owned-plans list shows top-level only; a nest/unnest control is the curation path. Modules stay as label-sections. Reuses the `NoteCollection` model, `getReadiness`, `ProgressReportService`, `ReadinessSummary`, and `getCollectionLabels` (no `ProfileType` branching). **Deferred** to later phases: recursive adopt-the-whole-Goal, per-module %, metadata/est-time/difficulty, arbitrary depth, direct notes on a Goal. This is a **deliberate, scoped reversal** of the "no parent/child collections" rule (two levels only). Architecture audit: `docs/product/STUDY_PLAN_HIERARCHY_PLAN.md`; Codex prompt: `docs/codex-prompts/v0.33.1-study-plan-hierarchy-phase1.md`.
