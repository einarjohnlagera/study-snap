...
`v0.48.0 - Retention Experiment: Open Loop & Digest Trigger` is in progress (on `releases/v0.48.0`, cut from `main` after v0.47.1 merged) — two independent retention experiments testing the co-dominant causes from the retention root-cause diagnosis: an open-loop first-quiz ending (frontend) and a due-concepts digest default-ON trigger fix with CTA/content work (backend + frontend). See `RELEASES.md`.
...
## v0.48.0 - Retention Experiment: Open Loop & Digest Trigger (in progress, base branch `releases/v0.48.0`)
