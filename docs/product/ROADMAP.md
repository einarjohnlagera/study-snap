...
| **Challenge Quiz Known-Limitations Cleanup — 3 ungated items logged in v0.60.1** (claim-release rollback under `@Transactional`, expiry-vs-completion lock race, Resume/Start Fresh missing at the collection premium-exam entry point) | Full plan (root causes, verified fix designs, file:line anchors) written directly into this doc's own `## v0.60.2 — Challenge Quiz Known-Limitations Cleanup` section above, verified by direct file read (not inferred) in the same session that kicked it off — no product or data gate, unlike every other Backlog Index candidate reviewed the same day | **Shipped 2026-07-26, pending human release signoff** on `releases/v0.60.2`: claim release commits independently of the failed generation transaction, expiry cleanup locks and rechecks the session status, and the collection Challenge Quiz launch carries the mode-selection entry marker | none — shipped | 2026-07-26 |
