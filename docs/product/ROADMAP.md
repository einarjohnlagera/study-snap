...
| Production performance audit (Private Library, Public Library, Note Collection detail, Dashboard reported slow) | `production-performance-audit-out/01-production-performance-audit.md` | **Run — 10 candidates (F1-F10), ranked and sequenced.** F1 (dashboard-overview lean projections) and F2 (`NoteService.listMine` lean projection + optional limit) are zero-decision byte-identical fixes, ready to scope. F3 (Public Library bounding) needs one human UX call (cap + "Load more" now vs. full server-side browse later) first | F1/F2: none, ready now. F3: UX decision. F7/F8 (real pagination for both libraries): next release, not this pass | 2026-07-17 |
