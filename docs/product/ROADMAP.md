...
`v0.58.0 - Explore Convergence` is the next planned version (Phase 2 of the Company Redefinition roadmap, not yet kicked off as of this writing — owner explicitly chose to proceed to Phase 2 without waiting for Phase 1's behavioral-read gate; see the Backlog Index row below for the full decision).
...
| **Company Redefinition (Boardready-Model Re-Architecture, 6 docs)** | `docs/claude-prompt/company-redefinition-out/01–06`, sourced from `company-redefinition-session-plan.md` (the R0–R5 prompt sequence). Origin: a boardready.ph-inspired owner proposal; a pre-Fable Claude Code challenge found it right-in-direction/wrong-in-sequence (2.4% W1→W2 retention flat across 3 releases, 0% exam-dated retention undiagnosed, pricing already ruled out as the retention lever). Owner explicitly chose to proceed without user interviews (cold outreach and in-app feedback surfacing have both gone unanswered). **v1.0.0 explicitly reserved for later** — owner decision (2026-07-23): tag it only once the redefinition's user-visible core is actually live and the product succeeds, not at the planning/strategic-commitment stage | **Phase 1 shipped and signed off as v0.57.0** (2026-07-23, see its ROADMAP/RELEASES section). **Phase 2 kicked off as v0.58.0 (Explore Convergence) without waiting for Phase 1's behavioral-read gate** — explicit owner decision (2026-07-23): the pre/post W1→W2 retention read needs a ~14-day window on both sides of the ship date and has not been pulled yet, but the owner chose to proceed on Phase 2 now rather than wait, an informed override of this row's own originally-stated gate. Phases 3–4 remain drafted, not ratified | Phase 1's retention read is still owed (informational at this point, not a blocker — Phase 2 already proceeded); Phase 3 needs its proposed adoption-volume gate (3a) and the review-queue dependency (3b) resolved; Phase 4 needs `05`'s "Owner must decide" §4 items actually decided | 2026-07-23 |
