...
> Last updated: v0.132.0 - 2026-09-08 (Released). Adds the three Review Set publication-boundary decisions settled 2026-09-08 — see the first section below. Previously v0.115.0 - 2026-09-05 (Released). **⚠️ THIS PREVIOUSLY READ "verdicts only, nothing implemented" — the DECIDED INSTEAD verdict below (transfer applicability authority at the PRIVATE→PUBLIC transition) HAS NOW SHIPPED as Slice 1. Every verdict below still stands as written; only its implementation status changed. Slices 0A/0B are owner-executed and unstarted, and Slice 2 is NOT started.** **Adds the Stage 1 verdicts of the Comprehensive Review Set Overhaul.** **REJECTED: adding an `ARCHITECTURE` Domain Context** — and NOT on the superseded "provable no-op" argument, which holds only for single-program notes. Rejected because `ARCHITECTURAL_DESIGN` already describes *"space planning and circulation, anthropometrics and ergonomics"* — verbatim the expressive-gap test — and is live on 10 cross-program notes. **DEFERRED: any biomedical/Medicine context, on an UNFILLABLE PROOF TABLE** — 2 notes exist and not one Nursing, Pharmacy, PT or Medicine note is cross-program, so the failure it would fix has never occurred; re-trigger is the first cross-program health note. **PRESERVED: `NURSING`** — nursing assessment/intervention/care-planning is a genuinely different treatment from diagnosis/differential/management, and nothing collapses it into medical treatment. **PROPOSED BUT DEFERRED: `COMPUTING_AND_INFORMATION_SYSTEMS`** (`quantitative = false`), at MEDIUM confidence, on classify-first-then-decide — the expressive gap is real but there is **no multi-program forcing case.** **REJECTED: note-copy options D and C — and the two rejections are DIFFERENT IN KIND, which matters.** **D is rejected on PRODUCT SEMANTICS** (refuted by 608 null-string and 551 multi-program source notes, which would leave copies with no representation at all). **C is rejected AS A SOLUTION SHAPE** — the settled decision is narrowly *"we are not solving this by making the learner's free-text value clear copied rows"*; **separately, and as a REPO FACT rather than a product argument, C as proposed cannot fire in the current model** because shadowing renders the learner's free-text value effectively shadowed on copied notes. **⚠️ Do NOT conflate the two, and do NOT extrapolate either into a claim that learner ownership does not matter — it does, and it is exercised at publication.** **DECIDED INSTEAD: transfer applicability authority at the PRIVATE→PUBLIC transition**, which un-shadows the learner's own field — authority transferred, not merely revoked. **REJECTED: backfilling the learner's string from the joined catalog name**, which would relaunder curator classification through a different column. **DECIDED: three provenance models stay SEPARATE** — note origin *terminates*, plan adoption *persists*, copied applicability needs **no stored provenance at all**; a universal inheritance framework would be wrong. **REJECTED: curriculum revision numbers and event sourcing** — snapshot plus timestamp suffices. **DEMOTED: the `ADR-001` ratio guard** — at 11:44 it now sits BELOW its ratified 8:21 baseline and can be satisfied by adding programs, so the composition matrix becomes the primary guard. **⚠️ PROCESS: the brief's own premises were six-for-six stale against code**, and `ADR-001` already named options D and C as "deliberately left open" — a parked decision was re-presented as a discovery. Previously v0.94.0 - 2026-08-29 (Released). **Adds the `v0.94.0` verdicts.** **DECIDED: a link redemption creates `PENDING` and the CREATOR confirms** — with email the inviter names the counterparty at invite time, so with a link redemption alone is one party agreeing twice; the confirm step relocates the same act rather than adding ceremony. **DECIDED: shareable links live in their OWN table** — `invited_email` is `NOT NULL`, and PostgreSQL treats NULLs as distinct in a unique index, so a nullable address would have silently disabled the email invitation's live-row uniqueness. **DECIDED: SPLIT the progress payload** rather than couple the toggles or fix only the copy, because it is the only option in which turning activity sharing off actually does something. **DECIDED: `hasActivity` is progress-shaped** — keeping a study-days signal would have left an activity-derived inference channel in a response the learner never granted activity for. **DECIDED: invitation expiry is surfaced OUTGOING-ONLY**, with a visibility window derived from `invitation-ttl-days` rather than a second hardcoded number. **REJECTED: a resend endpoint** — re-inviting already re-arms the same row. **RAISED AND OVERRULED: deferring shareable links** as a security design needing its own release; the owner scoped them with three constraints locked as anti-drift. **PROCESS: running a read-only audit agent concurrently with a mutation agent produced a false finding** — sequence them. Previously v0.93.0 - 2026-08-28. **Adds the `v0.93.0` verdicts.** **DECIDED: the progress read stays UNIDIRECTIONAL** — bidirectional progress is a new capability needing its own decision, and was NOT taken by reusing the symmetric grant check. **DECIDED: `PROGRESS` grants are learner-issued only**, because under a unidirectional read a supporter-issued row is unconsumable; a dead toggle is worse than an absent one. **DECIDED: no backfill granting `PROGRESS` to existing `ACCEPTED` relationships** — free while production was empty, and forbidden anyway because it converts an implicit rule into explicit consent nobody gave. **DECIDED: the DTO's `*SharedByMe` reflects the stored row while `*SharedWithMe` also requires `ACCEPTED`** — the status filter has a DIRECTION, and applying it to both would have made a learner's own toggle read OFF while the row was live. **REJECTED: repairing a self-fulfilling concurrency test** — it was removed instead, because a test that models its own answer is worse than an absent one. **REJECTED (again): auto-skipping the PostgreSQL harness when no database is reachable** — the opt-out must be an explicit flag a human types. Previously v0.92.0 - 2026-08-27. **Adds the `v0.91.0` and `v0.92.0` verdicts — read the Shared Learning Material and Activity Sharing sections below before re-proposing anything in that arc.** Previously v0.90.0 - 2026-08-27. **Adds the three Linked Learners surfacing verdicts taken 2026-08-27 — read them before re-proposing any of the four.** Previously v0.89.1 - 2026-08-20. **Stamp had gone four releases stale (v0.85.0) — restamped with the decisions most likely to be re-proposed.** **⚠️ `v0.89.0` shipped Learning Connections as a CAPABILITY, not a profile type** — a parent, tutor, sibling or mentor can make a quiz for someone, connect with them, and see their progress. **`ProfileType` answers "how do YOU learn?", not "may you help someone?"**, so the old design forced a parent to claim to be a Teacher — changing their own dashboard and practice options — just to help their child. **⚠️ Do NOT re-gate any of this on `ProfileType`; `PARENT` stays unimplemented with zero users.** **⚠️ THE PRIVACY LINE IS RATIFIED AND ABSOLUTE: a supporter sees readiness, progress and quiz performance, NEVER the learner's notes** — this protects the core loop, because learners who suspect notes are visible write less honestly. Teachers still exclusively keep DOCX export, multi-version exports, question-count control and the Exam Builder; only the single-quiz share link opened up. **Ratified 2026-08-19 (Q4):** connections are **invite + accept in BOTH directions, revocable either side**, and **acceptance is load-bearing** — without it anyone could claim a supporting relationship over any account by knowing an email address. **Ratified 2026-08-19 (Q5):** age is collected **at link time, never at signup**, with guardian consent below a threshold that is **configuration, and whose NUMBER remains owner-owned pending counsel** — do not let a shipped default be read as a legal position. **Rejected and recorded:** `learnerLevel` as an age proxy (breaks both ways), adults-only-first (excludes the motivating parent-child case), and a supporter `ProfileType`. Previously v0.85.0 - 2026-08-18. Adds the **Domain Context Catalog** decline (2026-08-17), which is the single most likely thing to be re-proposed from this span. Previously v0.73.0 - 2026-08-12.

---

## Review Set publication boundary — three decisions settled 2026-09-08, before the prompt was written

All three were the owner's, recorded at `v0.132.0` kickoff as genuine calls rather than assumptions, and
all three took the audit's recommendation.

1. **The narrowed public-view contract is ACCEPTED.** The public and new adopters are shielded from
   unpublished **additions** only; **removals, reorders and renames stay live.** The alternatives were a
   working/published column pair per field plus soft-delete, or a full snapshot — both disproportionate to
   a curator whose mid-expansion work is overwhelmingly *adding*. **⚠️ THIS IS A STATED LIMITATION, NOT A
   GAP TO CLOSE LATER BY DEFAULT.** Decision 1 is the one that sized the architecture, which is why it was
   settled before any code was written.
2. **The backfill stamps each collection's `created_at`, not `now`** — so a future public "Updated" date
   is not uniformly the deploy date for every set that existed beforehand.
3. **`last_update_published_at` preserves the capability for a public "Updated" date and ships NO UI.**
   ⚠️ Do not add that UI without a fresh decision.

**The architectural decision underneath all three: a PER-ROW PUBLICATION STAMP, never a snapshot,
version or history.** That is only sound because `applySourceUpdate` is additive-only — `MOVED`, renames
and removals are detected and reported but never applied — so the boundary only has to gate what becomes
visible as an ADDITION. **⚠️ If additive-only ever changes, this decision must be revisited, because the
stamp stops being sufficient.**

**Deferred, deliberately:** P3, the Review Set update **notification**, was cut on size, not doubt — it
is a separate producer and is now un-parked. **⚠️ The audit's §7 SUPERSEDES the notification plan's §8
drift-signature dedup, which must NOT be implemented.**

**Recorded at signoff, not decided:** the Publish control and the `Unpublished changes` indicator live
only on the Review Set's root page, while topics are actually added in the Builder. That is a known gap
carrying `[CHECKPOINT — due 2026-09-22, deploy-relative]`, whose kill criterion is any set holding
unpublished rows older than 14 days that was never really published.
