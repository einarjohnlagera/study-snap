...
> Last updated: v0.151.0 - 2026-09-16 (Released). **`v0.151.0` closed the scope half of the `v0.110.2` share-link fix below: "Regenerating a note also deactivates (never deletes) any live quiz share link" now holds for BOTH regeneration scopes, not just the combined one.** `StudyPackService.saveStudyPack` replaces a note's shared quiz in place on either `STUDY_PACK` or `NOTE_AND_STUDY_PACK`, but the deactivation call was gated to fire only on the combined scope — so the default, `STUDY_PACK`-only regeneration left a recipient gradable against replaced material. Fixed by moving the call out of that gate. The same gate, deliberately mirrored, was also found in the bulk-regeneration consequence-counting service backing the preflight modal's count and the batch receipt, and in the confirmation dialog's own warning copy — all three fixed together so the disclosure, the execution, and the receipt agree. *(previously v0.144.0)* - 2026-09-13 (Released). **⚠️ CORRECTS THE `v0.143.0`-DATED NOTE BELOW: the admin-repair-path leg is now closed too.** `AdminStudyPackTransactionHelper.regenerateOnePack`/`repairMalformedQuiz` (admin-only summary/quiz repair) now call the same `ExamQuestionPoolService.refreshPool` invalidation, with the same flush-before-lock ordering, immediately after each save. This was the fourth path `v0.143.0`'s exam-pool fix did not reach. **The Challenge question bank leg remains the one open leg of the original limitation** — it still has no pool mechanism and no invalidation path. *(previously v0.143.0)* - 2026-09-12 (Released). **⚠️ CORRECTS THE `v0.118.0`-DATED "KNOWN LIMITATION" BELOW: the exam question pool leg is now invalidated.** Long Exam and Board Exam pools are reset and re-dispatched on ANY Study Pack regeneration — the combined `NOTE_AND_STUDY_PACK` scope and the default `STUDY_PACK`-only scope alike (`ExamQuestionPoolService.refreshPool`, called unconditionally from `StudyPackService.generateStudyPackFromExistingNoteAsync`). **The Challenge question bank leg is UNCHANGED and still open** — it has no pool mechanism and no invalidation path. **⚠️ Also still open, found while fixing the pool leg: `AdminStudyPackTransactionHelper.regenerateOnePack`/`repairMalformedQuiz` (admin-only summary/quiz repair) replace Study Pack content in place with no invalidation call at all — a fourth path this fix does not reach**, tracked in `ROADMAP.md`'s Backlog Index, not yet fixed. *(previously v0.141.0)* - 2026-09-11 (Released). **⚠️ `v0.141.0` ATTEMPTED a delimiter-PAIRING change in quizzes and REVERTED it at signoff — a cold pass measured 385 production strings worse against 3 better. The defect below is STILL OPEN and nothing in quiz maths rendering changed.** KaTeX only activates inside `$…$`, `\(…\)`, `$$…$$`, `\[…\]`. A `$` used to open a span whenever the next character was not whitespace — so a currency amount like `$50,000` opened one. The closer is currency-aware (it rejects a `$` preceded by whitespace or an operator), so it walked past every intervening amount and closed on the FORMULA's final `$`, handing KaTeX an entire sentence; KaTeX failed on the prose and the fallback re-emitted the source, so **the reader saw the raw formula with backslashes visible**. An attempted `startsCurrencyAmount` gave the opener the same awareness **by looking at the single character after the digits — which cannot discriminate**, because `$1.5 \times 10^4$` puts a space exactly there. Reverted. **⚠️ TWO CLAUSES, BOTH LOAD-BEARING: a `$` immediately after the digits means the pair is already balanced (`NPV $10000$`, a real production string, stays maths), and a letter/backslash/operator after them means maths (`$3x^2$` stays maths). Dropping either is WORSE than the original defect**, because the closing `$` of a balanced pair would then be read as an opener and swallow the rest of the sentence. 44 stored strings were affected. **⚠️ Do NOT 'fix' maths rendering in `normalizeBareMath` — it returns early on ANY delimiter and 339/339 backslash-bearing questions already carry one, so it reaches almost nothing; the mechanism is pairing, not absence.** **`v0.121.0` changed what a SHARED-QUIZ RECIPIENT receives. MATCHING questions are now EXCLUDED from shared quizzes** — `PublicQuizItem` drops `questionGroup` and the recipient page has no matching control, so a block was arriving as independent MCQs and being SCORED that way. The filter is applied on the SHARED derivation (`resolveSharedQuiz`), which `resolveSharedQuestions` delegates to, so the recipient projection and the grader walk the SAME list — filtering only the projection would trip `answers.size() != questions.size()` and 400 every submit. **⚠️ EVERY IN-APP MODE STILL RECEIVES MATCHING NORMALLY — this is the shared-link path only.** Recipient answers are also **index-addressed with a Back control** (correctable, but still not skippable), and a completed shared quiz now emits `QUIZ_SHARE_LINK_COMPLETED`, only after grading succeeds. *(previously v0.118.0)* - 2026-09-05 (Released). **⚠️ QUIZ MASTERY IS NOW SCOPED TO THE QUIZ IT WAS EARNED ON.** Mastery is derived, never stored, and regeneration preserves `study_packs.id` by design — so a perfect Quick Review on the OLD quiz used to keep the Quiz tab (which reveals answers) unlocked on the NEW one. `findQuizMasteredAt` now also requires the mastering session to be no older than `notes.generation_enqueued_at`, which moves only on generation. **⚠️ KNOWN LIMITATION: the exam question pool and the Challenge question bank key on the same preserved `study_packs.id` and are NOT invalidated on content change** — after a regeneration both can still serve questions built from replaced content. **Regenerating a note also deactivates (never deletes) any live quiz share link.** *(previously v0.113.1)* - 2026-09-04 (Released). **`v0.113.1` hardened that anchoring: completing a plan-scoped session with an unattributable item now skips and logs it rather than throwing** (the throw rolled back an already-`COMPLETED` session that the recovery sweeper does not cover, bricking the plan), **a legacy plan-scoped session whose collection was deleted falls back to its pack anchor instead of 404ing**, and the anchor rule collapsed from three near-copies to ONE — `QuickReviewSessionEntity.validateAnchor`. **A session is anchored on a note/pack pair OR a Study Plan, never both** — plan-scoped Adaptive Practice no longer borrows a pack anchor, so it stops contending with note-scoped Adaptive and Interview Practice for one index slot. A plan-scoped session is entered and reloaded by its own session id, credits every note it sampled on completion, and survives its plan being deleted. Note-scoped Adaptive Practice is unchanged. **⚠️ Exactly five quiz modes still; this changed the ANCHOR, never the mode taxonomy.**. **`v0.110.2` is a PATCH that stopped a regeneration from silently corrupting a quiz someone else was already taking.** **⚠️ THE DEFECT, found during `v0.110.0`'s item 4 audit and carried unfixed through TWO releases:** `GeneratedQuizService.generate` reuses the EXISTING quiz row and overwrites its questions, so **the id never changes**, while `quiz_share_links.generated_quiz_id` points at it and the service referenced share links **zero times**. Regenerating an already-shared quiz therefore mutated the quiz a recipient could be part-way through — **a changed question count 400'd them on submit, and an UNCHANGED count graded them against questions they never saw with no error at all.** The silent case is the worse one and is what every non-`TEACHER` hits, since they always receive 10 questions. **FIX:** regeneration deactivates **every live link** for that quiz — **not just the newest**, because only `token` is UNIQUE, `createShareLink` mints a new row over an inactive one, and `findActiveLink` accepts ANY active token, so one quiz can own several live links. **⚠️ It DEACTIVATES, never DELETES:** deleting would force a new link and spend share-link quota because of the fix; left inactive the owner turns it back on knowingly, at which point it serves the new questions as their informed choice. The owner is warned beforehand only when a live link exists, and the panel **re-reads** the link afterwards. **⚠️ A SECOND DEFECT WAS FOUND WHILE WIRING THAT UP:** the share panel keys its effect on `generatedQuiz.id`, which a regeneration does **not** change, so it never re-ran and the owner would have kept seeing *"Sharing on"* for a link the server had just turned off. **⚠️ VERIFIED AT SIGNOFF: the deactivation is transactional with the quiz write and both catch blocks rethrow, so a link is never turned off for a regeneration that did not land; and there is NO second writer of `GeneratedQuizEntity.questions`.** **⚠️ KNOWN LIMITATION:** turning the link back on serves the NEW questions under the SAME token — deliberate and visible to the owner, but a recipient holding the old URL sees different questions. **The permanent fix is snapshotting the single-note shared quiz at share time (the `combined_quizzes` model), which is a migration plus a table and was deliberately NOT taken in a patch.**
