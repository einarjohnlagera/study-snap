...
**Current baseline:** `v0.57.0 - Practice-First Activation Onboarding` is the most recently **released** version (signed off and merged to `main` 2026-07-24). `v0.56.0 - Weak-Concept Explanation Links` released immediately before it. See "Current Release: v0.57.0" below. **No version is currently kicked off or in progress** — the obvious next step (`v0.58.0 - Explore Convergence`) was ratified 2026-07-23 and then explicitly *un*-ratified the next day; read "Company Redefinition & the 2026-07-24/25 Reprioritization" below before proposing what comes next, it supersedes the old single-retention-bet framing and is the single most important section in this file right now.
...
- **Phase 2 — IA / Explore convergence.** Nav becomes `Dashboard / My Reviews / Library / Explore / Progress`; Explore composites the existing Official catalog + Public Library; Progress promotes to top-level nav. Split into two chunks for risk reasons: `v0.58.0` (Explore, additive) and `v0.59.0` (Dashboard/Progress reorg, touches daily-used default states). **Re-gated 2026-07-24** — see the reprioritization below; do not kick this off under the old gate.
...
**New sequence — ratified by the owner 2026-07-24, nothing kicked off yet:** Phase 1 (shipped) → **Diagnostic Read** (ratified; Round 1 already run 2026-07-24/25, inconclusive by construction, re-read after 2026-08-06) → **Reusable Practice Assets & the Return Loop** (ratified, clear for `/kickoff` whenever scoped — its only gate, Phase 1 shipped, is already met; the reframed realization: turn on the dormant per-user pool for Board/Long Exam, extend the same per-user pattern to Challenge Quiz, persist generated questions as a revisitable set, add a "redo what you missed" surface reusing existing `ConceptHealth`/weak-concept machinery — Adaptive Practice stays explicitly excluded, see below) → Phase 2, re-gated (`v0.59.0` Progress needs Reusable Practice shipped first since there's nothing stable to show progress against otherwise; `v0.58.0` Explore needs the Diagnostic Read to actually show a discovery problem) → Phase 3/4 unaffected, still parked at their original gates. Full detail and the resequenced Phase 2 gates: `docs/product/ROADMAP.md`'s "Company Redefinition Roadmap — Phase Detail" section.
