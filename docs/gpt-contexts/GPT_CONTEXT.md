...
> Last updated: v0.110.1 - 2026-09-04 (Released). **`v0.110.1` is a PATCH that fixed the CRITICAL finding carried out of `v0.110.0`'s pressure test.** **⚠️ QUIZ CHOICE TEXT WAS DEGRADING ON EVERY READ, PRODUCT-WIDE.** `QuizValidationUtils.sanitizeChoiceTexts` strips a leading choice label with `replaceFirst` and is **NOT idempotent**, and it ran on every `QuizItem` construction rather than only when text first arrives from the LLM — so `"A. B. Smith"` stored as `"B. Smith"` and read back as `"Smith"`, and `"D.C. generator"` degraded to `"C. generator"` then `"generator"`, **compounding with each read** across every table holding a `QuizItem`. **⚠️ IT WAS FOUR REBUILD PATHS, NOT THE ONE THE SCOPE NAMED:** `@JsonCreator`, `QuizSessionStateUtils.extractQuiz` (every session load, and not a Jackson route at all), `ShareService.copyQuizItem`, and `QuizVersionShuffleUtils`. The sanitizing constructor now takes a `stripChoiceLabels` switch — **generation passes true, every read/copy/rebuild passes false** — and **only the label strip is conditional**: whitespace normalization stays unconditional, and `resolveCorrectIndex` still runs, because MULTI_SELECT and legacy `answer` rows derive their index on read. **⚠️ THE PRE-SIGNOFF COLD AGENT FOUND TWO REGRESSIONS THE FIX ITSELF INTRODUCED, BOTH FIXED BEFORE SIGNOFF:** a legacy row holding labelled choices beside a full-text answer lost `correctIndex` **entirely** (grading every response wrong) because the answer was still being stripped while the choices no longer were; and dropped whitespace normalization broke matching for a reason unrelated to labels. Both now compare **label-insensitively for the comparison only**, never rewriting stored text. **ALSO SHIPPED:** `GET /combined-quizzes` and `/library/combined-quizzes`, so an owner can find a combined quiz they already shared and **revoke a live public link** — previously reachable only by the post-assembly redirect. Its owner scope needed a **real-PostgreSQL** test: the mocked service test could not see a dropped predicate, which is the `v0.93.0` failure mode. **⚠️ OPEN RESIDUALS:** already-corrupted rows are **UNRECOVERABLE** and no backfill was written (a guess is worse than visible truncation); a pre-2026-04-08 row still **displays** its label, rendered as `"A. A. Encapsulation"` since the UI prepends its own letter; and the combined-quiz list truncates at 50, hiding the **oldest** rows — the forgotten links most likely to need revoking.
