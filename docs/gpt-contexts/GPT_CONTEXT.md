...
> Last updated: v0.101.0 - 2026-09-01 (Released). **`v0.101.0` shipped LANGUAGE AND OBSERVABILITY** — Slice 1 of the Review Sets audit: the generation meter became **`Quiz generations`** (pricing says **`generated quizzes`** — two constants on purpose), learner-facing "AI" copy was swept from 16 sites, and **`NOTE_ADDED_TO_COLLECTION`** now records the transition the retention hypothesis rests on, which nothing measured before. **⚠️ Category B disclosures (privacy, terms, "verify calculations", review-before-you-share) and "AI critique" were deliberately NOT swept — do not propose finishing the job.** `v0.100.0 - Domain Context Resolution` shipped 2026-09-01 and **was never stamped here**; it stopped canonical curated generation from falling back to the curator's own profile, surfaced the effective writing domain, and moved the multi-program Domain Context block from note SAVE to GENERATION. **⚠️ ORIENTATION LINE 3 BELOW IS STALE BY NINE RELEASES (it still says `v0.92.0`) — read this stamp for current state, not that line.** Previously v0.99.0 - 2026-08-31 (Released). **`v0.99.0` shipped CONNECTION COMPLETENESS — four items, finishing the two properties `v0.98.0` shipped half-built and closing every one of its Known limitations.** **An expiry stays visible after a paused or backlogged sweep:** retention was keyed on the DEADLINE, but `v0.98.0`'s own batch bound and pause hook create backlogs, so a late-swept request became `EXPIRED` with a timestamp already outside the window and **vanished without ever being shown as expired**. `V130` adds a separate `expired_at`; **⚠️ `expires_at` keeps meaning THE DEADLINE for every status and is never overwritten, re-purposed or backfilled — a NULL there is the entire mechanism protecting a consent-paused relationship**, which `v0.97.0` established after two wrong attempts. The backfill is `EXPIRED` rows only. **The onboarding bar now reaches `invite()`**, so *forming* a connection requires finished onboarding on every path; **⚠️ `accept()` is deliberately NOT gated** — every path creating a `PENDING` relationship is gated at creation and `onboardingCompletedAt` is never cleared on a live account, so gating it would put a third gate on the guardian-consent path. **⚠️ *"Every `ACCEPTED` relationship has two onboarded parties"* is PERMANENTLY FALSE for pre-gate rows** and no migration heals them. **Curator Domain Context copy stopped blocking classification** — `ENGINEERING_SCIENCES` now names building services, which is why ~41 notes generated with computation guidance silently off; **⚠️ NO enum value was added, and `ARCHITECTURE` specifically was not — it PASSED the bar and is still a provable no-op.** **⚠️ THE VERIFICATION LESSON, and it is the sharpest yet: THREE OF THE COLD AGENT'S FOUR CONFIRMED DEFECTS WERE INSIDE THE GUARD THIS RELEASE BUILT** — the cron contract test, written to stop guards from looking present and doing nothing. It pinned the **annotation** default while `application.yaml` declares 8 of 10 keys and a present property **wins**; it scanned one package, so a bare-literal cron elsewhere passed **both** assertions at once; and its disablement check was an unanchored leaf substring a **comment** could satisfy, which armed the data-deleting purge job for every test run. **A guard is worth only what it actually executes — and a guard written to enforce that rule is not exempt from it.** A fourth defect: nothing pinned the onboarding gate above the **write-once** birth-year write. **⚠️ AND ONE FINDING WAS THE SIGNOFF SESSION'S OWN — the `BP 344` correction cited the ratified spec as saying 344; the spec says `BP 334`, and a `RELEASES.md` misquote had been cited as proof of it. The change stands on the CURRICULUM sources; do NOT restore 334 from the spec.**
