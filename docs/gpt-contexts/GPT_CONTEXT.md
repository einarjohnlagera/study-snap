...
> Last updated: v0.78.0 (Released) - 2026-08-15
...
3. **Current state:** `v0.78.0 - Post-Mastery Next Step` is **Released 2026-08-15**. **No version is currently open.** It opened as one backend change — a learner who masters a pack is offered the next note in their Study Plan, filling a result-screen slot that was literally `null` — and **grew mid-flight on production evidence**: **69% of packed-note learners (189 of 274) have nothing in a plan at all**, so that suggestion could never fire for them. **106 of those already had a published plan for their exact program and had never adopted it**, so the Dashboard now names one matched plan with its coverage instead of a generic "browse curated plans" pointer, and the same plan fills the post-mastery slot when the note is in no plan. **The bet is narrower than it looks: those 106 already had a working route** (the pointer landed on Explore's Review Sets tab, already program-filtered) **and did not take it** — so this changed the invitation, not the route, and `[CHECKPOINT — due 2026-09-14]` tests whether the invitation was ever the constraint. **It amends `v0.67.0`'s "Explore Owns Discovery"**: the Dashboard may render ONE named recommendation, never a grid; Explore still owns browse. Previous: `v0.77.0 - Evidence-Gated Weak Concept Recommendation` (Released 2026-08-14).
...
8. **Eight dated checkpoints are live, the earliest 2026-09-10.** 09-10 H1+H5 proximal (carries a kill criterion) · **09-11 onboarding completion** *and* Knowledge Impact · **09-12 ×2, both from `v0.74.0`** (the Quiz-unlock rate, and whether removing Adaptive Practice from Quick Review cost it adoption) · 09-13 Explore engagement · **09-14 `v0.78.0`** (does a NAMED plan recommendation convert where a generic pointer did not — kill criterion on the proximal Dashboard arm) · 10-15 Challenge Quiz read (c) · 11-09 H1+H5 distal. **The live consequence is sequencing:** a proposal that changes what a checkpoint measures before it is read destroys the read. That is not hypothetical — it is why `v0.76.0` is a copy slice rather than anything better, and why the post-mastery next-step and the onboarding-vocabulary residuals are both parked until mid-September **on measurement grounds, not merit**.
