...
> Last updated: v0.69.0 (Released) - 2026-08-04
...
3. **Current state:** `v0.69.0 - Canonical Knowledge Foundation` released and **deployed to production 2026-08-04**. **No version in progress.** Next step is a `/kickoff` — `v0.70.0` already has inherited scope (see Current baseline).
...
- **`v0.70.0` inherited scope (from `v0.69.0`'s deferrals and post-deploy findings, 2026-08-04).** Not yet kicked off. (a) `course_programs` catalog + `program_families` — needs one production read of the exact 32 program strings; query written. (b) Question pool/bank re-keying — moves pool/bank `learner_level` off the *reader's* level onto the note's; needs a production read sizing the LLM-regeneration wave it triggers, and carries a read/write divergence where one call site still passes the reader's level. (c) **Authoring metadata must become editable on `STUDY_PACK_READY` notes** — today the editor is closed once a Study Pack exists, so Domain Context and Note Learner Level cannot be corrected after generation, and this currently **blocks R4 step 1 outright**. (d) The note detail page's inline "Edit details" panel duplicates the audience field but omits both new axes, so it looks like a complete authoring surface while silently missing what `v0.69.0` shipped.
