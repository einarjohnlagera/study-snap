...
> Last updated: v0.57.0 (Released) - 2026-07-25
...
**Current baseline:** `v0.58.0 - Reusable Practice Assets & the Return Loop` is **in progress** (kicked off 2026-07-24, base branch `releases/v0.58.0`) — the Diagnostic Read + Reusable Practice Assets sequence, ratified 2026-07-24, is now underway. `v0.57.0 - Practice-First Activation Onboarding` is the most recently **released** version (signed off and merged to `main` 2026-07-24). `v0.56.0 - Weak-Concept Explanation Links` released immediately before it. **`v0.58.0` no longer refers to `Explore Convergence`** — that Phase 2 chunk (ratified 2026-07-23, un-ratified the next day) shifts to `v0.59.0`; Dashboard & Progress Reorg shifts to `v0.60.0`. Read "Company Redefinition & the 2026-07-24/25 Reprioritization" below before proposing what comes next, it supersedes the old single-retention-bet framing and is the single most important section in this file right now.
...
## Company Redefinition & the 2026-07-24/25 Reprioritization (read this before proposing what comes next — it supersedes the "next obvious step" framing above)

**The strategic redefinition (2026-07-23).** The owner encountered `boardready.ph` (a simpler PH board-exam-review competitor with strong traction) and, working through a proposal with GPT, produced a company-level reframe: NoteLib as a **"learning OS"** — a learner's own notes become curriculum, curation turns that curriculum into a compounding reusable asset, and AI is the machinery behind the curtain, never the thing a learner is asked to trust directly. A pre-Fable Claude Code challenge found the proposal right-in-direction but wrong-in-sequence (grounded in the same 2.4%/0%-exam-dated retention data above), then a 6-session Fable plan produced a full design — `docs/claude-prompt/company-redefinition-out/01-06` (`01` strategic redefinition, `02` activation/onboarding, `03` information architecture, `04` reusable assets/Reviewer decision, `05` packaging/terminology, `06` unified 4-phase roadmap) — synthesizing this new direction onto the already-shipped architecture (Companion, Goal→Subject hierarchy, ConceptHealth, `ExamQuestionPool`) rather than replacing it. **`v1.0.0` is explicitly reserved for later** — the owner decided to tag it only once this redefinition's user-visible core is actually live and the product succeeds, not at the planning stage.

**The 4 phases, current status:**
- **Phase 1 — Practice-first activation onboarding.** **Shipped as `v0.57.0`, signed off 2026-07-24.** A `BOARD_EXAM` learner whose collected course/program already has a depth-qualifying Official Review Set skips note-authoring/generation entirely, adopts the set in one tap, and lands on its detail page (Today's Focus, Continue Studying one tap away) — not cold inside a quiz (a live-tested pre-signoff correction; the original design landed the learner directly in Quick Review, which felt jarring on real testing). Zero note-authoring, zero LLM call on this path. This release's own instrumentation is the acceptance signal — see the Diagnostic Read below for its status.
- **Phase 2 — IA / Explore convergence.** Nav becomes `Dashboard / My Reviews / Library / Explore / Progress`; Explore composites the existing Official catalog + Public Library; Progress promotes to top-level nav. Split into two chunks for risk reasons: `v0.59.0` (Explore, additive) and `v0.60.0` (Dashboard/Progress reorg, touches daily-used default states) — renumbered from `v0.58.0`/`v0.59.0` once Reusable Practice Assets & the Return Loop claimed `v0.58.0` at its 2026-07-24 kickoff. **Re-gated 2026-07-24** — see the reprioritization below; do not kick this off under the old gate.
- **Phase 3 — Cross-user question pooling + bounded reusable-object model.** Drafted, not ratified, **stays parked at its own adoption-volume gate** — do not accelerate this just because pooling is generally top-of-mind (see the pricing debate below for why this recurs). Its design already vetted the key question "what's the pool key?" and deliberately chose content lineage (`studyPackId`, one-hop copy) over anything coarser like a tag or topic, specifically for correctness (a question written against Note A's content isn't necessarily valid against Note B's content just because they share a label). Also has an unresolved review-queue dependency for its authoring slice.
- **Phase 4 — Packaging / terminology.** Drafted, needs an explicit owner decision (its source doc's own "§4 Owner must decide" section), no engineering dependency on anything else — free to move anytime once ratified.

**The 2026-07-24 reprioritization — a real-time signup surge reversed the Phase-2-next decision made hours earlier that same day.** ~15-29 signups landed in one evening (Facebook/LET-driven acquisition), and separately the product/UX side raised a realization: Challenge Quiz always generates fresh AI questions every session, and maybe quizzes should be reusable, improving assets rather than disposable output. Full reasoning: `docs/claude-prompt/company-redefinition-out/07-reprioritization.md`, cross-checked by an independent `advisor()` pass and an independent Fable session before being acted on. Verified against the actual code (not assumed): Quick Review already replays a stored quiz with zero LLM call per session (already the target state); Board/Long Exam have per-user pooling *built but dormant* (`examPoolPrewarmEnabled=false` by default — one flag from not regenerating fresh every time); Challenge Quiz regenerates fresh on start *and* on every "give me more" click, and the "give me more" path is *completely unmetered* (no quota decrement at all — a real, previously-untracked cost/integrity gap); no cross-user pooling is live anywhere; **no token/dollar cost metering exists anywhere in the system** (only integer per-action counters). Conclusion: the realization is **correct but was misfiled as a cost play** — at this scale cost is small and unmeasured anyway — its real value is a **retention primitive** (a learner who never gets a second crack at a missed question has no spaced-repetition mechanic to retain against), and it is explicitly **not** the same thing as Phase 3 (cross-user pooling of Official content, still parked) — it's a cheaper, **per-user, cross-session** reuse idea, net-new, on the roadmap nowhere until this pass found it.

**New sequence — ratified by the owner 2026-07-24:** Phase 1 (shipped) → **Diagnostic Read** (ratified; Round 1 already run 2026-07-24/25, inconclusive by construction, re-read after 2026-08-06) → **Reusable Practice Assets & the Return Loop** (ratified **and kicked off 2026-07-24 as `v0.58.0`**, In Progress; the reframed realization: turn on the dormant per-user pool for Board/Long Exam, extend the same per-user pattern to Challenge Quiz, persist generated questions as a revisitable set, add a "redo what you missed" surface reusing existing `ConceptHealth`/weak-concept machinery — Adaptive Practice stays explicitly excluded, see below) → Phase 2, re-gated and renumbered (`v0.60.0` Progress needs Reusable Practice shipped first since there's nothing stable to show progress against otherwise; `v0.59.0` Explore needs the Diagnostic Read to actually show a discovery problem) → Phase 3/4 unaffected, still parked at their original gates. Full detail and the resequenced Phase 2 gates: `docs/product/ROADMAP.md`'s "Company Redefinition Roadmap — Phase Detail" section.

**The Diagnostic Read — Round 1 run 2026-07-24/25, inconclusive by construction, re-read after 2026-08-06.** The actual deliverable (W1→W2 retention on the real surge cohort) is not yet available — the 07-23 signups don't close their 14-day eligibility window until ~2026-08-06, and nothing run so far substitutes for that. What Round 1 did establish: the historical activation-anchored retention read is still flat (~2.0%, statistically unchanged from the 2.4% baseline above — no measurable change either direction); a **chronic ~50% onboarding non-completion rate across recent signups generally** (59/117 non-surge-day vs. 19/29 on the surge day itself, 65.5% — **not a surge-specific problem**, if anything the surge completed onboarding somewhat better than baseline); and two funnel-shape over-reads were caught and explicitly retracted rather than reported as findings (a "practice-first converts 9/9 = 100%" claim at n=9 — encouraging, not validated; a theory that a 2-person funnel gap reflects UX friction rather than backend errors — too small a sample, weak inference from best-effort client analytics). Full results: `docs/claude-prompt/company-redefinition-out/08-diagnostic-read-methodology.md` "Results — Round 1." Also fixed during scoping: the existing W1→W2 definition anchored "activated" on `STUDY_PACK_GENERATED`, which never fires for a practice-first adopter (copies an already-generated pack, no LLM call) — every practice-first-onboarded learner was invisible to the old read; queries now report a signup-anchored read alongside the fixed, widened activation-anchored read side by side.

**Knowledge Impact (2026-07-25) — a separate, parked proposal, not one of the 4 phases.** GPT proposed a future creator-recognition dashboard ("your notes helped N learners") off the same growth (297 verified users, 12,211 views, 2,094 copies). CTO evaluation (`docs/claude-prompt/company-redefinition-out/09-knowledge-impact.md`) verified against code that ~232 of ~235 public notes are official-curated — only ~3 ever published by non-official users — so that engagement is almost certainly overwhelmingly official, not community-creator, activity (a safe inference, not yet a measured fact; no query has ever split it). Parked behind a two-query gate (count distinct non-official creators; attribute the views/copies split), not a veto — the fair counter is that near-zero publishing may itself be *caused by* creators getting no feedback, so this could be a small lever on the publish rate rather than only a reward waiting for scale. See `docs/product/ROADMAP.md`'s Backlog Index for the tracked row.

**The pricing debate (2026-07-25) — open, unresolved, nothing decided or implemented.** The owner argued NoteLib's current pricing (PLUS ₱179/PRO ₱249, FREE capped at 5 Challenge Quizzes/month against review sets with 40+ notes) can't convert PH students against boardready.ph's reported ₱99 one-time payment for ~unlimited (5/day) practice access, and proposed pooling quiz questions specifically to fund a price cut. Key findings from the discussion:
- **A load-bearing correction: NoteLib's PLUS/PRO are already one-time, manually-renewed passes, not recurring subscriptions** (`docs/features/billing.md`: "Renewal is manual; there is no recurring billing yet"; copy is explicitly one-time-pass-framed, no `/month` recurrence language — see "Plans, Pricing, and Payments" below, this was already documented). The "recurring vs. one-time" framing against boardready doesn't actually hold; the real, narrower gap is price *level* and quota *cadence framing* (monthly cap vs. daily cap), not billing model.
- **Pooling buys generosity, not a price cut — these are two different levers.** Pooling lowers the marginal cost of serving more quizzes to the same payer, which can justify being *more generous with quota at the same price* (strictly better: more perceived value, same revenue). It does not, on its own, justify *lowering the price* — that's a separate bet needing its own market/conversion evidence, which doesn't exist yet. The owner's proposed ₱99/₱149 floor is notably *below* the current promotional intro price (₱149/₱199), which itself has never been evaluated for whether it converts.
- **FREE quota being "never hit" doesn't settle the question either way** — it's consistent with two different stories: (a) users engage reasonably and the quota is genuinely sufficient (not a binding constraint), or (b) users disengage so early they never approach the quota at all, *because* the perceived value (a stingy-looking cap, an unrecognized brand, and — new finding — **only 3 notes, ever, published by anyone besides the official account**, i.e. the "your notes become a compounding public asset" flywheel this whole platform is architected around has essentially zero real-world traction yet) discourages engagement from the start. This is genuinely unresolved.
- **Proposed discriminating checks, none run yet, ranked cheapest/best-evidenced first (per an independent Fable consult):** (1) the *already-instrumented* `PAYWALL_VIEWED`→`UPGRADE_CLICKED`→checkout-complete funnel (`docs/features/pricing.md` "Upgrade-surface analytics") — measures price aversion at the actual decision point, never pulled; (2) intro (₱149/₱199) vs. standard price conversion rate — already exists in the data, never evaluated; (3) % of users with ≥1 *authored* note vs. zero, to properly size a "Students" (note-authors) vs. "Reviewers" (curated-content consumers, closer to boardready's audience) segmentation — publishing ≠ authoring, so the 3-published-notes fact indicts the *community/flywheel narrative* specifically, not necessarily the size of the private-authorship population. **Refinement (2026-07-25, mine):** measure **authored ≥1 note AND returned**, not authored alone — an onboarding-authored-then-churned note inflates the authoring segment without indicating a real authoring habit; (4) a Challenge-Quiz usage-count histogram (0-5) among FREE users — the weakest of the four, ranked last (a count of "1" is produced identically by a light user, an early bouncer, or someone who just signed up late in the month).
- **Directly relevant prior finding that argues against a pure pricing story:** exam-dated users (the closest proxy to boardready's audience) already retain at **0% (0/41)** vs. 2.83% (3/106) non-exam-dated — that's a return-visit problem a price change has no purchase on.
- **Second opinion sought and received (2026-07-25): GPT, independently prompted with Fable's opinion, reports ~90% alignment and converges on the same consensus** — do not change pricing yet, pull the paywall funnel, evaluate intro-vs-standard conversion, measure authored-note segmentation, wait for the 08-06 cohort. GPT independently named the same "blind-hypothesis" risk this record already tracks (retention fix → onboarding sequencing → reprioritization → pricing, one hypothesis swapped for the next before evidence lands) — a third independent voice landing on the same discipline is a stronger signal than an echo. GPT's one added framing: NoteLib may be "becoming two products" (note-authors vs. curated-content Reviewers). **Caution on this framing (mine):** with a small team and retention still unproven, declaring two products risks pre-authorizing investment in the *authoring* side before data supports it — this session's evidence (near-zero publishing, traction concentrated in curated consumption) currently tilts toward the Reviewer segment being the live one. Check (3) above should *reveal which segment is real*, not be read as a mandate to build for both — see also `company-redefinition-out/09-knowledge-impact.md`, which parks an authoring-side creator-recognition idea for exactly this reason.
- **GPT also flagged the 17% public copy rate (2,094/12,211 views) as a health signal worth monitoring alongside signups.** Real, but **use with a caveat (mine):** practice-first onboarding's "adopt" action *is* a copy, so an unknown fraction of those 2,094 copies are funnel adoptions, not organic browse-driven discovery. Disaggregate onboarding-adopt copies from browse copies before treating the raw rate as a health metric — otherwise it's partly a number our own funnel manufactures.
- **Consensus (three-way: mine + an independent Fable consult + an independent GPT second opinion, all converged):** do not cut PLUS/PRO prices now — sequence pricing behind the three zero-cost/zero-confound checks above and the 2026-08-06 Diagnostic Read close. Running a pricing change now, on an untested hypothesis, would be a fourth major lever floated in quick succession this week (retention fix → onboarding sequencing → reprioritization → pricing), each time diagnosis got inconvenient — named explicitly as a pattern to guard against, matching the three-prior-failed-releases pattern above. FREE-tier generosity (raising the Challenge Quiz cap) is comparatively no-regret since quota is never hit, and could be actioned any time without much downside, but should still be sequenced around the pending 08-06 read (or done now with the confound explicitly annotated) rather than folded in casually.
- **Decided firmly, not open for re-litigation:** Adaptive Practice stays **out** of any pooling scope. Its entire value is personalized reactivity to what one specific learner got wrong; pooling it — even by "concept" rather than tag — would dissolve the reason it's a paid differentiator, and the concept-normalization infrastructure it would need is the still-parked Smart Review Planning pipeline, not a bolt-on to Challenge Quiz pooling.
...
- **A pricing/quota reprioritization question is open as of 2026-07-25 — nothing has been decided or implemented.** See "Company Redefinition & the 2026-07-24/25 Reprioritization" above for the full debate (PH market price sensitivity vs. boardready.ph, the already-instrumented paywall funnel that hasn't been pulled, why Adaptive Practice pooling was ruled out). Do not propose a price or quota change here as if it's settled either way.
...
## Current Release: v0.57.0 - Practice-First Activation Onboarding

**Status: Released, signed off and merged to `main` 2026-07-24.** Theme: for a board-exam learner whose target course/program already has a curated Official Review Set, get them from signup to a real first practice question with zero note-authoring and zero AI generation. Phase 1 of the Company Redefinition roadmap — see the dedicated section above for full origin, phase context, and what comes next (unresolved as of this update).

- **Practice-first onboarding branch.** Immediately after onboarding Step 2, branch on `profileType === BOARD_EXAM` AND a depth-qualifying Official Review Set existing for the collected course/program. When true, skip Steps 3-4 and render one "Confirm & Practice" screen instead: confirm the target → show the matched Review Set → one-tap `Start this plan` → existing free/instant `adopt`/`adopt-goal` (zero LLM call) → land on the adopted Review Set's detail page. No qualifying set → unchanged 5-step flow. Other profile types completely unaffected.
- **Pre-signoff correction (live-tested):** the original design landed the learner directly inside a Quick Review session on the first ready note — live testing found this jarring (cold inside a quiz, no orientation). Changed to land on the Review Set's detail page instead (Today's Focus, Continue Studying one tap away), matching every other adopt entry point in the product. Also fixed: the step header showed a literal, misleading "Step 3 of 5" on this branch's only screen — now displays "Step 5 of 5" (display-only; underlying step state unchanged).
- **Interview-free validation instrumentation.** New `AnalyticsEventType` entries for the activation funnel (`ONBOARDING_V2_PRACTICE_FIRST_ELIGIBLE`, `ONBOARDING_V2_PRACTICE_FIRST_PLAN_ADOPTED`) so a pre/post W1→W2 retention read can be pulled — see the Diagnostic Read status above for where that stands.

Anti-drift: no new backend entity, no new endpoint, no migration — an onboarding routing change only, built entirely on existing free/instant adopt and the existing stored Study-Pack quiz. See `RELEASES.md` v0.57.0 and `docs/releases/v0.57.0.md` for full scope and known limitations.

---

## Previous Releases: v0.51.0 → v0.56.0 (condensed — see `RELEASES.md` for full detail)

**v0.56.0 - Weak-Concept Explanation Links.** Quick Review, Adaptive Practice, Challenge Quiz, and Board Exam Mode result lists now link weak/missed concepts that match a source-note Key Concepts entry straight to that entry (scroll-and-highlight on Note Detail); unmatched concepts stay plain text. Mid-release addition: a one-time Quiz-tab nudge toward Full Notes, and Key Concepts tab sorting by readiness. Pure navigation over existing data — not a retention mechanic (same-session learning aid, not a return-trigger).
