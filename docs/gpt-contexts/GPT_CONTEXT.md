...
> Last updated: v0.77.0 (Released) - 2026-08-14
...
3. **Current state:** `v0.76.0 - Messaging Architecture: The Money Surfaces` is **Released 2026-08-14**. **No version is currently open.** A copy-only slice finishing the pricing page and the in-app upgrade prompts. **It was chosen because nothing better was available, not because it is the highest-value work:** the earliest live checkpoint is 2026-09-10, so every stronger learner-facing candidate sits inside a measurement window it would contaminate. **Last two released:** `v0.75.0 - Authoring by Inference` (deployed 2026-08-14 — curator authoring now infers note depth from the Review Set or the author's profile) and `v0.74.0 - Quiz Progression` (deployed 2026-08-13 — the Study Pack Quiz tab is locked until a learner scores a perfect Quick Review). Before those: `v0.73.0 - Onboarding Redesign`, `v0.72.1 - Constraint Check` (which corrected the retention metric — the retention and activation lines below supersede anything you have seen about "2.4%"), and `v0.72.0 - Return Loop`. **If you are proposing notes or note collections, the authoring model changed materially in Release B — read the five-axes note-model line below, then `NOTES_AND_COLLECTIONS_CONTEXT.md`.**
