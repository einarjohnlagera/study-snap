...
> Last updated: v0.110.0 - 2026-09-04 (Released). **`v0.110.0` shipped SUPPORTER COMBINED QUIZ — slice 6, the LAST of the assessment sequence.** Someone helping a learner can build ONE quiz from several notes and hand it over; the recipient still needs no account and no connection. **⚠️ IT OPENED AT THREE ITEMS AND SHIPPED FIVE, because a cold audit found THREE OF THE KICKOFF'S FOUR CLAIMS WRONG.** **TWO LIVE DEFECTS were fixed on the ALREADY-SHIPPED single-note path:** shared quizzes mis-graded MULTI_SELECT so a fully correct answer scored ZERO while a partly correct one scored full marks (live in effectively every shared quiz, invisible to all ten existing tests), and a Plus/Pro supporter who picked 30 questions silently received 10 — **whose fix also removed an upgrade prompt selling Plus for a capability gated on the TEACHER PROFILE that no plan grants.** **NEW:** `combined_quizzes` (`V132`), an immutable snapshot with **no FK to `notes`** so deleting or regenerating a source note cannot alter a live shared quiz, plus an exclusive target arc on `quiz_share_links` (exactly one of `generated_quiz_id`/`combined_quiz_id`) that keeps ONE token namespace and ONE share-link meter. Assembly makes **no LLM call and charges nothing**. **⚠️ VERIFIED BY TWO COLD AGENTS IN ISOLATED WORKTREES — one falsifying claims, one pointed at the BLIND SPOT.** All five claims survived; the blind-spot agent found **four surviving mutants plus a product-wide JSONB leak THIS RELEASE INTRODUCED** (`isMultiSelect()` is an is-getter, so Jackson wrote a derived `multiSelect` key into six tables until `@JsonIgnore` was added) — all fixed before signoff. **⚠️ TWO OPEN ITEMS CARRY FORWARD:** a CRITICAL **pre-existing** choice-text corruption that COMPOUNDS ON EVERY READ product-wide (`"D.C. generator"` → `"C. generator"` → `"generator"`, reproduced at signoff — the sanitizer re-runs on deserialization and is not idempotent), and **NO in-product path back to an existing combined quiz or its live share link**, an owner-control gap whose own index is built and unqueried. **⚠️ The session-anchoring migration now has NO forcing slice — that is this release's own finding — so the plan/note session collision stays a named Known limitation with no scheduled fix.**
