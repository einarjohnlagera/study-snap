...
> Last updated: v0.63.0 (Released) - 2026-07-29
...
**Current baseline:** **No version currently in progress.** `v0.63.0 - Ask Companion` shipped and signed off 2026-07-29 (base branch `releases/v0.63.0`; signoff commit pushed, PR to `main` still pending as of this writing). It **reclaimed the `v0.63.0` minor-version slot from `Explore Convergence`** (now `v0.64.0`, still gate-blocked) — the sixth time a slot has changed hands this way this cycle. Two items shipped: **Ask Companion grounded Q&A** (PLUS/PRO — retrieval over a Review Set's authored Companion content, not generation; 20 sessions/month, 6-turn cap, cheapest configured model tier) and **twice-missed concept → Ask Companion** (a new consecutive-incorrect streak on `ConceptHealth` drives an "ask about this" CTA on Adaptive Practice/Challenge Quiz/Quick Review results). An owner-requested Opus pre-signoff pressure test (three parallel agents, given the release's size and two PRs touching the same shared method) found and fixed five issues before any learner hit them — most notably an OpenAI Responses-API content-type bug that would have broken every multi-turn conversation on its second question — and logged seven more as Known Limitations rather than silently dropping them. Full detail: `RELEASES.md` v0.63.0, `docs/releases/v0.63.0.md`. `v0.62.0 - Knowledge Impact` shipped and signed off immediately before this, same day (see condensed summary below). `v0.64.0 - Explore Convergence` is the reserved next slot but stays un-kicked-off, gated on the Diagnostic Read (due ~2026-08-06). **Read "Company Redefinition & Where Things Stand Now" below before proposing what comes next — it supersedes the old single-retention-bet framing and is the single most important section in this file right now.**
...
- **Phase 2 (IA / Explore convergence) — split, re-gated 2026-07-24, half shipped.** `v0.59.0` (Dashboard hero → Primary Review Set card, Progress promotion) shipped once Reusable Practice Assets shipped. `Explore Convergence` (new nav item, segmented Review-Sets/Notes control, Exam Hub additive official-set check) stays gated on the Diagnostic Read showing a discovery problem specifically — still un-kicked-off, now numbered **`v0.64.0`** after four slot reclaims by unrelated work whose own gates cleared first (`v0.60.0` Shared Official Pool Foundation, `v0.61.0` Challenge Quiz Quota Increase, `v0.62.0` Knowledge Impact, `v0.63.0` Ask Companion — all ratified/gate-cleared same-day work, not Diagnostic-Read-gated).
