...
3. **Current state:** `v0.83.0 - Target Audience Removal (Phase 2)` is **In Progress**, kicked off 2026-08-17 — it **removed Target Audience from note authoring and from Public Library discovery** and replaced the retired facet with an **Authored Depth `?level=` filter**. Previous: `v0.82.0 - Authored Depth Backfill` (**Released 2026-08-16, deployed 2026-08-17**), which amends `ADR-001` from **five note metadata axes to FOUR**, ratifying that **Target Audience is retiring** — on the narrow grounds that it reaches no generation prompt and its access-control purpose was never implemented. **⚠️ The ~99.3% Course/Program correlation is explicitly NOT the justification** (board-heavy-catalog artifact, not equivalence). `V117` backfills `notes.learner_level` on **curator-owned notes only** (`BOARD_TAKER`→`BOARD_EXAM_REVIEW`, `PROFESSIONAL`→`PROFESSIONAL`); the **4,645 learner-owned notes are never touched**, and **`STUDENT` stays NULL** because it spans four depths. **⚠️ `BOARD_TAKER` is NOT self-certifying — it has now failed in two audited programs**, so the exclusion is a **denylist** of non-licensure program values (Information Technology plus the academic levels), never derived from `exam_goal_slug`. **In production only ONE mapping is live:** no `PROFESSIONAL` note exists. **⚠️ As of `v0.83.0` the authoring field, the filter chips and `?audience=` ARE removed** — a legacy `?audience=` link now degrades to an unfiltered library rather than erroring. **The COLUMN is retained** and is still written from the owner's profile, because it is `V117`'s input and the 09-16 kill criterion cannot run without it. Its replacement, an Authored Depth `?level=` filter, is **partially populated**: 80 of 120 curator public notes formerly tagged `STUDENT` have NULL depth and sit outside every depth-filtered result until curators classify them. **Phase 2 was UNGATED from 09-13 at signoff, but dropping the column waits on `[CHECKPOINT — due 2026-09-16]`**, which needs `target_profile_type` to run its kill criterion. Previous: `v0.81.0 - Challenge Bank Integrity` (Released 2026-08-16).
