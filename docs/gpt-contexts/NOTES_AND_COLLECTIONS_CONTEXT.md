...
> Last updated: v0.119.0 - 2026-09-06 (Released). **That single-note operation is now also available in BULK to curators** — up to 50 notes per batch from Library selection, each still handled as its own atomic unit resolved from **its own** context, so no batch-wide metadata is applied over authored titles and tags. A preflight states both meters' cost and remaining allowance, the public notes affected and the shared quizzes that will be turned off; failures continue rather than stopping the batch, and **retry re-runs only the FAILED items as a new batch** — never the ones that already regenerated. **The Library also gained a Review Set membership filter.** **⚠️ Bulk changes CONTENT only — never membership, Section, ordering, Subject, Domain Context, Depth or Applicable Programs, and there is no propagation to learner copies.** *(previously v0.118.0)* - 2026-09-05 (Released). **A note's content and its Study Pack can be regenerated as ONE operation**, preserving note and pack identity and the note's `note_collection_items` row with its `position` and `label`. Learner copies are untouched — there is no propagation. **A failed regeneration leaves the ORIGINAL content and ORIGINAL pack in place and charges neither meter.** **⚠️ `PUT /notes/{id}` rejects with 409 `NOTE_GENERATION_IN_PROGRESS` while a note is `GENERATING`** — the only note content/metadata endpoint is a single upsert with mandatory content, so a metadata-only edit is not expressible. **⚠️ The AI metadata suggestion appears ONLY when a note has no Study Pack yet.** *(previously v0.116.0)* - 2026-09-05 (Released). **`v0.116.0` added learner-initiated ADDITIVE updates to an adopted Review Set — additions only; rename, reorder, retire and move are reported and never applied. Re-adoption remains a hard no-op. Adoption now stamps a Companion structure baseline.** Previously v0.115.0 - 2026-09-05 (Released). **`v0.115.0` changed §2's copied-note row: a learner-owned copy's program representations are frozen only WHILE PRIVATE — publishing clears the copied join rows and un-shadows the learner's own field, transferring classification authority to them. Curators are excluded.** Previously v0.113.1 - 2026-09-04 (Released). **`v0.113.1` added one nuance to the line below: a LEGACY plan-scoped session (pre-`V133`, so it still carries a valid pack/note pair) whose collection was deleted now falls back to that pack anchor on the session-addressed route instead of 404ing. A post-migration plan-scoped session has no pack anchor and correctly still 404s.** **Deleting a Study Plan no longer deletes the practice done against it.** Completed and forfeited plan-scoped sessions are orphaned rather than destroyed and stay reachable through the notes they sampled; only unfinished sessions on that plan are cleared. Notes are still never deleted by a collection delete.. **⚠️ THIS STAMP HAD BEEN STALE BY MORE THAN TWENTY RELEASES; the §0 lists below are current as of `v0.111.0` and the historical narrative that follows is NOT.** **The Domain Context taxonomy is ELEVEN values, not eight** — `ARCHITECTURAL_DESIGN`, `ARCHITECTURAL_HISTORY_AND_THEORY` and `PLANNING_AND_SITE_DEVELOPMENT` were added on ALE evidence, all `quantitative = false`. **⚠️ The catalog is FORTY-FOUR programs as of 2026-09-05, not 41 and not 21** — it grows by curator addition, so re-read it rather than quoting any figure here — that list was `V106`'s seed and had been stale for a month; proposing against it is what produced off-catalog recommendations. **⚠️ There is no `ARCHITECTURE` Domain Context — but do NOT read that as "Architecture has no Domain Context", which is the instruction that produced 215 contaminated ALE rows.** Architecture material routes to `ARCHITECTURAL_DESIGN`, `ARCHITECTURAL_HISTORY_AND_THEORY` or `PLANNING_AND_SITE_DEVELOPMENT`, and a 2026-09-05 audit **rejected adding `ARCHITECTURE`** on the ground that `ARCHITECTURAL_DESIGN` already covers space planning, circulation and anthropometrics. Previously v0.89.1 - 2026-08-20 (v0.89.1 is IN PROGRESS, not released — `v0.89.0` is the last released version). **`v0.89.1` makes `users.birth_year` correctable.** It was account-global and write-once, so a learner who declared an adult year — by mistake, or coached by a supporter who wanted the guardian-consent step gone — permanently disabled consent for **every future connection they would ever form**. **⚠️ The load-bearing half is the re-evaluation:** correcting to a younger year pauses every ACCEPTED connection that has no consent record, reverting it to PENDING (never REVOKED — the gate is about consent, not about ending a connection both people agreed to). Consented links are untouched; consent already given is not retracted. **⚠️ Birth year is still collected at LINK TIME only — never at signup, onboarding or profile** — and only the current value plus a last-changed timestamp is stored, with no history, because it is a minor's personal data. **⚠️ `v0.89.0` shipped Learning Connections as a CAPABILITY, not a profile type** — a parent, tutor, sibling or mentor can make a quiz for someone, connect with them, and see their progress. **`ProfileType` answers "how do YOU learn?", not "may you help someone?"**, so the old design forced a parent to claim to be a Teacher — changing their own dashboard and practice options — just to help their child. **⚠️ Do NOT re-gate any of this on `ProfileType`; `PARENT` stays unimplemented with zero users.** **⚠️ THE PRIVACY LINE IS RATIFIED AND ABSOLUTE: a supporter sees readiness, progress and quiz performance, NEVER the learner's notes** — this protects the core loop, because learners who suspect notes are visible write less honestly. Teachers still exclusively keep DOCX export, multi-version exports, question-count control and the Exam Builder; only the single-quiz share link opened up. Previously v0.89.0 - 2026-08-19. **`v0.89.0` shipped Support Another Learner in three phases.** (1) Creating a shareable quiz no longer requires the `TEACHER` profile — **`ProfileType` answers "how do YOU learn?", not "may you help someone?"**, so a parent previously had to misrepresent their own profile, which drives dashboard emphasis, quiz-mode availability and generation behaviour. (2) **Linked learners**: invite + accept in BOTH directions, revocable either side, acceptance mandatory; age captured AT LINK TIME (never at signup) with guardian consent below a configurable threshold whose number is **owner-owned pending counsel**. (3) **The product's FIRST cross-user read** — a supporter with an `ACCEPTED` link sees readiness, progress and quiz performance. **⚠️ THE PRIVACY LINE IS ABSOLUTE: never the learner's notes**, and the projection is counts-only — no concept names, note titles or collection titles. **⚠️ Viewing writes NOTHING** (no `ConceptHealth`, no streak, no study day), so supporter *view* frequency is unmeasurable by design. **⚠️ Account existence is still observable** via the inviter's own list; the fix is email-keyed invitations, and **multi-recipient invites must not ship before it**. **⚠️ `users.birth_year` is account-global and write-once with no correction path.** Owes `[CHECKPOINT — due 2026-09-19]`: zero `ACCEPTED` relationships kills the demand hypothesis. Previously v0.88.0 - 2026-08-19. **`v0.88.0` made Subject Plan sections authorable.** Sections are still DERIVED from the existing per-item `label` — **no table, no new entity, no third collection level** — but a curator can now create one from the Builder's per-note control, and **Bulk Generate takes an optional Section, pre-filled from the batch subject and editable**, so generated notes arrive already grouped. **`Group by subject`** sections a whole plan from the subjects the notes already carry. **⚠️ `Not in a section` is a RESERVED sentinel defined once in `collection-labels.ts`** — every authoring surface refuses it and both reading surfaces fold any casing into the bucket, because a second copy of that string lets a curator mint a section that renders identically. **⚠️ Only the sentinel comparison is case-folded; grouping between real sections stays case-sensitive.** Also: `subject` (64) and `course/program` (120) request bounds now match their columns — user input is REJECTED, **LLM-generated values are CLAMPED**, because rejecting one would discard an already-billed generation. Previously v0.87.0 - 2026-08-18. **`v0.87.0` made Bulk Generate failures legible: the receipt now records WHY each topic failed**, not just that it did — an application error contributes its code and user-facing message, anything else a generic reason plus the error type only, never a raw message. **⚠️ It changed NO validation** — the same inputs succeed and fail as before. **⚠️ The reason data is EPHEMERAL: receipts are deleted on read and expire after 24 hours, so it cannot be queried retrospectively** — the server log is the durable record; an empty `bulk_generation_result` table does not mean nothing is failing. **⚠️ `v0.87.0` opened as *Published Bounds* and rescoped on its own gate** when a live 14-topic probe showed the three unpublished word bounds (`title` 12, `overview` 90, `keyIdea` 40) never fire — 14/14 clean, nothing within 15% of a ceiling. **Do not re-propose that fix without new evidence.** Previously v0.86.0 - 2026-08-18. **`v0.86.0` added an age-based recovery sweep for generation work stranded in a non-terminal status** — 37 exam question pools (18 `GENERATING` + 19 `PENDING`, corrected 2026-08-18) (oldest 6.5 weeks) and 1 Long Exam session were permanently stuck in production, because the code that would restart them refuses while the row still reads `PENDING`/`GENERATING` and nothing swept them. Stale rows now resolve to `FAILED`, which the existing next-use refresh and Retry paths already recover from; **recovery never re-dispatches, never auto-regenerates, and never touches quota**. **⚠️ The note half is PROSPECTIVE — production had ZERO stuck notes; do not describe it as a backlog.** **`v0.86.0` also changed how generated-note bullets are length-checked:** `coreDetails` and `whyItMatters` are bounded by **characters** (240, the bound the prompt states and the schema enforces), not by a 28-word ceiling published nowhere — formula bullets are word-dense and character-light, so quantitative topics were failing a limit the model never saw. **⚠️ `title`, `overview` and `keyIdea` still carry unpublished WORD bounds (12/90/40) — if generation fails with `invalid title` / `invalid overview` / `invalid key idea`, that is why; publish the bound in the prompt rather than raising it.** Previously v0.85.0 — 2026-08-18. **`v0.85.0` made Domain Context declare whether its material is quantitative**, replacing a substring guess that matched 49 English keywords against the value's own display label — so computation guidance used to track a program's NAME, not its content. **⚠️ Impact is PROSPECTIVE: it changes the prompt for zero notes today**, because the property fires only on a classified note and classification is at 12.7% (121 of 956 curator public notes). `GENERAL_EDUCATION`, `PROFESSIONAL_EDUCATION` and `PROFESSIONAL_PRACTICE_AND_REGULATION` declare `false` and fall through to the unchanged keyword scan. Previously v0.83.0 — 2026-08-17. **⚠️ Target Audience is REMOVED from authoring, display and discovery as of `v0.83.0`** — see §0. It is no longer a request or response field; the column survives, written server-side, pending `[CHECKPOINT — due 2026-09-16]`. **`v0.79.0` made the 21-row `course_programs` catalog the first
...
| **Curated note copied by a learner** | **⚠️ CORRECTED 2026-09-05 — "as copied" implies POPULATED and is often NULL.** `create` sets the string `curator ? null : …`, so **608 of 1360 curated public notes (44.7%) carry a NULL string**, and their copies inherit that null. This row describes the V107-era shape, which is why all 536 EXISTING learner copies happen to agree — and not copies made today | copied rows (**never say "inherited"** — no live inheritance exists; the curator retains no authority). **⚠️ Both representations are FROZEN WHILE THE COPY STAYS PRIVATE**: shadowing skips the string and the Applicable Programs editor requires `isOwner && isCurator`, so the learner CANNOT edit either. That is a capability gap, not a consent gap. **⚠️ CORRECTED 2026-09-05 — `v0.115.0` ENDS THE FREEZE AT PUBLICATION, and this cell previously said "FROZEN" unqualified:** publishing a learner-owned copy CLEARS the copied rows, which un-shadows the string, so **the learner then states their own classification** — authority is TRANSFERRED, not merely revoked. A null string stays null (public but UNSHELVED) and is never backfilled from the joined catalog name. **⚠️ Curators are EXCLUDED — a curated note keeps its authored programs on publication, and without that exclusion the clearing would destroy the catalog** |
...
- **Adoptable Study Plans:** an admin can publish a collection as an adoptable plan over already-public notes (v0.31.0 exception to an earlier "no independently adoptable sub-plans" rule). Learners don't study the source plan directly — adopting creates a **private snapshot copy** into the learner's own library, reusing the public-note-copy spine with `includeStudyPack=true`. The published source plan stays a snapshot source, never a live MIRROR — and since `v0.116.0` a learner may PULL upstream ADDITIONS on demand via `applySourceUpdate`. Nothing syncs automatically, and rename, reorder, retire and move are reported without ever being applied. **⚠️ MEASURED 2026-09-05 — THE SNAPSHOT IS PERMANENT IN PRACTICE AND EARLY ADOPTERS ARE STRANDED.** Re-adoption is still a **hard no-op** — but since `v0.116.0` an adopted Review Set can pull upstream **ADDITIONS** through a separate update path (`GET`/`POST /collections/{id}/source-update`). **⚠️ Additions only: rename, reorder, retire and move are REPORTED and never applied**, because until source-at-sync provenance has history the system cannot tell learner customization from upstream change: `adopt()` bails at `:830-835`, *before* `copySourceItems` runs at `:840`, and **as of `v0.116.0` exactly ONE code path adds a source note to an existing adoption — `applySourceUpdate`, learner-initiated, additions only.** Production carries **92 learners, 523 adopted collections, 4,374 placements**, with **364 source placements and 68 whole Subject Plans stranded — 100% attributable to post-adoption growth, 0% to learner removal** — and **10 no-op re-adopt attempts by 4 distinct users, so learners are already asking.** **⚠️ BUT THE FIX IS NOT A SYNC ENGINE: `copyNote` is IDEMPOTENT per `(owner_user_id, copied_from_note_id)`, so re-running the existing copy pass is ALREADY additive and non-destructive** — every owned note resolves to the learner's own copy, preserving sessions, `ConceptHealth` and edits, and only genuinely new source notes mint rows. What is missing is the decision to call it, plus three provenance facts. **⚠️ Sequencing is a CORRECTNESS constraint: the learner-copy publish fix must land BEFORE the first update pass** — 100% of adopted placements' source notes carry program join rows while only 11.8% of existing copies do, so an update pass run today grows that population ~8x in one batch.
