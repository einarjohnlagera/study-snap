...
- **Whether a note is quantitative is a declared property of the Domain Context, not a guess about its name.** Each `DomainContext` value carries a `quantitative` flag beside its label, and `isQuantitativeContext` short-circuits to `true` when the effective domain declares it. A domain declaring `false` — `GENERAL_EDUCATION`, `PROFESSIONAL_EDUCATION`, `PROFESSIONAL_PRACTICE_AND_REGULATION`, `v0.111.0`'s `ARCHITECTURAL_DESIGN`, `ARCHITECTURAL_HISTORY_AND_THEORY` and `PLANNING_AND_SITE_DEVELOPMENT`, and `v0.145.0`'s `BASIC_MEDICAL_SCIENCES` — falls through to the pre-existing keyword scan over the effective authoring domain, subject, tags, concept hints and summary, exactly as an unclassified note does. **`BASIC_MEDICAL_SCIENCES`'s own false-negative repair is `"pharmacokinetic"` in `QUANTITATIVE_KEYWORDS`, measured against production rather than assumed** — see `ADR-001`'s `v0.145.0` revision-log entry for why the other two originally-proposed strings were rejected. **The declared check is additive by construction: it can only turn the signal on, never off.** Prefer declaring it on the enum over lengthening `QUANTITATIVE_KEYWORDS`; the keyword list is the fallback for unclassified notes, not the definition. The flag is compile-time and never persisted, and it does not change the prompt payload — `effectiveAuthoringDomain` still sends the display label — so past generations stay reproducible. **`v0.148.0` split 7 of the keywords (`ratio`, `solve`, `current`, `interest`, `integral`, `balance`, `units`) into `QUANTITATIVE_KEYWORDS_ANCHORED`, matched with word-boundary regexes instead of plain substring `contains`** — the unanchored form matched inside unrelated words (`ratio` inside "corporation", `solve` inside "resolve"), confirmed against ~4,890 affected notes in production. The anchored patterns also accept each keyword's plain plural/verb inflections (`ratios`, `solve`/`solves`/`solved`/`solving`, …), which a bare `\bkeyword\b` would otherwise miss on genuine quantitative content. `"nursing"` and `"accountancy"` ship in the same release as two more unanchored `QUANTITATIVE_KEYWORDS` entries — the same false-negative-repair pattern as `pharmacokinetic` — since some `domain_context IS NULL` content in those fields reached `quantitative=true` only via the substring bug being fixed.
...
The job runs every ten minutes by default, processes at most `200` candidates per surface per run, reports recovered count and oldest age, and has a deploy kill switch. Every bound, the cron and batch size are configuration-owned placeholders; they can be tightened after production observation without a code change. `V118` seeds existing non-terminal pool attempts with deploy time rather than reused-row `created_at`, so no live attempt is swept early and genuinely stuck rows become eligible one full bound after deploy.

**Notes with a null enqueue clock (`v0.154.0`).** `StudyPackService` is the single writer of `GENERATING` and stamps `generation_enqueued_at` in the same transaction, so a null clock today can only be a data anomaly — not a freshly enqueued row — and every occurrence is still warned in the job log so a future non-atomic writer would surface immediately, exactly as before. What changed: the sweep now also **recovers** these rows instead of leaving them for the self-service endpoint alone, using `updated_at` as the fallback clock and the same `noteBoundMinutes` bound — the identical rule `POST /notes/{id}/recover-stranded-generation` already applied to this row class, now enforced automatically as well as on request. `V118` seeded the clock for any note already `GENERATING` at deploy time, so this is prospective, defense-in-depth coverage rather than a fix for an active incident: a live read-only production check at the `v0.154.0` kickoff found zero notes in this state.

For a null-clock row still within its `updatedAt` bound (or on-demand at any point after the bound), the
note owner can also call `POST /notes/{id}/recover-stranded-generation`. The transaction locks and
rechecks the owner-scoped Note, requires `GENERATING`, and invokes the same `markNoteGenerationFailed`
transition as scheduled recovery. It never dispatches generation or charges quota. Success changes the
Note to `FAILED`, after which the normal Retry action is available; an early, repeated, or otherwise
ineligible request returns `409 GENERATION_RECOVERY_NOT_ELIGIBLE`. Note Detail shows this low-prominence
recovery action only for a first generation with no usable artifacts after the bound — the scheduled
sweep now usually beats the user to it, within one job interval of the bound elapsing.
