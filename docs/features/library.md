...
- Library cards receive ordered Applicable Program names in the paginated list projection and show those names, capped with a `+N` overflow indicator. Notes with no join rows show the legacy string. Cards never issue a per-note applicability request.
