...
- **⚠️ The three filter-pruning effects (subject, courseProgram, tags) were REMOVED in `v0.95.1` and must not be re-added.** They stripped a selected value whenever it was absent from the facet options, so applying a saved filter whose subject no longer had notes silently rendered the **whole library** instead of an empty result. **A filter that matches nothing is still a filter.** They predate the move to server-side filtering, when facets were derived client-side from loaded notes and an off-list value could not be rendered meaningfully; `getLibraryFilterOptions()` now takes **no parameters** and returns facets across the whole library, which turned the pruning from a rendering constraint into a silent discard of an explicit choice. The empty state (*"No notes match these filters"*) and its Clear filters button already handle the outcome, and the controls are comboboxes over a text input rather than `<select>`s, so they render a value that is not in their option list.
- **Each filter control renders while its own filter is ACTIVE, not only while its facet list is non-empty.** Gating purely on the facet list would hide the control for the last subject in a library, leaving the filter applied and its own clear button unreachable.
- **Deleting a note returns to the filtered library the reader came from.** Delete navigates to the same validated `ref` target the BackLink uses (`/library` or `/collections/` prefixes only, falling back to `/library`), so an active filter survives and a note opened from a collection returns to that collection. Before `v0.95.1` delete hardcoded `/library` while the BackLink honoured the ref — the two disagreed in the same file.
...
- `Study Pack Ready` matches notes with a Study Pack whose own status is `DONE`, regardless of whether
  the Note lifecycle currently reports `GENERATING` or `FAILED`, and remains visible for Student, Board
  Taker, and Teacher profiles.
...
- Facets use `UNION ALL` over mutually exclusive joined and zero-row legacy populations. A multi-program note therefore contributes once to each applicable program, so course/program facet counts can correctly sum above the note total; the More Filters panel no longer states this (corrected in `v0.71.1`/M11: the explainer sat under the Course / Program dropdown, which renders no counts at all, while the Subjects facet that *does* render counts is single-valued and can never exceed the total). The panel now explains applicability instead: *"A note can apply to several programs, so it appears under each one it serves."*.
