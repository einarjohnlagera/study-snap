...
- At narrow mobile widths, selections above eight programs initially show eight removable chips plus an explicit “Show all N selected programs” control. Expanding restores every individual remove action; collapsing keeps the surface's primary action reachable without changing the saved selection. Desktop continues to show the full wrapped chip row.
...
`course_programs.is_active` controls whether a catalog program can be newly selected for authoring. New programs are active by default. Setting a program inactive retires it from new authoring without deleting the catalog row or changing existing Notes: an inactive program already present in a Note's explicit selection continues to resolve as a normal removable chip. Inactive programs do not appear in individual suggestions and do not count as family members for expansion.
