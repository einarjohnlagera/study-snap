...
- **All four filter facets load for signed-out visitors as of `v0.83.2`.** Subject and Course/Program are sourced from `GET /subjects?scope=public` and `GET /course-programs?scope=public`, which were **not permitted anonymously before `v0.83.2`** — `SecurityConfig` ends `.anyRequest().authenticated()` and neither path was listed, so both 401ed and `Promise.allSettled` turned them into empty chip lists while Tags and Authored Depth populated normally. Both endpoints are permitted for **GET only**; the `scope=mine` branch inside each controller still requires a caller and is what keeps the widening from exposing a user's own values.
...
**`?level=` is slug-tolerant, matching `?subject=` and `?courseProgram=`.** `?level=senior-high`, `?level=SENIOR_HIGH` and `?level=senior%20high` all resolve to `SENIOR_HIGH`: the server normalises hyphens and whitespace to underscores in `LearnerLevel.fromSlug`, and `resolveLearnerLevel` does the same client-side. **Both sides must normalise or neither should** — if only the server does, a slug-shaped link filters the results while the chip renders unselected. `fromSlug` is deliberately separate from `fromString`, which is also used by the authoring metadata parser and by quiz level overrides; the looser contract belongs to the URL boundary alone.

Two tolerant parses guard `?level=`, and they read from **different** sources: the server uses `LearnerLevel.fromSlug`, while the client validates against the hand-maintained `LEARNER_LEVEL_OPTIONS` list in `lib/learning-profile.ts`. Both ignore unknown values, so an invalid parameter is safe either way. **But if a value is ever added to the `LearnerLevel` enum without being added to `LEARNER_LEVEL_OPTIONS`, the client will silently drop it** — the URL keeps the parameter while the page renders unfiltered. Keep the two in step when the enum changes. The chip list itself is not exposed to this: it is data-driven from `GET /notes/public/learner-levels`. **⚠️ Re-audited 2026-09-17 (the original 2026-08-17 figure below is stale by 3.5x and preserved only for history):** the true curator-owned public NULL-depth population is 286 notes (282 curator-owned across `STUDENT`/`BOARD_TAKER`/`PROFESSIONAL`, plus 4 learner-owned), against 1,896 total public notes — not the 80 first measured. 173 of those were created *after* the `v0.83.0` deploy via bulk-generate, which does not require Authored Depth on a `makePublic` batch, so the population was still growing while the original figure was being cited as static. See `docs/claude-plans/authored-depth-legacy-backfill-audit-and-plan.md` for the full re-audit and the resulting prevention/cleanup plan. Original 2026-08-17 figure, preserved: 80 of 120 curator-owned public notes formerly classified with the `STUDENT` audience remained NULL-depth, so the replacement mechanism was only partially populated until curators classified those notes.
