...
A note's context counts as quantitative in one of two ways, and the first is authoritative. **(1) Declared:** the effective Domain Context carries a `quantitative` property on the `DomainContext` enum itself — `ENGINEERING_MATHEMATICS`, `ENGINEERING_SCIENCES`, `CIVIL_ENGINEERING`, `PROFESSIONAL_PRACTICE_AND_REGULATION`, `NURSING` and `ACCOUNTANCY` declare `true`; `GENERAL_EDUCATION` and `PROFESSIONAL_EDUCATION` declare `false`. **(2) Inferred:** if the domain does not declare it (including when no Domain Context resolves at all), the course/program, subject, title, concept hints and summary are scanned for quantitative keywords (engineering, physics, math, accounting, finance, chemistry, statistics, …). The declared check runs first and only ever *adds* — a note the keyword scan would have caught is still caught, so `false` means "fall through to the scan", not "never quantitative". Do not read the keyword list as the definition of the signal; before v0.85.0 it was, and because one keyword is `engineering` the signal tracked a program's *name*, leaving 48.4% of the public catalog without guidance. When the context is quantitative, the prompt should allow computation questions, formula-based questions, word problems, unit conversions, and multi-step calculations when appropriate. Keep them multiple choice; ensure the computed answer matches one choice exactly; explanation should show short step-by-step solution flow.
