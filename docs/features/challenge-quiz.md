...
- Bank persistence is genuinely best-effort: generated rows are written in a **transaction separate from the Challenge session's**, so a duplicate or any other write failure is absorbed and the session still completes and still serves its questions. A failure loses that batch of bank rows — questions the learner never sees — rather than the session. **Catching the failure inside the session's own transaction would not work**: a failed flush marks it rollback-only and PostgreSQL aborts it, so the session's commit fails whatever the catch does. **Bank READ failures are a different and weaker guarantee, stated honestly:** `claimEligibleQuestions`, `countEligibleIncorrectQuestions`, `claimIncorrectQuestions` and `updateOutcomesAndReleaseClaims` catch `RuntimeException` and fall back to fresh generation, but they run **inside the session's transaction** — so that fallback holds only for failures which do not poison the transaction. A genuine database error still fails the session, because PostgreSQL aborts the transaction regardless of the catch. Do not read these as offering the same protection as the write path; only `releaseClaims` and the generated-write path are transaction-isolated. Adaptive Practice remains intentionally always-fresh and does not use this bank.
