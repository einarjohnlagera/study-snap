...
- Board Exam Mode uses its configured target count after representative Review-Set sampling; source count is
  not an exam-length multiplier.
- Board Exam Mode does not use progressive generation; question count is fixed at session start
- Board Exam generation completes asynchronously through this service after its start transaction commits.
...
- Bank rows are unique per `(user, Study Pack, question, learner level)`. A question regenerated after an authored-depth correction can coexist with the preserved row at the previous level instead of colliding with it. Nullable legacy levels use nulls-not-distinct uniqueness, so they retain the same duplicate protection.
- On both session start and `+5 Questions`, Challenge Quiz claims eligible banked questions before calling the LLM, then generates only the shortfall. A banked question must match the note's effective curriculum level (`notes.learner_level` -> reader level -> `COLLEGE`) and is deduplicated against the live session with `QuizDeduplicationUtils`.
...
- Bank persistence is **best-effort in intent but only partially guaranteed**, and the limit is worth stating plainly: generated rows are written inside the Challenge session's own transaction, so a constraint violation marks that transaction rollback-only and PostgreSQL aborts it — the surrounding catch cannot rescue the session. The widened uniqueness key `(user, Study Pack, question, learner level)` removes the deterministic collision, so a question regenerated after an authored-depth correction coexists with the preserved old-level row instead of colliding. **A concurrent same-level duplicate remains possible and would still fail the session.** Isolating the write in its own transaction was attempted and reverted, because the bank's session foreign keys cannot see an uncommitted session row. Bank **read** failures share the same limit: the fallback to fresh generation holds only for failures that do not poison the transaction. Adaptive Practice remains intentionally always-fresh and does not use this bank.
- When at least three eligible bank entries were last answered `INCORRECT`, the Challenge result handoff offers **Redo Missed Questions**. It starts a normal `CHALLENGE` session sourced only from those same-effective-curriculum-level claimed questions, with no LLM call and no Challenge quota consumption to *start*. The availability count and the claim query resolve the same effective curriculum level, so the CTA cannot diverge when the note and reader levels differ. The same-route handoff resets the completed result and reliably starts the redo session; if fewer than three entries remain eligible, it returns to a clean prestart with the backend's message. A bank failure or a claim race fails closed rather than starting a partial redo. Completion still records `ConceptHealth` through the ordinary Challenge path; this is an entry point, not a new quiz mode or sub-mode. The frontend's one-shot retrigger guard resets on every start outcome (success or error), so requesting Redo Missed Questions again later in the same page session (e.g. after redoing once, then missing questions again) reliably fires a second time instead of silently no-op'ing (v0.60.1). Redo session matching uses a dedicated provenance marker: a marked prior redo session still resumes, while an unrelated active Challenge session on the same Study Pack is forfeited and replaced with a fresh missed-only session instead of silently returning its original full question set (v0.60.3).
...
- **⚠️ The admin backfill reads the catalog in THREE bounded queries, never two per note (v0.125.0).** It used to load every `PUBLIC` `NoteEntity` — ~1,442 rows, each carrying `content` — plus every matching `StudyPackEntity`, then run `userRepository.findById` AND `existsByUserIdAndStudyPackId` **inside** the loop: ~2,884 queries and two full entity loads for one admin action. It now reads note and Study Pack **projections** (`NoteOwnerVisibilityProjection`, `StudyPackOwnerProjection` — id, owner, visibility/note id only), resolves Official authors once for the distinct owner set via `findAllById`, and batches the bank-existence check into `findOwnerStudyPackPairsByStudyPackIdIn`. **⚠️ WHICH notes are eligible is unchanged** — the shape and Official-author legs are one shared predicate the eager per-note hook also uses, and the bank check is still asked only about notes that already passed both, exactly as the old `||` short-circuit did, so `queued`/`skipped`/`rejected` mean what they meant before. **⚠️ It is deliberately NOT paged** (the response is three counts over the whole catalog, so a page would change what the numbers mean) and the **dispatch order stays `updated_at desc`**, because it decides which packs seed first when the executor queue fills.
...
- Template rows are read-only and are never shared as a learner's live bank rows. Eligible unseen template questions are copied into fresh rows under the learner's own user id and copied Study Pack id, claimed by the live session, tagged with the adopter note's effective curriculum level, and initialized as `UNANSWERED`. This preserves each learner's own outcomes and Redo Missed Questions history.
- The source read intentionally ignores the Official author's bank-row learner level. The copied row uses the adopter note's effective curriculum level so later ordinary claims use the same key. The adopter note already inherits its authored learner level from the Official source through note copying. Any template shortfall still uses the existing LLM path; private/non-Official Study Packs, Adaptive Practice, quotas, and the redo-missed claim path are unchanged.
...
The control saves the reader's profile level. It remains relevant to prompt scaffolding and is the curriculum fallback for notes without an authored learner level, but it does not change which questions are claimed for a note that carries its own learner level. The note's authored curriculum floor wins in that case.

Current save toast:

- `Learner level updated. Future Study Packs and quizzes will match this level.`

The retained toast predates note-level authoring and now overstates the control for notes with an authored learner level; changing that UI copy is outside this backend-only re-keying.
...
A note's context counts as quantitative in one of two ways, and the first is authoritative. **(1) Declared:** the effective Domain Context carries a `quantitative` property on the `DomainContext` enum itself — `ENGINEERING_MATHEMATICS`, `ENGINEERING_SCIENCES`, `CIVIL_ENGINEERING`, `NURSING` and `ACCOUNTANCY` declare `true`; `GENERAL_EDUCATION`, `PROFESSIONAL_EDUCATION`, `PROFESSIONAL_PRACTICE_AND_REGULATION`, `v0.111.0`'s `ARCHITECTURAL_DESIGN`, `ARCHITECTURAL_HISTORY_AND_THEORY` and `PLANNING_AND_SITE_DEVELOPMENT`, and `v0.145.0`'s `BASIC_MEDICAL_SCIENCES` declare `false`. **(2) Inferred:** if the domain does not declare it (including when no Domain Context resolves at all), the effective authoring domain (Domain Context label, else course/program), subject, tags, concept hints and summary are scanned for quantitative keywords (engineering, physics, math, accounting, finance, chemistry, statistics, …). The declared check runs first and only ever *adds* — a note the keyword scan would have caught is still caught, so `false` means "fall through to the scan", not "never quantitative". Do not read the keyword list as the definition of the signal; before v0.85.0 it was, and because one keyword is `engineering` the signal tracked a program's *name*, leaving 48.4% of the public catalog without guidance. When the context is quantitative, the prompt should allow computation questions, formula-based questions, word problems, unit conversions, and multi-step calculations when appropriate. Keep them multiple choice; ensure the computed answer matches one choice exactly; explanation should show short step-by-step solution flow.
