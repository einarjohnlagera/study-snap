...
`GoalCollectionDetailResponse` also carries `companionMayBeOutdated`. **Since `v0.116.0` it is no longer ADMIN-only:** it fires for a curator (ADMIN or TEACHER) and for the owner of an adopted plan, where it renders as a read-only notice — a learner cannot regenerate a Companion, so the signal is informational by design. **⚠️ It could never fire for a learner before `v0.116.0`, and the predicate was not the reason: adoption copied `companion` but never the structure snapshot, so the method returned `false` at its FIRST guard. Adoption now stamps a baseline from the LEARNER's own structure** — never the curator's, whose ids could never match. It is `true` only when the requester is ADMIN, Companion content exists, a saved structure snapshot exists, and the current structure no longer matches that snapshot. The comparison is intentionally structural and lightweight: child/note membership count and sorted ids only. It does not compare note body edits, generated content, concept counts, mastery, or per-user progress.
...
The structure snapshot is never COPIED on either adopt path, even when Companion itself is copied on a genuine cross-owner adopt — the new row's children/notes have freshly-generated ids, so a copied snapshot would never match and would flag the adopted copy as outdated immediately. **⚠️ CORRECTED IN `v0.116.0`: that argument is right and was read as settling the whole question, which it did not. Not copying is correct; leaving the snapshot NULL was not.** A null snapshot makes `companionMayBeOutdated` return `false` at its FIRST guard, so the signal could never fire for any adopted plan — 523 adopted collections carried a Companion-and-no-snapshot state and 82 of them a copied Companion. **Adoption now COMPUTES a fresh baseline from the LEARNER's own structure** (note ids for a leaf adoption, child ids for a Goal, stamped after the children are reparented), which is neither the curator's ids nor null. **⚠️ THIS SENTENCE DESCRIBED THE PRE-`v0.116.0` BEHAVIOUR AND IS SUPERSEDED — retained in corrected form because it is exactly the claim a later session would otherwise restore.** It read: *"`companionStructureSnapshot` stays null on every freshly-adopted row … until the new owner's first `PUT /collections/{id}/companion` save captures a real one."* A learner cannot make that call — every Companion write path is `assertAdmin` — so for them the snapshot stayed null forever and the staleness signal was unreachable by construction. **A freshly-adopted row that carries a Companion now carries a baseline computed at adoption**; a row with no Companion still carries no snapshot, and `companionMayBeOutdated` still reads `false` there (nothing to be stale).
