...
The collection detail page renders a Companion card in both top-level view branches when renderable content exists; the whole card is hidden when the Companion is null or only contains empty draft fields. As of the "View Full Guide" collapse (see Coach Experience, below), the card's five sections do not render inline — the card shows only its header and a "View Full Guide" toggle by default, and the sections render only once expanded. Overview, Study Strategy, Common Mistakes, Resources, and FAQ answers use the shared `SummaryMarkdown` renderer. Empty individual sections are skipped.
...
- **Expanded-guide visual treatment.** When expanded, `CompanionDisplayCard` still renders the same five authored sections in the same order, but each populated section now appears as an icon-led sub-panel. The coach-voice emoji is a leading icon chip rather than inline heading text; the markdown body still uses `SummaryMarkdown`.
