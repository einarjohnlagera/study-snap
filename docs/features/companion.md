...
- **Coach intro.** `CompanionCoachIntro` renders directly above the Companion card, only when an authored Companion exists *and has renderable content* (shared `hasRenderableCompanionContent` check — same gate `CompanionDisplayCard` uses, so a non-null Companion with only whitespace/empty fields shows neither card, not an orphaned intro with nothing beneath it). It picks one of two coach-voice lines based on whether `primaryStudyAction` (already resolved on the page) is set — a "there's more to do" tone vs. a "you've worked through everything" tone. It deliberately does **not** restate the action's title, the weekly countdown, or readiness numbers, since `PrimaryActionCard` / `GoalWeeklyCountdownCard` / `ReadinessSummary` already show those verbatim a few cards up — restating them here would read as a duplicate, not a coach. It also says nothing about the Companion or "the guide" itself — that's `CompanionDisplayCard`'s own "View Full Guide" affordance's job (below); having both cards point at the same hidden content read as redundant during review, so the intro stays Coach-only per the Coach vs. Companion split (`docs/product/ROADMAP.md`). No new data fetch, no new persisted state.
- **"View Full Guide" collapse.** `CompanionDisplayCard` is collapsed by default — regardless of viewport, unlike the note-list sections above it, which default to expanded on large screens. It always renders its header (`labels.companionSingular` eyebrow + "Learning {companionSingular}" title) and a "View Full Guide" / "Hide Full Guide" toggle; the five sections render only when expanded (local component state, not persisted — resets on navigation/reload). This is the actual fix for "feels like documentation": the live-signal cluster above it (countdown/primary-action/readiness/coach intro) is what a learner sees first, and the full authored guide is reference material, reachable but opt-in.
