...
- **Authoring copy names the outcome, never the mechanism.** The Domain Context and Authored Depth empty options currently read "Automatic — based on the program" and "Automatic — based on the reader", not "Use … fallback". "Fallback" is implementation vocabulary. Canonical curated generation no longer uses the curator's profile program as a Domain Context fallback; learner-owned material retains profile personalization. `NoteEditorForm` groups these controls under `Generation context`.
