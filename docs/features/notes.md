...
Each of the eight Domain Context enum values declares whether its catalog is quantitative; computation guidance no longer depends on the value's English display label containing a keyword. This declaration is additive: subject, tags, concept hints, and summary still pass through the existing quantitative-keyword scan, and a null Domain Context still uses the unchanged program-name fallback through that scan. No lookup or persistence is involved. The curator-only Domain Context select shows the selected value's covered-subject description at the point of choice, including Engineering Economics under Engineering Mathematics.
