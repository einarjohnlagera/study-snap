...
`DomainContext` is a closed architectural enum, **curated and governed by `ADR-001` rather than fixed at a number** — eleven values: the eight ratified at Release A plus three added in `v0.111.0`, listed below. Adding one is an architectural decision with an evidence bar; see that ADR's *Domain Context governance* section, and do not restate the bar here:
