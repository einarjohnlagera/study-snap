...
- **Authoring copy names the outcome, never the mechanism.** The Domain Context and Authored Depth empty options read "Automatic — based on the program" and "Automatic — based on the reader", not "Use … fallback". "Fallback" is implementation vocabulary; the underlying resolution chain is unchanged. `NoteEditorForm` groups these controls under `Generation context`.
