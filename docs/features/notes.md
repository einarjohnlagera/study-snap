...
- **Course / Program and Applicable Programs stay separate fields, and helper text is what keeps them distinct.** Course / Program is the curriculum the note is authored for; Applicable Programs is where learners should discover it. Because the first value seeds the second, authors reliably ask whether they are the same field — so both authoring surfaces state *why* it reappears: "Every program whose learners should discover this note. It starts from your Course / Program above — add others so one note can serve several curricula instead of being duplicated." Do not merge the fields, and do not delete that sentence; it is the fix for a real and repeated point of confusion.
...
- Applicable Programs are discovery-only metadata. They never enter `StudyPackGenerationContextResolver`, any generation request, or a prompt, and changing them never affects quota.
