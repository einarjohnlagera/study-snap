...
- **A learner-authored note carries no mechanically derived Applicable Program rows, and being served by the personal-string fallback is its canonical shape — not a degraded one.** A learner's free-text Course / Program lives only in `notes.course_program` and is never materialized into a `note_course_program` row; `V108` removed the rows `V107`'s unfiltered backfill derived from learner-owned non-copy notes, so a learner's edit to their program stays effective on both discovery and generation instead of losing to a join row they cannot reach. **Rows inherited by copying a curated note are preserved** — they were authored by a curator, and copy inheritance is the only legitimate route to a join row on a learner-owned note. Read semantics never consult ownership: joined programs first when they exist, otherwise the personal string. There is deliberately no learner-facing Applicable Programs UI, no `source` provenance column, and no re-derivation or clearing of rows on a learner save.
