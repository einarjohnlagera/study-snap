...
Each Domain Context enum value declares whether its catalog is quantitative (twelve values as of `v0.145.0` — the set is a **curated closed set governed by that ADR**, not a fixed constant and not one-per-program; see its *Domain Context governance* section for the bar a new value must clear, and do not restate that bar here); computation guidance no longer depends on the value's English display label containing a keyword. This declaration is additive: subject, tags, concept hints, and summary still pass through the existing quantitative-keyword scan, and a null Domain Context still uses the unchanged program-name fallback through that scan. No lookup or persistence is involved. The curator-only Domain Context select shows the selected value's covered-subject description at the point of choice, including Engineering Economics under Engineering Mathematics.
...
- **Catalog creation accepts zero, one, or several existing Program Families** (`CourseProgramCreateModal`, the shared component also used by the Admin catalog surface as of `v0.152.0`), not a single optional family — `CreateCourseProgramRequest.programFamilyIds` is a list. Assigning the new program to one or more families is what makes it participate in each family's unconditional authoring expansion; a program with no family remains individually selectable but is not inferred into any family. New-family creation is not part of note authoring.
...
- Domain Context stays visible from the start. It is optional for saving any note, but required to generate when the note has more than one program because the AI needs one authoritative academic domain; a program list is never sent to a prompt.
...
- **Note validity is not generation readiness.** Note Editor, Note Detail, Create a Note, and the admin Applicable Programs screen may save two or more programs with no Domain Context. Every note-to-Study-Pack generation path then rejects that saved state with `MultiProgramDomainContextRequiredException` before a `GENERATING` status, quota spend, rate-limit spend, or LLM call. Bulk/topic generation keeps its existing pre-generation check because it creates and generates in one operation.
- **Publishing a copied note transfers its program classification from the curator to the learner (`v0.115.0`).** A learner's copy inherits the curator's `note_course_program` rows verbatim, and public discovery is join-first — so before this, a learner who published their copy had the *curator's* rows decide which public shelves it appeared on, for a note the curator never published. At the `PRIVATE → PUBLIC` transition on a **learner-owned** note those copied rows are cleared. **⚠️ Curators are excluded, and that exclusion is load-bearing rather than defensive: without it, publishing would strip the authored applicability off every curated note and destroy the catalog.**
  - **It transfers authority rather than merely revoking it.** At zero join rows the note is no longer shadowed, so the learner's own Course / Program field becomes readable, is rendered, and is required — they state their own classification.
  - **A null string stays null: the note is public but UNSHELVED until the learner answers.** It is never backfilled from the joined catalog name, which would relaunder curator classification through a different column.
  - **Only the transition clears.** A resave of an already-`PUBLIC` note does not re-clear, or it would destroy programs the learner set after publishing.
  - **Visibility and program-specific discovery stay separate concerns.** `NoteVisibility` is unchanged, publishing is never blocked, and no discovery query changed.
- **An omitted `courseProgramText` retains the note's own stored value rather than stamping the owner's profile program (`v0.115.0`).** This branch became reachable *because* publishing un-shadows the field: a note with zero join rows is no longer shadowed, so every later edit reaches the resolver. `UpsertNoteRequest`'s `@JsonAlias` already mitigated one cause of a null arriving there; the resolver now handles the rest. The profile fallback survives only for a note with no stored value at all.
- **Learner updates still read the stored program set for shadowing, not validation.** A learner request carries no Applicable Programs while inherited rows survive untouched, so the stored count is required to decide whether the learner's personal Course / Program string is readable. Clearing Domain Context may make that string readable again on a 2+ program copy; it does not make the note generation-ready.
- **The depth control is labelled `Authored Depth`** (`v0.75.0` item 4, `ADR-001` constraint 4). The axis remains `notes.learner_level` and is still called Note Learner Level in `ADR-001` and the API contract — **the rename is copy-only.** Target Audience no longer occupies an authoring label.
...
- **Existing curator notes may carry Authored Depth derived once from Target Audience by `V117` (`v0.82.0`).** The historical migration applies only to NULL depths on `ADMIN`-owned notes: `BOARD_TAKER` maps to `BOARD_EXAM_REVIEW`, `PROFESSIONAL` maps to `PROFESSIONAL`, and `STUDENT` stays NULL. `BOARD_TAKER` notes stay NULL when any of their programs is on a denylist of values known not to be licensure programs — `Information Technology`, plus the academic-level values `Grade School`, `Junior High`, `High School` and `Senior High …` — whether the program appears in the catalog join or the free-text field. This is a one-time data migration, **not a runtime fallback**; normal note creation, editing, and generation must never derive depth from Target Audience.
- **Authoring copy names the outcome, never the mechanism.** The Domain Context and Authored Depth empty options read "Automatic — use note context" and "Automatic — based on the reader", not "Use … fallback". "Fallback" is implementation vocabulary. Curator note editing shows the resolver-derived outcome as either `Writing domain: <name>` or `Writing domain needs attention`; it never names a precedence level, and renders neither state until the existing Applicable Programs read succeeds. Canonical curated generation no longer uses the curator's profile program as a Domain Context fallback; learner-owned material retains profile personalization. `NoteEditorForm` groups these controls under `Generation context`.
- **The shared control derives Program Families from each catalog program's many-to-many `programFamilies` membership** (`course_program_family`, `v0.150.0`), not the vestigial scalar `course_programs.program_family_id` column — application code does not read that column. A family action unconditionally adds every member to the current selection without removing hand-picked programs, then shows the explicit removable chips immediately so the author can trim before saving. Subject, Domain Context, and learner level never condition expansion.
...
- Curator note saves carry the full catalog id set and reconcile join rows transactionally. `PUT /notes/{id}/applicable-programs` remains the dedicated full-set save path; unknown ids are rejected before any row changes, and retrying the same set is idempotent. Domain Context readiness is checked later, when generation is requested.
...
`DomainContext` is a closed architectural enum, **curated and governed by `ADR-001` rather than fixed at a number** — twelve values: the eight ratified at Release A, three added in `v0.111.0`, and one added in `v0.145.0`, listed below. Adding one is an architectural decision with an evidence bar; see that ADR's *Domain Context governance* section, and do not restate the bar here:
...
- `ARCHITECTURAL_DESIGN` — Architectural Design *(added `v0.111.0`)*
- `ARCHITECTURAL_HISTORY_AND_THEORY` — History and Theory of Architecture *(added `v0.111.0`)*
- `PLANNING_AND_SITE_DEVELOPMENT` — Planning and Site Development *(added `v0.111.0`)*
- `BASIC_MEDICAL_SCIENCES` — Basic Medical Sciences *(added `v0.145.0`; role-in-the-knowledge stays `NURSING`, role-only-in-the-audience belongs here — see `ADR-001`'s `v0.145.0` revision-log entry)*
...
- On `/profile`, the Note Editor, private Note Detail, the Dashboard lightweight profile-completion prompt, and **onboarding Screen 2** (added in `v0.128.0` — it was `v0.79.0`'s one deliberate exclusion, held out to protect a signup-funnel read that has since been discharged), `courseProgram` autocomplete suggests names from the live Course Program catalog first, followed by the retained hardcoded fallback and normalized saved/current values. A slow, failed, or empty catalog response falls back immediately to the hardcoded list; the current value remains selectable even when off-catalog. Free text stays allowed on every one of these surfaces — the `ADR-001` field-locking amendment remains unratified.
- Free text remains accepted on `/profile`, the Note Editor, private Note Detail and **onboarding Screen 2**. **The Dashboard lightweight profile-completion prompt is the exception and always has been** — it passes `allowCustom={false}`, so it accepts only offered options. Catalog-first ordering is convergence by default, not validation or a catalog lock, and no release has changed any surface's `allowCustom` setting — `v0.128.0` included, which added onboarding to the catalog-first set **without** locking its field.
- **⚠️ CORRECTED 2026-09-07 (`v0.128.0`).** This list previously ended with *"Onboarding deliberately continues using the hardcoded `COURSE_PROGRAM_SUGGESTIONS` list until after the 2026-09-11 completion checkpoint."* **That is no longer true and the line is removed:** the checkpoint was discharged early (read taken 2026-09-07; the deciding fact was an n=18 post-`v0.73.0` denominator, not the rate), the owner lifted the freeze, and onboarding joined the catalog-first surfaces. The source-text test that pinned the exclusion was removed with it, so **nothing now asserts onboarding uses the hardcoded list** — the behavioural guards in `app/onboarding/onboarding-course-program.test.tsx` assert the opposite.
...
- `notes.target_profile_type` remains required in storage but is not an authoring field. `NoteService.mapOwnerProfileTypeToNoteTarget` writes `BOARD_EXAM -> BOARD_TAKER`, `PROFESSIONAL -> PROFESSIONAL`, and `STUDENT` for every other owner profile on create. It never becomes a runtime depth fallback.
- Domain Context and Authored Depth are visible only to Teacher/Admin authors in the product UI, on both authoring surfaces (Note Editor and Note Detail's inline metadata panel). Both are optional single-selects; blank remains a real null state rather than a fabricated default. Because `PUT /notes/{id}` is a full replace, a surface that hides these selects must send the note's stored values back untouched rather than the empty draft — hiding a field must never null it.
...
- leaves `subject`, `courseProgram`, and `tags` unset/defaulted for later user review; `NoteService.create` supplies the retained storage-only target value
...
- editable metadata: `title`, `courseProgram`, `subject`, `tags`, plus `domainContext` and `learnerLevel` for Teacher/Admin authors (`private-note-detail-page-client.tsx` → `handleSaveMetadata`)
- locked field: `content`
- helper text: `Note content cannot be edited after generating a Study Pack. You can still update the title, course/program, subject, and tags.` — Teacher/Admin authors get the longer variant naming Domain Context and Authored Depth as well
