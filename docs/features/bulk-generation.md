...
The section field is meaningful only with a selected target plan, so it is hidden and omitted from the request otherwise. It is capped at 120 characters and begins as a copy of the visible batch subject (itself capped at 64, so the section cap is never the binding limit on the pre-fill), and stops tracking subject edits after the curator edits the field. Blank values are omitted, and a value colliding with the reserved **Not in a section** bucket name is refused inline on the field before the batch is queued — no LLM call, no quota spend. On completion, the backend validates the value through the collection item's existing optional-label guard and adds successful notes with that assignment; a membership failure remains logged and swallowed so it cannot fail the generated batch.
