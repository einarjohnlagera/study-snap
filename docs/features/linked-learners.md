...
| `EXPIRED` | An unconfirmed relationship request timed out; nobody is said to have withdrawn it | No action can revive the row; a new invitation may create a new relationship |
...
The original five interleavings are pinned by `LinkedLearnerConcurrencyTest`, which runs two transactions on two threads and asserts persisted state rather than trusting a returned DTO. **The expiry interlock is stronger evidence:** `requestExpiryRacesCannotStrandAnAcceptedLearnerInEitherCommitOrder` in `NativeQueryPostgresIntegrationTest` uses real PostgreSQL rows, real provisional declarations and real threads for sweep versus acceptance, revocation and birth-year correction, in both commit orders. It asserts the forbidden state directly: no `ACCEPTED` relationship may have both a provisional declaration and a null account birth year. **⚠️ The grant-versus-revoke race is NOT in the Mockito harness:** the conditional insert's `ACCEPTED` predicate is executed for real by `NativeQueryPostgresIntegrationTest`.

Live duplicate rows for the same supporter → learner direction are prevented by a partial unique index covering `PENDING` and `ACCEPTED`. `REVOKED` and `EXPIRED` history do not block a fresh invitation. A database check and a service guard both prevent self-linking. Invitations carry their own partial unique index over inviter and address, active only while the invitation is `PENDING`; their separate status constraint remains `PENDING | ACCEPTED | REVOKED`, because invitation expiry is represented by the carrier's `expires_at`, never `EXPIRED`.
...
Only `markAcceptedIfPending(...) == 1` promotes the provisional value to the write-once account column, and promotion never overwrites a non-null value. Acceptance clears the request's `expires_at`, then deletes the provisional row after promotion in the same transaction. Revoking or expiring the relationship deletes it without promotion, and relationship/account deletion removes it through the foreign-key cascade. The row is not declaration history. An unconfirmed redemption leaves no account-level birth-year trace.
...
**Every path that FORMS a connection requires a FINISHED onboarding**, not merely a verified email — invitation-link endpoints (`v0.97.0`), email invitation acceptance (`v0.98.0`) and **inviting** (`v0.99.0`). **⚠️ `accept()` is deliberately NOT gated, and that is a call-graph conclusion rather than an omission:** every path creating a `PENDING` relationship now gates onboarding at creation, `acceptInvitation` calls `accept()` internally and is itself gated, and `onboardingCompletedAt` is never cleared on a live account. **Accepting therefore INHERITS the requirement from whichever path created the relationship**, and gating it again would put a third gate on the guardian-consent path. Originally, in `v0.97.0`, this applied to the link endpoints alone — every invitation-link endpoint, and **as of `v0.98.0` email invitation ACCEPTANCE too**, since the two form the identical relationship with identical guardian-consent handling and identical cross-user capacity. **⚠️ The email path's tightening is scoped to the ACTING endpoint alone: `listInvitations` and the relationship list are deliberately NOT gated**, because making an invitation invisible to its recipient converts an error into an absence, and an absence gives them nothing to act on — the opposite of what `v0.90.0` made invitations visible for. On both paths the caller is routed to `/onboarding` and returns to complete the action. Originally, in `v0.97.0`, this applied to the link endpoints alone — `create`,
...
The relationship request has a deliberately separate clock: `studysnap.linked-learners.request-ttl-days` (default and minimum 30). Its deadline is always relationship `created_at + request-ttl-days`; using redemption-carrier expiry, deployment time or a shorter duration would invalidate the dated provisional-row production read. A scheduled sweep changes only due `PENDING` requests to terminal `EXPIRED`. It never sweeps `ACCEPTED`, and acceptance clears the request deadline. A consent pause returning an accepted relationship to `PENDING` therefore has no request deadline and is not mistaken for an unconfirmed redemption.

**⚠️ A relationship still awaiting guardian consent DOES expire, and that is a decision, not an oversight.** `accept()` returns early when consent is required, leaving the row `PENDING` with its deadline intact, so a request blocked on a guardian's permission lapses 30 days after creation like any other. The reasoning, recorded because the opposite intuition is natural: such a relationship has **never been `ACCEPTED`**, so expiry ends a request nobody completed rather than severing a confirmed connection; the protective outcome for a minor is **no connection**, not one pending indefinitely while their declared birth year is retained; and re-inviting is always available. **⚠️ This is the opposite of the consent PAUSE above** — that row was accepted once, so it is protected. The two look alike (`PENDING`, consent outstanding) and are distinguished solely by whether `expires_at` is set. The connection card shows the date so the supporter can act before it.

The two clocks must stay separate. Invitation/link expiry says whether a carrier can be used. Request expiry says how long the relationship created by a used carrier may await confirmation. Expiring the request does not unredeem or re-arm its carrier, and there is no un-expiry endpoint: trying again means sending a new invitation. The connection surfaces describe this as a timeout, distinctly from deliberate revocation, while every authorization read continues to require exactly `ACCEPTED` and returns the same not-found contract for terminal relationships.
...
**⚠️ The RELATIONSHIP list applies the same bounded-retention rule as of `v0.98.0`:** live rows are always returned, and terminal (`REVOKED`, `EXPIRED`) rows only while `coalesce(revoked_at, expired_at)` is within `request-ttl-days` — the row's own terminal timestamp. **⚠️ `expired_at` is a SEPARATE column from `expires_at` as of `v0.99.0`, and the distinction is load-bearing:** `expires_at` is the DEADLINE and `expired_at` is when the sweep actually acted. They are equal only while the sweep runs daily and unbacklogged — and `v0.98.0` added a batch bound and a pause hook, so a late-swept request keyed on the deadline arrived already outside the window and **vanished without ever being shown as expired**. **⚠️ `expires_at` is never overwritten**, and a NULL there still means the row is not on the expiry clock at all. **⚠️ The status allowlist is load-bearing, not redundant:** a `PENDING` request past its deadline but not yet swept must stay visible, or a live request vanishes from its owner's list before it has actually expired. **⚠️ A terminal row missing its timestamp is retained, never hidden.** This is bounded retention, not dismissal — nothing deletes a relationship row, and re-inviting a hidden pair mints a new relationship as before.
