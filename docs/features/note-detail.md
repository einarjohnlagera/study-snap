...
- Teacher/Admin see the derived authoring outcome from the initial Applicable Programs response: `Writing domain: <name>` when the generation resolver resolves one, otherwise `Writing domain needs attention`. The display never exposes resolver precedence and renders neither state if that read fails.
