...
- when the panel is closed, the header shows **one muted line** combining the primary Course / Program with a reach count — `Accountancy · Applies to 6 programs`. **Program names are never listed here.** Applicable Programs answer *who can discover this note*, and discovery is already complete by the time this page renders; the names' only actionable home is the Edit details panel, which is also the only place they can be changed. The count is **suppressed at exactly one applicable program**, because a note seeded from its Course / Program has exactly one, so it would be noise on nearly every note.
- **Do not restore a pill list or a bordered metadata block here.** A previous pass rendered the full set as pills, which consumed roughly a quarter of the mobile viewport and pushed `Start Quick Review` and the entire content tab bar below the fold — authoring metadata outranking the product. It also wrapped a single line in a border for plain student owners, who never see Applicable Programs at all. The rule across surfaces: **names appear where you author them (Edit details, admin table); counts appear where you read (cards, Note Detail).**
