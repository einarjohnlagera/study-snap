...
- when the panel is closed, the primary Course / Program and a non-empty Applicable Programs set render read-only for authorized Teacher/Admin authors inside **one bordered metadata block**, each under its own quiet uppercase label. **They are deliberately not merged into a single "primary first, then the rest" list:** a curated set may legitimately exclude the note's primary program (the zero-join-row guard in `library.md`), so a merged list would assert a containment that is not guaranteed. Applicable Program pills carry a background fill rather than a bare border — an unfilled low-contrast border disappears in dark mode, which is why the Subject badge uses a tint.
