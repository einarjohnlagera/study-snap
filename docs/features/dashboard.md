...
- With no primary, resolve the first exact course/program-matched public plan plus owned collections. An unadopted match renders one named recommendation with its title and `readyCount of itemCount notes practice-ready`, reusing the existing Start/adopt path.
...
- An already adopted match is not recommended again. The valid Primary continue path remains the owned-plan route.
