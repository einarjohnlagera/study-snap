...
Dashboard adds a profile-neutral `People you support` card whenever `GET /linked-learners` returns `PENDING` or `ACCEPTED` relationships where the caller is the supporter. **⚠️ An ALLOWLIST, not a denylist — corrected in `v0.97.0`:** it previously read `status !== "REVOKED"`, which would have rendered the new terminal `EXPIRED` status as a live connection. Accepted connections link to the relationship-scoped progress view; pending connections **name the actual blocker** — the learner's birth year, guardian consent outstanding, or consent recorded and activation still finishing. **⚠️ What `PENDING` means depends on HOW the relationship was created, and this paragraph previously stated only half of it.** For an **email-keyed** invitation (`V122`) a relationship row is created only at acceptance, so `PENDING` does not mean the invitation is unaccepted and the card must not say acceptance is still required. **For a shareable invitation LINK (`v0.94.0`) the opposite is true:** redemption creates a `PENDING` row immediately with the redeemer as initiator, and the link's creator is the party who must confirm — so for those rows `PENDING` means exactly that acceptance is outstanding, and the card **must** say so. The two are distinguished by `incomingInvitation` (the server's own *`PENDING` and the caller is the invited party*), which drives the `AWAITING_CALLER_CONFIRMATION` reason. Only a `PENDING` row with no blocker **and** no incoming-invitation flag falls through to deliberately neutral copy, which is where a legacy pre-`V122` row lands. **⚠️ Do not re-simplify this back to a single rule** — it was one rule until `v0.95.0`, and the consequence was that a creator whose link had just been redeemed was told only that the connection was *not active yet*, never that they were the one holding it up. The vocabulary lives in `frontend/lib/linked-learner-status.ts`, shared with the Learning Connections page so the two cannot drift. The card links to `/linked-learners` for management and does not turn supporting someone into a `ProfileType`, replace the caller's own learning Dashboard, or expose learner notes.
