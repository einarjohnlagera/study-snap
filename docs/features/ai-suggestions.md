...
Keep generated metadata suggestions collaborative and easy to review after Study Pack generation.

## Current Scope

The shared Suggestions modal appears after generation when the product suggests:
...
- `Tags`
  - `Keep My Tags`
  - `Merge My Tags + Suggested Tags`
  - `Use Suggested Tags Only`

## Default Selections

- existing title -> `Keep My Title`
- empty title -> `Use Suggested Title`
- existing subject -> `Keep My Subject`
- empty subject -> `Use Suggested Subject`
- existing tags with new suggested tags -> `Merge My Tags + Suggested Tags`
- existing tags with no new AI tags -> `Keep My Tags`
- empty tags -> `Use Suggested Tags Only`
