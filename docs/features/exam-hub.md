...
Completing Long Exam, Board Exam, or Interview Practice records concept-level signals into `ConceptHealth`, which is the only store `/progress` reads. The write records fully-correct concepts to `lastCorrectAt` and missed concepts to `lastIncorrectAt`. New Long Exam and Interview Practice questions carry a separate `keyConcept` for Progress recording; it is distinct from the report-facing `concept`, which remains the free-form domain/readiness label used in exam reports. Legacy questions without `keyConcept` fall back to `concept`. **For Long Exam, Board Exam and Interview Practice, every generated item carries its source Study Pack id, and completion buckets correct and missed concepts by that source before persisting; it never broadcasts one source's concept list to sibling packs.** **Long Exam** additionally intersects against each source pack's `keyConcepts` as a secondary safety filter, and concepts that do not match are ignored. **⚠️ Board Exam does NOT** — it completes through `ChallengeQuizService`, which calls `recordCorrectAnswers` / `recordIncorrectAnswers` rather than the `…ForKnownConcepts` variants, so no `keyConcepts` intersection is applied and non-matching concepts are recorded. That asymmetry pre-dates `v0.104.0` and is unchanged by it. A missing or unreadable source pack is skipped so completing the exam still returns its report. Pre-v0.104.0 sessions without an item stamp retain their historical Long Exam source-list fallback. **Interview Practice gained the same per-source attribution in `v0.107.0`** — it stamps `sourceStudyPackId` at its generation seam and buckets by `(sourceStudyPackId, concept)` on completion, like Long Exam, and it intersects against each source pack's `keyConcepts` as Long Exam does. **⚠️ The `v0.104.0` Known limitation recording it as the third broadcasting instance is CLOSED. ⚠️ `ConceptHealth` rows written by Interview Practice sessions completed BEFORE that release remain over-attributed — no backfill shipped, matching `v0.104.0`'s precedent.** Pre-`v0.107.0` in-flight Interview Practice sessions whose items carry no stamp retain the historical broadcast fallback. Forfeit paths do not record either signal.
