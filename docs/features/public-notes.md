...
Post-copy landing must match the CTA's verb. `Quiz yourself on this note` (a quiz promise) auto-launches Quick Review on the copy (`redirectTarget="quick-review"` → `?copied=1&generate=1&startQuickReview=1`). `Copy Study Pack` and `Copy to My Library` (copy promises, not quiz promises) land on the **copied note's detail page** so the viewer sees the full Study Pack and chooses their next action — including Challenge Quiz — rather than being dropped straight into a quiz. `Copy Study Pack` uses `redirectTarget="generate"` (lands on detail; generates only if the copy lacks a ready Study Pack); `Copy to My Library` uses `redirectTarget="library"`. Do not route copy-verb CTAs to `quick-review`.
