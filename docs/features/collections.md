...
**Frontend consumption (Dashboard):** `DashboardStudyPlanSection` (`frontend/app/dashboard/dashboard-study-plan-section.tsx`) accepts an optional `primaryCollectionId` prop. When set, it looks the id up via `listCollections()` and, if found, renders that owned collection instead of the course/program-matched public recommendation — skipping the adopt flow entirely (the CTA always resolves to the existing "already owned" render path, i.e. `router.push` straight to `/collections/{id}`, never `adoptGoal`/`adoptStudyPlan`). The heading swaps from `Recommended {labels.singular}` to `labels.primarySingular` (a field on `CollectionLabels` in `frontend/lib/collection-labels.ts`: "Primary Study Plan" / "Primary Review Set" / "Primary Lesson Plan" / "Primary Collection", resolved the same profile-aware way as every other collection label), and course/program recommendation details do not apply once the plan is an explicit choice. If the id isn't found among the user's owned collections (a defensive fallback — the backend invariant should keep this reference valid), rendering falls through to the existing course/program-matched flow, same as if no primary were set at all. It is wired into both Dashboard call sites (`STUDENT`/`PROFESSIONAL` and `BOARD_EXAM`). The onboarding call site does not pass this prop because a new user going through onboarding has no owned collections yet. As of the 2026-07-30 "Explore Owns Discovery" addition, `/collections` no longer consumes `DashboardStudyPlanSection`; it is a pure owned-workspace surface.
...
- `courseProgram` and `profileType` come from `getMe()`; labels resolve through `getCollectionLabels`. Until Dashboard's separately sequenced pointer migration lands, its full recommendation still links into this standalone route. `/collections` no longer does. The route remains directly addressable for deep links, SEO, and anonymous access, while authenticated navigation-level discovery is owned by `/explore`.
- Recommended states: loading skeleton, error + retry, a guidance state when no course/program is set (links to `/profile`), and an empty state when the track has no published plans. Browse All has its own loading skeleton, error + retry, and distinct empty state for the genuine system-wide case where no official public root collections exist. One section failing does not blank the other. `BackLink` is entry-point-aware (v0.45.1): it reads an allowlisted `?ref=` query param (`/dashboard`, `/public/library`, `/collections`, and sub-paths of each) and returns there with a matching label, falling back to `canAdopt ? "/dashboard" : "/public/library"` when `ref` is absent or not on the allowlist — the allowlist stays specific-prefix so it can't be used as an open redirect. Every existing link into this page (both Dashboard `See all N` links and the Dashboard empty-state link) appends its own path as `ref`. As of the 2026-07-30 "Explore Owns Discovery" addition, `/collections` no longer links here directly — its header CTA now points to `/explore` instead (see above).
