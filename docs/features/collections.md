...
Item labels are neutral backend data. The frontend interprets them as section/module names, and the Teacher Exam Builder uses them as initial section/week/topic names. The backend does not group or otherwise interpret them, but bulk generation can write the curator-supplied section value when adding generated notes to a target plan.
...
- **canonical**, non-empty `label` is the single source of truth for grouping. Canonical means trimmed **and internal whitespace runs collapsed to one space**, case preserved — `canonicalSectionLabel` in `frontend/lib/collection-labels.ts`, from which `normalizeSectionValue` is derived so the display and comparison forms cannot drift. **⚠️ It was TRIM-ONLY until `v0.123.0`, and that is what let a stored `"Cash  and Receivables"` name a section no canonical form could equal** — a Builder card then rewrote the same value on every page load and, before the retry bound, retried without limit.
- labels are user-defined free text, not course/program, subject, learner level, audience, or taxonomy data.
- a section is the set of items sharing the same case-sensitive **canonical** non-empty label. **Two labels differing only by internal whitespace are therefore the SAME section since `v0.123.0`**; previously they were two sections that rendered identically.
...
- reordering preserves the displayed relative order of notes and sections. The detail page's dormant organize-mode path scopes drag to a section, while the Builder deliberately allows cross-section note drag. Builder drag results remain local until **Save changes** submits the whole collection and rewrites every row position; notes in untouched sections retain their relative display order. **⚠️ Since `v0.140.0` that control lives in a sticky bar at the bottom of the Builder, NOT in the "Your notes" card header** — the header scrolls out of view on a long plan (production holds a 79-note plan), which left a curator working hundreds of pixels from the only control that commits their work. **There is exactly one Save and one Discard on the page, both in the bar**, asserted by test; the header carries status text only. ⚠️ **That guard matches `/save/i` STRUCTURALLY, not an exact label** — the first version pinned the string `"Save changes"` while the control it replaced was called `"Save order"`, so restoring the header button under its own name passed. **⚠️ On mobile the bar CLAIMS THE BOTTOM VIEWPORT while it is up** (`useBottomViewportClaim(leafOrderDirty)`): `MobileBottomTabBar` is `fixed bottom-0 z-20` and the bar is `sticky bottom-4 z-30`, so without the claim it would pin directly over the navigation and win on stacking order. The tab bar returns the moment changes are saved or discarded — the claim tracks the BAR, never the page, because claiming it while merely browsing would delete the curator's navigation for no reason.
...
**Frontend consumption (Dashboard + onboarding):** `DashboardStudyPlanSection` (`frontend/app/dashboard/dashboard-study-plan-section.tsx`) accepts optional `primaryCollectionId` and `discoveryPresentation?: "full" | "pointer" | "recommendation"` props. `"full"` is the default and preserves onboarding's course/program-matched recommendation and adoption flow. Both persistent Dashboard call sites pass `"recommendation"`: while a configured primary lookup is pending they render nothing; a valid owned primary renders the existing continue card unchanged; otherwise the component reuses `listCourseProgramStudyPlans(normalizedCourseProgram)` plus `listCollections()`, selects `publicPlans[0]`. **Those two reads are skipped entirely whenever a configured primary is still resolving or resolves to a real plan** — that learner renders the continue card and can never see a recommendation, so fetching for them would restore the per-load public-plan call `v0.67.0` removed with no reachable output, and renders one unadopted named match with `readyCount`/`itemCount` coverage through the existing Start/adopt path. No course/program, no match, or either read failing falls back to the existing dashed Explore pointer; an already adopted match is not recommended again. The former `viewAllHref` and `browseWhenEmpty` props remain absent, and no plan grid is restored. The heading for a valid primary still swaps from `Recommended {labels.singular}` to `labels.primarySingular` (a field on `CollectionLabels` in `frontend/lib/collection-labels.ts`: "Primary Study Plan" / "Primary Review Set" / "Primary Lesson Plan" / "Primary Collection", resolved the same profile-aware way as every other collection label), and its CTA goes directly to `/collections/{id}` without adopting again. `/collections` no longer consumes this component; it is a pure owned-workspace surface. **`suppressPointerWhenNoPrimary?: boolean` (v0.67.1)** suppresses only the no-match/no-program/error pointer fallback; the STUDENT/PROFESSIONAL zero-note Dashboard call site passes `true` since `DashboardEmpty` already shows an equivalent "ready-made plan" link there. A valid primary and a genuine named match are unaffected. The legacy `"pointer"` mode remains available and does not fetch public plans.
...
- **PROMOTED, then cleared, on reparent** — `updateParent` (`PATCH /collections/{id}/parent`) clears `targetCompletionDate` whenever a top-level Goal is nested under a new parent (becoming a child), in the same branch that sets `parentCollectionId`. **⚠️ Since `v0.127.0` it first PROMOTES that date to the parent, because it is LEARNER-ENTERED AND UNRECOVERABLE — nothing else records it, and a learner who set a date and later adopted a parent Goal was silently losing it.** The **earliest date wins**, since a completion target is a deadline and the nearest one binds; a parent that already has a nearer date keeps it. This is the same rule `persistAdoptedGoal` already applied on the adoption path — **the two paths had disagreed, and that asymmetry was the defect.** **⚠️ THE CLEAR ON THE CHILD IS NOT THE DEFECT AND MUST STAY:** Without this, a dated Goal that gets nested and later detached back to top-level (`updateParent(null)`) would resurface its stale date instead of starting fresh — the same category of invariant `reassertPrimaryInvariant` already enforces for the Primary Review Set on this same method, just for the target-date field instead.
...
**⚠️ Since `v0.124.0` the collection DETAIL page (`/collections/{id}`) does NOT fetch the note library on load.** `listNotes()` is unbounded (the backend caps nothing when `limit` is null) and every row carries `contentPreview` and `summaryPreview` — the two fields that pushed `/notes/public` past Next.js's 2 MB data-cache limit on 2026-08-31. It used to run on every collection open to derive exactly two things: an **ADMIN-only** visibility map, and the primary exam's Study Pack id. The pack id now arrives on the item itself as `NoteCollectionItemResponse.studyPackId` (free — the projection was already loaded for the due-concept lookup), and the visibility map loads in its own effect **only when the viewer is an ADMIN**. **⚠️ A learner opening a collection issues zero `listNotes()` calls; do not reintroduce one.** **⚠️ `v0.125.0` CLOSED THE CAVEAT BY DELETING THE SECOND COPY. That paragraph used to read: the detail page's own `AddNotesModal` still calls unbounded `listNotes()` on open, and the claim holds only because the modal is unreachable — `setAddOpen(true)` appeared nowhere on this page. It has been REMOVED outright (along with the page's duplicate `filterPickerNotes`, its `handleAdd` and its `addOpen` state) rather than bounded, because bounding it would have duplicated the Builder's debounce-and-search machinery into code nothing renders, where no test could ever exercise it. `GET /notes` therefore has no caller at all on this page other than the ADMIN-only visibility load. ⚠️ Note addition lives in the BUILDER; if a detail-page picker is ever wanted again, build it on the Builder's bounded-and-searched shape, not on the deleted one.** The admin visibility load is deliberately outside `loadCollection` because `authUser` resolves in its own effect — gating the loader on `isAdmin` would re-arm the whole load, which is the defect `v0.124.0` fixed in the builder.
...
The builder page header (leaf and Goal) has a single primary action, `Add {subjectSingular}`; there is no standalone header `Refresh`. A `Refresh` control lives inside the `Add notes` modal instead (next to the search field), scoped to re-fetching just the note list so newly created notes appear as selectable — and **since `v0.123.0` that note list is fetched LAZILY, on first picker open, rather than on page load**, because its only builder consumers are the picker and the add-note handlers that read its selection. **⚠️ One automatic attempt only: a failed load surfaces an error and does not retry, because the retry re-armed itself through the effect's own dependency and hammered the endpoint.**

**⚠️ SINCE `v0.125.0` THE PICKER FETCH IS BOUNDED AND ITS SEARCH RUNS ON THE SERVER, AND THE TWO ARE ONE FEATURE.** The modal requests `listNotes(50, query)` → `GET /notes?limit=50&search=…`; 50 is `NoteController.PRIVATE_NOTES_MAX_LIMIT`, which the server clamps to anyway. **⚠️ THE ORDER WAS A CORRECTNESS CONSTRAINT, NOT A PREFERENCE: `v0.123.0` deliberately declined to bound this list because it filtered CLIENT-SIDE over the whole library, so a limit alone would have made every note past it permanently unaddable — an invisible correctness loss traded for an invisible performance win. Never bound it again without a search beside it.** Three consequences are load-bearing:
- **The server search must match the same FOUR fields the client filter matched** — title, subject, course program and tags, case-insensitive substring. A title-only server search looks correct in every demo and silently makes tag-findable notes unreachable. It is `NoteLibraryRepositoryImpl.appendOwnedNoteSearchFilter`, a **separate** method from `appendSearchFilter`, which serves the public library and is deliberately not widened.
- **The picker's selection holds the NOTES, not just their ids.** `notes` is now the current result page rather than the whole library, so a note selected under one query is absent from the array once the query changes; deriving "Selected (N)" or the optimistic add rows from `notes` drops it silently. The modal keeps a `Map<id, note>`, and `onAdd` hands the selected notes to the add handlers.
- **A truncated list says so.** The picker's empty state already treats "No notes available." as a terminal claim about the library; showing 50 of 1,442 with no sentence attached is the same false claim, so a filled page renders "Showing your 50 most recently updated notes. Search to reach any of the others."

**⚠️ The loop guard survived the rewrite in a different form.** The picker-open effect no longer depends on `refreshingNotes` (that dependency plus a `finally` reset is what turned a rejected fetch into 3,743 calls in five seconds). A `lastPickerRequestRef` records the query whose fetch has already been *started* — set synchronously, before the first await, and never cleared on failure — so a failed search cannot retry itself, while a *changed* query still refetches. A request-sequence ref discards out-of-order responses. Refresh remains the deliberate retry — it does not refetch the collection/goal shape and does not affect the modal's current selection (v0.36.1). The Subject-block row itself still switches from stacked to horizontal layout at the `xl` breakpoint rather than `lg`, because the persistent app-shell sidebar (present from `md` up) consumes real width the viewport-relative breakpoint doesn't otherwise account for (v0.36.1).
...
- sections are the existing item `label` values, with `null` / blank labels grouped as **Not in a section**.
...
- `/collections/[id]` opens in read mode by default. Note cards show title, subject/course metadata, execution status, entitled due-concept signals, and admin private badges. Leaf plan curation links to `/collections/{id}/builder`; the old inline `Organize` toggle is no longer exposed from detail. Its dormant implementation remains as reference code only and is not the authoring contract.
...
- When at least one item has a trimmed non-empty `label`, `/collections/[id]` groups the notes into collapsible section cards (`section name + item count + chevron`). Section order follows the first/minimum `position` in each section, items stay in `position` order within each section, and null/empty labels render under a trailing **Not in a section** section. Section cards start collapsed below the `lg` breakpoint and expanded at `lg` and wider; collapsed sections render only the header row. When collapsed, the card also shows a title peek: the first 1–3 note titles joined by `·`, with a `+N more` suffix when there are more.
- The Builder renders **Not in a section** as a non-editable, non-draggable trailing bucket and suppresses its heading when it is the only group. Other headers are editable up to 120 characters; rename into an existing normalized name requires confirmation and combines the two groups in one whole-plan write. The Builder's section cards start expanded and can be collapsed at every breakpoint.
...
- Item labels are edited in the Builder through profile-aware **Section** / **Part** assignment controls with max length `120`: users can choose an existing name, type a new one, or clear the value to return the item to **Not in a section**. The control persists through `PUT /collections/{id}/items/order`; no new section entity, endpoint, taxonomy, or grouping interpretation is added.
