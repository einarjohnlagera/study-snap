...
- Admins see a published/private **status badge that is itself the publish control** (Notion-style): clicking it (`aria-label="Publish settings"`, gear affordance) opens the publish modal. There is no separate `Publish settings` menu item or `Share` button.
