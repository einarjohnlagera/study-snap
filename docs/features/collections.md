...
**Superseded by v0.43.0 — recorded here so the old shape isn't reintroduced.** Through v0.42.1, `GoalWeeklyCountdownCard` rendered as its own "This Week" card above `ReadinessSummary`. As of v0.43.0's Coach/Progress/Companion hierarchy redesign, `GoalWeeklyCountdownCard` is deleted and its data redistributed — there is no separate countdown card anymore:
