...
- **Out of scope by the same rule, deliberately:** flashcards, memorization, learn articles, the Ask Companion guide FAQ (`collection-detail-page-client.tsx`), and the landing-page FAQ. These render cards, articles, or Q&A prose — not quiz question/option/explanation — so they do not fall under the rule. Whether they *should* render math is a separate product decision, not a gap in this one.
