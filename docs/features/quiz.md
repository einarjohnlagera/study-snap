...
- Session Review export is one of the clarity exceptions:
  - use icon + `Export` label on both desktop and mobile
  - do not collapse it to icon-only

## Session Review PDF Export
...
- styling rules:
  ...
  - minimal green/red correctness accents
  - no UI-like card chrome inside the exported document
- subtle branding only:
  - footer: `Generated by NoteLib`
  - no intrusive marketing blocks or large watermarks
- export must use persisted session data only and must not call the LLM
