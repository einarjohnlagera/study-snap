...
3. ⚠️ **Delimiter PAIRING — A KNOWN OPEN DEFECT. An attempted fix was REVERTED at `v0.141.0` signoff and must not be restored as written.** Defence 1 tells the model *"never use `$` as a currency symbol"*, and **the model does it anyway**. A `$` opens a span whenever the next character is not whitespace, so `$50,000` opens one; the closer correctly skips every intervening amount and closes on the **formula's** final `$`, handing KaTeX an entire sentence. KaTeX fails on the prose and the fallback re-emits the source, so the reader sees the raw formula. **⚠️ THE FIX ATTEMPT MADE IT WORSE, MEASURED: `startsCurrencyAmount` decided money-vs-maths from the SINGLE CHARACTER after the digits, and that character cannot discriminate — the dominant legitimate maths shape puts a space or an operator exactly where the predicate looked for a currency boundary.** `$1.5 \times 10^4$` matched `1.5`, saw a space, was classified as money, and its opener skipped — so a string that **rendered correctly before** printed raw after. Replaying 1,158 affected production strings through both versions: **385 WORSE, 3 BETTER**, concentrated in explanations and working solutions. **⚠️ Any future attempt must decide on the WHOLE CANDIDATE SPAN — e.g. refuse the currency reading when the span up to its valid close contains a LaTeX command or a maths relation — and must be measured against that corpus before shipping.** The regression guards now live in `quiz-question-text.test.tsx` (scientific notation, an arithmetic step, a ratio) alongside a skipped test carrying the original defect.
...
**⚠️ BUT DO NOT READ THAT AS "HAND REGENERATION FIXES THE REMAINING CASES" — corrected 2026-09-10/11.** Re-running that query returns `NEEDS_FIX` = 15, and **7 of those were generated in the last 14 days**, so the population is not a fixed historical backlog that regeneration drains. More importantly, **the defect that actually reaches readers is in the DISPLAY path, not the stored text** — so regenerating a pack repairs nothing while the renderer keeps mis-pairing. ⚠️ Defence 3 above is what would close it for every affected string at once, and it is **STILL OPEN**: the `v0.141.0` attempt was reverted at signoff as a net regression. **⚠️ And a fix aimed at `normalizeBareMath` would be a NO-OP** — it returns early on **any** delimiter, and 339 of 339 backslash-bearing questions plus 498 of 498 explanations already carry one, so the repair is wired correctly at every call site and reaches almost nothing in the live corpus. A session proposed exactly that fix and was wrong; the mechanism is pairing, not absence.

**Where maths renders, as of `v0.141.0`.** `renderMathText` (which calls `normalizeBareMath` internally, so every call site gets defence 2 for free) covers quiz questions, choices, explanations and working solutions, plus the shared Study Pack page's key concepts and full notes. `SummaryMarkdown` covers summaries and, since `v0.141.0`, normalises bare maths before `remark-math` tokenises — that plugin handles **delimited** maths only, so an undelimited expression in a summary previously printed literally. **⚠️ Known gap:** `normalizeBareMath` returns early on any delimiter anywhere in the string it is handed, and a summary is one long multi-paragraph string, so a summary already containing a single `$` is left entirely alone.
...
Configured review weekdays are matched in `Asia/Manila`, the retention email budget zone. Null or empty review days no longer preserve the pre-v0.72.0 digest cadence: `v0.148.0` assigns those learners a deterministic weekday (`Math.floorMod(userId.hashCode(), 7)`) instead of matching every day, to fix a real weekday-clustering effect this cadence produced (see `retention-emails.md`). The Quick Review page records the digest landing and the first submitted answer once each so trigger-to-answer conversion can be measured without changing scoring, ConceptHealth, or readiness.
