...
- A per-subject readiness bar with `X% ready` or `Not started`, plus counts for mastered, due, and not started concepts. Each row links to `/library?subject={subject}` (private Library, filtered).
