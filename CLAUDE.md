...
Base branch for this release: `releases/v0.33.0`. The constraint carried over from v0.32.2 is **near-zero W1→W2 retention (5.6%, recent cohorts ~0%)** — users activate but don't return (longest observed streak: ~2 days). This release attacks retention through the natural trackable unit, the **Study Plan**: turn it from a static folder / one-time adoption surface into a **readiness journey** that gives a learner a number that only moves by coming back to practice. Two tracks: **Track A** removes publish/discovery friction so curated plans actually reach learners (activation); **Track B** (headline) introduces **plan- and subject-scoped readiness** with charts. Full scope in `ROADMAP.md`. Locked rules:

- **Retention is the goal; readiness is the lever.** The headline is a per-plan / per-subject readiness view ("how ready am I for CPALE, what's left") — a reason to return. Adoption/discovery polish is activation support, not the retention bet; don't let Track A crowd out Track B.
- **Deliberate reversal of "plans don't duplicate Progress."** `docs/features/collections.md` previously locked that no mastery %, weakest-subject, or readiness belongs on Study Plans. This release **consciously revisits** that for a *dedicated readiness surface*. The plan **execution-detail rows** (the action/next-step list) keep their no-mastery rule — readiness lives on its own surface, not crammed into those rows.
- **Readiness is derived, not stored.** It reuses the existing **ConceptHealth recency spine** (same source as `/me/progress`) — no new mastery signal, no new persisted progress field on collections, no new generated content, no new AI/LLM call. Aggregation only.
- **Publish decouple, not weaker validation.** Course/program + description persist independently of publishing (fixes the silent metadata loss); **Publish stays gated** on its existing rule (every note public, at least one note) with a clearer blocker message. Do not remove publish validation — only stop discarding metadata when it fails.
- **Discovery stays course/program-scoped.** Recommended plans are matched to the learner's own course/program (not an all-programs browse — that is Public Library's job for *notes*). Surface them on the user's own Study Plans page by reusing the Dashboard "Recommended" section pattern (not tabs).
- **Anti-drift:** no quota / billing / price / checkout change; no new quota category; no per-profile pipeline fork; charts are the *expression* of readiness, never a standalone vanity dashboard.
- **Out of scope / deferred:** teacher bulk-quiz generation & teacher-flow polish (still no teacher cohort — re-pointed to v0.34.0 candidate); live-link / shared-progress plans (different architecture, belongs with teacher-flow).
