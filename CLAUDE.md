...
Current version: **v0.140.0 — Pending Work in Reach** (Released, kicked off 2026-09-10, signed off 2026-09-10, base branch `releases/v0.140.0`, cut from `main` after `v0.139.0` merged and tagged. Source: `docs/claude-plans/authoring-and-quiz-legibility-fix-plan.md` §§5-7 and **§10** — owner-reported 2026-09-05 from real use with screenshots. **⚠️ READ §10 FIRST; it is the plan's own record of where the code contradicts the tightening, and one finding removes a premise this release would otherwise ship on.** **⚠️ THIS IS A LEGIBILITY FIX, NOT A DATA-LOSS DEFECT — do not oversell it.** Navigation protection ALREADY EXISTS, verified at kickoff at `study-plan-builder-page-client.tsx:1663-1671` (`beforeunload` + an in-app click interceptor, both gated on `leafOrderDirty`), so **work is not silently lost today**; the failure is a curator who drags on a long plan, scrolls away from the header holding Save, and redoes the arrangement. **The trigger is VERIFIED LIVE: a 79-note plan, two 77-note plans, a 76-note plan and five more at 59+, with 12 authors creating 1,768 notes in the 14 days to 2026-09-10.** **SCOPE:** (1) **the dirty-state sticky bar** — shown only while pending, holding the status text plus Discard and Save; **⚠️ NO TWO EQUALLY-PROMINENT SAVE CONTROLS**, the buttons at `:2496-2515` MOVE INTO it; (2) **`[DECISION]` — copy wording is OWED FROM THE OWNER before implementation**: `Unsaved changes` OVER-claims (the combobox pick is already persisted) and `Order changes not saved` UNDER-claims (`leafOrdersMatch:118-125` compares `noteId` sequence AND `label`, so a pending drag can also move a note between sections) — **the semantic requirement is fixed even though the words are not: describe drag-originated order AND placement, and never promise isolation**; (3) **§7 navigation protection, NARROWED ON EVIDENCE** — its premise *"current silent loss is unacceptable"* is FALSE (§10 Finding C); what remains is that the existing `confirm()` offers TWO choices where the owner requires THREE, plus §7's named coverage gap; (4) **three Backlog rows corrected** — LaTeX (b) shipped `v0.100.0` 2026-08-29 while the row reads *"candidate for `v0.96.0`"*; the public-catalog unbounded read shipped **ALL THREE LEGS in `v0.119.1`** (2026-09-06) while the row reads *"STILL OPEN IN CODE … NOBODY ACTED"*; and **that row was "verified" as unfixed by the `v0.138.0` pass**, recorded as an error of that pass rather than quietly repaired. **⚠️⚠️ THE STRUCTURAL FINDING: every stale row found across `v0.138.0`, `v0.139.0` and this kickoff was stale in the SAME DIRECTION — overstating open work — and every one shipped in a release that never re-read the row. `v0.138.0` added a verification procedure to KICKOFF; NOTHING UPDATES A ROW WHEN THE THING IT DESCRIBES SHIPS.** That is a missing SIGNOFF step. **The near-miss that makes it concrete: on 2026-09-10 the onboarding redesign — shipped as `v0.73.0` a month earlier — came within one verification step of being scoped and rebuilt as an 8-screen rewrite of a 2506-line file.** **⚠️ ANTI-DRIFT: autosave-per-drop stays REJECTED (it raced itself; two releases were paid to close it); do NOT change the combobox flush — a section pick persisting pending drags is DELIBERATE (*"flush, never discard"*) and requirement 9 is satisfied vacuously, so fix the COPY not the behaviour; item 4 already shipped in `v0.117.0` and the sticky bar disappearing when a section is picked is BENIGN and must not be "fixed"; the Challenge Quiz bank-write isolation is NOT in this release — verified genuinely unshipped at `ChallengeQuizQuestionBankService.java:118-125` (the `REQUIRES_NEW` at `:235` is `releaseClaims`, a different method) but it carries `[DECISION]` with three shapes differing in FAILURE SEMANTICS; no `frontend/app/onboarding` work — the `2026-09-11` checkpoint closed **KILL CRITERION NOT CLEARED** and its own wording says reopen the framing rather than iterate on onboarding polish; no new drag or animation dependency; `globalThis` never `window`.** **Routing: CLAUDE CODE inline** — frontend-only, one component, well under the Codex threshold. **Tier: one `advisor()` call on the diff before commit.** Full scope in `RELEASES.md`.)
