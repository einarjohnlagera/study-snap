...
Current version: **v0.160.0 — Study Plans by Semester** (Released 2026-09-26 on `releases/v0.160.0` pending the release PR to `main`; kicked off 2026-09-26, base branch `releases/v0.160.0`, cut from `main` after `v0.159.0` merged as #1446 and tagged; `v0.159.0` is deployed and verified: Render live 2026-09-25 15:24Z, Vercel production 15:28Z). **Owner-picked scope:** Degree Study Journeys **Phase A0** (ADR-003, documentation first) and **Phase A** (`docs/claude-plans/degree-study-journeys-stage1-architecture-audit.md`, §21 is the implementor handoff): Academic Term as two nullable columns (`term_label`, `term_order`) on the child Subject Plan row, term grouping and compact Subject cards on the Year page, a curator term combobox, and `academic_term` in the curriculum pipeline. Codex in slices (backend, frontend, pipeline); additive in both deploy directions (two optional fields on the existing collection update, and the release notes must say so). **⚠️ THE TERM MUST BE CARRIED AT BOTH CHILD-COPY BUILDERS** — `persistAdoptedPlan` (`NoteCollectionService.java:1956-2017`, serves `adopt()` and `adoptGoal()`) and the Official-update addition `createSubjectAddition` (`:2476-2515`) — and NOT at `persistAdoptedGoal`, the root copy, which has no term. **Anti-drift:** all child terms NULL means existing Review Set rendering is UNCHANGED (no term UI, full-size cards; five live Review Sets); density gated on the same condition as grouping, never a count; Official update additive-only; no Degree progress in any phase; the hierarchy stays at two persisted levels; academic placement never touches the Note (ADR-001); no Degree landing page, entity or `journey_key` fallback; Phase B is the NEXT release (Backlog Index), Phase C out. **Signed off 2026-09-26 with two owner decisions added mid-release: a curated Year is all-termed or none (enforced in the pipeline and by refusing to publish a partial Year), and a term is settled before publication and frozen after; retroactive terms on a published Year are unsupported.** Full scope in `RELEASES.md`.)
...
2. User triggers generation → `StudyPackService.startAsyncGenerationFromNote()` → marks note `GENERATING` and enqueues work
