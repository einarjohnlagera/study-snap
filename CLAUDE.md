...
Current version: **v0.161.0 — Scannable Study Plans** (In Progress, kicked off 2026-09-27, base branch `releases/v0.161.0`, cut from `main` after `v0.160.0` merged as #1451 and tagged; `v0.160.0`'s deploy state and its two post-deploy read-only `SELECT`s (`term_label` count; non-admin roots with `last_update_published_at`) had not been reported when this release opened; see `RELEASES.md`). **Owner-picked scope:** Degree Study Journeys **Phase B**, frontend only, no schema (`docs/claude-plans/degree-study-journeys-stage1-architecture-audit.md` §13, §18, §9.6): Sections collapsed by default at every breakpoint with a single-Section auto-expand exception; an `Expand all` / `Collapse all` toggle; `Not started` instead of `0% · 0 due` at the Section summary row and the shared plan-summary header. **Anti-drift:** Sections stay a computed grouping (no entity); no persistence of expansion state; no accordion single-open; no terminology resolver; no backend, migration or Term/Degree work; the `v0.160.0` term grouping and compact cards are untouched. **⚠️ Carried from `v0.160.0`, binding:** Academic Term is settled BEFORE publication and frozen after (a published Subject Plan's term cannot change, an adopted copy never edits one, a partially-termed Year cannot be published); retroactive terms on a published Year are unsupported and logged as the placement-revisions Backlog row; a Year's subjects need one `plan_no` each in the plan file to carry terms. Full scope in `RELEASES.md`.
