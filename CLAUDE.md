...
- **Bulk-tool quota awareness & gating.** Bulk generate folds the note-generation quota into the Topics cap (`X / min(50, remaining)`), hard-caps add/queue at remaining note generations, soft-confirms a Study-Pack shortfall (notes still get content), and the backend rejects an over-quota batch at submit. Bulk import surfaces remaining OCR/image-scan quota (Draft-only; OCR consumed only for images/scanned PDFs). Reuse the generalized `NearLimitBanner`; read remaining values from `/api/me/plan` only — **never recompute quota from local note lists** (AGENTS.md). No new quota category.
