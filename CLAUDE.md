...
Current version: **v0.143.0 — No Way Out** (Released, kicked off 2026-09-11, signed off 2026-09-12, base branch `releases/v0.143.0`, cut from `main` after `v0.142.0` merged as #1380 and tagged — Vercel and Render both confirmed live on `61153cc6`. Two live defects, both re-verified against current code rather than taken from Backlog Index prose after a fresh candidate survey found four of five surveyed items unbuildable, three of those four because their own rows overstated open work already shipped. **ITEM 1 — LONG EXAM'S FOCUS-MODE TRAP:** `long-exam/page.tsx:255` calls `useExamFocusMode(phase === "running")` with no `!submitting` guard, while its Leave button is `leaveDisabled={submitting}` (`:966`) — if a completion request hangs, the learner is trapped behind a hidden header with no working exit. Challenge Quiz already fixed the identical trap in `v0.131.0`: `challenge-quiz/page.tsx:1516` reads `useExamFocusMode(phase === "running" && !submitting)`, with a comment explaining why the guard is load-bearing. Long Exam was left out of that release's diff. Fix: apply the same guard. **ITEM 2 — EXAM QUESTION POOLS SURVIVE A REGENERATION THAT REPLACES THEIR SOURCE CONTENT:** `ExamQuestionPoolService.initiatePoolForMode` (`:220-227`) early-returns whenever a pool already exists and is READY/PENDING/GENERATING. Regeneration keeps the same `studyPackId` (the documented in-place versioning rule, so quiz/session history stays linked), so the `initiatePool` call after regeneration (`StudyPackService.java:933`) is a silent no-op — Long Exam and Board Exam keep serving questions drawn from content that no longer exists. The fix pattern already exists three lines above the omission, in the same method: `deactivateShareLinksForNote` (`:908`) does the analogous thing for shared quiz links, citing `v0.110.2`'s precedent by name in its own comment; no equivalent exists for the exam pool. **⚠️ EXPLICITLY NOT IN SCOPE: the Challenge Quiz question bank was flagged as carrying the same shape of staleness gap, but this kickoff traced only as far as confirming the seed path it found (`queueOfficialChallengeQuizTemplateSeed`) is official-template-only, not the per-user bank — where the per-user bank is actually populated is unknown. Do NOT scope a third invalidation seam on a structural analogy; verify it first, the same discipline that ruled out the false candidates below.** **⚠️ FOUR STALE BACKLOG ROWS WERE FOUND AND CORRECTED REACHING THIS SCOPE, EACH WITH commit-LEVEL EVIDENCE:** Official Review Set publication P3 (claimed un-parked, shipped `v0.135.0` commit `83074463`); "Support Another Learner" Phase 1 (claimed `[DECISION]`-blocked, shipped `v0.89.0` commit `cbc7d13c` — also a duplicate row for the same shipped arc as "Learning Connections"); Onboarding Intent Router's C8/C9 (claimed open, shipped commit `826ca155`, five weeks stale). Two checkpoints due the same day as this kickoff were also resolved: `[CHECKPOINT — due 2026-09-11]` `v0.114.0` CLOSED (production Render log confirms `DELAYED_ACQUISITION_AND_HOLD` + `open-in-view=ON` across 8+ boot samples, kill criterion (i)); `v0.62.0` Knowledge Impact's conditional-rate checkpoint RE-DATED to 2026-10-09 (its own "fired ZERO times" premise was itself stale — 3 events now exist from 1 viewer, 2 days old, too recent to read the conditional rate). Full detail, including the Adaptive-Practice population-block and the Learning-Connections deferral that were correctly gated rather than stale, in `RELEASES.md` and `ROADMAP.md`'s kickoff scan note. **⚠️ ANTI-DRIFT: item 2 must not change `additionsAvailable()` or any other unrelated Review Set method it does not touch; do NOT scope the Challenge Quiz bank without first tracing where it is actually populated; do NOT let this kickoff's stale-row corrections cascade into a broader Backlog Index sweep — three corrections were made because they were found, not because a sweep was run.** **Verification tier: ONE SCOPED COLD AGENT, falsification-framed, for item 2 only** — it changes what questions a learner is served and touches a `@Transactional` regeneration path carrying two quota meters. Item 1 is a single-expression fix with a direct precedent in the same codebase; a normal test plus `advisor()` is enough. **Routing: CLAUDE CODE inline** for item 1; **CODEX** for item 2. Full scope in `RELEASES.md`.)
