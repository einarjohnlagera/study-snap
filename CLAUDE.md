...
Current version: **v0.133.0 — Education Family** (Released, kicked off 2026-09-08, signed off 2026-09-08, base branch `releases/v0.133.0`, cut from `main` after `v0.132.0` merged as #1347 and tagged). Source: `docs/claude-plans/program-family-generalization-and-education-family.md`. **⚠️⚠️ THE BRIEF'S CENTRAL PREMISE IS FALSE AND THAT IS THE MOST LOAD-BEARING FACT HERE: there is NO Engineering-specific authoring shortcut, so there is NOTHING TO GENERALIZE.** `applicable-programs-combobox.tsx:290` interpolates the family name and count, and families are derived DYNAMICALLY from the catalog (`:88-111`); **there is no Engineering literal anywhere in it.** It says *"Add all 18 Engineering programs"* only because Engineering is the only family with members. **⚠️ SO THIS IS A DATA CHANGE PLUS ONE ADMIN FORM FIELD. DO NOT MODIFY THE COMBOBOX — if a diff touches it, the scope has drifted.** All four authoring surfaces already share it. **SCOPE: (1)** one migration — insert the `Education` family, assign the EXISTING `Education` row to it, **RENAME `Special Needs Education – Generalist` → `Special Needs Education` keeping its `id`** (do NOT insert a second row), and insert Elementary / Secondary / Early Childhood / Technical-Vocational Teacher / Physical Education and `Teacher Certification`; **(2)** an admin family selector on create — `POST /course-programs` ALREADY accepts and validates `programFamilyId`, the form just has no field, and **this is what stops every future family member being another migration**; **(3)** helper-text copy polish, **⚠️ with resolver mechanics dropped from it.** **⚠️⚠️ THE MIGRATION SEED IS NOT THE LIVE CATALOG: `V106` seeds THREE Engineering programs, production has EIGHTEEN, and no later migration inserts any — the catalog was extended through `POST /course-programs` in production, so A DUPLICATE AUDIT FROM THE REPO ALONE IS UNSOUND.** Run `docs/claude-plans/v0.133.0-education-family-precondition-read.sql` (READ-ONLY) BEFORE writing the migration. **⚠️ `GET /course-programs/similar?name=` already exists** — the duplicate check the brief asks for is built. **⚠️ THE RENAME IS SAFE FOR `note_course_program`** (it joins by `course_program_id`, and the row keeps its `id`) **BUT NOT for five free-text `course_program` columns** (`notes`, `note_collections`, `users`, `bulk_generation_result`, `official_study_plan_wishlist` incl. `normalized_course_program`) — nothing breaks, but a stale string stops matching the catalog. **Decision 1 (RENAME, settled 2026-09-08) is CONDITIONAL on that read returning ZERO hits; if not, those strings are a further OWNER decision AND A PRODUCTION WRITE.** **⚠️ TWO DECISIONS OPEN: (2)** do the new programs carry `exam_goal_slug = 'let'`? recommend only where learners genuinely sit the LET, not as a family proxy; **(3)** ship the admin selector now? recommend YES. **⚠️ ANTI-DRIFT: NO new Domain Context** (`GENERAL_EDUCATION`, `PROFESSIONAL_EDUCATION`, `PROFESSIONAL_PRACTICE_AND_REGULATION` already cover LET); **Program Family must NEVER select or override Domain Context, nor be inferred from `GENERAL_EDUCATION`**; **NEVER feed Program Family or an expanded list to the LLM — `StudyPackGenerationContext` has no field for either, so it is STRUCTURALLY IMPOSSIBLE TODAY; keep it that way**; do NOT delete the existing `Education` program; **NO backfill of `note_course_program`**; do NOT infer Applicable Programs from Review Set membership; **family membership is NOT dynamic inheritance** — expansion writes explicit program IDs at authoring time; **NEVER use a credential abbreviation (BEEd, BSEd, CPE/DPE) as a canonical name** — `Teacher Certification` is endorsed because `Professional Education` already exists as a `DomainContext` AND as a Subject and would collide on two axes; do NOT block mixed-family selections. **⚠️ NO Review Set publication work — `v0.132.0` owes `[CHECKPOINT — due 2026-09-22, deploy-relative]` and its F5 gap is recorded, NOT this release's to fix. NO Learning Connections work — `[CHECKPOINT — due 2026-09-19]`, denominator ONE.** **⚠️ TESTS: the rename guard must link a Note BEFORE the rename** (a fixture created after it passes trivially), **mixed-family selection needs TWO families in the fixture** (a single-family fixture proves nothing), and the Engineering-still-expands test is the migration's regression guard. **VERIFICATION: a single `advisor()` call** per the audit's §9 — **⚠️ but re-decide if the shape changes: `v0.132.0` was tiered at one cold agent, ran two, and they found four blocking defects.** **Routing: CLAUDE CODE INLINE** — a data migration, one admin form field and tests. Full scope in `RELEASES.md`.
