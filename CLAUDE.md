...
Current version: **v0.132.0 — Publication Boundary** (Released, kicked off 2026-09-08, signed off 2026-09-08, base branch `releases/v0.132.0`, cut from `docs/planning-publication-boundary-and-plan-corrections` so that branch's audit commit rides in via the release PR). **SCOPE IS SLICES P1 AND P2** of `docs/claude-plans/official-review-set-update-publication-boundary.md`. **THE GAP IS TOTAL: there is exactly ONE source state, so every curator edit is INSTANTLY learner-facing** — adopters are immediately "behind", and new adopters and the public see unfinished curriculum work. **⚠️⚠️ THE FIX IS A PER-ROW PUBLICATION STAMP, NOT A SNAPSHOT ARCHITECTURE.** That works because `applySourceUpdate` is ADDITIVE-ONLY — `NoteCollectionService:2050` states *"THIS RELEASE REPORTS AND NEVER APPLIES"*, and `MOVED` changes are detected and reported but never applied — so the boundary only has to gate what becomes visible as an ADDITION. **Do NOT build snapshots, versions or history.** **(P1)** three columns plus a backfill; filter drift detection, adoption copy and `getPublic` to **published rows only**; an atomic, locked, idempotent `Publish update` service and endpoint. **ONE MIGRATION — the whole contract lives here.** **(P2)** curator UX: the `Publish update` action, an `Unpublished changes` indicator, and a confirmation carrying additions-only counts. **⚠️ P1 AND P2 SHIP TOGETHER AND THAT IS DELIBERATE: P1 alone creates an endpoint with no way to press it, while silently stopping every curator edit from reaching learners — the `v0.130.0` empty-inbox shape.** **⚠️⚠️ THREE OWNER DECISIONS ARE OPEN AND DECISION 1 SIZES THE ARCHITECTURE — settle it BEFORE the Codex prompt:** (1) accept the narrowed public-view contract (§6)? audit recommends YES; (2) backfill timestamp — audit recommends the collection's `created_at`, not `now`; (3) make `last_update_published_at` the public "Updated" date? audit recommends preserving the capability and shipping NO UI. **⚠️ ANTI-DRIFT: P3 (the Review Set update notification, formerly Stage 6) IS DEFERRED ON SIZE — do NOT build it.** This audit UNBLOCKS it: **§7 SUPERSEDES the notification plan's §8 drift-signature dedup, which must NOT be implemented.** **⚠️ NO Learning Connections work — `[CHECKPOINT — due 2026-09-19]` is eleven days out and its denominator is ONE.** **⚠️ Curriculum update != Note content overwrite; one learner remains ONE adopter across every update; publishing makes changes AVAILABLE for review and never forces learner synchronization.** **⚠️ Adoption counts must keep working — this release changes what `getPublic` can SEE and `v0.129.0`'s count reads the same rows.** **⚠️ No public "Updated" date UI. No quota, entitlement or pricing change; onboarding untouched.** **VERIFICATION: AT LEAST ONE SCOPED COLD AGENT, DECIDED AT KICKOFF — this moves a VISIBILITY BOUNDARY and carries a BACKFILL over production rows.** **⚠️ THE `v0.131.0` LESSON APPLIES DIRECTLY: that release picked its tier by the letter of the gate, shipped, and a cold agent then found a trap it had introduced. Decide the tier from the SHAPE of the change, not the item count.** **⚠️ PRE-DECLARED GUARDS: (1)** an unpublished edit is invisible to `getPublic`, to a NEW adopter AND to drift detection — assert all three; **(2)** publish is idempotent and atomic; **(3)** an existing adopter's already-copied content is UNTOUCHED by a publish; **(4)** the backfill leaves every pre-existing set PUBLISHED so nothing vanishes from Explore on deploy — **assert against real migrated rows, not a hand-built fixture**; **(5)** `v0.129.0`'s adoption count is unchanged for a set with no unpublished edits; **(6)** **⚠️ A RENDERED CONTROL THAT IS DISABLED IS NOT A CONTROL** — `v0.131.0` verified an exit by its RENDER gate and missed that it was `disabled`. **Routing: CODEX** — migration + service + controller + frontend. **⚠️ Call `advisor()` BEFORE writing the prompt.** Full scope in `RELEASES.md`.
