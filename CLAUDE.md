...
Current version: **v0.150.0 — Membership, Not a Slot** (Released, kicked off 2026-09-16, signed off 2026-09-16, base branch `releases/v0.150.0`, cut from `docs/correct-stale-title-policy-backlog-row` rather than `main` — that branch carries PR #1402's still-open Backlog corrections, and cutting from it avoids a merge conflict on `ROADMAP.md`'s 1,000+ line Backlog Index table; it rides into `main` via this release's own signoff PR, following the `v0.120.0`/`v0.111.0` precedent for a blocked base branch. Source: `docs/claude-plans/program-family-many-to-many-final-plan.md` (FINAL, Opus architecture audit, independently verified by a peer "Feature Planner" session 2026-09-15; owner approval given directly to this session 2026-09-16, confirmed against the plan's own spot-checkable claims — ADR-001's exact quoted text, zero frontend clients for `updateCourseProgram`, and production family membership counts all verified rather than trusted). **SCOPE: Program Family membership becomes many-to-many** — a Course/Program may belong to zero, one, or several families, replacing the nullable single-FK `course_programs.program_family_id` that made real overlap cases (Computer Engineering and Architectural Engineering are both current Engineering members) structurally impossible to express without silently evicting a program from its other family. **Root cause of the reported bug, CONFIRMED IN CODE, not assumed:** the Note-authoring "Add Course/Program" family picker (`applicable-programs-combobox.tsx:119-128`) derives its options by scanning catalog rows that already carry a family, so a brand-new, empty family (Health Sciences, Accounting, and two more the owner had since created — Computing & Technology, Built Environment & Design) contributes zero entries and is invisible there, even though the canonical Admin family list (`GET /course-program-catalog/families`) shows it correctly. Not hardcoding, not stale cache (no cache library exists in this frontend at all), not a legacy enum. **Prerequisite, satisfied at this kickoff: ADR-001 amendment.** `ADR-001:92` literally forbade "any preset table beyond `course_programs.program_family_id`" — verified verbatim against the file before treating it as blocking. Owner-ratified storage-neutral replacement text (plan §A) lands as Slice 0, the first commit on this release branch, preserving the constraint's substance (expansion stays unconditional and membership-driven) while permitting many-to-many storage; naming which table implements it moves to `DATA_MODEL.md`, not the ADR, so a future storage change never requires re-touching ADR-001. **New table `course_program_family`** (`V146`, additive — verified free against the current migration set at kickoff, which ends at `V145`): copies every existing single-FK membership (Engineering 18 + Education 8 = 26 relationships), asserting parity at the RELATIONSHIP level inside the migration itself (a `NOT EXISTS` check per legacy pair, not a count comparison, which cannot detect a row copied to the wrong family while a different one went missing) and aborting the migration via `RAISE EXCEPTION` on any mismatch. `course_programs.program_family_id` is retained, unread and unwritten by application code after cutover — explicitly no dual-write, since a program with two families has no single correct legacy-column value to write. **Catalog API becomes additive:** `GET /course-program-catalog` gains `programFamilies: [{id,name}]`; the deprecated `programFamilyId`/`programFamilyName` scalars stay populated (alphabetical-first, read-time, sourced from the new table) for one release of old-frontend tolerance. `PATCH /course-program-catalog/{id}` becomes an authoritative `programFamilyIds` list replacement — verified as a free breaking change, since `frontend/lib/api.ts` has zero calls to `updateCourseProgram` (grepped directly, not trusted from the plan). **New Admin Edit action** lets an admin change an existing Course/Program's family memberships end-to-end for the first time — `v0.149.0`'s release notes claimed this worked; it did not, through any UI, and that correction is carried in `RELEASES.md`. **Explicitly out of scope, per the owner's own enumerated non-goals list:** Course/Program renaming; any Exam Goal work (editing, read-only display, or otherwise — not even a column reference); `is_active` gaining a write path (a real, separate gap this audit found — new application code writes it nowhere at all, not even at creation, where the column's own DB-level `DEFAULT TRUE` does the work, not an `INSERT` column; tracked as its own Backlog Index row); the two legacy fused catalog rows (preserved untouched, and their retirement is now understood to be BLOCKED on the `is_active` gap above, not merely deferred data work); family-side membership editing (Family → Programs direction); family or program deletion; react-query/TanStack/websockets/polling (no query cache exists in this frontend today and none is introduced); dropping `course_programs.program_family_id` or the deprecated response scalars (a later, separate contract release, only after this one is confirmed live on both platforms). **Post-deploy data operation (owner-run, not code):** populate all four empty families — Health Sciences, Accounting, Computing & Technology, Built Environment & Design, 17 memberships total, including two deliberate overlap cases (Computer Engineering → Engineering + Computing & Technology; Architectural Engineering → Engineering + Built Environment & Design) — via the Admin UI as the PRIMARY path per the owner's explicit instruction, which doubles as this release's own end-to-end production acceptance test, specifically because `v0.149.0` shipped two half-features in this exact code area that no prior verification pass caught. A documented SQL fallback exists only for if the UI route is blocked. **Routing: Codex** (migration + new endpoint + multi-surface frontend together) — `docs/codex-prompts/v0.150.0-program-family-many-to-many.md` (Long mode, gitignored, not committed), audited against the plan's locked constraints (no dual-write, plain `LEFT JOIN` + group-in-Java rather than `json_agg`/`array_agg` since this repository is invisible to the PostgreSQL native-query harness and its only real guard is H2-in-PostgreSQL-compatibility-mode, additive DTO, the create-modal-must-not-expand-onto-the-Note boundary, zero Exam Goal work) before dispatch, plus an addendum on the H2 harness's existing `stripInformationalPlPgSqlBlock` helper, which truncates any migration at its first `DO $$` — meaning `V146`'s load-bearing parity `RAISE EXCEPTION` cannot literally execute inside the test harness, same as `V106`'s already-informational block; the test must instead independently execute the same discriminating `NOT EXISTS` query as a plain assertion, precise about what it does and doesn't prove, since the real protection comes from Flyway running the actual block against production PostgreSQL at deploy time. **Verification tier: one scoped cold agent, falsification-framed** — the trigger is evidentiary, not precautionary: `v0.149.0` shipped two half-features in this exact code area undetected by that release's own two falsification passes, and this release adds a migration moving the only copy of a real relationship. Full scope in `RELEASES.md`.)
...
**⚠️ ALSO BEFORE CLOSING: MARK THE BACKLOG ROW OF EVERY ITEM THIS RELEASE SHIPPED — THE MISSING STEP THAT COST FIVE STALE ROWS.** Added 2026-09-10 in `v0.140.0`; the full gate is in `.claude/commands/signoff.md`. **A Backlog row is written when work is PROPOSED and never touched again, because the release that SHIPS it has no step that re-reads it.** `v0.138.0` added a verification procedure to **kickoff**, which catches a stale row only when someone later happens to scan it — and in the meantime the Index is the input to every kickoff's scope decision. **⚠️ EVERY STALE ROW FOUND SO FAR WAS STALE IN THE SAME DIRECTION — OVERSTATING OPEN WORK — WHICH IS WHY IT IS DANGEROUS RATHER THAN UNTIDY: such a row gets OFFERED TO THE OWNER AS A RELEASE CANDIDATE.** The Study Plan Builder reorder row (`NOT SHIPPED` for 41 releases) was offered as a `v0.137.0` candidate; the LaTeX row named `v0.96.0` while the fix shipped in `v0.100.0`, **39 releases** earlier; the public-catalog unbounded read read *"STILL OPEN … NOBODY ACTED"* while all three legs had shipped in `v0.119.1`, **the very next release after the gate it named**; and on 2026-09-10 the onboarding row came within one verification step of being scoped as an 8-screen rewrite of a 2506-line file that already existed. **⚠️ AND DO NOT MARK A ROW SHIPPED FROM THE RELEASE NOTES ALONE:** `v0.138.0`'s pass "verified" the unbounded-read row as still unfixed because it checked that the legacy branch **existed** (true) and concluded **unfixed** (false — it existed *and had been bounded*). **Open the code and read what it DOES, not whether it is there.**
...
8. Scan `ROADMAP.md`'s Backlog Index: bump `Last reviewed`, check whether any `Gate` condition became true, and verify every `docs/claude-prompt/*-out/` planning directory, **every `docs/claude-plans/` file AND every `docs/claude-findings/` file** still has a row — this is the only enforced checkpoint against a large planning effort (a multi-document Fable session, a paused exploration) silently going unindexed across release cycles. The `docs/claude-plans/` half was added 2026-08-10 after the `v0.71.1` kickoff found five files there unindexed; the Backlog Index intro carries the narrow *release artifact* exemption that covers finished one-off consultation prompts and sizing queries. Check all three directories — the invariant always said "or session-plan file," but the checklist only ever named some of them. **⚠️ `docs/claude-findings/` was added 2026-09-04 at the `v0.112.0` kickoff, after the scan found a production outage file dated that day AND a second incident file that had gone unindexed through FOUR consecutive kickoff scans — a scan performed exactly as written could not have caught either, because the step did not name the directory they live in.** **⚠️ `docs/claude-plans/done/` and `docs/claude-findings/done/` exist as of 2026-09-14 — this scan checks the LIVE directories only, not `done/`.** A file whose plan is fully consumed (shipped in full, or the document itself fully superseded with no open thread left in it) moves to its sibling `done/` subdirectory in the same commit that closes its Backlog row; the move itself is what discharges the row from future step-8 scans — do not leave a row pointing at a `done/`-moved file's old path. **Do not move a file whose plan is only partially shipped** (open Workstream, a `[BLOCKED — owner validation required]` item, a deferred verdict) — it stays live alongside whatever sibling plan carries its open thread forward, even if the file's own primary claim already shipped. When in doubt, leave it live: a plan wrongly filed as done is invisible to this scan, which is the same failure direction as a stale Backlog row overstating open work, just unrecoverable by a later pass that only reads `Last reviewed` dates.
