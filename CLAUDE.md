...
Current version: **v0.131.0 — Inbox Polish** (Released 2026-09-08, kicked off 2026-09-07, base branch `releases/v0.131.0`, cut from `main` after `v0.130.0` merged as #1344 and tagged `3cec79bb`). **THREE FRONTEND FIXES against the SHIPPED `v0.130.0` inbox**, from `docs/claude-plans/notification-inbox-polish.md` (owner report). **⚠️ FRONTEND ONLY — NO backend, NO migration, NO contract change, NO new notification type.** **(1)** Close the desktop panel on **outside click** and on **`Escape`**, then **delete the `Close` button** — today that button is the only way to close it, and the component's sole `addEventListener` is a `matchMedia` listener. **⚠️ The inbox is the ODD ONE OUT**: the avatar menu in the same header, the theme toggle and the export dropdown all already do this — copy the avatar-menu pattern. **⚠️ MOBILE NEEDS NO CHANGE** (`AppModal` already handles both) — do NOT add a second handler there. **(2)** Make the bell a **TOGGLE**: `openInbox` sets open and refetches unconditionally, so clicking an open inbox re-opens and refetches. When open, close **without** refetching. **⚠️ Do NOT drop load-on-open** — only the re-click while open skips the fetch. The trigger also owes a stateful `aria-label` and an `aria-expanded`. **(3)** Hide the bell **while a quiz is in progress**. It is already hidden for Long Exam and Board Exam only — the header is wrapped in `{!isExamFocusActive ? (` and there are exactly TWO `useExamFocusMode` consumers. Extend to ordinary Challenge Quiz, Quick Review, Adaptive Practice, Interview Practice and the shared quiz. **No new mechanism.** **⚠️⚠️ ITEM 3 IS FOUR SURFACES, NOT FIVE, AND NO EXIT WORK IS OWED — a kickoff finding claiming Quick Review's running branch has no in-page exit was WRONG and corrected the same day** (it has a *Leave Quiz* button; the audit grepped for links rather than `requestLeave`). **⚠️ DROP THE SHARED QUIZ: `app-shell.tsx:593` returns early for `/quiz/` and renders NO header, so the bell is already absent and `useExamFocusMode` there would be a SILENT NO-OP** — the `v0.116.0`/`v0.117.0` class. Superseded text: Its four `BackLink`s all sit in non-running branches, and **focus mode hides the ENTIRE header plus the mobile tab bar**, so applying it as written would **TRAP the learner**. Audit each of the five surfaces' RUNNING branch and add an exit where missing **before** the hook goes in. **⚠️ A `BackLink` elsewhere in the file does NOT satisfy this — that is exactly what made Quick Review look safe. ⚠️ Guard 7 is asserted PER SURFACE.** **⚠️ ANTI-DRIFT: Do NOT let the outside-click handler treat the BELL as 'outside'** — it fights the toggle, and **guard 2 exists for it: a fixture that clicks the page BODY passes while the bell double-fires.** **Do NOT change badge semantics** (announcements never inflate the number; zero renders no badge element at all) **or read/dismiss semantics** (no mark-all-read on open; dismissal never alters product state). **NO Learning Connections work and NO Stage 5/6/7 event — `[CHECKPOINT — due 2026-09-19]` is twelve days out and its denominator is ONE.** **⚠️ §8's drift-signature dedup is SUPERSEDED and must NOT be implemented**; Stage 6 now belongs to `official-review-set-update-publication-boundary.md`. **⚠️ This release SUPERSEDES `shared-quiz-recipient-experience-plan.md` §10's 'do NOT expand focus mode to other quiz modes'** — withdrawn 2026-09-07; do not re-derive the narrowing from that file's history. **No quota, entitlement or pricing change; onboarding untouched.** **VERIFICATION: A SINGLE `advisor()` CALL. Routing: CLAUDE CODE inline.** **⚠️ CARRIED LESSONS FROM `v0.130.0`, all three earned there: a `jest.mock` factory is an ALLOW-LIST** — a module the component imports but the factory omits fails silently at the call site, and `app-shell.test.tsx` was green *because* its poll threw; **a mutation must be PRESENT *and* able to REACH its subject** to prove anything; and **never revert a mutation by restoring the file from HEAD** — it discards the real fix alongside the mutant. Full scope in `RELEASES.md`.
