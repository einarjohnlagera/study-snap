...
Current version: **v0.129.0 — Adoption Signal** (Released, kicked off and signed off 2026-09-07, base branch `releases/v0.129.0`, cut from `main` after `v0.128.0` merged). **Scope is STAGE 2 — and only stage 2 — of `docs/claude-plans/in-app-notifications-and-review-set-adoption-signals-stage1.md`:** show how many learners have adopted an Official Review Set, on Explore/discovery cards (compact, `1.2K adopted`) and on Official Review Set detail (fuller wording). **⚠️ THAT PLAN'S OWN INSTRUCTION IS TO KEEP THIS SEPARATE — *"Stage 2 is independent of every other stage and is the cheapest real user-visible win — one index, one query, two surfaces. Do not fold it into the notification work."* NOTHING from Stages 3-7 is in scope: no `notifications` table, no inbox, no bell, no badge, no announcement, no fan-out, no polling, no scheduled sweep.** **THE COUNT:** `SELECT COUNT(*) FROM note_collections WHERE source_plan_id = :officialId AND owner_user_id <> :officialOwnerId`. **⚠️ NO `DISTINCT` — `V76`'s partial UNIQUE index on `(owner_user_id, source_plan_id)` already makes one-learner-one-adoption a DATABASE INVARIANT**, so rows-per-source equals learners-per-source and the count cannot inflate. **⚠️ THE ONLY SCHEMA CHANGE IS ONE INDEX ON `source_plan_id`**, needed solely because the existing index leads on `owner_user_id` and so cannot serve a `source_plan_id` filter. **PRODUCTION READS RUN 2026-09-07, READ-ONLY, BEFORE SCOPING:** provenance completeness **91.2%** (536/588), so §10's *"wait if materially low"* does **NOT** trigger; curator self-copies are **zero** (keep the exclusion for correctness anyway); distribution is **40, 40, 40, 40, 40, 30, 28, 15×7, 8, 4×7, 1, 1**. **⚠️⚠️ THE READ FOUND A DISPLAY PROBLEM THE PLAN COULD NOT HAVE ANTICIPATED, AND IT IS NOT A COUNTING BUG: ADOPTING A GOAL FANS OUT TO ITS CHILDREN.** `LET Comprehensive Review` reads 40 and each of its four children **also** reads 40; `PNLE Core Nursing Review` reads 15 and its seven children **each** read 15. **The counts are CORRECT — each child's `source_plan_id` genuinely points at the child source — and they must NEVER be summed into the parent.** But they read as duplicated down a Goal's subject list. **⚠️ AND THE FAN-OUT IS NOT UNIFORM, SO IT CANNOT BE SPECIAL-CASED AWAY: `CPALE` is 8 against children at 4, and `ALE` is 30 against a child at 28** — adopters who joined before a child existed. **⚠️⚠️ CORRECTED 2026-09-07 BEFORE ANY CODE WAS WRITTEN — THE FAN-OUT IS REAL IN THE DATA BUT INERT ON BOTH SURFACES IN SCOPE, AND THE FIRST WORDING OVERSTATED IT.** `listPublic` calls `findByVisibilityAndParentCollectionIdIsNullOrderByUpdatedAtDesc`, so **Explore renders TOP-LEVEL collections only** — children are fetched solely to roll up item counts and are never cards. `getPublic`'s `toPublicDetailResponse` returns a `childCount` **number**, not child summaries, so the public detail page has no per-child list to hang a count on. **⚠️ THEREFORE THERE IS NO "duplicated down a Goal's subject list" TO FIX IN THIS RELEASE — do NOT build a suppression rule, a roll-up, or a parent/child display heuristic for a problem no surface currently exhibits.** The fact is kept because it goes live the instant any surface renders children with counts; it is not a `v0.129.0` deliverable. **✅ SMALL-COUNT THRESHOLD (§37) DECIDED BY THE OWNER 2026-09-07: 5.** Show the count only at **5 or more** adopters; below that **omit it entirely** — no "fewer than 5", no range, since either leaks the smallness the threshold exists to hide. **⚠️ DISPLAY POLICY ONLY — the queried count stays exact.** **⚠️ AND THE THRESHOLD IS CURRENTLY INERT TOO, WHICH IS THE OTHER HALF OF THE SAME CORRECTION: every top-level PUBLIC set is ALREADY ≥8** — LET 40, ALE 30, PNLE 15, CPALE 8, and those four are the entire Explore surface. **The `4`s and `1`s are all CHILDREN**, and the `1`s are PRIVATE. So threshold 5 hides **nothing today** — it is a DEFENSIVE policy for the first small set that gets published top-level, not an active filter. **⚠️ Do NOT claim it "hides nine sets"** — that counted rows the surfaces never render. **⚠️ Do NOT re-derive it from a later distribution read — it is a decision, not a computed value.** **⚠️ ANTI-DRIFT: NO Learning Connections work — `[CHECKPOINT — due 2026-09-19]` is TWELVE DAYS OUT and decides whether that arc continues at all.** **⚠️ Do NOT write "N learners adopted this"** — the repo cannot prove every counted owner is a learner, and `ProfileType` must NOT be used to filter adoptions to justify the word. **⚠️ Do NOT roll child adoptions into a parent's count. Do NOT rank Review Sets by adoption count. Do NOT show a count in onboarding** — social proof distorts the one flow where NoteLib is making the recommendation. **⚠️ Do NOT add a denormalized or event-maintained counter; query-time only. Do NOT expose adopter identities, avatars or a feed. Do NOT reconstruct provenance from titles or note overlap** — the 8.8% without it are excluded and under-count; report that, do not fix it. **⚠️ No quota, entitlement or pricing change; onboarding untouched.** **VERIFICATION: a single `advisor()` call**, per the plan's own Verification section — one index, one query, two read-only surfaces, no authorization or privacy boundary moved, and an index-only migration that writes no data. **⚠️ PRE-DECLARED GUARDS: (1)** adopt → delete → re-adopt returns the count to its prior value; **(2)** applying a source update does NOT change the count (`applySourceUpdate` mutates rows and creates none); **(3)** adopting a child Subject Plan leaves the parent's count unchanged; **(4) ⚠️ Explore issues ONE grouped count query for N cards — ASSERT THE QUERY COUNT, NOT THE RENDERED NUMBERS**, because a test checking only the displayed figures passes an N+1 implementation. **⚠️ CARRIED LESSON, TWICE-BURNED: confirm a mutation is PRESENT before trusting a green suite.** **⚠️ AND THE `v0.128.0` LESSON ON TOP OF IT — ANCHOR EVERY QUOTED DEFECT TO THE CURRENT FILE AT KICKOFF:** that release opened on an item that had already shipped nine days earlier in `v0.96.0`, because the kickoff copied planned-scope prose forward instead of re-reading the code. **Routing: CLAUDE CODE inline for the frontend surfaces; Codex for the index + count query if it grows past the query and its test.** Full scope in `RELEASES.md`.
