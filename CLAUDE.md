...
Current version: **v0.137.0 — Deploy Integrity** (Released, signed off 2026-09-09, kicked off 2026-09-09, base branch `releases/v0.137.0`, cut from `main` after `v0.136.0` merged as #1359. **⚠️⚠️ OPENED AS *Deploy and Read* AND RE-SCOPED THE SAME DAY BECAUSE ITS PREMISE WAS FALSE — recorded, not overwritten, because the owner chose it over two feature candidates on that premise.** **READ-ONLY QUERIES DISPROVED IT: `V141` RAN 2026-09-08T12:23:16Z, `V142` RAN 2026-09-08T15:47:36Z, the profile-string write is ALREADY APPLIED, and the BACKEND has auto-deployed every release all along.** Only `notifications`/`announcements` at ZERO rows survived. **⚠️⚠️ THE DETECTOR EXISTED AND WAS UNUSED — the `v0.133.0` row said `V142` HAS NOT BEEN RUN in its Gate cell while its own Status cell said the read closed *"one day after the deploy"*. A row that contradicts itself is free to detect.** **⚠️ THE REAL DEFECT: `v0.136.0` DID NOT AUTO-DEPLOY ON EITHER PLATFORM.** Render's config is correct and every prior release fired in 2–3 seconds; Vercel has a Production deployment for every prior release merge and ZERO for `4ee2c752`. No repo webhooks, no CI — both are GitHub Apps on the SAME push event, and neither received it. **⚠️⚠️ THE SKEW IS LIVE: Vercel serves `v0.135.0`, Render runs `v0.136.0`, and `v0.135.0` calls `/creator-impact/me` with NO parameters — a 400 under `v0.136.0`. Your Impact is BROKEN for the owner until Vercel ships.** The change was breaking in BOTH directions and `v0.136.0` recorded no deploy ordering. **SCOPE:** (1) restore the frontend deploy — **OWNER ACTION**; (2) a detector for a merge that produced no deploy, **shape decided first — there is no `.github/workflows/` and this is not automatically a CI problem**; (3) two standing rules — an API form removed/renamed/made-required is breaking in BOTH directions and owes a deploy-ordering statement, and **a ROADMAP claim about PRODUCTION STATE is a snapshot, not a fact**; (4) the `2026-09-10` and `2026-09-11` reads. **⚠️ ANTI-DRIFT: no `frontend/app/onboarding` work before the `2026-09-11` read (62.4% baseline, cannot be re-run); no Learning Connections work (`[CHECKPOINT — due 2026-09-19]`, denominator ONE); do NOT change any analytics event or gating condition a live checkpoint reads; do NOT add a second `/impact` entry point before its own read; do NOT reach for CI as a reflex.** **⚠️ THE PRODUCTION DATABASE IS READ-ONLY FOR CLAUDE — the deploy and every write are the owner's.** **DEPLOYED 2026-09-09: backend `v0.136.0`, frontend `v0.135.0`, migrations `V143`.** **Routing: CLAUDE CODE.** Full scope in `RELEASES.md`.
