...
Current version: **v0.146.0 — Knowledge, Not Lost** (Released, kicked off 2026-09-14, signed off 2026-09-14, base branch `releases/v0.146.0`, cut from `main` after `v0.145.0` merged as #1388 and tagged — Vercel and Render both confirmed live on `1be308b7`. PR #1389 (implementation) and PR #1390 (pre-signoff findings) merged into the release branch. Source: `docs/claude-plans/note-visibility-learning-status-stage1.md` (Stage 1 audit) and `docs/claude-plans/artifact-first-learning-availability-stage2.md` (Stage 2 implementation plan, final decision block approved by the owner at this kickoff; both untracked on disk, indexed in `ROADMAP.md`'s Backlog Index). **SCOPE: artifact-first learning availability is this release's entire code scope.** Every learning action (Quick Review, Challenge Quiz, Adaptive Practice, Flashcards, Memorization, Long/Board Exam eligibility, Review Set premium-exam launch, public note pages) currently keys off Note lifecycle (`NoteStatus`/the `studyPackStatus` string it produces) rather than whether the artifact it consumes actually exists — so a note whose regeneration is running or has failed hides an intact, complete Study Pack, which is **the only generation-failure pattern that has ever occurred in production (7 of 7 historical failures — re-verified read-only at this kickoff, 2026-09-14: all 7 are `GENERATED`/`DONE` today, i.e. fully recovered with zero current live instance; likewise zero `study_packs` rows have an empty `quiz` and zero notes are currently `GENERATING`, so the Quick Review guard and the stranded-generation endpoint below both close latent, zero-current-instance gaps, not an active incident)**. The fix: a new derived fact `studyPackDone` (`StudyPackEntity.status == DONE`), plus `hasKeyConcepts` added only where no artifact signal exists today (`NoteCollectionItemResponse`; a `hasQuizQuestions` field on the same DTO was implemented and then reverted before commit — it would have widened a shared "lean projection" used by Dashboard/Progress/Adaptive Practice to pull the full `quiz` JSONB column, violating an existing performance guard test, and had zero real consumers), computed by a new stateless `StudyPackArtifactFacts` utility and added as additive fields to four DTOs (`NoteResponse`, `NoteListItemResponse`, `NoteCollectionItemResponse`, `PublicNoteDetailResponse`). Every consumer of the old lifecycle-first gate is repointed at the new fact: the four quiz-mode entry gates, `collection-exam.ts`'s two predicates, the public note page and copy CTA, Note Detail's quiz-action row and flashcard entry point, and the Library `STUDY_PACK_READY` SQL predicate. Also in scope: a new Quick Review backend guard (`QuickReviewNotAvailableException` on an empty-quiz pack, mirroring Challenge Quiz's existing guard); a real recovery action on the Flashcards/Memorization guard components (they promise retry today but cannot perform it); and one new, narrowly-scoped owner-callable endpoint, `POST /notes/{id}/recover-stranded-generation`, that lets a note stuck indefinitely in `GENERATING` with a null `generation_enqueued_at` (never reached by the recovery sweeper) be manually flipped to `FAILED` via the sweeper's own existing `markNoteGenerationFailed` — zero new business logic, zero quota interaction, so it cannot double-charge. **⚠️ ANTI-DRIFT: no database migration, no new persisted state** — every fact is derived at response-build time from data already stored. No change to `PRIVATE`/`PUBLIC` visibility, no new Library filter, no sixth exam mode, no `ConceptHealth`/mastery/readiness semantics change, no quota/pricing change, no automatic generation or regeneration, no Cross-Note Review design, no backend sweeper redesign (the stranded-generation fix is deliberately the smallest safe escape hatch, not a fix to the sweeper's null-timestamp exclusion), and no re-opening of `v0.143.0`'s exam-pool invalidation work or its two adjacent seams (`deactivateShareLinksForNote`, Challenge Quiz question bank). Backend entitlement enforcement (`FeatureGateService`) is untouched everywhere — `NoteResponse.adaptivePracticeAvailable` keeps its existing artifact-AND-entitlement meaning unchanged; the frontend Adaptive Practice page instead composes two more precise signals it already has (`note.quiz.length`, `usageSummary.adaptivePracticeAvailable`). **Deferred, not dropped** (see the Stage 2 plan's §L for the full list): Long Exam's late-discovered-primary vs. Board Exam's pre-checked primary (error-UX only); `DONE`-but-empty-quiz sampling into Long/Board Exam (latent, zero instances); three duplicate lifecycle-badge implementations; ~15 scattered inline artifact-presence checks not migrated onto the new utility; "Quiz Ready" naming; Cross-Note Review. **Verification tier: ONE SCOPED COLD AGENT, FALSIFICATION-FRAMED** — not the full three-agent pressure test (no permission-boundary, first-of-kind cross-user-read, or money/quota-semantics trigger fires) and not a bare `advisor()` call (a new mutation-shaped endpoint plus a load-bearing deploy-ordering dependency are exactly what a single summary call is weakest at catching). Hand the agent the Stage 2 plan's §D/§E/§J and ask it to disprove: (a) `studyPackDone` is actually derived from `StudyPackStatus`, not accidentally presence-only, in the final diff; (b) the four entry-gate swaps each use the field their own mode requires rather than being copy-pasted identically; (c) the deploy-ordering statement in §J is actually true of the shipped diff. **Deploy ordering: backend first (additive DTO fields), frontend second** — the frontend changes make `studyPackDone` load-bearing for the Long Exam entry gate and the Review Set premium-exam predicate, so an old backend serving no `studyPackDone` field would make Long/Board Exam appear to have zero eligible sources; do not deploy frontend before backend this release. **Routing: CODEX** — multi-system (frontend + backend together), >5 files, >100 LOC, requires applying `AGENTS.md`'s anti-drift rules across ~15+ consumer sites. Implemented via a Long-mode Codex prompt (`docs/codex-prompts/v0.146.0-artifact-first-learning-availability.md`, gitignored), audited before commit (found and fixed a SQL syntax bug, a performance-guard violation, and a UX regression), merged as PR #1389. **The scoped cold falsification review ran post-merge** (not pre-merge as originally planned — the implementing session judged the diff ready to commit first and ran the cold review as a pre-signoff gate instead) and confirmed all three claims it was asked to check; it also caught a stale doc and a missing regression test, fixed in PR #1390. Full scope in `RELEASES.md`.)
