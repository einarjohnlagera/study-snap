...
Current version: **v0.151.0 — No Backdoor Left, Round Two** (Released, kicked off 2026-09-16, signed off 2026-09-16, base branch `releases/v0.151.0`, cut from `main` after `v0.150.0` merged as #1405 and tagged — Vercel and Render both confirmed live on `52dac3a2`. **SCOPE: one bug-fix pair, same root defect class in two independent backend code paths, plus the frontend confirmation copy that inherited the same wrong assumption.** Closes the same gate gap `v0.143.0`/`v0.144.0` already closed for the exam question pool, this time for shared quiz links: `StudyPackService.java:928` called `generatedQuizService.deactivateShareLinksForNote` only inside `if (regeneratingNoteContent)`, but `POST /notes/{id}/regenerate` defaults to `NoteRegenerationScope.STUDY_PACK`, which reaches the same shared worker method with that flag `false` — so a `STUDY_PACK`-only regeneration replaces the quiz content (`saveStudyPack` does this regardless of scope) without deactivating any live share link pointing at it. Found 2026-09-11 while tracing `v0.143.0`'s own scope; re-verified read-only at kickoff, unchanged since 2026-09-12: exactly 1 active `quiz_share_links` row, not currently exposed (its quiz predates the link's own creation). Fix: dropped the deactivation call out of the `if` gate, the same one-line shape `v0.143.0` already used one line above it for `examQuestionPoolService.refreshPool`. **⚠️ SCOPE GREW MID-IMPLEMENTATION — the full backend build (not the original scoping, not the targeted test class) surfaced a second, independent instance of the identical scope gate in `NoteRegenerationConsequenceService.notesWithLiveShareLink`**, the bulk-regeneration path's consequence-counting method, backing both the preflight modal's `sharedQuizzesToDeactivate` count and the per-item batch receipt's `hadLiveShareLink` flag (captured before dispatch, from the same method, per `NoteBulkRegenerationService.processItem`). Its own Javadoc said the gate was deliberate, mirroring the single-Note primitive — which made it stale, not correct, the moment the primitive's gate came out: left alone, the bulk path would have gotten *worse* than the original bug — the preflight would promise a curator zero deactivations for a `STUDY_PACK`-only batch, the run would deactivate some anyway, and the receipt (reading the same gated method) would falsely confirm nothing happened. Fixed together: the scope condition and its Javadoc were removed from `notesWithLiveShareLink`, since a live share link is now a consequence of either scope. **The confirmation-dialog copy had the same assumption baked in:** `bulk-regenerate-modal.tsx` suppressed its shared-quiz warning behind `combined &&`, so even with the backend fixed the curator would never have seen it on the default scope — fixed by dropping that gate, since `sharedQuizzesToDeactivate` is now meaningful on either scope. `docs/features/bulk-regeneration.md` and `docs/features/study-pack-generation.md` both carried the stale "`NOTE_AND_STUDY_PACK`-only" claim explicitly and are corrected. **Explicitly out of scope:** the sibling Challenge question bank invalidation gap (Leg 2 of the same defect class) — still needs a tracing pass on where the per-user bank is populated before it is scopeable, not a build-now item. **Routing: Claude Code inline** (isolated, clear root cause once traced, direct precedent from prior fixes of the same defect class). **Verification tier delivered: one `advisor()` call on the diff, plus one scoped cold agent at signoff** (owner-requested; also matched this repo's own "delivery introduced a defect the same session then fixed" trigger — the session's first-pass fix was itself incomplete, twice). The cold agent refuted 5 of 7 claims outright and surfaced 2 real but narrow, pre-existing bulk-regeneration races (a preflight/dispatch readiness-window gap, and a TOCTOU on the per-item receipt's stale `hadLiveShareLink` read) — neither introduced by this diff, both recorded as Known Limitations in `RELEASES.md` rather than fixed, since fixing either is a design change beyond this bug's scope. Full detail in `RELEASES.md`.)
