...
Current version: **v0.156.0 — Say What You Meant to Show** (In Progress, kicked off 2026-09-22, base branch `releases/v0.156.0`, cut from `main` after `v0.155.0` merged as #1422 and tagged. **SCOPE: make the existing announcement `ctaLabel` field — already authored in Admin, already stored, already transmitted to the client, never rendered — visible as a real call-to-action in the notification inbox, and close the density/hit-area/touch-target gaps around it.** Source: `docs/claude-plans/actionable-announcements-campaign-feedback-stage1-plan.md`, a Stage 1 audit + plan (Opus-drafted, independently re-verified, tightened through two rounds of owner decisions 2026-09-22). **This is Release A / Slice 1 of a two-release split — Release B (Campaign Feedback: new `/feedback` route, `campaign_feedback_responses` table, new endpoints, provisional `v0.157.0`) is explicitly out of scope for this release** and stays queued until its one open owner input (the campaign's `closesAt` timestamp) is resolved; its Codex prompt already exists (`docs/codex-prompts/v0.157.0-campaign-feedback.md`, untracked) but must not be dispatched from this release. **Frontend-only — no migration, no API/DTO change.** Scope: `frontend/components/notifications/notification-inbox.tsx` renders `notification.ctaLabel` as a non-interactive `<span>` inside the existing body `<Link>` (never a nested link/button — the trap this release must avoid), extends `aria-labelledby` to include it (WCAG 2.5.3 Label in Name), adds `line-clamp-3` to the body, bumps the dismiss button to a 44px (`min-h-11 min-w-11`) touch target, and moves row padding onto the `<Link>`/dismiss button so the full card width is tappable. `frontend/app/admin/announcements/page.tsx` gets a live body character counter with a ~160-char soft-target helper and Link-label helper text — field names ("Link label"/"Link path") are unchanged, zero production usage. Tests: `notification-inbox.test.tsx` (28 existing tests stay green plus new CTA render/absent, anti-nesting, accessible-name, and clamp coverage) and `announcements/page.test.tsx` (counter). Docs: `docs/features/notifications.md` gets the CTA affordance contract, the span-not-link rule, and the stored-vs-displayed body split. **Anti-drift:** does not make `ANNOUNCEMENT` badge-eligible (would resurrect the `v0.134.0` immortal-row defect), does not touch badge-decrement logic, does not build any part of Release B. Zero `announcements` rows exist in production, so this ships with no live-data risk. **Routing: Claude Code inline** (frontend-only, ~2 source + 2 test files, no new infrastructure — too small for a Codex prompt). **Verification tier: one `advisor()` call on the diff** — no permission/money/quota surface touched. Full scope in `RELEASES.md`.)
