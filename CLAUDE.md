...
- **Partial-outcome reporting, not new infrastructure.** When a bulk topic's content generation fails, no note row is created — that behavior stays (a note without content is not persisted). The gap is *reporting*: the endpoint returns `queuedTopics = N` before background generation runs, so the immediate `Queued N notes` toast over-promises when topics fail. Surface the real outcome (e.g. "3 of 5 generated; 2 couldn't be generated") **without** adding a batch-job entity, progress table, or status enum — those remain locked from v0.29.0. This subsumes the deferred "quota ran out mid-batch" messaging (same background-reporting gap).
