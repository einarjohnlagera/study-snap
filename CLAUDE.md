...
Current version: **v0.149.0 — Precision Before Coverage** (Released, kicked off 2026-09-15, signed off 2026-09-15, base branch `releases/v0.149.0`, cut from `main` after `v0.148.0` merged as #1398 and tagged — Vercel confirmed live on `eb9d1fa`, Render backend deploy `dep-dakev48ae00c73d1gn10` confirmed live for the same commit. Source: `docs/claude-plans/program-family-health-accounting-expansion-final-plan.md` (FINAL, Product UX-approved, tightening pass 2 of 2; untracked on disk, indexed in `ROADMAP.md`'s Backlog Index at this kickoff). Supersedes `docs/claude-plans/program-family-health-accounting-expansion-product-ux-consultation-prompt.md` (pass 1) — that file's facts are preserved as historical trace only, its governance rule and Accounting-family verdict are both revised in the final plan. **SCOPE: two new Program Family shortcuts (Health Sciences, Accounting) on the existing generic `program_families` mechanism, plus the admin capability and legacy-catalog cleanup that mechanism was missing.** **Health Sciences (LOCKED per Product UX, not reopened this release):** Nursing, Medicine, Pharmacy — evidence is 18 notes carrying a genuine 3-way tag accumulated across five weeks, plus 21 more notes across two independently-invented fused catalog rows, two separate curator actions converging on the same trio. Physical Therapy and Radiologic Technology excluded (no comparable co-selection evidence). **Accounting (4 members):** Accountancy, Management Accounting, Accounting Information Systems, Internal Auditing — decided from the CPALE curriculum's own subject-plan structure (Management Services and Auditing sections are, by curriculum design, shared core coursework for these four program types), explicitly NOT from the 35-note bulk-tagging action that touched 10 programs (that action proves curators need cross-program bulk assignment as a workflow — Question A in the plan's governance framing — not that all 10 programs belong in one family — Question B, answered independently). **Business Administration is explicitly EXCLUDED** — its genuine overlap concentrates in RFBT + finance-flavored Management Services content, ~22% of the CPALE plan, which fails the family's own precision-before-coverage bar. Entrepreneurship, Economics, Senior High–ABM, Real Estate Management, CMA, and CFA are excluded too (thin/no signal, or — CMA/CFA — an unresolved credential-vs-program taxonomy question). **⚠️ Flagged for the owner, not blocking this kickoff: the Business Administration exclusion rests on external domain knowledge about how Philippine accounting-track programs share curricula, not a row-level fact the database can confirm — the plan's own §E says so explicitly and asks for a sanity check before the family ships.** **New `is_active BOOLEAN NOT NULL DEFAULT TRUE` column on `course_programs`** (confirmed absent from the schema at kickoff; reuses the existing `discount_vouchers`/`quiz_share_links` convention rather than a new abstraction) retires the two legacy fused catalog rows ("Nursing · Medicine", "Nursing · Pharmacy") from new authoring without a frontend denylist. **⚠️ ANTI-DRIFT: the column-add migration and the 2-row `is_active` backfill are separate steps, not one migration** — the backfill deploys only after Health Sciences family population is confirmed live in production, so a curator is never stranded between losing the fused shortcut and gaining its replacement. **New ADMIN-only `PATCH /course-program-catalog/{id}`** (confirmed no such endpoint exists at kickoff — `CourseProgramCatalogController` is GET-only) with `UpdateCourseProgramCatalogRequest(UUID programFamilyId)`, nullable, mirroring `NoteCollectionController`'s existing PATCH convention; needs a real `MockMvc` request test (`Content-Type: application/json`, not a bare service-method call — this repo's own `v0.119.0` lesson), ADMIN-only guard, assign/change/clear, unknown-program, unknown-family cases. **List-endpoint filtering:** the authoring combobox's pickable list excludes `is_active = false` rows; an already-selected inactive row must still resolve and render as a normal chip; **confirm which endpoint the admin catalog management screen uses and keep that one unfiltered** — filtering the wrong list would hide an inactive row from the screen meant to manage it. **Frontend family-chip redesign** in `applicable-programs-combobox.tsx`: none selected `Family · N`, partial `Family · N remaining` (click adds only the missing ones), full `✓ Family · N` — LOCKED, inert, non-interactive (status text or a disabled element, never `aria-pressed`, since the Note never persists "family selected," only individual programs do); removing a member after full immediately reverts to partial. **⚠️ ANTI-DRIFT: do NOT touch `availableProgramFamilies`/`handleFamilyExpansion`** — already family-count-agnostic, confirmed unchanged at kickoff. 18+ selected-program mobile wrapping gets an explicit acceptance check at implementation time (plan §K: render Engineering's 18 at desktop + 375px mobile, pass/fail on wrapping/clickability/fold-visibility) rather than a pre-guessed threshold fix. One low-risk copy unification: `bulk-generation-page-client.tsx`'s looser Domain Context helper phrasing matched to the other two surfaces' wording. `docs/features/program-families.md` updated for both new families, `is_active`, and the reverse Domain-Context guard. **Explicitly out of scope:** Finance as a Course/Program (deferred to CPALE curation itself, trigger likely to fire soon per plan §G — 16 of 359 planned CPALE notes are textbook Finance topics, not yet authored); the CPALE TSV's own `applicable_programs` under-tagging (`Accountancy` on all 359 rows, zero cross-program exceptions) and several RFBT titles baking "Accountancy"/"Business Law" into title text (a Note Title doctrine violation) — both curriculum-content issues for the strategist pipeline, flagged to the owner separately, not fixed here; legacy-Note normalization for the 21 fused-row-tagged notes (deferred/follow-up); Program Family overlap (one nullable FK, single family per program) stays unsupported, a known recorded limitation. Data operations — assigning the 7 catalog rows to their new families via the new endpoint, and the later `is_active` backfill — are owner-run post-deploy, sequenced per plan §O, not part of this release's code diff. **Routing: Codex** (backend migration + new endpoint + frontend together, per this file's own task-routing table) — implementation slices per plan §Q: (1) backend catalog lifecycle + family-reassignment API, (2) frontend family-chip redesign, (3) docs update; one Codex prompt or two, decided at prompt-writing time. **Verification tier: at minimum one scoped cold agent, falsification-framed** — re-decide once the actual diff exists. Three triggers already fire at kickoff: a new ADMIN-only mutation endpoint with no prior tests to anchor against; a new lifecycle column consumed by two different list paths where filtering the wrong one hides inactive rows from the admin screen meant to manage them; and the Business Administration exclusion is the plan's own flagged external-domain-knowledge conclusion, not a verifiable row. Full scope in `RELEASES.md`.)
