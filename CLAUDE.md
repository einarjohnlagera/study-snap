...
Current version: **v0.137.0 — Deploy and Read** (In Progress, kicked off 2026-09-09, base branch `releases/v0.137.0`, cut from `main` after `v0.136.0` merged as #1359. **⚠️⚠️ THIS IS DELIBERATELY NOT A FEATURE RELEASE, AND THAT IS AN OWNER DECISION TAKEN AT THIS KICKOFF over two available feature candidates.** **THE REASON: `v0.132.0` THROUGH `v0.136.0` ARE MERGED AND NONE IS DEPLOYED OR EXERCISED AGAINST LIVE DATA** — `V141` and `V142` have both never run (`V142` queues behind `V141`), and `notifications` and `announcements` each hold ZERO rows, so the whole notification substrate has only ever run against H2 and a test container. **⚠️ EVERY CHECKPOINT IN THE SEPTEMBER CLUSTER IS DEPLOY-RELATIVE, SO NONE CAN BE READ ON UNDEPLOYED CODE — a sixth undeployed release adds work no checkpoint can retire.** **SCOPE:** (1) the deploy itself — **OWNER ACTION** — `V141` then `V142` then deploy, **with `docs/claude-plans/v0.133.0-owner-profile-string-update.sql` in the SAME maintenance step, pending since the `v0.133.0` signoff**; (2) read the cluster in date order (`2026-09-10`: `v0.72.0` proximal retention + `v0.134.0` `deploy + 1 day`; `2026-09-11`: onboarding funnel + `v0.62.0` Knowledge Impact); (3) whatever a failed read forces, each row carrying a kill criterion stated before the read; (4) correct the stale prompt pointer on the Study Plan Builder row. **⚠️ ANTI-DRIFT — THIS RELEASE EXISTS TO MEASURE, SO IT MUST NOT DISTURB WHAT IS MEASURED: NO `frontend/app/onboarding` work before the `2026-09-11` read (62.4% baseline, cannot be re-run); NO Learning Connections work (`[CHECKPOINT — due 2026-09-19]`, denominator ONE — Stage D2 is exactly the nudge that moves the metric it kills the arc on); do NOT change any analytics event, fire site or gating condition a live checkpoint reads — `v0.136.0` did that to `KNOWLEDGE_IMPACT_DASHBOARD_VIEWED` two days before its read; do NOT add a second entry point to `/impact` before its own `deploy + 30 days` read, which adding one would destroy; do NOT read a checkpoint early.** **⚠️ A QUIET READ IS NOT A PASS — on rows with a denominator clause (`notifications` at zero, one curator, zero digest opt-ins), `no data` means NOT YET MEASURABLE → RE-DATE.** **⚠️⚠️ THE MIGRATIONS, THE PENDING PROFILE-STRING WRITE AND THE DEPLOY ARE ALL OWNER ACTIONS — the production database is READ-ONLY for Claude, always.** **Routing: CLAUDE CODE** for the reads and doc corrections; any code a read forces is re-routed on its own shape. Full scope and anti-drift in `RELEASES.md`.
