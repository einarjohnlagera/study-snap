...
Current version: **v0.156.0 — Say What You Meant to Show** (Released, signed off 2026-09-22, kicked off 2026-09-22, base branch `releases/v0.156.0`, cut from `main` after `v0.155.0` merged as #1422 and tagged. **Pre-signoff verification: one scoped cold agent (Opus, no inherited context), framed as falsification against 12 specific implementation claims — all 12 confirmed, no code defects found;** three doc-precision fixes applied from its findings (reworded an overclaimed browser-verification line to foreground the reproducible compiled-CSS check, added a re-date clause to the `[CHECKPOINT — due 2026-10-08]` row for if `CAMPAIGN_CLOSES_AT` moves, swapped that checkpoint's arbiter query from an owner-typed title match to the machine-determined `cta_path`). Source: `docs/claude-plans/actionable-announcements-campaign-feedback-stage1-plan.md`, a Stage 1 audit + plan (Opus-drafted, independently re-verified, tightened through two rounds of owner decisions 2026-09-22). **⚠️ Originally scoped as two releases (Release A frontend-only, Release B backend+frontend as a separate `v0.157.0`) — owner decision 2026-09-22 folded both into this one release.** **Release A (frontend-only, SHIPPED — PR #1423, merged): the existing announcement `ctaLabel` field — already authored in Admin, already stored, already transmitted to the client, never rendered — is now visible as a real call-to-action in the notification inbox, with the density/hit-area/touch-target gaps around it closed.** `notification-inbox.tsx` renders `notification.ctaLabel` as a non-interactive `<span>` inside the existing body `<Link>` (never a nested link/button — the trap this release had to avoid), extends `aria-labelledby` to include it (WCAG 2.5.3 Label in Name), adds `line-clamp-3` to the body (NOT paired with `block` — Tailwind emits `.block` after `.line-clamp-3` in this project's compiled CSS, so `block` would silently override the clamp; caught by `advisor()` before commit, confirmed by compiling this project's actual Tailwind output, independently re-confirmed the same way by the pre-signoff falsification pass), bumps the dismiss button to a 44px touch target, and moves row padding onto the interactive elements. `admin/announcements/page.tsx` gets a live body character counter (~160-char soft target) and Link-label helper text. No migration, no API/DTO change, does not make `ANNOUNCEMENT` badge-eligible. **Release B (backend + frontend, SHIPPED — PR #1424, merged): Campaign Feedback — a bounded, single-instrument in-app research campaign** (`/feedback` route, `campaign_feedback_responses` table `V148`, `GET`/`POST /feedback/campaign`). One fixed instrument, not a campaign framework — `CAMPAIGN_ID` is a named `String` constant, never an enum or registry; `PrimaryBlocker`/`QuizIssue`/`PlanIssue` do get real enums (genuine closed option sets). Close boundary is a configured property (`notelib.campaign.study-friction-2026-09.closes-at`, fail-closed on missing/malformed value, bound as `String` and parsed to `Instant` explicitly since `@Value` has no `Instant` converter here), **owner-set 2026-09-22 to `2026-10-06T00:00:00Z`** (~2 weeks after deploy — this was the prompt's one open input, now resolved). Locked precedence: the unique-constraint duplicate check runs before the close-date check, so a learner who already responded never sees a "closed" rejection on re-submit. Does not touch `notification-inbox.tsx`, `AnnouncementEntity`, or the Admin announcements page (Release A's surface). Does not relax `developer.txt:105` or touch anything gated by `ADR-002` (unrelated `v0.155.0`-incident threads). The permanent free-text Send Feedback channel is untouched. **Routing: Codex** (`docs/codex-prompts/v0.157.0-campaign-feedback.md`, Long mode, untracked/gitignored per convention) — new endpoint, migration, service logic. **Verification: `/audit-diff` on delivery**, per the standing rule for Codex-delivered work; Release A's tier was one `advisor()` call on the diff. Full scope in `RELEASES.md`.)
