...
Current version: **v0.159.0 — Nothing Lost in the Batch** (Released 2026-09-25, kicked off 2026-09-25, base branch `releases/v0.159.0`, cut from `main` after `v0.158.0` merged as #1443 and tagged; `v0.158.0` is deployed and verified: Render live 06:30Z, Vercel production 06:33Z). **Signed off; the release PR to `main` is owner-merged and nothing is deployed until then.** **Shipped:** (1) **Notifications Release A**, bulk-only async-completion notifications (PR #1444; `NotificationCategory` now has independent `badgeEligible`/`retentionExpirable` flags and `ASYNC_RESULT(true, true)`), and (2) the **`connection-timeout: 5000` 500-cause read** (the owner's VERDICT is still pending on its Backlog row; any change is its own item). **Still open:** the `[CHECKPOINT — due 2026-09-27]` click/open read, the 2026-09-28 publication-boundary read, and the new Release A checkpoint (deploy + 30 days). **Anti-drift:** no single-note notification and no "Study Packs are ready" for bulk generation; never a `finally` in `NoteBulkRegenerationService.processBatch`; never `deliver()` inside a transaction; no retry promise or reconciled "N of M" in copy; no migration/endpoint/DTO/`notification-inbox.tsx` change; Release B (continuity) is deferred; no retention Stage 3; do not cap or reorder `INACTIVITY` here. Full scope in `RELEASES.md`.)
