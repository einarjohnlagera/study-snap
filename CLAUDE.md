...
Current version: **v0.126.0 — Context Budget** (Released, kicked off and signed off 2026-09-07, base branch `releases/v0.126.0`, cut from `main` after `v0.125.0` merged and tagged). **⚠️ THIS LINE USED TO BE 322,329 CHARACTERS — 63 CHAINED `Previous:` BLOCKS FROM `v0.37.0` TO `v0.125.0`, ~80,000 TOKENS ON EVERY SESSION, ABOUT 90% OF THIS FILE.** It was replaced, not truncated: every one of those blocks is duplicated in `RELEASES.md` (verified across the FULL range before removal — `v0.107.0` alone had 32 matches there against one line here), and the two versions with no `## ` section of their own were checked individually (`v0.37.0` is a rule citation that survives inside its rule; `v0.111.1`'s naming ruling is in `RELEASES.md`'s `v0.110.1` section). **⚠️ THE RULE GOING FORWARD, AND IT IS WHY THE LAST PASS LAPSED: KICKOFF REPLACES THIS BLOCK, IT DOES NOT PREPEND TO IT.** The 2026-07-10 archive pass was a one-off with no recurring step, so nothing triggered it again for 85 releases; `.claude/commands/kickoff.md` and `signoff.md` now carry that step. **⚠️ For any release before the open one, read `RELEASES.md` — it is the canonical record and always was; this line was only ever a copy.** **SCOPE — five items, all docs and `.claude/commands`, NO product code: (1)** archive `RELEASES.md` v0.41.0 → v0.120.x into `docs/archive/RELEASES_ARCHIVE.md`, keeping current + last 5 live plus the one-line index; **(2)** archive `ROADMAP.md`'s `(Released)` per-version retrospectives into `ROADMAP_ARCHIVE.md`; **(3)** this line; **(4)** trim `AGENTS.md`'s preamble narrative to durable rules plus a pointer; **(5)** patch `kickoff.md`/`signoff.md` so the convention cannot lapse again. **⚠️ IT IS RESUMING A DOCUMENTED CONVENTION, NOT INVENTING A TRIM** — `docs/archive/README.md` states the rationale verbatim (*"moved here specifically so it doesn't cost tokens to read on every pass… a MOVE, not a delete"*) and names the design as *current + last few versions*; `RELEASES.md` holds 116 sections. **⚠️ ANTI-DRIFT: ITEMS 1 AND 2 ARE MOVES — content preserved VERBATIM, exactly as the 2026-07-10 pass did.** **⚠️ ITEMS 3 AND 4 DELETE FROM THE DOCUMENTS THAT GOVERN THIS PROJECT'S ANTI-DRIFT, so `## Key conventions`, `## Task routing`, the PRODUCTION DATABASE READ-ONLY rule, the source-of-truth list and EVERY `AGENTS.md` Anti-Drift Rules entry MUST survive untouched — only shipped-release narrative goes.** **⚠️ Do NOT prune the ROADMAP Backlog Index (plan item 6, deliberately excluded): it mixes LIVE `[CHECKPOINT — due …]` gates with closed rows, and kickoff step 8 requires every `docs/claude-plans/` and `docs/claude-findings/` file to keep a row — it needs judgment, not a script.** **⚠️ Do NOT split `AGENTS.md`'s Required Product Architecture section (plan item 7, held back) — this repo has TWICE recorded that a rule which stops being found stops being followed.** **⚠️ NO product code, NO migration, NO quota/entitlement/meter change, NO behaviour change of any kind.** **⚠️ `frontend/app/onboarding` STAYS FROZEN (`[CHECKPOINT — due 2026-09-11]`, FOUR DAYS OUT) and NO Learning Connections promotion before `2026-09-19` — this release touches no product surface at all, which is precisely why it can run inside the twelve-read window.** **VERIFICATION: a single `advisor()` call on the diff** — no permission substrate, no cross-user read, no money semantics, no migration, and by construction no behaviour change. **⚠️ PRE-DECLARED GUARD: a `grep` for each surviving governing rule BEFORE and AFTER, plus a re-measurement of all four files. A diff that merely looks smaller proves nothing — the discriminating check is that every named rule is still present and that `RELEASES.md` + `RELEASES_ARCHIVE.md` together still contain every archived section byte-for-byte.** **Routing: CLAUDE CODE inline.** Full scope in `RELEASES.md`.
