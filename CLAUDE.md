...
Current version: **v0.148.0 — Say What You Mean** (Released, kicked off 2026-09-15, signed off 2026-09-15, base branch `releases/v0.148.0`, cut from `main` after `v0.147.0` merged as #1393 and tagged — Vercel confirmed live on `06b191a2`, Render backend deploy `dep-dak8vh1srm7s73bqpicg` confirmed live for the same commit. **SCOPE: two small, unrelated backend correctness fixes, each one file, no migration, no new endpoint.** **Item 1 — `QUANTITATIVE_KEYWORDS` substring-anchoring fix.** `isQuantitativeContext` (`OpenAiLlmStudyPackService.java:1649`) uses plain `String.contains` for all 50 keywords, so several match as embedded substrings of unrelated words: `ratio` ⊂ `corporation`/`operations`/`administration`, `solve` ⊂ `resolve`, `current` ⊂ `currently`, `interest` ⊂ `interested`. Measured read-only against production: ~4,890 notes are currently "quantitative via keywords only." Anchoring 7 proven-bad keywords (`ratio`, `solve`, `current`, `interest`, `integral`, `balance`, `units`) with word boundaries flips notes to non-quantitative on a future regeneration — never retroactively, since the flag is computed fresh at generation time and never persisted anywhere (confirmed: no DB column, no entity field). Sampled the flip set: genuinely non-computational content. **Two amendments closed before commit, both found via pre-commit `advisor()` review (the Nursing/Accountancy one independently also flagged earlier by a cold-agent falsification pass):** (1) anchoring alone would declassify Nursing/Accountancy content with `domain_context IS NULL` that reaches `quantitative=true` today only via this same accidental substring match — `nursing`/`accountancy` ship as two new unanchored keywords in the same diff; re-measured with `course_program` joined into the haystack (the original estimate omitted it), this rescues 466 notes, more than the original ~370 estimate. (2) a bare `keyword` misses the keyword's own plural/verb forms (`ratio` doesn't match "ratios," `solve` doesn't match "solving") — 28% of the flip set triggered only on an inflected form, so each of the 7 anchored patterns now also accepts its plain plural/verb inflections without reopening the substring bugs anchoring closed. **Final measured flip count with both amendments and course_program in the haystack: 754 notes** (the original ~1,520-1,586 estimate used a narrower haystack and pre-inflection patterns). `pharmacokinetic` (added `v0.145.0`) stays deliberately unanchored — its match depends on unanchored substring matching, and its explanatory code comment gets rewritten so a future session doesn't break it by "fixing" it. **⚠️ ANTI-DRIFT:** no resolver rewrite — same haystack construction, same `domainContext().isQuantitative()` short-circuit, same overall function shape; anchoring is a second, additive matching branch for exactly 7 keywords (each with its plain inflections), not a semantic overhaul of the scan. Test owed: a haystack containing only `corporation` → not quantitative; one containing `current ratio` → still quantitative; plural/verb inflections match on their own; nursing/accountancy rescue proven via `courseProgram` (the field production actually uses); existing `pharmacokinetic` test stays green as the canary. `docs/features/study-pack-generation.md` updated to describe the anchoring split and the `nursing`/`accountancy` false-negative repair. **Item 2 — due-concepts-digest day-of-week clustering fix.** `RetentionService.isEligibleReviewDay` returns `true` unconditionally for the 143 users with `review_days IS NULL`, so they are checked every day the digest job runs and gated only by a flat 7-day cooldown, which locks them onto whichever weekday they first landed on, forever. Measured read-only against production (Asia/Manila, the job's actual `EMAIL_BUDGET_ZONE`): Mon 107, Tue 101, Wed 98 vs. Thu 9, Fri 8, Sun 2 over 28 days — a real, confirmed 3-day cluster. Fix: for null-`review_days` users, `isEligibleReviewDay` gets a deterministic default day — `Math.floorMod(user.getId().hashCode(), 7)` compared against today's `DayOfWeek` — instead of "any day," and `dueConceptsDigestCooldownDays`'s null-branch changes from the global 7-day config to **6 days** (not the committed-user value of 1). Purely computed at read time from the existing `id` column — no new column, no migration, no backfill, no write to `review_days`. **A cold-agent falsification pass corrected two claims from this release's own scoping, rather than letting them stand:** (1) the originally-claimed "3.5x peak reduction" was a units error (compared users-per-bucket to sends-per-week); the real, reproduced improvement is **1.5x** peak-day reduction (26.8 → 18.0 sends/week on the worst day). (2) This is **not** a live email-cap breach fix — `dispatchDueConceptsDigestEmails` never consumes the `EMAIL_DAILY_LIMIT` budget (confirmed unbudgeted), and `sendDailyEmails()` (the budgeted path) runs before it in the daily job, so same-day collision with the 100/day cap cannot occur the way the original finding implied; this fix smooths an already-unbounded channel's shape for 143 users, not a breach fix — say so plainly rather than overclaiming. **⚠️ ANTI-DRIFT:** cooldown must be 6, not 1 — a cooldown of 1 was shown to produce two digests 2 days apart for a concrete example user during the first transition week; must use `Math.floorMod`, not `%`, since `UUID.hashCode()` can be negative. Known limitation: a user whose last digest landed close to their newly-assigned day may see one earlier-than-usual digest in the first week post-deploy — bounded, one-time, stated rather than silently accepted. `dispatchDay.getValue() - 1` is used (not `.ordinal()`) to compare against the hash bucket. `StudySnapProperties.Retention.dueConceptsDigestCooldownDays` (default 7, no `application.yaml` key) is removed rather than left orphaned — the uncommitted cooldown is now the compile-time constant `UNCOMMITTED_DUE_CONCEPTS_DIGEST_COOLDOWN_DAYS = 6`, deliberately not configurable. Feature docs re-read and corrected to match: `docs/features/retention-emails.md`, `docs/features/quiz.md` (its "null/empty review days preserve the pre-`v0.72.0` cadence" line was exactly the claim this fix makes false), and `docs/features/email-preferences.md`; frontend review-days copy was swept and found already accurate, no change needed. Tests owed: `RetentionServiceTest`'s null/empty-`review_days` tests are rewritten (the old "always eligible" assertion is no longer true), with a new negative-case test proving the day-gate excludes a mismatched day; `RetentionEmailScheduler`'s and `RetentionEmailSchedulerTest`'s existing 7-day-cooldown assertions are updated. **Routing: Claude Code inline** (isolated bug fixes with a clear root cause each — Item 1 touches 1 production file, Item 2 touches 3: `RetentionService.java`, `RetentionEmailScheduler.java`, `StudySnapProperties.java`). **Verification tier: one `advisor()` call per item** — no auth/quota/money/production-data semantics change for either, and both were already pressure-tested pre-implementation by a cold Opus agent during scoping, falsification-framed against the specific claims above. Full scope in `RELEASES.md`.)
