...
Current version: **v0.72.1 — Constraint Check** (In Progress, base branch `releases/v0.72.1`). Opened on a production read — is the binding constraint retention *rate* or activation *volume*? **The read ran 2026-08-11 and the volume hypothesis failed** (activation is already 52.2%, which structurally caps the volume lever below the rate lever), so the Onboarding Intent Router residuals were not taken and the release **rescoped to validating the retention metric's own measurement window**. Read-only until that answers. Renamed from "Activation", which named the disproved hypothesis; previous: **v0.72.0 — Return Loop** (Released 2026-08-11), which shipped H1+H5 (commitment device + pre-decided return action) against the project's oldest constraint, W1→W2 retention at **2.4%**, under a decision rule pre-committed by an earlier release, and carries two dated checkpoints (proximal **2026-09-10**, distal **2026-11-09**) recorded in `ROADMAP.md`'s Backlog Index. Before that, **v0.71.2 — Catalog Management** closed the Applicable Programs arc that ran from **v0.71.0** (Release B of `docs/architecture/ADR-001-canonical-knowledge-architecture.md`) through **v0.71.1**. Full shipped scope is in `RELEASES.md`; forward-looking scope and candidates are in `docs/product/ROADMAP.md`.
