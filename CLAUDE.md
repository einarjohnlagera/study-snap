...
Current version: **v0.155.0 — Say What You Checked** (Released, signed off 2026-09-22, kicked off 2026-09-21, base branch `releases/v0.155.0`, cut from `main` after `v0.154.0` merged as #1420 and tagged. **SCOPE: fix a real quiz-grading correctness defect a learner caught and reported, and ship the validator that would have rejected it at generation time.** Source: `docs/claude-findings/2026-09-19-quick-review-percentage-increase-correctness-incident.md` (full incident audit, §A–T), owner decisions locked 2026-09-21 (§Q.1). **What happened:** a learner answered a Quick Review question correctly, was graded wrong, re-ran the quiz picking the answer they knew was wrong to confirm the bug, then reported it. **Root cause:** the LLM emitted the wrong answer *letter* while its own explanation derived the correct value — a stored MCQ's `correctIndex` pointed at a wrong choice while its `explanation`/`workingSolution` both derived and stated the right one. Confirmed generation-pipeline defect (class A — generation inconsistency), not parsing, persistence, shuffling, assembly, evaluation, or rendering — all four downstream layers traced and confirmed correct. A deterministic corpus scan (zero LLM calls, re-run twice) across 115,333 production questions in four stores found **30 confirmed defects** (corrected 2026-09-22 from an originally-claimed 31 — a "duplicate defect" in pool `2437d442` was found on live re-read to be a different, already-correctly-keyed question with a different choices array, not a duplicate), each independently hand-verified by re-deriving the correct answer from the question's own stated inputs, not trusted from its own suspect explanation. Realized learner exposure is exactly one person, two sessions — every other instance sits in never-served exam pools or the owner's own test account. **Two independent halves, one code, one data:** (1) an owner-run repair SQL (`docs/claude-plans/2026-09-21-quiz-answer-key-repair.sql`, 38 idempotent statements across four sections — 12 `study_packs` rows, 14 `exam_question_pool` rows/14 fixes, 10 `challenge_quiz_question_bank` rows with zero real exposure, and a retroactive session/`concept_health` correction kept deliberately separate per the owner's explicit "do not silently rewrite history" condition) — **Claude does not execute this, production write-only, owner's to run**; (2) a Codex-routed backend fix: **H4** (the internal-consistency validator, the actual fix — MCQ-with-numeric-choices only, rejects when the keyed choice's text doesn't appear in `explanation + workingSolution` while another choice's does; locked retry chain per owner decision — retry once, then omit, never fail the whole pack), **H1** (schema tightens `answer` to the A/B/C/D/null enum), **H2/H3** (two dead-code removals — `QuizValidationUtils.randomizeChoices`, an answer-identity-corruption hazard with only its own test as caller; `QuizQuestionEntity`/`QuizQuestionRepository`, zero production rows, zero references outside themselves), **H3b** (MATCHING block-integrity enforcement at generation, measured 8% yield on production blocks). **Explicitly deferred, not this release:** **H5** (relax `developer.txt:105` so explanations must state the answer's value while still forbidding letter references) — owner-approved (§Q.1 item 3) but ships as its own later prompt, once H4's rejection-rate baseline exists in production, so a post-ship rate change stays attributable to one change at a time. **H6** (replace the A/B/C/D letter contract with verbatim answer-text identity) — owner-approved in concept (§Q.1 item 4) but gated on `docs/architecture/ADR-002-quiz-answer-identity-by-text.md`, currently **PROPOSED, not Accepted**; not implemented until ratified. Structural answer-key validation (index-in-range, exactly-one-correct, duplicate choices) is explicitly not the fix — the corpus scan found zero violations of any of those checks across all 115,333 questions. **Anti-drift:** H4 evaluates MCQ-with-numeric-choices only, every other question type passes through unchanged; passing H4 is never to be represented as "verified correct" anywhere, only as internally consistent; no file under either ADR, `developer.txt:105` (or a sibling file's equivalent line), `StudyPackGenerationContextResolver`, or any Note-persistence path is touched. **Routing: Codex** (`docs/codex-prompts/v0.155.0-quiz-answer-key-integrity-validator.md`, Long mode) — shared backend generation infrastructure across every quiz mode. **Verification tier: one scoped cold agent, falsification-framed** — trigger: a generated-content semantics change reachable from every quiz mode. Full scope in `RELEASES.md`.)
