...
Current version: **v0.153.0 — The Missing Telemetry** (Released, signed off 2026-09-18, kicked off 2026-09-17, base branch `releases/v0.153.0`, cut from `main` after `v0.152.0` merged as #1410 and tagged — Vercel and Render both confirmed live on `2547da67`. **SCOPE: stop re-investigating a recurring production outage for a fifth time; ship the instrumentation that would actually answer it.** Source: `docs/claude-plans/2026-09-17-pool-exhaustion-instrumentation-fix-plan.md` (Prod Investigator session, written on request from a peer session relaying the owner's report that prod goes down almost daily), built on `docs/claude-findings/2026-09-10-prod-pool-exhaustion-trigger-unresolved.md` (§11 adds this occurrence). **The `[CHECKPOINT — due 2026-09-17]` in `ROADMAP.md`'s Backlog Index fired at kickoff:** today's incident (05:56:29–05:58:46 UTC, ~90s impact, already recovered) is a confirmed **fourth** occurrence of the identical signature (2026-09-04, 2026-09-05, 2026-09-10, now today) — HikariCP pool exhaustion (`active=20/20`) causes `DataSourceHealthIndicator` to starve on the same pool, so the platform restarts an instance whose only problem was that it was busy. Every discriminating check from the prior three investigations repeats identically: no connection leak, no OOM, no recent deploy, the database near-idle, nothing scheduled. **Per the checkpoint's own stated kill criterion, this release does NOT attempt a fifth root-cause hunt** — three priors plus a cold-agent falsification pass already narrowed the mechanism as far as existing telemetry allows (a non-DB, non-CPU blocking wait under 60 seconds, under OSIV). **The owner asked directly whether this could be a docker-compose/app-config issue: ruled out** — `docker-compose.yml` is local-dev-only and never touches the deploy path; Render builds `backend/Dockerfile` directly. **Scope, two independent legs plus one owner action:** (1) Leg A2 (backend) — Hikari-saturation-triggered diagnostic logging of in-flight request paths (`activeConnections >= maximumPoolSize` with threads waiting, sustained, captured via a `ThreadLocal` registry set in a servlet filter at request entry, polled against `HikariPoolMXBean`) — the one thing that would have answered all four incidents on the spot; explicitly not a general APM integration. (2) Leg B (backend config) — lower `server.tomcat.threads.max` (currently 25) to at or below `spring.datasource.hikari.maximum-pool-size` (20), closing a structural exposure where ~21 concurrent requests alone can exhaust the pool regardless of query speed, independent of whatever is actually holding connections. **⚠️ Both are `${ENV_VAR:default}` (`SERVER_TOMCAT_THREADS_MAX`, `DB_POOL_MAX_SIZE`) and Render env vars cannot be read by any tool available to Claude — the owner must confirm on the dashboard whether either is overridden before this ships, or a YAML edit is a silent production no-op.** (3) A1 (owner action, not code) — confirm whether Render's own per-request logging can be enabled for this service; deploy/env/plan-tier actions are owner-only. **Explicit non-fixes, carried forward — do not re-propose:** raising `maximum-pool-size` further (already raised once, 10→20, after 2026-09-04; the identical failure recurred three more times at 20 since — holds are duration-bound, not throughput-bound); touching `spring.jpa.open-in-view` (real blast radius, needs a staging run first); adding PgBouncer (addresses too-many-clients, not connections-held-too-long); chasing the unconfirmed "synchronous external call" lead without new evidence. Leg A2 and Leg B are independent — neither blocks the other. **Pre-declared guards:** a test that actually saturates a small test Hikari pool and asserts the saturation log line fires and names the blocking path (not just that the detector compiles); a test asserting the detector does NOT fire under ordinary, non-saturated concurrent load; for Leg B, confirm the application context still starts and a burst of ~20 concurrent requests queues at the Tomcat acceptor rather than erroring. **⚠️ FOLDED IN MID-CYCLE, 2026-09-17, explicit owner call rather than opening a second release:** a second, unrelated fix from the Authored Depth Backlog Index row, re-audited the same day (`docs/claude-plans/authored-depth-legacy-backfill-audit-and-plan.md`) — the real curator-owned public NULL-depth backlog is 282 notes, not the 80 originally checkpointed, 173 created after `v0.83.0` shipped via a bulk-generate path that never inspects depth. (4) F1 (frontend) — a non-blocking publication-time warning in the existing *Make public* dialog and the Bulk Generate form when a note with no Authored Depth is about to publish; ships before any cleanup of the existing backlog. (5) F2 (multi-system) — a missing-depth count/filter added to the existing curator-scoped Admin Applicable Programs surface, which today has no way for a curator to see which of their own notes are missing metadata. **F1/F2 explicit non-fixes:** no Course/Program→Depth inference (different semantic axis, `ADR-001:62,68,485`); no learner-facing "Unclassified" filter chip; no hard publication requirement yet (`NoteBulkGenerationService.java:336-346` swallows a publish exception into `log.warn`, so a hard throw there would silently fail a whole `makePublic` batch — deferred pending a 30-day post-F1 inflow re-read); no bulk depth editor (no bulk write path exists for `learnerLevel` today, and 80-plus one-time edits cost less than the endpoint a bulk tool would need). The two halves of this release share no code or files; folded together only to avoid a second open branch mid-cycle. **Routing: Codex** for Leg A2 (backend service + filter + config, anti-drift care against scope-creeping into a general APM layer) and for F2 (backend DTO + filter + frontend section, scope locked to the existing admin surface); **Claude Code inline** for Leg B (one YAML line, existing pattern) and for F1 (two existing components, copy + one conditional each, well under ~50 LOC). **Verification tier: Leg A2 — one scoped cold agent, falsification-framed** (four-incident production-reliability history; hand it the plan plus finding §11, ask it to disprove the detector fires under load and doesn't false-positive under normal traffic); **Leg B, F1, F2 — one `advisor()` call each** — no full pressure test for either half. **Backlog Index obligations at this release's own signoff:** the pool-exhaustion checkpoint row is NOT resolved until Leg A2 has shipped and fired at least once in its own guard test — a fifth production occurrence is not something to wait for; the Authored Depth row needs a new `[CHECKPOINT — due <F1 deploy + 14 days>]` added for the manual cleanup's completion re-read once F1's actual deploy date is known. Full scope in `RELEASES.md`.)
