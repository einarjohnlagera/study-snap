...
Current version: **v0.33.0** — see `RELEASES.md` for in-progress scope, `docs/product/ROADMAP.md` for sequencing.

## Active release: v0.33.0 — Study Plans as a Retention Engine

Base branch for this release: `releases/v0.33.0`. The constraint carried over from v0.32.2 is **near-zero W1→W2 retention (5.6%, recent cohorts ~0%)** — users activate but don't return (longest observed streak: ~2 days). This release attacks retention by placing **one ConceptHealth-derived readiness signal** where it has reach. **Track A** removes publish/discovery friction so curated plans reach learners (activation). **Track B** (headline) is readiness — an audit found it already half-built (`/me/progress` is a full subject-mastery dashboard *already Free*; note detail already shows per-concept "due" badges but *gated to PLUS/PRO*), so the work is reach + the missing plan scope, not a new dashboard. Full scope in `ROADMAP.md`. Locked rules:

- **Retention is the goal; readiness is the lever — organize it as signal vs. detail.** *Signal* (compact "62% ready · 3 due") goes at high-traffic touchpoints (the per-note rollup on note detail, in this release). *Detail* (overall ring + per-subject/per-concept bars) goes on dedicated surfaces: the plan readiness sub-route and `/me/progress`. Adoption/discovery polish (Track A) is support, not the retention bet.
- **Track B ships as two Codex prompts:** ① shared `ReadinessSummary` component + the owner-scoped plan readiness sub-route `/collections/[id]/readiness` (reached by a "Check readiness" CTA, **not** a tab, **not** inline on execution rows); ② the per-note readiness rollup on note detail + the Free-gate split. Claude audits each diff before commit.
- **Deliberate reversal of "plans don't duplicate Progress."** `docs/features/collections.md` previously locked that no mastery/readiness belongs on Study Plans. This release **consciously revisits** that for the *dedicated readiness sub-route only*. The plan **execution-detail rows** keep their no-mastery rule — readiness never goes on those rows.
- **Readiness is derived, not stored, and must match `/me/progress`.** Reuse the existing **ConceptHealth recency spine** and `ProgressReportService` classification/`masteryPercentage` (extract shared logic) — no new mastery signal, no new persisted field on collections, no new generated content, no new AI/LLM call, no new thresholds. Current-state only (ConceptHealth is point-in-time; no trend chart, no snapshot table).
- **Free-gate split is access, not billing (deliberate, value-ladder-preserving).** Ungate the note readiness **signal** to Free (the rollup + which concepts are due); keep the per-concept review-**timing** detail ("Due — 3d ago") PLUS/PRO. This aligns note detail with `/me/progress` (already Free) and gives the Free cohort its return trigger. Touch only the existing `FeatureGateService` / `canViewConceptHealth` path — **no price, quota, pass-duration, or checkout change.**
- **Publish decouple, not weaker validation.** Course/program + description persist independently of publishing; **Publish stays gated** (every note public, at least one note) with a clearer blocker. Do not remove publish validation — only stop discarding metadata when it fails.
- **Discovery stays course/program-scoped.** Recommended plans match the learner's own course/program (not an all-programs browse — that is Public Library's job for *notes*); surface via the Dashboard "Recommended" pattern (not tabs).
- **Anti-drift:** no quota / billing / price / checkout change; no new quota category; no per-profile pipeline fork; no new chart library (reuse the Progress bar pattern + inline SVG); charts express readiness, never a standalone vanity dashboard.
- **Out of scope / deferred:** dashboard + plan-list readiness **badges** (signal reach) → v0.34.0; teacher bulk-quiz & teacher-flow polish (no teacher cohort — v0.34.0 candidate); live-link / shared-progress plans (different architecture, belongs with teacher-flow).
