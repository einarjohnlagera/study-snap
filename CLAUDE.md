...
Current version: **v0.151.0 — No Backdoor Left, Round Two** (In Progress, kicked off 2026-09-16, base branch `releases/v0.151.0`, cut from `main` after `v0.150.0` merged as #1405 and tagged — Vercel and Render both confirmed live on `52dac3a2`. **SCOPE: one isolated backend bug fix.** Closes the same gate gap `v0.143.0`/`v0.144.0` already closed for the exam question pool, this time for shared quiz links: `StudyPackService.java:928` calls `generatedQuizService.deactivateShareLinksForNote` only inside `if (regeneratingNoteContent)`, but `POST /notes/{id}/regenerate` defaults to `NoteRegenerationScope.STUDY_PACK`, which reaches the same shared worker method with that flag `false` — so a `STUDY_PACK`-only regeneration replaces the quiz content (`saveStudyPack` does this regardless of scope) without deactivating any live share link pointing at it. Found 2026-09-11 while tracing `v0.143.0`'s own scope; re-verified read-only at this kickoff, unchanged since 2026-09-12: exactly 1 active `quiz_share_links` row, not currently exposed (its quiz predates the link's own creation). **Fix: drop the deactivation call out of the `if` gate**, the same one-line shape `v0.143.0` already used one line above it for `examQuestionPoolService.refreshPool`. **Explicitly out of scope:** the sibling Challenge question bank invalidation gap (Leg 2 of the same defect class) — still needs a tracing pass on where the per-user bank is populated before it is scopeable, not a build-now item. **Routing: Claude Code inline** (single file, clear root cause, direct precedent from two prior fixes of the same defect class). **Verification tier: one `advisor()` call** — no auth/quota/money/production-data semantics change, no migration, no new endpoint. Full scope in `RELEASES.md`.)
