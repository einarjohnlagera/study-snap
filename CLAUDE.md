...
Current version: **v0.141.0 — Formulas That Render** (Released, kicked off 2026-09-10, signed off 2026-09-11, base branch `releases/v0.141.0`, cut from `main` after `v0.140.0` merged as #1373 and tagged. Source: the `v0.140.0` cold pressure test and the raw-LaTeX Backlog row — **⚠️ READ THE ROW FIRST; the previous session proposed the WRONG fix and the row records why.** **⚠️⚠️ A LIVE LEARNER-FACING DEFECT: a quiz question mentioning MONEY and a FORMULA in the same sentence prints the formula as RAW LATEX.** `isInlineDollarOpen` (`quiz-working-solution.tsx:62`) opens a math span on any `$` not followed by whitespace, so **`$50,000` opens one**; `findInlineDollarCloseIndex` (`:68`) correctly skips intervening `$` preceded by a space, walks past `$5,000` and `$A = …`, and closes on the formula's FINAL `$`. KaTeX is handed the whole sentence, fails, and `renderMathSegment` re-emits the source. A sibling case fails the other way, absorbing prose into run-together italics. **22 questions + 49 explanations, verified twice against production.** **⚠️⚠️ DO NOT FIX THIS IN `normalizeBareMath` — IT WOULD BE A NO-OP, AND THE LAST SESSION PROPOSED EXACTLY THAT AND WAS WRONG.** `renderMathText` already calls it at every call site (`:206`) and it returns immediately on ANY delimiter (`math-normalization.ts:264-266`) — **339/339** backslash-bearing questions and **498/498** explanations already carry one, so the repair is wired correctly and reaches nothing. **The premise was true of the CODE and false of the CORPUS; verifying a repair is wired up says nothing about whether it reaches the data.** **⚠️ THE CURRENCY LOGIC IS NOT ABSENT, IT IS ONE-DIRECTIONAL:** the CLOSER is currency-aware and four passing tests cover currency-ONLY strings (`$10-$20`, `$5+$3`, `$12/$4`, two amounts); the untested case is currency AND a formula together, which is what finance questions produce. **SCOPE:** (1) pair `$` correctly when both appear; (2) `SummaryMarkdown` never calls `normalizeBareMath`, so bare math in a summary is never repaired (**36** summaries); (3) `app/shared/study-packs/[id]/page.tsx:69,75,81` render summary/keyConcepts/fullNotes as raw `{value}` with NO math rendering — **⚠️ scoped honestly: ONE linked-learner relationship, SIX share events in 90 days, `share_token` = 0 across 7,573 packs; it is in scope as the same defect class, not as urgent work.** **⚠️ GUARDS: the opener heuristic has been WRONG TWICE and `:55-60` records the last time — a fix captured `"10-"` as LaTeX, which KaTeX renders HAPPILY because a trailing binary operator is legal, so the fallback never fired and the reader silently saw a subtraction with the `$` eaten. A test asserting "no crash" or "something rendered" passes under both defect and fix; assert WHAT THE READER SEES.** **⚠️ ANTI-DRIFT: do NOT widen `ALLOWED_COMMANDS` as the remedy (the `\to` / `\text{}` / `^\circ` gaps are a SEPARATE finding, not in this release); do NOT let any display path rewrite stored content (`v0.110.1` destroyed stored choice text that way); do NOT regress the four currency tests; NO `rehype-katex` (`summary-markdown.tsx` deliberately uses `remark-math` as a TOKENIZER through the single existing KaTeX call); no new maths or markdown dependency; NO hand regeneration of stored packs — the defect is in the DISPLAY path, which is the framing the raw-LaTeX row carried wrongly since `v0.78.0`; `globalThis` never `window`.** **Verification tier decided at kickoff: THREE SURFACES ⇒ signoff owes ONE SCOPED COLD AGENT, falsification-framed** — `v0.140.0`'s found a blocking defect its 2,375 passing tests did not, and disproved an already-merged claim. **Routing: CLAUDE CODE inline** — frontend-only, three files. Full scope in `RELEASES.md`.)
