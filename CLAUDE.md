...
Current version: **v0.128.0 — Onboarding Unfrozen** (Released, kicked off and signed off 2026-09-07, base branch `releases/v0.128.0`, cut from `main` after `v0.127.0` merged and tagged). **⚠️ THE ONBOARDING FREEZE IS LIFTED (owner, 2026-09-07), AND THE READ IT PROTECTED WAS TAKEN FIRST RATHER THAN ABANDONED — that is the whole justification and a later session must not read this as drift.** `[CHECKPOINT — due 2026-09-11]` froze `frontend/app/onboarding` to protect a signup-funnel read against a **62.4% completion baseline that cannot be re-run**. The read was taken read-only on 2026-09-07, four days early: **393 signups all-time, 249 completed, 63.4% — essentially flat against the baseline.** **⚠️⚠️ AND THE FINDING THAT ACTUALLY DISCHARGES THE CHECKPOINT IS THE DENOMINATOR, NOT THE RATE: the cohort since the `v0.73.0` redesign (2026-08-12) is EIGHTEEN SIGNUPS — 15 completed, 83.3%.** At ~0.7 signups/day, waiting to 2026-09-11 adds about three more. **83.3% on n=18 is noise, not a result, so THE CHECKPOINT WAS NEVER GOING TO BE ANSWERABLE ON ITS OWN DATE — the freeze was protecting a read that cannot be taken.** That is the underpowered-denominator failure this repo's own checkpoint doctrine names, and it is why lifting is not a trade-off. **⚠️ Do NOT re-freeze onboarding for this checkpoint; do NOT quote 62.4% or 83.3% as a current figure — the first is a stale baseline and the second has n=18.** **SCOPE — the work the freeze was blocking, and nothing else: (1)** Summary maths renders. `components/ui/summary-markdown.tsx` runs `react-markdown` + `remark-gfm` with **no math plugin**, and the fix was blocked *solely* because `app/onboarding/page.tsx` renders `SummaryMarkdown`. **⚠️ THE NAIVE FIX IS WRONG AND `v0.95.1` RECORDED WHY: `_` IS MARKDOWN EMPHASIS, so `$x_1 + x_2$` becomes `<em>` before any post-processing could see it — it needs `remark-math` for TOKENIZATION ONLY, rendering through the EXISTING KaTeX setup so there stays ONE math configuration.** `katex` is already a dependency and `renderMathText` already handles `$…$`, `$$…$$` and `\[…\]`. **⚠️ `v0.95.1` fixed leg (a), the plain-text note surfaces; this is leg (b) and the two must not be conflated with `v0.86.0`'s CORRUPTED-ESCAPE item, which is a different defect on stored content.** **(2)** catalog-first suggestions in onboarding — the deferred half of `v0.79.0`, **and by that release's own baseline the load-bearing one**, since onboarding was excluded precisely to protect the now-discharged read. **⚠️ ANTI-DRIFT: NO Learning Connections work — `[CHECKPOINT — due 2026-09-19]` is TWELVE DAYS OUT, its kill criterion keys on `ACCEPTED`, and it decides whether that arc continues at all; touching it would contaminate the one number that decides.** **⚠️ Do NOT change what `BOARD_EXAM_STARTED`, `ADAPTIVE_PRACTICE_STARTED`, `QUIZ_SHARE_LINK_*` or `GUIDANCE_TIP_SHOWN` record.** **⚠️ NO migration, no quota/entitlement/meter change, no new mode or sub-mode, no `ProfileType` gate.** **⚠️ Item 2 must NOT lock the course/program field or add a request queue — `v0.79.0` shipped the COUNTER-PROPOSAL and free text STAYS ALLOWED; the `ADR-001` amendment remains unratified and shipping this does not ratify it.** **⚠️ Do NOT add, remove or reorder an onboarding FLOW step beyond what these two items require** — the freeze is lifted, not the judgment behind it. **VERIFICATION: ONE SCOPED COLD AGENT framed as falsification.** Item 1 changes a renderer used by **nine consumers including SEO-indexed public pages**, and item 2 touches the signup path. **⚠️ PRE-DECLARED GUARDS: (a) a Summary containing `$x_1 + x_2$` must render maths and NOT `<em>` — a fixture without an underscore passes under the defect and proves nothing; (b) a Summary containing NO maths must render BYTE-IDENTICALLY to today, since nine surfaces share this renderer; (c) onboarding must still complete end to end with a catalog-sourced program AND with free text, because free text staying allowed is the counter-proposal's whole point.** **⚠️ CARRIED LESSON FROM `v0.127.0`: confirm a mutation is PRESENT before trusting a green suite — one silently failed to apply and the run passed.** **Routing: CLAUDE CODE inline.** Full scope in `RELEASES.md`.
