...
Current version: **v0.157.0 — Watching More Closely** (Released 2026-09-24 on `releases/v0.157.0`, kicked off the same day from `main` after `v0.156.0` (#1425); **undeployed until the release PR merges to `main`, and `V149` runs on that deploy**). Landed five already-open PRs on the release branch instead of `main` (Vercel Web Analytics #1426, the only code one; pool-observability plan #1427; the 2026-09-22 restart finding #1428; GPT context #1429; the retention channel doctrine #1430) and implemented two of the plans they carried, each on the owner's explicit go-ahead: **pool diagnostics that see background threads** (a two-thread scheduler subclass plus a decorator on four executors) and **retention email click/open tracking plus one daily budget over all five retention types**. Production data read before signoff showed `INACTIVITY` pinned at the 60/day cap, so **send order is priority: the digest runs first, then `WEAK_CONCEPT`, then `INACTIVITY`** — do not reorder without reading `docs/features/retention-emails.md`. **Anti-drift for what follows:** retention Stages 2-3 do not start until the three `[CHECKPOINT]` rows in `ROADMAP.md` fire (dates provisional, assume a 2026-09-24 deploy); `WEEKLY_SUMMARY` and `KNOWLEDGE_IMPACT_DIGEST` are budget-starved by design-for-now and gated on opt-in growth, not fixed; the click/open instrumentation is unverified emitting until deploy + 3 days. Full scope in `RELEASES.md`.)
