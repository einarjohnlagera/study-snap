...
Current version: **v0.159.0 — Nothing Lost in the Batch** (In Progress, kicked off 2026-09-25, base branch `releases/v0.159.0`, cut from `main` after `v0.158.0` merged as #1443 and tagged; `v0.158.0` is deployed and verified: Render live 06:30Z, Vercel production 06:33Z). **Owner-picked scope:** (1) **Notifications Release A**, bulk-only async-completion notifications (`docs/claude-plans/learning-relevant-notifications-stage1-plan.md`; backend only, no migration; Codex-routed; **BLOCKED until the owner decides the `NotificationCategory` badge/retention flag split**), and (2) the **`connection-timeout: 5000` follow-up** (a proper read-only 500-cause read, then the owner's verdict; any change is its own item). **Anti-drift:** no single-note notification and no "Study Packs are ready" for bulk generation; never a `finally` in `NoteBulkRegenerationService.processBatch`; never `deliver()` inside a transaction; no retry promise or reconciled "N of M" in copy; no migration/endpoint/DTO/`notification-inbox.tsx` change; Release B (continuity) is deferred; no retention Stage 3; do not cap or reorder `INACTIVITY` here. Full scope in `RELEASES.md`.)
