...
Current version: **v0.142.0 — Awareness Before Action** (Released, kicked off 2026-09-11, signed off 2026-09-11, base branch `releases/v0.142.0`, cut from `main` after `v0.141.0` merged as #1377 and tagged. Source: `docs/claude-plans/v0.142.0-adoption-and-notifications-plan.md`, written from a tightened product spec the owner returned after a second GPT opinion, verified against code and against a 2026-09-11 read-only production read. Polishes two already-shipped surfaces the owner is now actively announcing to users: the in-app notification inbox and the adopted Review Set update panel. **⚠️⚠️ ITEM A5 IS A LIVE BUG ON 100% OF TODAY'S NOTIFICATION POPULATION: all 42 production notifications share one type (`REVIEW_SET_UPDATE`), all 42 carry a CTA, and the CTA `<Link>` at `notification-inbox.tsx:163-169` marks read but never closes the panel** — every notification in production is one tap from this exact defect, and 41 of 42 are still unread. Ship it first, inline-sized, its own PR. **⚠️ THE OWNER'S STATED REASON FOR THE CARD REDESIGN WAS WRONG, THE REDESIGN ITSELF IS NOT: borders already exist (`border-b border-border`, `:154`) and are merely invisible with one notification on screen; what is genuinely missing is a background distinction for unread beyond `font-medium` vs `font-semibold` (`:157`).** **SCOPE:** (1) A5 fix; (2) card redesign — single tap target, no separate CTA link or Mark-read button, dismiss stays distinct, a free relative timestamp (`createdAt` already on `NotificationResponse`); (3) Review Set update panel — replace two uncapped raw-diff lists with meaning-based partitioning (`SKIPPED_NOT_PUBLIC` currently sits under a heading claiming "no action taken", which is false), aggregate the three per-note fan-out types (`REORDERED`/`RETIRED`/`MOVED`) into counts, move to a compact summary plus a progressive-disclosure "Review update" surface; (4) rename **Apply additions** → **Add N new topics**, counting `ADDED_NOTE` alone — NOT `additionsAvailable()` (`NoteCollectionService.java:3523-3529`), which also counts `ADDED_SUBJECT_PLAN` and would overstate the count. **⚠️ SECTION GROUPING DEFAULTED AT KICKOFF TO OPTION A — SUBJECT PLAN GROUPING ONLY, NO SECTION LEVEL** — a Section is a string label on `NoteCollectionItemEntity.label`, not an entity, and the update payload carries no Section field; adding `sourceSectionLabel` (Option B, a real but deferred fast-follow — the variable is already in scope at the `ADDED_NOTE` construction site, no extra query) would make this a backend release and raise the verification tier. Revisit if the owner wants the extra hierarchy level. **⚠️ THE RENAME'S COLLISION RISK IS CLEARED: a read-only production check for "Apply additions"/"upstream"/"addition" across all notification titles, bodies and CTA labels returned ZERO ROWS** — but `docs/features/collections.md:932` and `frontend/app/collections/[id]/page.test.tsx:736,758` still reference the old label and must be swept in the same PR. **⚠️ ANTI-DRIFT: do NOT let notification activation apply an update — navigation and mark-read only, the update stays a separate explicit action; do NOT title-infer the Subject Plan grouping — use stored `sourcePlanId`/`subjectTitle` identity; do NOT change `additionsAvailable()`'s boolean-gate semantics; do NOT add pagination, a new diff engine, or new notification infrastructure — Option A needs none of it; do NOT sweep `AGENTS.md`'s preamble or the Backlog Index further as a side effect — item 6 of `docs/claude-plans/context-doc-token-reduction-plan.md` ran ONCE at this kickoff (six Backlog Index rows moved to `docs/archive/ROADMAP_ARCHIVE.md`), items 4/6/7 stay otherwise held.** **Verification tier decided at kickoff: ONE SCOPED COLD AGENT MINIMUM, falsification-framed** — two PRs land on the same shared classification surface, and this release changes what a user-facing claim means (the "no action taken" heading), the exact class that has cost three releases running when swept only by diff. **Escalates to the full three-agent test if Option B ships instead of A.** **Routing: CODEX** for both workstreams; A5 ships first as its own inline-sized PR. Full scope, verified production reads, and every rejected alternative are in the plan file above and in `RELEASES.md`.)
