...
Current version: **v0.145.0 — Knowledge, Not Role** (Released, kicked off 2026-09-14, signed off 2026-09-14, base branch `releases/v0.145.0`, cut from `main` after `v0.144.0` merged as #1386 and tagged — Vercel and Render both confirmed live on `22983935`. PR #1387 merged into the release branch at `94d2bbd3`. Source: `docs/claude-plans/domain-context-biomedical-business-calibration-stage2.md` (Stage 2, tightening the Stage 1 audit of the same name; both untracked on disk at the owner's instruction, indexed in `ROADMAP.md`'s Backlog Index). **WORKSTREAM A — `DomainContext.BASIC_MEDICAL_SCIENCES` (`quantitative = false`) is this release's entire code scope.** Six canonical, multi-program Pharmacology notes (`Antibiotics: Mechanism of Action and Resistance` and five siblings) clear `ADR-001:397` clause (a)'s ~10-note floor and are *currently mis-instructed* — forced onto `NURSING` because no coarser value exists, generating under a `DOMAIN_CONSTRAINT` naming a professional role their content never uses. Boundary test, validated against all 18 multi-program Pharmacology notes' real summaries: *does a professional role appear in the knowledge itself, or only in who is reading it?* — 6 mechanism-framed (this release's population), 11 role-framed (stay `NURSING`), 1 unclassified (curator follow-up). `QUANTITATIVE_KEYWORDS` gains exactly one string, `"pharmacokinetic"` — measured, not assumed: two of Stage 1's three proposed strings were wrong (`"half-life"` matches zero notes anywhere in the corpus; `"clearance"` is a live false positive, 77 corpus-wide matches, mostly building/construction, including a false positive on `PROFESSIONAL_PRACTICE_AND_REGULATION`, a documented `quantitative = false` decision). **This closes a pre-existing Quick Review computation-guidance gap, NOT a regression** — the regression Stage 1's condition was meant to prevent was measured and affects zero notes; writing it up as prevention would be the `v0.116.0`/`v0.117.0` failure mode. **WORKSTREAM B (Accountancy/Business/Finance) SHIPS NOTHING** — pre-CPALE calibration only. `ACCOUNTANCY` keeps `quantitative = true` unchanged, decided by measurement (`false` would be a 5-save/4-lose trade across its 154 notes, all currently `domain_context IS NULL`); no new Domain Context is minted; the re-audit trigger (≥10 canonical notes stably shared across 2+ live programs via a committed CPALE plan) stands at 0. **⚠️ OWNER DECISION 3 (widening the PPR description for Business Law/RFBT) IS BLOCKED, NOT DROPPED:** gated on a two-arm comparison requiring a production WRITE (`domain_context` set on three real notes) that is the owner's to run, never Claude's, under the read-only production rule — approving the plan is not approving the write. Exact `UPDATE`/verify/revert SQL, the three notes' UUIDs (independently confirmed read-only at kickoff), and the pass/fail condition are in `docs/claude-plans/domain-context-ppr-validation-armB.sql`, handed to the owner directly (on disk, deliberately not committed — a production `UPDATE` statement should not sit in git as a would-be runbook). **Re-checked read-only at signoff: still not run, all three notes still `domain_context IS NULL`.** Ships as a follow-up PR into a later release only if Arm B passes; otherwise `BASIC_MEDICAL_SCIENCES` ships without it — independent array entries, bundled by convenience only. **⚠️ ANTI-DRIFT: no database migration** (`notes.domain_context` is `VARCHAR`, zero CHECK constraints) **, no backfill of the six mechanism-framed notes (curator follow-up, outside this release), no `isQuantitativeContext` resolver rewrite (one array element only), no program catalog or Program Family change, no Workstream B implementation.** The `QUANTITATIVE_KEYWORDS` unanchored-substring defect (`ratio` ⊂ `corporation`, etc. — proved against production) is reported in the Backlog Index and explicitly out of scope; fixing it with word boundaries would silently break the `"pharmacokinetic"` entry this release depends on. **Verification tier: ONE `advisor()` call** — no new endpoint (no `MockMvc` test owed), `api.ts`'s change is a TypeScript union member only (no `api-*.test.ts` test owed) — both stated explicitly rather than skipped silently; tests for the new value and keyword land in the same diff per the unexercised-change rule. **Deploy ordering: ship together** — frontend-first silently drops a curator's save via `fromString`'s `null`-on-unknown return. **Routing: CLAUDE CODE inline** — no endpoint, migration or service-logic change beyond one array element; the judgment-heavy parts are curator-facing prose and the ADR entry. Full scope in `RELEASES.md`.)
