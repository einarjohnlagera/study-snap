...
Current version: **v0.139.0 — Reopened** (Released, kicked off 2026-09-10, signed off 2026-09-10, base branch `releases/v0.139.0`, cut from `main` after `v0.138.0` merged as #1363 and tagged. Source: `docs/claude-findings/2026-09-10-september-checkpoint-reads.md` — **read it first; item 1 is a pre-committed rule firing, not a metrics chore.** **⚠️⚠️ THE `v0.72.0` RETENTION CHECKPOINT FIRED ITS KILL CRITERION: nine learners were shown the review-commitment prompt over a month and ZERO committed; one declined.** **⚠️ THE ZERO IS REAL AND TWO INDEPENDENT INSTRUMENTS AGREE** — `COMMITTED` and `DECLINED` fire from the SAME LINE (`review-commitment-prompt.tsx:102`) and `DECLINED` fired once, so the call site provably executes; independently `SELECT count(*) FROM users WHERE cardinality(review_days) > 0` returns **0**, which rules out the analytics-delivery bias that made `v0.80.0` necessary. The pre-committed rule said the framing *"reverts to unconfirmed and is **reopened rather than iterated on with further nudge tuning**"*. **⚠️ OWNER OVERRIDE, RECORDED: the owner elected to REDESIGN THE PROMPT — which is the nudge tuning that rule names — after being told so. A pre-committed rule quietly stepped over stops being a rule.** **⚠️ n=9 IS SMALL BUT NOT NOTHING: a ~10% true commit rate is not excluded by chance; ≥30% effectively is. The redesign is a BET, not a correction.** **SCOPE:** (1) **instrumentation FIRST and it is NOT optional** — the prompt fires only after a successful save (`:96-107`) so **8 of 9 vanished unrecorded**, and all 11 `DUE_CONCEPTS_DIGEST_LANDED` rows carry a **NULL `user_id`**, which made the checkpoint's second metric *never computable*; (2) **redesign the ask** — **⚠️ the constraint is the TRIGGER, not the copy: `isFirstCompletedSessionEver === true` means ONE impression per learner EVER**, asking for a weekday multi-select plus an exam date before any payoff is visible; (3) **`scripts/check-deploys.sh` must report a confirmed drift as drift** — **VERIFIED against today's live miss**: it prints *"VERCEL … BEHIND"* then **exits 2** (*"could not check"*) because `drift=1` at `:63` is discarded by the `exit 2` at `:82`. **⚠️⚠️ CORRECTED 2026-09-10, SAME DAY: VERCEL DID NOT MISS `v0.138.0`. It deployed `05c367c4` at 01:21:06Z, **4 minutes 24 seconds after the 01:16:42Z merge** — it was IN FLIGHT when this session checked, and the check was read as an absence. **The true record is ONE confirmed miss (`v0.136.0`), not two of three: `v0.137.0` and `v0.138.0` both auto-deployed normally.** ⚠️ **THIS IS `v0.137.0`'s OWN RULE BROKEN THE DAY AFTER IT WAS WRITTEN** — a production-state reading taken at one instant and asserted as a standing property. **⚠️ AND IT IS A REAL LESSON FOR THE DETECTOR, NOT JUST AN EMBARRASSMENT: testing for ABSENCE requires waiting past the normal deploy latency, or the test manufactures its own false positive.** Observed latency is ~2–5 minutes on both platforms. **Item 3's defect is UNAFFECTED and still real** — it was reproduced by mutation against the pre-fix script, independently of any live drift.**  **The upstream cause of the ONE real miss (`v0.136.0`) is GitHub push-event delivery loss, is INFERRED, and is not fixable from this repo; this release fixes the REPORTING.** **⚠️ ANTI-DRIFT: do NOT ship item 2 without item 1; do NOT re-date the `v0.72.0` checkpoint as though it had not fired; do NOT claim the Vercel auto-deploy is fixed; do NOT make `/actuator/metrics` public to make a checkpoint readable — the `v0.134.0` row's "externally readable" claim is WRONG and `application.yaml`'s own comment says so, correct the ROW not the security posture; do NOT add an analytics event without a fire site (`v0.116.0`/`v0.117.0`); no Learning Connections work (`[CHECKPOINT — due 2026-09-19]`); no `frontend/app/onboarding` work before the `2026-09-11` read.** **⚠️ GUARDS: assert the dismiss event fires on the CLOSE path specifically (a test asserting "some event fires" passes today); assert the PERSISTED digest row carries a non-null `user_id`, not that the client sent one; assert EXIT CODE 1 — the message already prints, the exit code is the whole bug.** **⚠️ The four `FAILED` notes from `v0.138.0` are still the OWNER's to re-run; the backend went live 01:18:59Z so that clock has started.** **Routing: CODEX for items 1–2, CLAUDE CODE inline for item 3.** Full scope in `RELEASES.md`.)
