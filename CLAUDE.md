...
Current version: **v0.158.0 — Reading the Evidence** (Released 2026-09-25, kicked off 2026-09-24, base branch `releases/v0.158.0`, cut from `main` after `v0.157.0` merged as #1440 and tagged; `v0.157.0` is deployed and verified: Render live 02:40Z, `V149` applied 02:39Z, Vercel production 02:43Z). **Signed off; the release PR to `main` is owner-merged.** It discharged evidence already owed: **three past-due `[CHECKPOINT]` reads** (`connection-timeout: 5000` 5xx, due 2026-09-18; Learning Connections demand, due 2026-09-19; publication boundary, due 2026-09-22 — each row's own kill criterion is authoritative, and a fired criterion becomes its own owner-scoped item, never an inline fix), the **first readings of the `v0.157.0` retention instrumentation** (dispatch order in the logs; the 2026-09-27 "is it emitting" read; owner still owes Resend click/open tracking and the `email.clicked`/`email.opened` webhook events), and **one ready fix: the `RetentionEmailScheduler.runMonthly()` zone pin** (Claude-direct, own branch + PR). **Anti-drift:** no retention Stage 3 (waits on the three retention `[CHECKPOINT]` rows); do not cap `INACTIVITY`'s share (gated on opt-in growth); do not reorder retention dispatch — send order is budget priority (`docs/features/retention-emails.md`); nothing here is feature scope, so choose feature scope explicitly. **Still open:** the `[CHECKPOINT — due 2026-09-27]` click/open read (on or after that date), and owner decisions on `INACTIVITY` email effectiveness and the `connection-timeout` verdict. Full scope in `RELEASES.md`.)
