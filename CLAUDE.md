...
Current version: **v0.144.0 — No Backdoor Left** (Released, kicked off 2026-09-13, signed off 2026-09-13, base branch `releases/v0.144.0`, cut from `main` after `v0.143.0` merged as #1383 and tagged — Vercel and Render both confirmed live on `c899d418`. One item, the fourth and last known path through the same "derived artifacts" defect class `v0.143.0` mostly closed. **ADMIN REPAIR PATHS BYPASS EXAM-POOL INVALIDATION:** `AdminStudyPackTransactionHelper.regenerateOnePack` (`:66-67`) and `repairMalformedQuiz` (`:121-123`) each replace a Study Pack's `summary`/`quiz` in place with no call to `ExamQuestionPoolService.refreshPool`/`initiatePool`, so a `READY` exam pool keeps serving questions built from replaced content — this is the fourth path `v0.143.0`'s own fix (`StudyPackService.java:905-912`) does not reach, found by that release's scoped falsification pass and tracked as its own Backlog row. **Fix scoped on precedent, not exposure evidence**: two read-only production queries run 2026-09-12 (`docs/backlog-rows-475-476-production-read`, PR #1384, merged into this release branch) found the admin-repair path's call volume is permanently unknowable (no admin-action audit trail exists, and the one plausible proxy — a `"|"` "enriched summary" marker — turned out to be a content-shape artifact of ordinary summary generation, not a repair-provenance signal). The fix ships anyway because it is one file, two call sites, and the exact pattern `v0.143.0` already built, tested and mutation-verified — `examQuestionPoolService.refreshPool` for both modes, inside the same `@Transactional` method, with the same flush-before-lock ordering that release's falsification pass found necessary. **The same PR also verified the sibling Backlog row (shared quiz links, `STUDY_PACK`-only regeneration) as NOT currently live** — production carries exactly one active share link, unaffected — so that row stays open but deprioritized, not folded into this release. **⚠️ ANTI-DRIFT: do not touch the Challenge Quiz question bank (leg 2 of the same defect class — still unscoped, needs its own tracing pass before it needs a release); do not add an admin-action audit log as part of this fix — that would answer future volume questions but is separate, unscoped infrastructure work; do not extend the fix to any other `AdminStudyPackTransactionHelper` method beyond these two.** **Verification tier: a single `advisor()` call is enough** — isolated bug fix in one file, direct precedent already established and mutation-verified in `v0.143.0`, no new authorization/quota/production-data semantics. **Routing: CLAUDE CODE inline** — isolated fix, 1 file, clear root cause, existing pattern. Full scope in `RELEASES.md`.)
