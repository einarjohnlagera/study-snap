...
Current version: **v0.147.0 — The Escape Hatch** (Released, kicked off 2026-09-14, base branch `releases/v0.147.0`, cut from `main` after `v0.146.0` merged as #1391 and tagged — Vercel and Render both confirmed live on `6d840811`. Source: `docs/claude-findings/2026-09-12-bulk-regeneration-modal-wedged-stale-batch-id.md` (Prod Investigator finding) and `docs/claude-plans/2026-09-12-bulk-regeneration-404-terminal-state-fix-plan.md` (fix plan), both untracked on disk, indexed in `ROADMAP.md`'s Backlog Index; recommended as the next open release at both the `v0.145.0` and `v0.146.0` kickoffs before finally being taken up here. **SCOPE: fix the Bulk Regenerate modal permanently wedging on a stale/expired batch id — this release's entire code scope, one file, frontend only.** `bulk-regenerate-modal.tsx` seeds `batchId` from `sessionStorage` with no TTL awareness (`:78`); receipts expire 24h after creation (`NoteBulkRegenerationReceiptService.RECEIPT_TTL_HOURS`), so an expired or unknown batch id 404s (`NoteBulkRegenerationReceiptService:55`, deliberately indistinguishable from "not yours" — an information-disclosure decision this release must not change). The poll's `catch {}` (`:130-138`) swallows every failure including that 404, and the stop condition (`:126`) requires a `200` (`finished`/`stale`), so the poll runs forever at its 3s cadence while the stored `batchId` keeps the control that starts a new batch permanently hidden behind the progress view (`:483`). Owner has a workaround (clear `sessionStorage` / reopen tab), so this is not an emergency, but it recurs on every future expired batch. **Leg A** discriminates the 404 as terminal — catch `ApiRequestError.status === 404` specifically, stop polling, clear the stored id, return to preflight, surface the backend's own message ("That regeneration batch is no longer available.") rather than inventing new copy; every other failure stays transient and keeps retrying. **Leg B** adds an explicit "start a new batch" / dismiss action that clears the stored id independent of the poll, so a curator is never dependent on the poll noticing anything to escape a stuck view — independently useful, ships now rather than deferred. **⚠️ ANTI-DRIFT:** do NOT extend the 24h receipt TTL (deliberate retention choice — a longer TTL only moves the threshold and leaves the wedge intact past it); do NOT remove `sessionStorage` persistence (deliberate — lets a curator navigate away and return to a running batch); do NOT make the `catch` rethrow everything (a transient non-404 failure must still be swallowed and retried); do NOT change the 404 contract's indistinguishable unknown/not-yours/expired semantics or add a distinguishable "expired" status (would leak batch existence to a non-owner); do NOT touch `queueBatch`'s write-before-return ordering (`NoteBulkRegenerationService.java:238-243` — it is what makes "no rows" a reliable diagnostic elsewhere); do NOT fold in the unrelated `INVALID_REFRESH_TOKEN` 401 finding (unproven relation, would change the verification tier); A1 alone (discriminate on 404 status), not A1+A2 (a retry-count bound addresses a different, unconfirmed failure mode). No backend change, no migration, no new endpoint. **Test guards owed, per the fix plan's §6** (a fixture that mocks a successful receipt passes under both the defect and the fix and proves nothing): (1) reject-with-404 → polling stops, stored key cleared, preflight view renders; (2) non-404 rejection → still swallowed and retried on the next tick; (3) mount-from-storage → seed `sessionStorage` before mount, assert the progress view opens; (4) Leg B dismiss → clears the key, next mount shows preflight; (5) confirm what `getBulkRegenerationReceipt` actually throws on a 404 rather than assuming its shape. **Routing: CLAUDE CODE inline** — frontend only, one file, clear root cause, existing `ApiRequestError.status` pattern already used elsewhere in the tree. **Verification tier: ONE `advisor()` call** — no auth/quota/money/production-data semantics change, no migration, no new endpoint. Full scope in `RELEASES.md`.)
