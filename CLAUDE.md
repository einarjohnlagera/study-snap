...
Current version: **v0.152.0 — The Missing Half of v0.150.0** (Released, signed off 2026-09-17, kicked off 2026-09-16, base branch `releases/v0.152.0`, cut from `main` after `v0.151.0` merged as #1407 and tagged — Vercel and Render both confirmed live on `ee5c5d98`. Delivered via PR #1408 (Slices 1-3) and PR #1409 (a cold-agent-found fix, same day, before this ever deployed) — not yet merged to `main` as of this signoff. **SCOPE: family-first Admin catalog management UX, over an already-correct data model — no schema change, no ADR amendment.** Gives the many-to-many Program Family architecture (`v0.150.0`) the curator UX it needed to actually get finished. Production is the evidence: Engineering (18/18) and Education (8/8) were fully populated the day `v0.150.0` shipped; three weeks and one release later, Health Sciences and Computing & Technology are still at zero members, Accounting 2/5, Built Environment & Design 1/8 — the many-to-many model did not fail, the one-program-at-a-time admin workflow (open a program, pick its one family from a `<select multiple>`, repeat) made finishing the backfill through it tedious enough that it didn't get finished. Source: `docs/claude-plans/program-family-catalog-management-ux-overhaul-plan.md` (FINAL, owner-approved, subagent audit + owner-tightening pass) and its companion Codex prompt `docs/codex-prompts/v0.152.0-program-family-catalog-management.md` (Long mode, Slices 1-3 only). **Four owner-consolidated slices, dependency order 1→2→3→4:** (1) backend catalog contracts — `POST /course-program-catalog/families` gains optional `programIds` (atomic create-with-members); new `PATCH /course-program-catalog/families/{id}` (rename + family-side membership replace, one transaction); family duplicate-name predicate weakened relative to the program one, fixed to match; rename gets `id <> ?` self-exclusion; new `V147__program_family_initial_membership.sql` — additive only, exact-name inner joins over a locked 50-pair matrix, `ON CONFLICT DO NOTHING`, no `RAISE`, no fuzzy matching, does **not** write the vestigial `course_programs.program_family_id` (production at kickoff: 29 existing pairs, 21 to insert; **⚠️ by signoff (2026-09-17), ordinary Admin-UI curator work had already closed 32 of the 50 pairs — `V147` had not yet merged to `main` or run, so the real insert count on deploy will be ≤18, confirmed by a post-deploy anti-join, not asserted here; either number is correct since the migration is additive and idempotent regardless**). (2) shared `CatalogMultiSelect` (`selectedSummary: "count" | "chips"`) + `CourseProgramCreateModal` extraction, replacing both remaining raw `<select multiple>` instances and collapsing two divergent create forms (Admin's weaker single-family one, note-authoring's already-multi-family one) into one component, one contract. (3) family-first Admin IA — `/admin/course-programs` gains a two-tab switch (`?view=families|programs`, URL-reflected), Program Families as the default/primary tab (table: name, member count — zero-member families included, never `is_active`-filtered — Edit), Course / Programs demoted to the inverse-convenience secondary tab. (4) verification + production acceptance + docs (Claude Code, not sent to Codex) — a scoped cold agent, then an anti-join of the same 50-pair matrix against `course_program_family` in production (expect 0 missing pairs — the primary proof, not a family-count check, since a count can be right for the wrong reason), then `docs/features/program-families.md` corrected (its "a family is created empty" and "membership is set on program creation or edited later" claims are both now false). **Owner-locked anti-drift invariant, binding on every new method:** "Program Family name is display data, Program Family ID is identity" — V147's exact-name matching is a scoped migration-only exception (runtime-generated UUIDs with no environment-portable literal) and must not be copied into any application code. **No ADR-001 amendment** — its amended clause 2 is already storage-neutral and ratifies many-to-many; nothing here changes what expansion means, only who can edit membership from which side. **Explicitly out of scope:** family/program deletion, the `is_active` write path, a `Business & Finance` family, a general Popover/Command primitive (the new control is a catalog picker for small in-memory lists, not a platform layer), the legacy fused rows (`Nursing · Medicine`, `Nursing · Pharmacy` stay unassigned), dropping `course_programs.program_family_id` (vestigial, kept). No Program Family reaches a prompt, is persisted on a Note, or triggers a live update to existing Notes — untouched by a management view, a rename, or a backfill. **Routing: Codex** (new endpoint + migration + service logic, multi-system frontend+backend, ~17 must-change files — three independent task-routing triggers). **Verification tier: one scoped cold agent, falsification-framed**, elected at kickoff rather than deferred to signoff — all three implementation slices touch the shared catalog create/membership path (this repo's own "two or more PRs touched the same shared method" trigger). Seven claims to disprove: family-side replace cannot evict a program from another family; rename preserves id, every membership, and every note's applicability; V147 is additive, idempotent, and cannot fail a fresh-database Flyway run; V147 does not write `course_programs.program_family_id`; no ordinary user can create a shared catalog entry through any path; creating a program with two families adds only that program to the note; the new multi-select's checkbox `checked` state is real, not `AddNotesModal`'s list-membership hack. Full scope in `RELEASES.md`.)
