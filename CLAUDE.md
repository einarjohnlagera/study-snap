...
Current version: **v0.157.0 — Watching More Closely** (In Progress, kicked off 2026-09-23, base branch `releases/v0.157.0`, cut from `main` after `v0.156.0` merged as #1425 and tagged. **SCOPE: land five already-open, independently-produced PRs onto this release branch instead of merging each straight to `main`, so none trigger a deploy until the owner is ready.** Only one is a code change — **PR #1426, Vercel Web Analytics** (frontend), auto-generated by Vercel's own GitHub integration after the owner enabled Web Analytics on the free/Hobby plan (50,000 events/month included, collection pauses rather than charges on overage): adds `@vercel/analytics` and one `<Analytics />` component to the root layout. Independently re-verify its own build/lint/test claims before signoff rather than trusting the PR body. **The other four are docs-only plans and findings, not diffs:** **PR #1427 — pool observability scoping.** Starts the `threads.max` checkpoint clock (owner confirmed removing Render's `SERVER_TOMCAT_THREADS_MAX` override 2026-09-19) and scopes — does not implement — closing the saturation detector's two known gaps (non-request-thread registry coverage, scheduler contention) via a design verified against this project's actual Spring 7.0.5 jar (`javap`-confirmed `TaskDecorator` support on both `ThreadPoolTaskExecutor` and `ThreadPoolTaskScheduler`). **PR #1428 — 2026-09-22 production restart finding.** Same-day incident where the known four-occurrence pool-exhaustion signature is explicitly absent (verified: none of that pattern's log lines appear); trigger left genuinely unidentified rather than rounded up to a guess. **PR #1429 — GPT context docs refreshed to `v0.156.0`.** `GPT_CONTEXT.md` and `SURFACES_AND_FEATURES_CONTEXT.md` brought current for the notification-CTA and Campaign Feedback work; other modules (`MONETIZATION_CONTEXT.md` 55 releases stale, `STRATEGY_AND_ROADMAP_CONTEXT.md` self-flagging 43+ releases stale) deliberately left flagged, not silently touched. **PR #1430 — retention communication channel doctrine.** Resolves "should retention email move to in-app notification" with a channel-role doctrine (email = outside-app re-engagement, in-app = discrete in-product change, Dashboard/Coach = current state) rather than a binary answer. Key finding: `WEAK_CONCEPT` and `DUE_CONCEPTS_DIGEST` already have live Dashboard current-state surfaces (`dashboard-focus-areas-card.tsx`, `today-focus-card.tsx`), so no in-app notification is recommended for either — not because the evidence says no, but because the in-product half is already built as standing state. Also found: the daily email budget check counts every `email_log` writer (password-reset/verification/subscription mail included), not just retention types; `UNFINISHED_NOTE` is confirmed dead configuration (zero dispatch sites). **Explicitly not implemented — gated on two named owner decisions before Stage 1b/2/3 proceed.** **Anti-drift: this release does not implement anything the four docs-only PRs scope** — they remain plans and findings until their own gated owner decisions clear; do not treat their presence in this release as authorization to dispatch their Codex prompts. Full scope in `RELEASES.md`.)
