...
Current version: **v0.134.0 — Notification Foundations** (Released, kicked off 2026-09-09, signed off 2026-09-09, base branch `releases/v0.134.0`, cut from `main` after `v0.133.0` merged as #1349 and tagged). Source: `docs/claude-plans/attention-notifications-email-expansion-stage1.md` (Stage A audit). **SCOPE IS STAGE B ITEMS 1–3 + 5 ONLY, AND IT IS DDL-FREE.** **(1)** taxonomy split — `NotificationType` becomes producer-level identity, a new `NotificationCategory` carries badge policy, and `actionableTypes()` derives from the category rather than a boolean on the type; **(2)** widen `dedupKey` to take a String discriminator (today `NotificationService:104-106` returns `type.name() + ":" + entityId`, so every future producer shares one key space and two holding the same entity UUID collide silently); **(3)** the response carries `actionable`/`category` and the frontend stops comparing to the `"ANNOUNCEMENT"` literal (`notification-inbox.tsx:110,117`); **(5)** retention expires non-actionable unread rows — `deleteReadOrDismissedBefore` deletes only read/dismissed rows, so **an unread row is immortal** (396 permanent rows per `EVERYONE` announcement, forever); **90 days reusing the existing constant, NO second knob** (D7). **⚠️⚠️ THIS RELEASE SHIPS NO USER-VISIBLE CHANGE AND THAT IS DELIBERATE — do not read it as stalled, and do not "fix" it by adding a producer.** The numeric badge **has never rendered in production and could not have**: `countActionableUnread` filters `{ACTION_REQUIRED}` and **ZERO code paths produce that type** (`AnnouncementService.deliverOne` is the only caller of `deliver` and always passes `ANNOUNCEMENT`). **⚠️ THE ENTIRE ARGUMENT IS THE ROW COUNT: `notifications` is EMPTY in production — RE-VERIFIED READ-ONLY 2026-09-09 AT KICKOFF (0 notifications, 0 announcements, 0 dedup keys, 0 `ACTION_REQUIRED`), not carried over from the audit — so items 1–3 are pure Java and item 5 is one JPQL predicate — every one becomes a data migration with a backfill and a reconciliation the day the first real notification lands. That window closes silently.** **⚠️⚠️ SPLIT INTO TWO PARTS 2026-09-09, AND THE DDL-FREE RULE APPLIED TO PART 1 ONLY. PART 1 (items 1–3 + 5) MERGED AS PR #1350.** Item 4 was excluded because `V141` and `V142` were unrun — **BOTH RAN 2026-09-08** (verified read-only against `flyway_schema_history`: V141 12:23, V142 15:47, both `success = true`), so that premise expired and **the owner elected to complete ALL of Stage B**. **PART 2 = items 4 (the `idx_notifications_inbox` migration), 6 (fan-out executor) and 7 (four counters).** **⚠️⚠️ ITEM 6 IS NOT "MOVE A CALL TO AN EXECUTOR" — THE AUDIT'S §18 MISSED THIS: `AnnouncementPublishResponse` returns `delivered`/`skipped` computed SYNCHRONOUSLY from the fan-out, and `admin/announcements/page.tsx:182-184` renders them, with 22 backend assertions depending on them. Async delivery makes those numbers unknowable at response time.** Agreed design: **resolve the audience synchronously** (one query, so the admin keeps a real count), hand the list to the executor, and return **`(announcement, recipientCount, queued)`** — dropping `delivered`/`skipped`, with admin copy that says delivery is *in progress*, never that it completed, and **the retry affordance preserved** (discard-with-log per D6). **⚠️ Executor is core 1 / max 2 with an EXPLICIT rejection handler — sized against the 20-connection pool that has failed twice, NEVER like `llmParallelTaskExecutor` (4/8) (R5/R13). NO `@EnableAsync`.** **⚠️ THE INVARIANT MOST LIKELY TO BREAK SILENTLY IS R9: fan-out must stay OUTSIDE any transaction**, or one duplicate key marks the whole fan-out rollback-only. **⚠️⚠️ THE VERIFICATION TRAP IS THE MOST LIKELY WAY THIS RELEASE SHIPS A NO-OP: the audit's own "behaviour must be identical after items 1–3" is the `v0.116.0`/`v0.117.0` SILENT-NO-OP SHAPE** — `actionableTypes() == {ACTION_REQUIRED}` passes **before and after**, so a green run on it is evidence about nothing. **AT LEAST ONE ASSERTION MUST BE ONE THAT CANNOT PASS AGAINST `main`, and the category PARTITION test is it** (badge-eligible and retention-expirable are complements: one flag, two derived sets, a test asserting they partition — two hand-maintained lists is how they drift). **⚠️ AND THE AUDIT'S "Verification the first producer owes" SECTION IS A STAGE D LIST, NOT THIS RELEASE'S — seven of its ten items presuppose a Review Set update producer that is not shipping; pasting it in over-commits.** Stage B's real list: `actionableTypes()` still resolves to exactly `{ACTION_REQUIRED}`; the category partition; a **non-actionable** unread row past 90 days is deleted while an **actionable** one is not; the `GET /notifications` JSON actually carries the new field, asserted through `NotificationControllerTest`'s existing **real request** (`.contentType(MediaType.APPLICATION_JSON)`, never a direct handler call — `v0.119.0`); and **the frontend tests asserting the `"ANNOUNCEMENT"` literal must MOVE WITH the change** or they pass for the wrong reason. **⚠️ ANTI-DRIFT: NO new enum values without a producer** — `NotificationType` keeps EXACTLY its two values; adding `REVIEW_SET_UPDATE`/`CONNECTION_REQUEST`/`NOTE_SHARED` recreates the exact defect the audit opens with (a type with zero producers whose only tests hand-build a state no code path reaches). **`ACTION_REQUIRED` survives as an explicitly TRANSITIONAL placeholder for BACKWARD-COMPATIBLE TAXONOMY TRANSITION, which Stage D replaces with `REVIEW_SET_UPDATE` — SAY SO IN THE ENUM'S JAVADOC** or a future session reads it as permanent. **⚠️ Do NOT justify it as "retained so badge behaviour stays exercised" (owner amendment, 2026-09-09): a domain-model value is not justified by the tests it keeps alive.** **⚠️ STAGE D IS BLOCKED — `docs/claude-plans/attention-notifications-stage1-tightening-addendum.md` proves its proposed dedup key `REVIEW_SET_UPDATE:<adoptedCollectionId>` is a PERMANENT SILENT SUPPRESSION BUG (one notification per learner per set, EVER); the fix is source-side, `REVIEW_SET_UPDATE:<sourceCollectionId>:<lastUpdatePublishedAt epochMilli>`, and costs no migration. This release is UNAFFECTED.** **`NotificationCategory` stays DERIVED IN JAVA, never a stored column** (a stored category is a second source of truth that can disagree with the type). **NEVER call `deliver()` from inside a transaction** — it is deliberately non-transactional so a dedup-index `DataIntegrityViolationException` cannot mark a whole fan-out rollback-only, and item 2 edits the very key that catch depends on. No `exists` check before an insert — the unique index is the mechanism. **`findVisibleInbox` and `countActionableUnread` predicates change TOGETHER** (the `dismissed_at IS NULL` leg was a `v0.130.0` pressure-test fix). `read_at` = awareness, `dismissed_at` = inbox visibility, **no `resolved` column**; opening the panel marks nothing read; poll the count only, never more often than 60 s; never render a literal `0`. **⚠️ NO Learning Connections work — `[CHECKPOINT — due 2026-09-19]`, denominator ONE. NO `frontend/app/onboarding` work — `[CHECKPOINT — due 2026-09-11]` is two days out.** **⚠️ R1 IS A LIVE PRODUCTION FINDING THIS RELEASE DOES NOT FIX: `EMAIL_DAILY_LIMIT` is 100 with a 40 reserve, observed peaks are 156–159/day, and the budget gates `INACTIVITY` ONLY — four other channels are unbudgeted. It blocks every future email producer (Stage E) and is indexed in the Backlog Index, not scoped here.** **VERIFICATION: Part 1 was a single `advisor()` call. ⚠️ PART 2 IS RE-DECIDED TO ONE SCOPED COLD AGENT, framed as FALSIFICATION** — three gate triggers fire: **(a)** delivery introduced a defect this session that the session then fixed (Part 1's frontend tests passed against the OLD implementation and a backend mutant survived — the measured blind-spot signal); **(b)** async ordering is a bug class the gate names as hard to reason about serially; **(c)** a second change touches `publish`/`fanOut`/`deliver`, already edited by Part 1. **⚠️ The claim most worth attacking is *"fan-out still runs outside any transaction"* — invisible to a green suite.** `model: "sonnet"` suffices for claim-checking. **Routing: CODEX** — backend enum + service + JPQL + DTO plus the frontend component is multi-system, so a prompt comes first; **call `advisor()` before writing it.** Full scope in `RELEASES.md`.
